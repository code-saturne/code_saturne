...
\item A Python interpreter, with Python version 3.4 or above.
\end{itemize}
...
a Python (3.4 or later) interpreter and a {\tt make} tool should be sufficient.
For parallel runs, an MPI library is also necessary (MPI-2 or MPI-3 conforming).
...
\item Catalyst (\url{http://www.paraview.org/in-situ/}) or full ParaView
    ...
    \item \texttt{eos} 1.2 or above may be used for thermodynamic properties of fluids.
...
\item PETSc (Portable, Extensible Toolkit for Scientific Computation,
      ...
      \texttt{--prefix} configure option are correctly detected by the
      \CS build scripts.

\end{itemize}

For developers, the GNU Autotools (Autoconf, Automake, Libtool)
will be necessary. To build the documentation,
...
and builds on relatively recent systems should
be straightforward.
...
\textbackslash \\
\textcolor{Violet}{-DPARAVIEW\_USE\_PYTHON}=ON
\textbackslash \\
...
\begin{itemize}
\item GNU Autotools: Autoconf, Automake, Libtool (2.2 or 2.4).
...
\fbox{\begin{minipage}{\textwidth}\texttt{\\
\$ for m in `<top\_source\_dir>/salome/build-aux/list\_salome\_modules.py <salome\_root\_directory>`; do module load \$m; done
}\end{minipage}}

With some builds from the \url{http://www.salome-platform.org}
downloads, building CFDSTUDY may fail due to the \texttt{omniidl} script containing
non-existent paths. In this case, add:

\texttt{OMNIIDL=<salome\_root\_directory>/INSTALL/omniORB/bin/omniidl}

to the \CS \texttt{configure} options to work around this issue.
...
\$ \textcolor{Magenta}{module load} intel\_compilers/2019.0.045 \\
\$ \textcolor{Magenta}{module load} open\_mpi/gcc/4.0.1 \\
...
\textbackslash \\
\textcolor{Violet}{--with-blas}=/opt/mkl-2019.0.045/mkl \textbackslash \\
...
\textbackslash \\
\textcolor{Violet}{--with-cgns}=\textcolor{OliveGreen}{\$CS\_OPT}/cgns-3.4/arch/athos \textbackslash \\
...
\textcolor{Violet}{--with-cgns}=\textcolor{OliveGreen}{\$CS\_OPT}/cgns-3.4/arch/xc30 \textbackslash \\
...
for separate front-end and compute systems, for coupling with \syrthes 4,
