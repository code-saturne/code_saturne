...
\item BLAS (Basic Linear Algebra Subroutines) may be used for certain
      ...
      With optimized BLAS such as Atlas, MKL, ESSL, and others,
      operations such as $y \leftarrow ax+y$ may be faster than with the usual
      compiler optimizations, leading to a slight overall performance
      increase. Optimized BLAS libraries may be very fast for BLAS3
      (dense matrix/matrix operations), but the advantage is usually much
      less significant for BLAS 1 (vector/vector) operations, which are
      the only ones \CS has the opportunity of using.
      Starting with version 2.3, \CS uses its own dot product
      implementation in any case (using a superblock algorithm, for
      better precision), and external BLAS are only used for
      $y \leftarrow ax+y$ operations, so possible gains are very limited.
      The \texttt{cs\_blas\_test} unit test allows comparing unit benchmark
      performance with or without BLAS (though the timing method
      may have limited precision). The Intel MKL BLAS may also be used
      for matrix-vector products, but this is currently only
      used in unit benchmark mode.
      In addition, when using threaded BLAS, threading might oversubscribe
      ...
      Use of BLAS libraries is thus useful as a benchmarking feature,
      but often not worth the trouble for production builds.
...
developpers building code from a repository can be expected to
...
\textcolor{Violet}{--prefix}=/home/user/Code\_Saturne/2.1/arch/\textcolor{Magenta}{dbg}
\textbackslash \\
\textcolor{Violet}{--with-med}=/home/user/opt/med-3.0 \textbackslash \\
...
\fbox{\begin{minipage}{\textwidth}\texttt{\\
\$ \textcolor{OliveGreen}{SRC\_PATH}=/home/projects/Code\_Saturne/2.1 \\
\$ \textcolor{OliveGreen}{INSTALL\_PATH}=/home/projects/Code\_Saturne/2.1 \\
...
\textcolor{Violet}{--with-hdf5}=\textcolor{OliveGreen}{\$CS\_OPT}/hdf5-1.8.6/arch/ivanoe
\textbackslash \\
