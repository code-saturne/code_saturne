...
\section{Compilers and interpreters{sec:compiler}}
...
\begin{itemize}
\item A C compiler, conforming to the C99 standard.
\item A Fortran compiler, conforming at least to the Fortran 95 standard
      and supporting at least the ISO\_C\_BINDING Fortran 2003 module.
\item A Python interpreter, with Python version 2.6 or above.
      The SALOME platform is currently only compatible with Python 2,
      so Python 3 is only possible when not building with SALOME support.
\end{itemize}

For some external libraries, such as Catalyst (see \ref{sec:{sec:list_ext_lib}}),
a C++ compiler is also required.

In practice, the code is known to build and function properly at least with the
GNU compilers 4.4 and above (up to 4.9 at this date), Intel compilers 11 and
above (up to 14 at this date), IBM XL Fortran 14 and C 12 compilers.
...
the optional Paraview/Catalyst support).
...
\fbox{\begin{minipage}{\textwidth}\texttt{\\
\$ /home/user/Code\_Saturne/4.0/src/code\_saturne-\verscs/configure \textbackslash \\
\textcolor{Violet}{--prefix}=/home/user/Code\_Saturne/4.0/arch/prod
\textbackslash \\
...
\fbox{\begin{minipage}{\textwidth}\texttt{\\
\$ \textcolor{OliveGreen}{SRC\_PATH}=/home/projects/Code\_Saturne/4.0 \\
\$ \textcolor{OliveGreen}{INSTALL\_PATH}=/home/projects/Code\_Saturne/4.0 \\
...
In that case, caution must be exerciced so that the user will load
...
The global default MPI execution commands and options may also be overriden.
...
are overriden; defaults continue to apply for all others.
