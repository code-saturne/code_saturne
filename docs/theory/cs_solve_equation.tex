...
\end{equation}
En utilisant l'\'{e}quation de conservation de la masse (cf. \fort{cs\_velocity\_prediction}),
...
qui est ensuite r\'{e}solue par \fort{cs\_equation\_iterative\_solve}.
\begin{equation}
...
le cas par exemple o\`{u} $T_s^{imp}$ change de signe\footnote{cf. \fort{cs\_velocity\_prediction}}.
\\\\
...
se reportera \`{a} la documentation sur la fonction \fort{cs\_solve\_equation\_scalar}.
...
(cf. sous-programme \fort{cs\_solve\_navier\_stokes}), on aboutit \`a :
\begin{equation}
\begin{array}{lll}
