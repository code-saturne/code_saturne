...
same point, namely the cell center\footnote{%
The center of a cell is a geometric point associated with the cell and
located preferably inside the cell. Nevertheless, the word \emph{center} shall
...
the second line is obtained using once again Green's formula. In formula \eqref{eq:spadis:convection_def}, one still has to express the face value
 $\varia_\face$ and also the value of the mass flux $\left(\rho \vect{u}\right)_\face\cdot \vect{S}_{\bij}$: all the available convective schemes (\emph{upwind, centered, SOLU, etc.}) are presented in \S~\ref{sec:spadis:convection}. Let $\dot{m}_{\bij}$ be the outgoing
...
These points are respectively the projection of the centers $\centi$ and $\centj$
...
%-------------------------------------------------------------------------------
\subsection{Centered}
For a centered scheme, the convective flux reads:

\begin{equation}
C_{\bij}^{centered} \left( \dot{m}_{\bij} , \, \varia \right)  \equiv \left(\varia_\fij^{centered} - \varia_{\celli} \right) \dot{m}_{\bij} ,
\end{equation}
with
\begin{equation}
\varia_\fij^{centered} = \alpha_{\bij} \varia_{\centip} + \left( 1 - \alpha_{\bij} \right) \varia_{\centjp}.
\end{equation}
...
%
\begin{equation}
\varia_\fij^{centered} = \alpha_{\bij} \varia_{\celli} + \left( 1 - \alpha_{\bij} \right) \varia_{\cellj}
+
...
Extrapolation  of the upwind value at the faces center.}%
...
the centered or SOLU scheme to the first-order upwind scheme (without blending). Additionally, the default option to deal with $\varia_\fij$ is
computed as a weighted average between the upstream value and the centered value (blending), according to users' choice.
\end{remark}
...
%-------------------------------------------------------------------------------
\subsection{Green Gauss iterative process}\label{sec:spadis:iteratif_gradient}
...
%-------------------------------------------------------------------------------
\subsection{Green Gauss iterative process for vectorial fields}\label{sec:spadis:iteratif_gradient_vectors}
...
The idea is to evaluate the gradient of the variable at the cell faces using the value of the gradient at the cell centers.
...
the centers $\centi$ and $\centj$ oriented from cell $\celli$ to $\cellj$.
...
to compute the hydrostatic pressure with an a priori momentum equation to obtain
...
stratified areas, near horizontal walls or also near high head losses zones.
...
\end{equation}
$\epsilon$ is set to $1$ if non orthogonalities are reconstructed, $0$ otherwise.
