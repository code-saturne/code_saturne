...
%$\sum\limits_{\fij \in \Facei{\celli}} P_{\fij} \vect{S}_{\iface}+\sum\limits_{\fib \in \Faceb{\celli}} P_{\fib} \vect{S}_{\ib}$,
...
Outward (with respect to the cell $\celli$) normals are respectively denoted  $\vect{S}_{\iface}$ and $\vect{S}_\ib$,
which means that $\vect{S}_{\iface}$ is oriented from $\celli$ toward $\face$.
...
D_{\iface} \left( K_\face , \, \varia\right) \equiv   K_\face \grad_\face \varia \cdot \vect{S}_{\iface}.
 \end{equation}
 Note that the diffusive flux $D_{\iface} \left( K_\fij , \, \varia\right) $ over the interior face $\fij$ lost by the cell $\celli$
 is gained by $\cellj$, in other words:
 \begin{equation}\label{eq:spadis:diffusion_flux_symmetry}
D_{\iface} \left( K_\fij , \, \varia\right) = - D_{\jface} \left( K_\fij , \, \varia\right) .
 \end{equation}
...
Eventually, a weighting factor $\alpha_{\iface}$ is defined to measure the distance of the cell center $\centi$ to the face $\fij$ relatively
to the other cell center $\centj$:
\begin{equation}\label{eq:spadis:pond_def}
\alpha_{\iface}=\dfrac{\overline{\centjp - \centf}}{\overline{\centjp - \centip}}.
\end{equation}
Note that the distances  $\overline{\centjp - \centip}$ and $\overline{\centjp - \centf}$ are defined algebraically, that is:
\begin{equation}
\begin{array}{r c l}
\overline{\centjp - \centip} & \equiv & \dfrac{\left(\centjp - \centip\right) \cdot \vect{S}_{\iface}}{\norm{\vect{S}_{\iface}}}, \\
\overline{\centjp - \centf} & \equiv & \dfrac{\left(\centjp - \centf\right) \cdot \vect{S}_{\iface}}{\norm{\vect{S}_{\iface}}},
\end{array}
\end{equation}
and are supposed to be positive if the mesh is \emph{star-shaped}. Note also that $\alpha_{\iface}$ is oriented from $\celli$ to $\cellj$ and
%
\begin{equation}
\alpha_{\iface} + \alpha_{\jface} = 1.
\end{equation}
...
numerical fluxes $C_{\iface} \left( \dot{m}_{\iface} , \, \varia \right)$ calculated at the interior faces,
...
The value of the convective flux $ C_{\iface} \left(\dot{m}_\fij , \, \varia \right) $ depends on the numerical scheme. Three different types of convection schemes are available in \CS.
...
\begin{equation}
C_{\iface}^{upwind} \left( \dot{m}_{\iface} , \, \varia \right)  \equiv \left(\varia_{\fij}^{upwind} - \varia_{\celli} \right) \dot{m}_{\iface},
\end{equation}
with
\begin{equation}
\varia_{\fij}^{upwind} =
\left\lbrace\begin{array}{l}
\varia_\celli \text{ if } \dot{m}_{\iface}  \geqslant 0,\\
\varia_\cellj \text{ if } \dot{m}_{\iface} < 0.
\end{array}\right.
\end{equation}
...
\begin{equation}
C_{\iface}^{centred} \left( \dot{m}_{\iface} , \, \varia \right)  \equiv \left(\varia_\fij^{centred} - \varia_{\celli} \right) \dot{m}_{\iface} ,
\end{equation}
with
\begin{equation}
\varia_\fij^{centred} = \alpha_{\iface} \varia_{\centip} + \left( 1 - \alpha_{\iface} \right) \varia_{\centjp}.
\end{equation}
...
%
\begin{equation}
\varia_\fij^{centred} = \alpha_{\iface} \varia_{\celli} + \left( 1 - \alpha_{\iface} \right) \varia_{\cellj}
+
...
\begin{equation}
C_{\iface}^{SOLU} \left( \dot{m}_{\iface} , \, \varia \right)  \equiv \left(\varia_{\fij}^{SOLU} - \varia_{\celli} \right) \dot{m}_{\iface} ,
\end{equation}
with
\begin{equation}
\varia_\fij^{SOLU} =
\left\lbrace\begin{array}{l l}
\varia_{\celli} +\grad_{\celli} \varia \cdot \left(\centf - \centi\right)  & \text{ if } \dot{m}_{\iface}  \geqslant 0,\\
\varia_{\cellj} +\grad_{\cellj} \varia \cdot \left(\centf - \centj\right)   & \text{ if } \dot{m}_{\iface} < 0 .
\end{array}\right.
\end{equation}
...
has been written as a sum of the
numerical fluxes $D_{\iface} \left( K_{\fij} , \, \varia \right)$ calculated at the internal faces,
...
The value of the diffusive flux $D_{\iface} $ depends on the \emph{reconstruction} of the field $\varia$ and also on the interpolation at the face  of the diffusivity $K$ from the cell values. Two interpolations are available:
%
\begin{enumerate}[ label=\roman{*}/, ref=(\roman{*})]
\item a \emph{harmonic} interpolation which reads:
\begin{equation}\label{eq:spadis:harmonic_viscosity}
K_{\fij}^{harmonic} \equiv  \dfrac{K_{\celli} K_{\cellj}}{\alpha_{\iface} K_{\celli} + \left( 1 - \alpha_{\iface}\right)K_{\cellj}}
\end{equation}
...
\begin{equation}
D_{\iface}^{NRec} \left( K_{\fij} , \, \varia \right)  =  - \dfrac{K_{\fij} \norm{\vect{S}_{\iface}}}{\overline{\centjp - \centip} } \left( \varia_{\celli} - \varia_{\cellj}\right).
\end{equation}
...
\begin{equation}
D_{\iface}^{Rec} \left( K_{\fij} , \, \varia \right)  =  - \dfrac{K_{\fij} \norm{\vect{S}_{\iface}}}{\overline{\centjp - \centip} } \left( \varia_{\centip} - \varia_{\centjp} \right).
\end{equation}
...
\begin{array}{r c l}
D_{\iface}^{Rec} \left( K_{\fij} , \, \varia \right)  &=&  - \dfrac{K_{\fij} \norm{\vect{S}_{\iface}}}{\overline{\centjp - \centip} } \left( \varia_{\celli} - \varia_{\cellj} \right) \\
&-& \dfrac{K_{\fij} \norm{\vect{S}_{\iface}}}{\overline{\centjp - \centip} }  \dfrac{1}{2}\left( \grad_{\celli} \varia + \grad_{\cellj} \varia \right) \cdot \left( \left(\centip - \centi\right) - \left(\centjp - \centj\right) \right),
\end{array}
\end{equation}
%
which ensures the second-order discretization in space for $\varia$.
...
\varia_{\fij}\,{\vect S_{\iface}}
+\sum\limits_{ \fib \in \Faceb{\celli}}
\varia_{\fib}\,{\vect S_{\ib }}, \\
&=& \displaystyle
 \sum\limits_{ \fij \in \Facei{\celli}}
\varia_{\centf}\,{\vect S_{\iface}}
+\sum\limits_{ \fib \in \Faceb{\celli}}
\varia_{\centf}\,{\vect S_{\ib }}, \\
&\simeq &  \displaystyle
\sum\limits_{ \fij \in \Facei{\celli}} \left[ \varia_{\cento}+ \grad_{\cento} \varia \cdot \left(\centf - \cento\right) \right] \vect{S}_{\iface}+
\sum\limits_{ \fib \in \Faceb{\celli}} \left[ \epsilon_{\delta \varia} A_{\fib} + B_{\fib} \varia_{\ipf} \right] \vect{S}_{\ib} ,\\
 & = &\displaystyle
\sum\limits_{ \fij \in \Facei{\celli}}
\left[
\left( \alpha_{\iface} \varia_{\centi} +
(1 - \alpha_{\iface}) \varia_{\centj} \right) \right] \vect{S}_{\iface} +
\sum\limits_{ \fij \in \Facei{i}} \left[
\grad_{\fij} \varia  \cdot  \left(\centf - \cento\right) \right] \vect{S}_{\iface} \\
&+&\displaystyle
\sum\limits_{ \fib \in \Faceb{\celli}} \left[ \epsilon_{\delta \varia} A_{\fib} + B_{\fib} \varia_{\ipf} \right] \vect{S}_{\ib} .
\end{array}
\end{equation}
...
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}}
\left[\alpha_{\iface} \varia_{\celli}
+ (1 - \alpha_{\iface}) \varia_{\cellj}  + \frac{1}{2}
\left( \grad_{\celli} \varia +\grad_{\cellj} \varia\right) \cdot \left(\centf - \cento\right)  \right] {\vect S_{\iface}}\\
&+& \displaystyle
\sum\limits_{\fib \in \Faceb{\celli}}
...
\displaystyle
\norm{\vol{\celli}} \grad_{\celli} \varia -
\sum\limits_{ \fij \in \Facei{\celli}} \frac{1}{2} \grad_\celli \varia \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right)-
...
(\alpha_{\iface} \varia_{\celli} + (1 - \alpha_{\iface}) \varia_{\cellj})\right] \vect{S}_{\iface} \\
&+&
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}} \frac{1}{2} \grad_{\cellj} \varia \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right) \\
&+&
\displaystyle
...
\int_{\vol{\celli}} \grad \varia\, \dd \vol{} =\sum\limits_{\fij \in \Facei{\celli}} \varia_{\fij} \vect S_{\iface} + \sum\limits_{\fib \in \Faceb{\celli}} \varia_{\fib} \vect{S}_{\ib}, \\
 &=& \displaystyle
 \sum\limits_{ \fij \in \Facei{\celli}}
 \left[ \alpha_{\iface} \varia_{\centi} +
(1 - \alpha_{\iface}) \varia_{\centj} \right] \vect S_{\iface}
...
\grad_\celli^{NRec} \varia= \dfrac{1}{\norm{\vol{\celli}}} \left[
  \sum\limits_{\fij \in \Facei{\celli}} \left[\alpha_{\iface} \varia_{\centi} + (1 - \alpha_{\iface}) \varia_{\centj}) \right] \vect S_{\iface}
...
\sum\limits_{ \fij \in \Facei{\celli}} \frac{1}{2}  \left(\centf - \cento\right) \otimes \vect{S}_{\iface} -
...
&+&
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}} \frac{1}{2} \grad_{\cellj}^k \varia \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right) \\
&+&
\displaystyle
\sum\limits_{\fib \in \Faceb{\celli}}\left[ \epsilon_{\delta \varia} A_{\fib} + B_{\fib} \varia_{\celli} \right] \vect{S}_{\ib},
\end{array}
...
\left(\grad_{\celli}^k \varia + \grad_{\cellj}^k \varia \right) \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right) \\
&+&
\displaystyle
...
= \sum\limits_{ \fij \in \Facei{\celli}}
\variav_{\fij} \otimes {\vect S_{\iface}}
+\sum\limits_{ \fib \in \Faceb{\celli}}
\variav_{\fib} \otimes {\vect S_{\ib }}, \\
&=& \displaystyle
 \sum\limits_{ \fij \in \Facei{\celli}}
\variav_{\centf} \otimes {\vect S_{\iface}}
+\sum\limits_{ \fib \in \Faceb{\celli}}
\variav_{\centf} \otimes {\vect S_{\ib }}, \\
&\simeq &  \displaystyle
\sum\limits_{ \fij \in \Facei{\celli}} \left[ \variav_{\cento}+ \gradt_{\cento} \variav \cdot \left(\centf - \cento\right) \right] \otimes \vect{S}_{\iface}+
...
\sum\limits_{ \fij \in \Facei{\celli}}
\left[
\left( \alpha_{\iface} \variav_{\centi} +
(1 - \alpha_{\iface}) \variav_{\centj} \right) \right] \otimes \vect{S}_{\iface} +
\sum\limits_{ \fij \in \Facei{i}} \left[
\gradt_{\fij} \variav  \cdot  \left(\centf - \cento\right) \right] \otimes \vect{S}_{\iface} \\
&+&\displaystyle
\sum\limits_{ \fib \in \Faceb{\celli}} \left[ \epsilon_{\delta \variav} \vect{A}_{\fib} + \tens{B}_{\fib} \cdot \variav_{\ipf} \right] \otimes \vect{S}_{\ib} .
\end{array}
\end{equation}
...
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}}
\left[\alpha_{\iface} \variav_{\celli}
+ (1 - \alpha_{\iface}) \variav_{\cellj}  + \frac{1}{2}
\left( \gradt_{\celli} \variav +\gradt_{\cellj} \variav \right) \cdot \left(\centf - \cento\right)  \right] \otimes {\vect S_{\iface}}\\
&+& \displaystyle
\sum\limits_{\fib \in \Faceb{\celli}}
...
\displaystyle
\norm{\vol{\celli}} \gradt_{\celli} \variav -
\sum\limits_{ \fij \in \Facei{\celli}} \frac{1}{2} \gradt_{\celli} \variav \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right)
&=&
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}}\left[
(\alpha_{\iface} \variav_{\celli} + (1 - \alpha_{\iface}) \variav_{\cellj})\right] \otimes \vect{S}_{\iface} \\
&+&
\displaystyle
\sum\limits_{\fij \in \Facei{\celli}} \frac{1}{2} \gradt_{\cellj} \variav \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right) \\
&+&
\displaystyle
...
\gradt_\celli^{NRec} \variav= \dfrac{1}{\norm{\vol{\celli}}} \left[
  \sum\limits_{\fij \in \Facei{\celli}} \left[\alpha_{\iface} \variav_{\centi} + (1 - \alpha_{\iface}) \variav_{\centj}) \right] \otimes \vect S_{\iface}
...
\displaystyle
 \tens{1} -
\dfrac{1}{\norm{\vol{\celli}}}\sum\limits_{ \fij \in \Facei{\celli}} \frac{1}{2}  \left(\centf - \cento\right) \otimes \vect{S}_{\iface}
,\\
\tens{R}^{k+1}_{\celli} &=&
\displaystyle
...
&+& \displaystyle
 \dfrac{1}{\norm{\vol{\celli}}}
\sum\limits_{\fij \in \Facei{\celli}} \frac{1}{2}
\left(\gradt_{\celli}^k \variav + \gradt_{\cellj}^k \variav \right) \cdot \left( \left(\centf - \cento\right) \otimes \vect{S}_{\iface} \right) \\
&+& \displaystyle
 \dfrac{1}{\norm{\vol{\celli}}}
