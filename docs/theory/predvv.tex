...
\fort{cs\_solve\_navier\_stokes}}, $\tens{R}$ the Reynolds which appears when applying the
...
applying a fixed-point subiteration over the subroutine \fort{cs\_solve\_navier\_stokes}
...
(\rho\,\vect {u})$ is not problematic for the diagonal dominance of the matrix as it is exactly counterbalanced by the convection term (cf. \fort{cs\_solve\_equation\_scalar}).
...
\\
$\bullet$ If the source terms are extrapolated and we iterate over \fort{cs\_solve\_navier\_stokes}\\
\begin{itemize}
\item [-]\var{TRAV} receives the source terms which are recomputed during each iteration over \fort{cs\_solve\_navier\_stokes} but are not extrapolated
...
\item [-]\var{TRAVA} receives the source terms whose values do not change during the course of the iterations over \fort{cs\_solve\_navier\_stokes} nor are they extrapolated
($T_s^{imp}\,u^n\,,-\rho\,\tens{K}_{d}\,u^n\,,\,-\Gamma^n\,u^n,...$).\\
\item [-]\var{PROPCE} receives the source terms that have to be extrapolated.\\
\\
\end{itemize}
$\bullet$ When there is no iteration over \fort{cs\_solve\_navier\_stokes}, \var{TRAVA} has no use and its contents are directly stored in \var{TRAV}.\\
\\
...
Therefore, when there is neither source term extrapolation nor iteration over \fort{cs\_solve\_navier\_stokes}, all the source terms go directly into \var{TRAV}.\\
\\
\begin{itemize}
...
\item If source term extrapolation is used, at the first iteration over \fort{cs\_solve\_navier\_stokes}, the vector \var{TRAV} (or \var{TRAVA}) receives $-\theta_S$ times the contribution at time $n-1$ of the source terms to be extrapolated\footnote{Because
...
\item The value of the term corresponding to the turbulence model is only calculated during the first iteration over \fort{cs\_solve\_navier\_stokes} then added to \var{TRAVA}, \var{TRAV} or \var{PROPCE} depending on whether the source terms are extrapolated, and/or whether we iterate over \fort{cs\_solve\_navier\_stokes}.\\
...
reconstruction gradients) associated to the operator $\mathcal{E}$ (nonlinear)\footnote{ Ensuring coherence with the definition of the operators $\mathcal{EM}$ and $\mathcal{E}$ employed in \fort{cs\_solve\_navier\_stokes} }.
...
\begin{itemize}
\item During the first iteration over the subroutine \fort{cs\_solve\_navier\_stokes}, the implicit and explicit parts of the user-prescribed source terms are computed by a call to the subroutine \fort{ustsns}.\\
...
{\tiny$\blacksquare$} The explicit part ($T_s^{exp}$) is added to \var{TRAVA}, \var{TRAV} or \var{PROPCE} depending on whether the source terms are extrapolated or not, and whether we iterate over \fort{cs\_solve\_navier\_stokes}.\\

\item The mass accumulation term ($\dive(\rho \vect {u})$) is calculated by calling the subroutine \fort{divmas} with the mass flux as argument. During the first iteration of the subroutine \fort{cs\_solve\_navier\_stokes},
...
\item The implicit and explicit mass source terms, if any (~$\var{NCESMP}>0$~), are computed at the first iteration by the subroutine  \fort{catsma} over \fort{cs\_solve\_navier\_stokes}. $\Gamma\,\vect{u}_i$ is added to \var{TRAV}, \var{TRAVA} or \var{PROPCE} for potential extrapolation. $\Gamma\,\vect{u}^n$ is added to \var{TRAV} or \var{TRAVA} and
$-\Gamma$ to \var{ROVSDT}.
\\
...
\item When strengthened transient velocity-pressure coupling ($\ \var{IPUCOU} = 1\ $) (only available with a first-order scheme, where there is neither source term extrapolation nor iteration over \fort{cs\_solve\_navier\_stokes}) is implemented, \fort{codits} solves the following equation:
