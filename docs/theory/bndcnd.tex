...
\caption{Values and unit of $\alpha$ common cases.}\label{tab:bdncnd:diffusivity}
\end{table}
...
Equaling this to the heat flux on the first fluid cell this to the heat flux on the first fluid cell boundary gives $ h_{ext} \left(T_{REF} - T_w \right) = h_{int} \left( T_\centip T_\centf \right)$. This relation between the variable and its gradient at the boundary is called a Robin or mixed boundary condtion.

More generally, to impose an external Dirichlet $\varia^{imp, \, ext}$ at some distance outside the boundary face,
this is done by a user prescribded externalent $h_{ext}$
...
\begin{remark}
Both $\varia^{imp, \, ext} $ and $ h_{ext} $ must be specified by the user. The boundary code is $1$ (see \tablename~\ref{tab:ICODCLadm_condli}). Take care that an outgoing flux is counted positive.
\end{remark}
...
when a wall boundary value is prescribed and a wall function is set by the user. The dedicated boundary conditions of the turbulent variables ($k$, $\varepsilon$, $R_{ij}$) are detailed in \chaptername~ \ref{chapter:turbul}.
 Here is a link to the \doxygenfile{clptur_8f90}{programmers reference of the dedicated subroutine}.
...
\vect{A}^f_\ib & = & -h_{fluid}  \left(\tens{1} - \vect{n} \otimes \vect{n} \right) \cdot  \vect{u}_{wall}
 - h_{hint} \left( \vect{n} \cdot \vect{u}_{wall}  \right) \vect{n}
, \\
...
A dedicated boundary condition for the velocity gradient is therefore derived in presence of wall function. 
...
Let us start with the Bernoulli relation ship between a boundary face $\face$ linked to a point on the same
