...
We have, for each cell $\Omega_I$ of center $I$, the following relation\footnote{For further details regarding $\mathcal{EM_{\it{disc}}}$, the discrete operator acting on a scalar $a$, the reader can refer to the subroutine
\fort{matrix}.}:
\begin{equation}\notag
...
to solve the velocity-pressure system (\var{NTERUP}$>$ 1), $a^{n+1,0}$ is initialized by
...
initialized by $\var{RHSINI}$ and completed with the reconstructed
...
The array $\var{RHSINI}^{\,k+1}$ initializes the entire right side
...
(first-order accurate in space upwind scheme, centered scheme with second-order spatial discretisation, second-order
linear upwind "SOLU" scheme or a weighted average (blending) of one of the second-order schemes (either centered or SOLU) and the first-order upwind scheme, with potential use of a slope test).\\
This contribution (convection-diffusion) is then added in to the right side of the
equation  $\var{RHS}^{\,k+1}$ (initialized by $\var{RHSINI}^{\,k+1}$).
\begin{equation}\notag
\begin{array}{ll}
...
of the approximate operator so that the centered scheme without reconstruction, for example, is
