...
$\Neigh{i}$ the set of the centers of the neighbouring cells of
${\Omega_i}$ and $\gamma_b(i)$ the set of the centers of the boundary
...
\footnotetext{Extrapolation  of the upwind value at the faces center.}
...
the values \var{PVAR(II)} and \var{PVAR(JJ)} at the cell centers are needed.
\begin{equation}
\begin{array}{ll}
\var{PIF} &= \var{PVAR(II)} \\
\var{PJF} &= \var{PVAR(JJ)} \\
...
\begin{equation}
\begin{array}{ll}
P_{IF} &= \var{BLENCP} P^{\,\it (center\  or\  SOLU) }_{\,IF} + (1 - \var{BLENCP})\  P_{II}\\
P_{JF} &= \var{BLENCP} P^{\,\it (center\  or\  SOLU) }_{\,JF} + (1 - \var{BLENCP})\  P_{JJ}\\
\end{array}
\end{equation}
$\bullet$ calculation of \var{FLUI} and \var{FLUJ},\\
