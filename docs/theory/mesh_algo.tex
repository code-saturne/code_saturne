...
The unwarping algorithm is a smoother which 
the principle is to mitigate the local defects by averaging
the mesh quality. It moves vertices by an iterative
process which, we expect, converges to a mesh with better averaged 
warping criteria.
...
Let $f$ be the face defined by $P_{i=1:Nbv(f)}$, $O_{f}$ the center 
of gravity and $\overrightarrow{n_{f}}$ the face normal, the warping
...
faces by an iterating process. At each iteration the algorithm tries to
...
In most cases, a vertex shares many faces $f_{j=1:Nbf(P_{i})}$, its final
displacement being:

$$\forall f \in \mathcal{M}, \; \forall i \in [1,Nbv(f)], \qquad
 \lambda_{P_{i}}^{f}=\sum_{j=1:Nbf(P_{i})}\lambda^{f_{j}}_{P_{i}}$$

This displacement technique may pose problems because the contributions
$\lambda^{f_{l}}_{P}$ and $\lambda^{f_{k}}_{P}$ can be ``contradictory''.
More, if a small and a large face are neighbours, the large face contribution
impose a too big displacement to the shared vertices and the warping criterion
can be deteriorated.
...
Every iteration, for each face the algorithm calculates the warping criterion.
It is used to give more weight to the warp faces. After renormalisation of the 
...
Every iteration, for each vertex $P \in \mathcal{M} $, the algorithm calculates 
the vertex tolerance defined as:
$$\forall P \in \mathcal{M}, \qquad
vtxtol_{P} = \frac{\min_{P \in nb(P)} PP\prime}
{\max_{Q\in \mathcal{M}}{\min_{Q\prime\in nb(Q)} QQ\prime}}$$
where $nb(Q)$ are the first neighbours of the point $Q$.
...
As we observed, this ``local coefficient'' is less efficient that the first one.
The first definition is used in \CS.
...
To ensure a better convergence, all vertices movement are multiplied by a scale 
factor $Cm$ (where $Cm \leqslant 1$). This coefficient helps converging to the
best solution by preventing too big a movement in the wrong direction.
In \CS the default value is set to 0.10.

The unwarping displacement is now:
...
\paragraph*{1st case: the algorithm converges}
...
\paragraph*{2nd case: the algorithm diverges}
...
\paragraph*{3rd case: the maximum number of iterations is reached}
...
fit\_by\_feature allows to fix boundary faces according to a feature angle.
