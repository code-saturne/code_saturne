...
at the cell center by the volume of the cell.
...
Taking $Neigh(i)$ as the set of the centers of the neighbouring cells of
${\Omega_i}$ and $\gamma_b(i)$ the set of the centers of the boundary faces of
${\Omega_i}$, we obtain:
...
\end{equation}
%
\item a second-order centered scheme:\\
$\vect{F}_{\,ij}((\rho \vect{u}), \vect{u})=
\vect{F}^{\text{\it{centered}}}_{\,ij}((\rho\vect{u}),\vect{u})$\\
with:
\begin{equation}
\vect{u}_{\fij}=
...
\begin{remark}
When a centered scheme is used, for the simple reason of numerical stability
we actually write:
...
The face value of the viscosity $\mu_{\fij}$ is calculated from the cell center
...
As for the numerical flux calculation in the centered scheme, we legitimately
...
t^n,\delta P^{n+\theta})
\end{equation}
and we compute the new velocity field at the center of the cells thanks
to the equality:
%
