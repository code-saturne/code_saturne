...
The continuity equation (also called mass equation) is displayed in \eqref{eq:mass}.

\begin{equation}\label{eq:mass}
\dfrac{\partial \rho}{\partial t} + \dive(\rho \vect{u})=\Gamma.
\end{equation}

In fact, to compute a given unknown $\varia$ (and in particular for the
velocity prediction),
\nomenclature[gphi]{$\varia$}{scalar unknown field}
 the equation 
$\displaystyle \frac{\partial \rho}{\partial t} + \dive(\rho \vect{u}) = \Gamma$ 
is used to re-write the term 
$\displaystyle \frac{\partial \left( \rho \phi \right)}{\partial t}$ as follows:
\begin{equation*}
\dfrac{\partial \left( \rho \varia \right)}{\partial t} = \rho \dfrac{\partial  \varia}{\partial t} - \varia \,\dive(\rho \vect{u})
+ \Gamma\,\varia^{in}.
\end{equation*}
...
$\tens{K}$ is a symmetric positive tensor, by definition. 
...
%
\begin{itemize}
\item with an eddy-viscosity model (\emph{EVM}) such as the $k-\varepsilon $ 
model, the closure requires a turbulent viscosity $\mu _{t}$. 
With formally the same definition for $\tens{\tau}$ as in Equation \eqref{eq:base_introd_tau},
...
\tens{\tau}$ (Equation \eqref{eq:base_introd_tau}):
\begin{equation}
...
\begin{itemize}
       ...
       \item $S^2$ is scalar strain rate defined by
	  \begin{equation}
	    \begin{array}{rcl}
...
\begin{equation}
 \dd s = \frac{C_p}{T} \dd T - \left.\der{\left( 1/\rho \right)}{T}\right|_P  \dd P.
\label{eq:Ds_maxwell}
\end{equation}

In the r.h.s. of Equation \eqref{eq:Ds}, the second term becomes:
\begin{equation}
...
So the l.h.s. of Equation \eqref{eq:enthalpyT} becomes:
\begin{equation}
