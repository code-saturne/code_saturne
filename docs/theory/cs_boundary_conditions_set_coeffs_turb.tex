...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This function is dedicated to the calculation of the wall boundary conditions. The notations introduced in \var{cs\_boundary\_conditions\_set\_coeffs} for the general boundary conditions will be used.

The wall boundary conditions refer to all the boundary conditions for the velocity, the turbulent variables ($k$, $\varepsilon$, $R_{ij}$), the temperature when it has a prescribed value at the wall (or the enthalpy and more generally the {\it VarScalaires}\footnote{As in \fort{cs\_boundary\_conditions} the {\it VarScalaire} are any solution of a convection-diffusion equation apart from the velocity, the pressure and the turbulent variables  $k$, $\varepsilon$ and $R_{ij}$. More specifically, the name {\it VarScalaire} can refer to the temperature, the enthalpy or a passive scalar.  } to treat at the wall by using a similarity law for the associated boundary layer). For the {\it VarScalaire} in particular, when the boundary conditions at the wall are of Neumann type (homogeneous or not), they are treated in  \fort{cs\_boundary\_conditions} and don't present them here. In particular, the boundary conditions of the {\it VarScalaires} are not treated here because their treatment at the wall is of homogeneous Neumann type.

We present the calculation of the pair of coefficients $A_b$ and $B_b$ which are used during the computation of certain discretized terms of the equations to solve, and which allow in particular to determine a value associated with the boundary faces $f_{b,int}$ (at a point located at the "center" of the boundary face, the center of mass of its vertices) using the formulae   $f_{b,int} = A_b+B_b\,f_{I'}$ ($f_{I'}$ is the value of the variable at point $I'$, the projection of the center of the boundary cell onto the line normal to the boundary face and passing through its center: see figure~\ref{Base_Clptur_fig_flux_clptur}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The velocity at the wall is noted
...
taken at the center $I$ of the boundary cell).
...
at the center of cell adjoint to the boundary face  (with the $R_{ij}-\varepsilon$ model, we use half the trace of the Reynolds stress tensor).
...
\left\{\begin{array}{l}
u_k = u^*\\
u^* \text{ solution of } \left\{\begin{array}{lll}
\displaystyle\frac{u^r_{\tau,I'}}{u^*} &=
\displaystyle\frac{1}{\kappa}ln(y^+)+5,2 &\text { for }y^+>y^+_{lim}\\
\displaystyle\frac{u^r_{\tau,I'}}{u^*} &= y^+                         &\text
{ for } y^+\leqslant y^+_{lim}
\end{array}\right.\\
\qquad\qquad\text{ with} y^+=\displaystyle\frac{u^*\,I'F}{\nu}
                                               \text{ and }\kappa=0,42
\end{array}\right.
\end{array}
\end{equation}
...
($\rho_I$ is the density at the center of cell $I$).
...
make explicit the velocity at the wall in the final expression:
\begin{equation}
\begin{array}{l}
...
A pair of coefficients $A$ et $B$ is deduced (separately
for each velocity component):
\begin{equation}\label{Base_Clptur_eq_AB_vitesse_rij_clptur}
\begin{array}{l}
\text{\bf Coefficients associated with velocity BCs }(R_{ij}-\varepsilon)\\
\left\{\begin{array}{l}
\left\{\begin{array}{lll}
...
from the above equation (for each velocity component separately).
...
\fort{cs\_boundary\_conditions} (in what follows only the most essential part of the
...
is coherent with the face gradient computation options detailed in
 \fort{cs\_boundary\_conditions} ($h_{int}=\displaystyle\frac{\alpha}{\overline{I'F}}$).
...
The values of all the physical properties are taken at the cell centers,
