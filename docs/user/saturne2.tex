...
The keywords are classified under headings. For each keyword of the Kernel of \CS,
...
for the variable N\\
\hspace*{1.3cm}= -999: by default: chronological records are generated on
...
which may be taken into account are the following:
\begin{list}{-}{}
...
centers and stored as fields. The standard post-processing
actions may therefore be activated, like the writing in the listing or
the output of result files (EnSight, MED, ...).
...
\item define for each average {\tt imom} the time step number or time value
      at which the calculation of the cumulated value must begin, by means of
      the array {\tt ntdmom(imom)} or {\tt ttdmom(imom)}.
\end{list}

The total number of averages ($p$={\tt nbmomt}) is automatically determined by
the code from the values of {\tt idtmom}. The user must not specify it.
...
\hspace*{1.3cm} - if {\tt idfmom(ii,imom)} is positive, it refers to the
                  index-number of a solved variable, like for instance
...
the calculation should begin. If its value is negative, {\tt ttdmom(imom)}
is used instead.\\
useful if and only if the user wants to calculate time averages}
...
\hspace*{1.3cm}\phantom{= {\tt ipol*1000+j} with} {\tt j}= 3: GMRES\\
...
van Driest damping). Most of these keywords are simple copies of the
...
{maximum number of implicitation iterations of of the structure displacement}
...
statistics\\
if the recording of the boundary statistics is non-stationary, then
