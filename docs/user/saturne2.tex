...
that may be taken into account are the following:
\begin{list}{-}{}
...
actions may therefore be activated, such as the writing in the listing or
the output of result files (EnSight, MED, ...).

To calculate $p$ time averages of the type $<f_1*f_2...*f_{n(imom)}>$,
the user must:
\begin{list}{-}{}
\item make sure that $p\leqslant${\tt nbmomx} (does not overstep the maximum
      number of averages),
...
gradient for the pressure {\tt ivar=ipr} and for any variable which is not
...
the linear equation systems: {\tt nitmax(ivar)}\\
