...
as example it the \texttt{SRC/USER\_EXAMPLES/cs\_user\_parameters-base.c} file.\\
...
\minititre{Notes}
$\bullet\ $The notation ``e'' refers to a double precision real. For
           instance, 1.8e-2 means 0.018. \\
$\bullet\ $The notation ``{\tt cs\_math\_big\_r}'' (which can be used in the code)
...
in \CS such as \texttt{nt\_max}, \texttt{nt\_cur}, etc.
...
(\texttt{cs\_glob\_physical\_model\_flag[CS\_JOULE\_EFFECT]\index{CS\_JOULE\_EFFECT}}) and an electric arcs module
(\texttt{cs\_glob\_physical\_model\_flag[CS\_ELECTRIC\_ARCS]\index{CS\_ELECTRIC\_ARCS}}).
