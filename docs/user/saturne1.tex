...
Documentation:     & online documentation, also available at:
                     \url{http://www3.ensight.com/EnSight10\_Docs/HowTo.pdf}\\
\hline
\end{tabular}
...
Developers can also refer to \cite{boucker00} and \cite{theory}.
...
\item A parallel calculation may be stopped in the same manner as a
      sequential one using a \texttt{control\_file} file (see paragraph
...
The \texttt{cs\_user\_boundary\_conditions} subroutine may be used with
the compressible module to define (or redefine) boundary conditions.
...
The subroutine works like \texttt{cs\_user\_initialization} may be used for
advanced initialization of velocity, turbulence and passive scalars. Concerning
pressure, density, temperature and specific total energy, only 2 variables out
of the 4 are independant. The user may also initialise the variable pair he wants
...
\texttt{iccfth} used for the call to \texttt{cfther}.
