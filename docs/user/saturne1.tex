...
function \texttt{cs\_user\_source\_terms} in the \texttt{cs\_user\_source\_terms} file is similar.
...
$\text{\texttt{st\_imp}}_i=\Omega_iS_{impl,i}$\\
$\text{\texttt{st\_exp}}_i=\Omega_iS_{expl,i}$
...
ensure that $\text{\texttt{st\_imp}}_i=\Omega_iS_{impl,i}$ is always negative
...
function \texttt{preduv}). The user must therefore make sure it is
...
$\displaystyle\text{\texttt{st\_imp}}_i=\Omega_i\frac{dF}{d\varphi}(\varphi_i^{(n)})$\\
$\displaystyle\text{\texttt{st\_exp}}_i=
...
the user must set:\\
$\text{\texttt{st\_imp}}_i=-2K\Omega_i\varphi_i^{(n)}$\\
$\text{\texttt{st\_exp}}_i=K\Omega_i[\varphi_i^{(n)}]^2$


%==================================
\subsubsection{For user variables}
%==================================

\noindent
\textit{Function called every time step.}

The source terms in the transport equations relative to solved variables
(velocity, turbulence, thermal, user scalars, ...)
can be defined using the GUI or the \texttt{cs\_user\_source\_terms} user-defined function.

this function is called every time step, once for each variable. The user must set
the appropriate values in the arrays \texttt{st\_imp} and \texttt{st\_exp} for the
variables to which a source term is applied.

Source terms can be entered directly using the GUI when they are
proportional to the volume of the cells or the volume of the fluid in the cell.

For example if we have in \texttt{cs\_user\_source\_terms} user file:
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=12cm]{gui_add_scalar.png}
\caption{Adding the scalar}
\label{fig:Def-scal}
\end{center}
\end{figure}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=11.25cm]{gui_source_term_meg.png}
\caption{initialization of source terms}
\label{fig:Ini-scal}
\end{center}
\end{figure}
