...
Once these steps completed, the user should go in the directory \texttt{CASE1/DATA} and type de command line \texttt{./SaturneGUI setup.xml} to load the calculation file into the interface.\footnote{
When working on a new cas, running \texttt{./SaturneGUI} with no argument can be used to create
a new setup.}
A window similar to \figurename\ref{fig:3_e1} will appear. Click on ``Run computation'' button (also in the ``Tools'' menu), and select the heading ``Prepare batch calculation'', see \figurename~\ref{fig:43_e1}. After having chosen the number of processors, press ``start calculation'' to run the calculation.
...
\begin{list}{-}{}
\item Calculation environment: case path info,
      definition of notebook (parametric) variables.

\item Mesh: definition of the mesh file(s),
      mesh preprocessing options, and mesh checking mode.

\item Calculation features: choice of physical model, ALE mobile mesh features,
      turbulence model, thermal model, coupling with \syrthes...

\item Fluid properties: reference pressure, fluid characteristics, gravity.
...
\item Volume zones: initialisation of the variables, and definition of
      the zones where to apply head losses or source terms.

\item Boundary zones: definition of the boundary conditions for
...
\item Time settings: time stepping scheme, number of time steps,
      management of calculation restart from a previous run.

\item Numerical parameters: advanced parameters
      for the numerical solution of the equations.

\item Postprocessing: parameters concerning the time averages, time step,
      location of
...
\item Performance settings: advanced parallel computing settings
      (partitioning, IO, ...).
...
\item On clusters with batch systems, the launching of a parallel run
      ...
      by the \texttt{--nprocs} option for \texttt{code\_saturne~ run}
      in the \texttt{runcase} file, or by setting the
