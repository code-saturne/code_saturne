...
This document gives practical information for the usage of \CS.
...
\end{center}
The mesh files should be copied in the directory \texttt{MESH}
...
%%%  to data storage, others to intensive I/O).
...
his value have priority over the value defined in the preference file
so if necessary, it
...
to skip the preprocessing stage.
...
allowing the use of the same partitioning in an furher calculations.
By default, this file is output when using graph-based partitioner, which may
use randomization and do not guarantee a reproducible output, and is not output
when using a deterministic space-filling curve based partition.
...
in the \texttt{DATA} directory and its name may either must be be unchanged, or
...
Note that this output is still an experimental stage, and is heavily
...
those that are to a given setup.
...
be stored in the arrays \texttt{propce} or \texttt{propfb}}
...
\underline{About \texttt{propce} and \texttt{propfb}}\\
In \CS, the physical properties\footnote{other variables are stored in the
arrays \texttt{propce} and \texttt{propfb}. They are not
...
\texttt{propce} and \texttt{propfb} arrays.
...
\texttt{propce(iel,ipproc(icdtmo(idtmom(imom))))}.\\
...
The formula editor is a window with three tab:
\begin{list}{$\bullet$}{}
...
Physical properties are stored in \texttt{propce} (cell center) and \texttt{propfb}. For instance, \texttt{propce(iel, ipproc(irom  ))} is \texttt{rom(iel)}, the mean density (in $kg.m^{-3}$).\\
...
the code needs to
be given some supplementary pieces of information:
...
\item The 1st call (initialisation) all the boundary faces that will be treated
 ...
 The parameter \texttt{isuit1} also must be be defined, this indicates if the
...
(parameter stored in lagpar, whose default value is 100). A zone type
