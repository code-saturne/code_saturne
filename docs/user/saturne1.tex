...
\item If $C_p$ is variable, it can be specified in the interface or by
      ...
      \texttt{field\_get\_val\_s(iprpfl(icp), cpro\_cp)}.
\end{list}
...
To assign a variable value to $C_p$, the user \textbf{must} specify it in the
      ...
      \texttt{field\_get\_val\_s(iprpfl(icp), cpro\_cp)} in \texttt{cs\_user\_physical\_properties}.
...
\texttt{field\_get\_val\_s(iprpfl(ivisct), cpro\_cp)}). The
...
\item activate, for the variables which are specific to the activated specific
      ...
      by calling \texttt{field\_get\_val\_s(iprpfl(iprop), cpro\_prop)})
      are listed below:
      \begin{list}{$\rightarrow$}{}
...
\texttt{call field\_get\_val\_s(iprpfl(irom), cpro\_rom)}.\\
