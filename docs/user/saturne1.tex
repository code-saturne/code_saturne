...
Fortran subroutines or developing inside the main solver.
\end{list}
...
sometimes marked by the following strings:
...
but also for some arrays of variable associated options.
...
rather than in a domain-sized array of reals.
\begin{list}{$\bullet$}{}
...
\item If $k$ is constant, it can be specified in the interface or by
      ...
      in \texttt{cs\_user\_parameters.f90}, (in \texttt{usipsu}).
\item If $k$ is variable, it can be specified in the interface or by
      ...
      \texttt{iel}, the value of $k$ is then given in \texttt{usphyv} and associated
      with the field whose id is given by calling
...
The user file \texttt{cs\_user\_les\_inflow.c} allows to generate the
...
subroutine. On the contrary, if Dirichlet values are given for these
...
\texttt{cs\_user\_les\_inflow.c} user file. The user file contains 3
subroutines:
...
\item By default, the $C_p$ coefficient and the
      ...
      space, with the values \texttt{cp0} and the value associated
      to the field's \texttt{diffusivity\_ref} keyword.
      specified in the interface or in \texttt{cs\_user\_parameters.c}.\\
