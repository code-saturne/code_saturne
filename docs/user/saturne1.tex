...
access to the most needed information (especially the user's and programmer's
...
it seems to ``cheat'' with CGNS version numbers and to confuse CGNS 3 and 3.1 specs.
...
must be less than or equal to \texttt{nscamx}}
...
\variabsize{npromx}{Maximum number of physical properties. These will
be stored in the arrays \texttt{propce}}

\variabsize{nproce}{Number of properties defined at the
cells. These will be stored in the array \texttt{propce}}
...
be less than or equal to \texttt{nclcpm}}
...
\minititre{Note 3: face-node connectivity}\label{sec:prg_nodfac}
...
{\bf The user must not modify the existing modules.} This would require the
...
\variab{ifacel}{ifacel(2,nfac)}{ia}{Index-numbers of the (only) two neighbouring
...
Regarding the quality of mesh, its ideal value is 0.5}

\variab{surfan}{surfan(nfac)}{ra}{Norm of the surface vector of
the internal faces}

\variab{surfbn}{surfbn(nfabor)}{ra}{Norm of the surface vector of the boundary faces}
...
Progressively, this system is being replaced by the \variab{field} module,
so some variables and properties are now only accessible as fields
(either through their name or their id). This is the case for example
for mass flux across interior and boundary faces.
...
deducted. The total solved pressure is stored in the PROPCE array}.
...
\underline{About \texttt{propce}}\\
In \CS, the physical properties\footnote{Other variables are stored in the
arrays \texttt{propce}. They are not, strictly speaking,
``physical properties'' but it is convenient to have them
...
corresponding to the specific heat, when it is variable
...
\item If $k$ is constant, it can be specified in the interface or by
      calling \texttt{field\_set\_key\_id(ivarfl(isca(iscal)), kivisl, -1)}
...
\item If $k$ is variable, it can be specified in the interface or by
      calling \texttt{field\_set\_key\_id(ivarfl(isca(iscal)), kivisl, 0)}
      ...
      \texttt{field\_set\_key\_id(ivarfl(isca(iscal)), kivisl, ...)}.
\end{list}
\end{list}
...
(1$\leqslant$ivar$\leqslant$nvar$\leqslant$nvarmx), rank in \texttt{icodcl} and
\texttt{rcodcl} of the boundary conditions. See note 2}
...
boundary face \texttt{ifac} (see user subroutine \texttt{cs\_user\_boundary\_conditions})}
...
Driest-wall damping) and when \texttt{icdpar=1}. The dimensionless wall distance $y^+$
...
See \texttt{ickpdc} and the user subroutine \texttt{cs\_user\_head\_losses.f90}}

\variab{ncepdc}{ncepdc}{ia}{Number of cells in
which a pressure drop is imposed. See the user subroutine \texttt{cs\_user\_head\_losses.f90}}
...
subroutine \texttt{cs\_user\_mass\_source\_terms}}
...
injection at imposed value). See the user subroutine \texttt{cs\_user\_mass\_source\_terms}}

\variab{ncetsm}{ncetsm}{i}{Number of cells with
mass sources. See the user subroutine \texttt{cs\_user\_mass\_source\_terms}}
...
value. See the user subroutine \texttt{cs\_user\_mass\_source\_terms}}
...
must be given to allow the construction of this connectivity. See note 3
...
\variablist{eptp}{eptp(nvp,nbpmax)}{ra}
...
\variab{eptpa}{eptpa(nvp,nbpmax)}{ra}{Variables forming the state vector
...
\variablist{ipepa}{ipepa(nivep,nbpmax)}{ia}{Integer variables
...
\variablist{pepa}{pepa(nvep,nbpmax)}{ra}{Real variables
related to the particles. They are marked out by the following ``pointers'':
\begin{list}{$\rightarrow$}{}
...
trajectography and backward coupling steps, \texttt{eptp} is updated with
\texttt{vitpar}}
...
\texttt{cs\_user\_parameters.f90} is in fact constituted of 4 separate subroutines:  \texttt{usipph}, \texttt{usipgl},
...
For $k$ in the $k-\varepsilon$, $R_{ij}-\varepsilon$, v2f or $k-\omega$
models:\\
{\texttt{rtp(iel,ikiph) = 1.5*(0.02*uref)**2}
(in $R_{ij}-\varepsilon$,  $R_{ij}=\frac{2}{3}k\delta_{ij}$)\\
For $\varepsilon$ in the $k-\varepsilon$, $R_{ij}-\varepsilon$ or v2f models:\\
...
For $\alpha$ in the EBRSM and BL-v2/k models:\\
\texttt{rtp(iel,ial) = 1}\\
...
\item For calculations with constant and uniform time step
      ...
      given in the parametric file of the interface or in \texttt{cs\_user\_parameters.f90}.
...
\hspace*{1cm}\texttt{rcodcl(ifac,ivar,3)}=imposed flux value (depending on the variable, the user may refer to the case \texttt{icodcl}=3 of \S~\ref{sec:prg_clnonstandard} for the flux definition).
...
To assign a variable value to $C_p$, the user \textbf{must} specify it in the
      ...
      \texttt{propce(iel,ipccp)} in \texttt{cs\_user\_physical\_properties}. NB: completing the array
...
\item In the same way, to have variable diffusivities for the scalars
      \texttt{iscal}, the user \textbf{must} specify it in the interface (with a user law) or calling \texttt{field\_set\_key\_id(ivarfl(isca(iscal)), kivisl, 0)}
...
\begin{list}{$\bullet$}{}
       ...
       \item \texttt{fmt\_name}: choice of the output format:
        \begin{list}{$\rightarrow$}{}
         ...
         \end{list}
The options are not case-sensitive, so {\em EnSight} or {\em CGNS} are also valid.
 ...
 tensor if its dimension is 6 or 9); not implemented yet.
       ...
       \item \texttt{time\_dep}: indicates if the post-processing
             ({\em i.e.} visualization) meshes (or ``parts'') are:
        \begin{list}{$\rightarrow$}{}
...
associated with writers 1 and 2 will not be modifiable, but a
mesh associated with writer 1 only will be modifiable. The
...
{lstcel}, \texttt{lstfac}, and \texttt{lstfbr}will be treated directly by
 the subroutine, thereby avoiding unused copies and simplifying the code
         \end{list}
...
Both the interface and the subroutine \texttt{cs\_user\_parameters.f90} allow autonomous management of the
``automatic'' chronological record files:
...
For gas combustion or if the GUI is not used for coal combustion, the 3 subroutines \texttt{usebu1, usd3p1, uslwc1, uscpi1} and \texttt{uscpl1} can be used to complete \texttt{cs\_user\_parameters.f90} for the
considered specific physics. These subroutines are called at the calculation start.
They allow to:
\begin{list}{$\bullet$}{}
\item activate, for the variables which are specific to the activated
             specific physics module, chronological records at the probes defined in
...
Physical properties are stored in \texttt{propce} (cell center). For instance, \texttt{propce(iel, ipproc(irom  ))} is \texttt{rom(iel)}, the mean density (in $kg.m^{-3}$).\\
...
\begin{list}{$\bullet$}{}
\item In the first call (initialisation) all of the boundary faces that will be treated
...
\item \texttt{rgpt1d(nfpt1d)\index{rgpt1d}}: geometry of the pseudo wall mesh (refined
 as a fluid if \texttt{rgt1d} is less than 1)
...
\item The third call (at each time step) is to confirm that all the arrays
...
flux conditions(in $W.m^{-2}$, negative value for energy entering the wall).
...
\minititre{Use of the subroutine \texttt{uslag1}}
...
\variab{ifrlag}{ifrlag(nfabor)}{ia}{In the Lagrangian module, the user defines
\texttt{nfrlag\index{nfrlag}} boundary zones from the color of the boundary faces,
or more generally from their properties (colors, groups...), from the boundary
conditions defined in \texttt{cs\_user\_boundary\_conditions}, or even from their
coordinates. To do so, the array \texttt{ifrlag(nfabor)} giving for each face
\texttt{ifac} the number \texttt{ifrlag(ifac)} corresponding to the zone to
which it belongs, is completed. The zone numbers ({\em i.e.} the values of
\texttt{ifrlag(ifac)}) are chosen freely by the user, but must be strictly
positive integers less than or equal to \texttt{nflagm\index{nflagm}}
(parameter stored in \texttt{lagpar}, whose default value is 100). A zone type
is associated with every zone; it will be used to impose global boundary
conditions. \emph{WARNING: it is essential that every boundary face belongs to a zone.}}

\variab{iusncl}{iusncl(nflagm)}{ia}{For all the \texttt{nfrlag} boundary zones previously identified, the number of classes \texttt{nbclas}\footnote{A class is
a set of particles sharing the same physical properties and the same characteristics concerning the injection in the calculation domain} of entering particles
is given: \texttt{iusncl(izone)} = \texttt{nbclas}. By default, value for the number of particle classes is zero. The maximum number of classes is
\texttt{nclagm\index{nclagm}} (parameter stored in \texttt{lagpar}, whose default value is 20).}
...
keywords \texttt{ientrl\index{ientrl}}, \texttt{isortl\index{isortl}},
\texttt{irebol\index{irebol}}, \texttt{idepo1\index{idepo1}},
\texttt{idepo2\index{idepo2}}, \texttt{iencrl\index{iencrl}}).

\begin{list}{$\bullet$}{}

 \item if \texttt{iusclb(izone)} = \texttt{ientrl, izone} is a particle injection zone. For each particle class associated with this zone, information must be provided (see below). If a particle trajectory might cross an injection zone, then this particle leaves the calculation domain.

 \item if \texttt{iusclb(izone)} = \texttt{isortl}, the particles interacting with the zone \texttt{izone} leave definitely the calculation domain.

 \item if \texttt{iusclb(izone)} = \texttt{irebol}, the particles undergo an elastic rebound on the boundary zone \texttt{izone}.

 \item if \texttt{iusclb(izone)} = \texttt{idepo1}, the particles settle definitely on the boundary zone \texttt{izone}. These particles leave the calculation domain and are deleted from the calculation

 \item if \texttt{iusclb(izone)} = \texttt{idepo2}, the particles settle definitively on the boundary zone \texttt{izone} and they are kept in the calculation
   domain: the particles do not disappear after touching the boundary zone. However, using \texttt{idepo2} type zones necessitates more memory than using \texttt{idepo1} type zones.
 ...
 \item if \texttt{iusclb(izone)} = \texttt{iencrl}, the particles which are coal particles
       ...
       in mineral matters must be given in the
       \texttt{uslag1} subroutine.
...
\item \texttt{ruslag(iclas,izone,idebt)}: allows to impose a particle mass
      ...
      statistical weight \texttt{pepa(jrpoi,npt)} is recalculated in order to
...
\noindent
The user subroutine \texttt{uslain} can be used to complete \texttt{uslag2} when the particles must be injected in the domain according to fine constraints (profile, position, ...): the arrays \texttt{eptp}, \texttt{pepa} and \texttt{ipepa} can be modified here for the new particles (these arrays were previously completed automatically by the code from the data provided by the user in \texttt{uslag2}).

\noindent
In the case of a more advanced use, it is possible to modify here all the arrays \texttt{eptp}, \texttt{pepa} and \texttt{ipepa}. The particles already present in the calculation domain are marked out by an index varying between 1 and \texttt{nbpart}. The particles entering the calculation domain at the current iteration are marked out by an index varying between \texttt{nbpart}+1 and \texttt{nbpnew}.
...
%==================================
\minititre{User-defined volume statistics: \texttt{uslast} and \texttt{uslaen}}
%==================================
\noindent
An intervention in both \texttt{uslast} and \texttt{uslaen} subroutines
is required if the user wishes to obtain supplementary volumetric statistics.

\noindent
The subroutine \texttt{uslast} is called at the end of every Lagrangian
iteration, it allows therefore the modification of particless-related variables, or the extraction and preparation of data to display in
...
\begin{list}{-}{}
\item the calculation of the statistics is unsteady: \texttt{statis} is
      reset at every Lagrangian iteration;
\item the calculation of the statistics is steady: the array
...
steady in second part.

\noindent
In the subroutine \texttt{uslaen}, the variable whose volumetric statistic is wanted is
stored in the array \texttt{statis}. In the framework of steady statistics,
...
\hspace*{1cm}- either the duration of the steady statistics
...
\noindent
This subroutine is also used for the calculation of the average corresponding to the
cumulated value obtained in the subroutine \texttt{uslast} and for standard volumetric statistics. Several examples are
therefore provided.
...
\noindent
An intervention in the \texttt{uslaed} subroutine is required if supplementary user
variables are added to the particle state vector (arrays \texttt{eptp}
and \texttt{eptpa}). This subroutine is called at each Lagrangian sub-step.
...
total) available in \texttt{eptp(jvls(i),nbpmax)} and in
 \texttt{eptpa(jvls(i),nbpmax)},
...
particulate variables contained in \texttt{eptp} and \texttt{eptpa}. \\
...
\minititre{Particle cloning/merging technic}
%-------------------------------------------

\noindent
An intervention in the \texttt{uslaru}subroutine is required when the particle
...
zone of importance 2, it will  undergo a splitting process (clonning): the particle is replaced by two
identical but idependant particles, each having one-half the statistical weight of the initial
...
\item \texttt{call field\_set\_key\_id(ivarfl(isca(itempk)), kivisl, ...)}: molecular thermal conductivity, constant (\texttt{-1}) or variable (\texttt{0}).
...
{\em WARNING: In the electric module, all of the physical properties are considered as variables
 and are therefore stored in the \texttt{propce} array. \texttt{cp0}, \texttt{viscls0} and \texttt{viscl0}
 are not used}
...
The algebraic variables related to the electric module are provided by default as long as
...
This initialisation can be performed in the Graphical User Interface (GUI)
 or in the subroutines \texttt{usipph} and \texttt{usstr1}. Firstlyl,
...
When starting a calculation using ALE, or re-starting a calculation with ALE, based
 ...
 \text{usipph}, so that the code can deal with the arrays \text{xstr0}, \text{vstr0} and \text{xstreq}.
...
{damping coefficient of the structure, where \texttt{i},\texttt{j} is the array of
...
The selection of an atmospheric flow model can be made in the Graphical User Interface (GUI)
 under the heading ``Thermophysical models'' in the item ``Calculation features''
...
\begin{list}{-}{}
 ...
 \item ``dry atmosphere'': potential temperature is used as thermal variable in order to simulate dry, thermally-stratified
atmospheric flows,
