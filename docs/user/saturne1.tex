...
throughout the code kernel and the user subroutines. It also provides an easily
...
On the left panel, several leaves are available :
\begin{list}{$\bullet$}{}
\item \textbf{Introduction}: general introduction,
\item \textbf{Running a computation}: general user guide sections,
...
The case generator command \texttt{code\_saturne~create} places an example of launch script,
...
The main function, \texttt{define\_domain\_parameters}, allows defining
most parameters relative to case execution for the current
domain, including advanced options not accessible
through the GUI, or overriding options set through the GUI.

%==================================
\subsection{Graphical User Interface}
%==================================
\label{sec:prg_gui}%
A Graphical User Interface (GUI) is available with \CS.  This GUI creates or
reads an XML file according to a specific \CS schema which is then interpreted
by the code.

In version \verscs, the GUI manages calculation parameters, standard
initialisation values and boundary conditions for standard physics, pulverised
fuel combustion, gas combustion, atmospheric flows, Lagrangian module,
electrical model, compressible model and radiative transfers (user subroutines
can still be completed though).

The GUI is optional. Each setting or definition that can be specified through
the GUI can also be specified in the user subroutines. In case of conflict, all
calculation parameters, initialisation value or boundary condition set directly
in the user subroutines will prevail over what is defined by the GUI. However,
it is no longer necessary to redefine everything in the user subroutines. Only
what was not set or could not be set using the GUI should be specified.

{\em WARNING: }
There are some limitations to the changes that can be made between the GUI and
the user routines. In particular, it is not possible to specify a certain number
of solved variables in the GUI and change it in the user routines (for example,
it is not possible to specify the use of a $k-\varepsilon$ model in the GUI and
change it to $R_{ij}-\varepsilon$ in \texttt{cs\_user\_parameters.f90}, or to
define additional scalars in \texttt{cs\_user\_parameters.f90} with respect to
the GUI). Also, all boundaries should be referenced in the GUI, even if the
associated conditions are intended to be modified in
\texttt{cs\_user\_boundary\_conditions}, and their nature (entry, outlet,
wall\footnote{Smooth and rough walls are considered to have the same nature},
symmetry) should not be changed.
...
entered in the GUI will not be taken into account.

The GUI is launched with the \texttt{./code\_saturne} command in the directory
\texttt{DATA}. The first step is then to load an existing parameter file (in
order to modify it) or to open a new one. The settings to be given or checked
for a standard calculation are the following:

\begin{list}{-}{}
\item Calculation environment: definition of the mesh file(s),
      stand-alone execution of the Preprocessor module
      (used by the GUI to get the groups of the boundary faces).
...
up of some simple calculations with the \CS GUI.

To launch \CS using an XML parameter file, the name of the file must be given
using the \texttt{--param} option of \texttt{code\_saturne run} in the launch
script (see \S\ref{sec:prg_runcase}). When the launch script is edited from the
GUI (Calculation management $\rightarrow$ Prepare batch analysis), this option
...
As a reminder, it is generally preferable to use the GUI for as many settings
...
Note that when upgrading to a new \CS version, the GUI can
automatically update the XML file (and in case a few elements cannot be
updated, a warning will be issued). Whereas although an effort is made
so as not to break user-defined functions too often, user-defined functions
are guaranteed to be "stable" only within a same release series
(i.e. 6.1.*).

In general, user functions and subroutines are called after GUI-defined
settings are loaded, so that when a same parameter is specified both
in the interface and in a user-defined function or subroutine,
the value in the user function has priority, or rather has the last word.

%==================================
\subsubsection{User source files needed without the GUI}
%==================================

\begin{list}{}{}

\item {\bf For all physical models:}

    \begin{list}{}{}
        \item {\em  very useful without Graphical User Interface:}
        \begin{list}{-}{}
            \item \texttt{\texttt{cs\_user\_model}}
        ...
        \item {\em  very useful:}
        \begin{list}{-}{}
            \item \texttt{cs\_user\_physical\_properties.c} to manage variable physical
                  properties (fluid density, viscosity ...)

            \item \texttt{cs\_user\_initialization.c} to manage the non-standard initialisations
        \end{list}
    \end{list}

  \item{\bf For the ``gas combustion'', ``pulverized fuel combustion'',
        and ``heavy fuel combustion'' models:}
  ...
  \item{\bf For the ``atmospheric module'' models:}

    \begin{list}{}{}
       \item {\em compulsory without Graphical User Interface:}
        \begin{list}{-}{}
            \item \texttt{usppmo} (in \texttt{cs\_user\_parameters.f90})
                  to select the atmospheric module
        \end{list}
    ...
    \end{list}
     ...
     \item{\bf For the ``electric module'' (Joule effect and electric arcs):}
    ...
    \item{\bf For the ``Lagrangian module'' (dispersed phase):}
      ...
      (the continuous phase is managed in the regular)
    \begin{list}{}{}
        \item {\em compulsory without Graphical User Interface:}
        \begin{list}{-}{}
   ...
   \item {\bf For the ``compressible module'':}
    ...
    \begin{list}{}{}
        \item {\em compulsory without Graphical User Interface:}
        \begin{list}{-}{}
            \item \texttt{cs\_user\_model} (in \texttt{cs\_user\_parameters.c})
                  to select the compressible module
        \end{list}
        \item {\em very useful:}
        \begin{list}{-}{}
...
All checkpoint/restart files are binary files. Nonetheless, they may
be dumped or compared using the \texttt{cs\_io\_dump} tool.
