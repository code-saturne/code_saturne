...
\subsection{Run cases}\ref{sec:run}
...
the setup between two runs of the same case, an external script has to be used for
...
The purpose of this functionality is to create new data based on several
runs of cases, and to plot them (see section \ref{sec:curves}).

The markup \texttt{<postpro>} has to be added as a child of the condidered study.
...
\item How to make a grid convergence study ?
...
In order to change the name of the mesh inside the file of parameters of
the code a script \texttt{grid.py} has to be written in the directory
\texttt{MESH} of the study or in the directory \texttt{DATA} of the related
case:
