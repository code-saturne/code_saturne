...
from code_saturne.Pages.OutputSurfacicFieldsModel import OutputSurfacicFieldsModel
...
class StandardItemModelOutput(QStandardItemModel):
    ...
    def populateModel(self):
        ...
        for label in self.mdl.list_label:
            row = self.rowCount()
    ...
    def setData(self, index, value, role=None):
        row = index.row()
        if index.column() == 0:
            label = from_qvariant(value, to_text_string)
...
class OutputSurfacicFieldsView(QWidget, Ui_OutputSurfacicVariablesForm):
    """
    """
