...
class ThermalRadiationModel(Model):
    ...
    def _defaultRadiativeTRansferValues(self):
        """
    ...
    def _getCoalModel(self):
        ...
        if (ind == 1) or (ind == 2):
            ...
            val = self._defaultRadiativeTRansferValues()['radiative_model']
        return val
    ...
    def getRadiativeModel(self):
        ...
        if self.isCoalCombustion():
            ...
            if model not in self.radiativeModels:
                model = self._defaultRadiativeTRansferValues()['radiative_model']
    ...
    def getNbDir(self):
        ...
        if nb == None:
            nb = self._defaultRadiativeTRansferValues()['directions_number']
            self.setNbDir(nb)
        return nb
    ...
    def getRestart(self):
        ...
        status = node['status']
        if not status:
            status = self._defaultRadiativeTRansferValues()['restart_status']
    ...
    def getTypeCoeff(self):
        ...
        if self.isCoalCombustion():
            ...
            if not type:
                type = self._defaultRadiativeTRansferValues()['type_coef']
    ...
    def setTypeCoeff(self, type):
        ...
        """
        self.isInList(type, ('constant', 'variable', 'modak'))
        if self.isCoalCombustion():
            self._setTypeCoalCoeff(type)
        node  = self.node_ray.xmlInitNode('absorption_coefficient', 'type')
        node['type'] = type
    ...
    def getAbsorCoeff(self):
        ...
        if self.isCoalCombustion():
            ...
            if val == None:
                val = self._defaultRadiativeTRansferValues()['value_coef']
    ...
    def getFrequency(self):
        ...
        if freq == None:
            freq = self._defaultRadiativeTRansferValues()['frequency']
    ...
    def getIntensityResolution(self):
        ...
        if intens == None:
            intens = self._defaultRadiativeTRansferValues()['intensity']
    ...
    def getTemperatureListing(self):
        ...
        if tp == None:
            tp = self._defaultRadiativeTRansferValues()['tempP']
    ...
    def getTrs(self):
        ...
        if idiver == None:
            idiver = self._defaultRadiativeTRansferValues()['idiver']
            self.setTrs(idiver)
        return idiver
