 ...
 * \param[in]  epsil_cell   work array
 */
/*----------------------------------------------------------------------------*/

void cs_lagr_car(int iprev, const cs_real_t dt[], cs_real_t taup[],
                 cs_real_3_t tlag[], cs_real_3_t piil[], cs_real_33_t bx[],
                 cs_real_t tempct[], cs_real_3_t gradpr[],
                 cs_real_33_t gradvf[], cs_real_t energi[], cs_real_t energip[],
                 cs_real_t epsil_cell[]) {
  ...
  // CH: add local table for epsilon attached to particles
  cs_real_t *epsil_loc;
  BFT_MALLOC(epsil_loc, p_set->n_particles, cs_real_t);
  // CH: add local table for k attached to particles
  cs_real_t *energi_loc;
  BFT_MALLOC(energi_loc, p_set->n_particles, cs_real_t);
  ...
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    ...
    if (cell_id >= 0) {
      ...
      if (cs_glob_lagr_time_scheme->iadded_mass == 1)
        taup[ip] *= (1.0 + 0.5 * cs_glob_lagr_time_scheme->added_mass_const *
                               rom / p_rom);
      ...
      if ((cs_glob_lagr_model->physical_model == 1 &&
        ...
        cs_user_lagr_rt_t(ip, rep, rel_vel_norm, rom, p_rom, xnul, xcp, xrkl,
                          tempct, dt);
  ...
  /* Compute turbulent kinetic energy and epsilon
  ...
  if (cs_glob_lagr_time_scheme->idistu == 1) {
    ...
    if (extra->itytur == 2 || extra->iturb == 50) {
      ...
      for (cs_lnum_t cell_id = 0; cell_id < ncel; cell_id++) {
        ...
        epsil_cell[cell_id] = extra->cvar_ep->vals[iprev][cell_id];
      }
      ...
      if (extra->cvar_rij == NULL) {
        ...
        for (cs_lnum_t cell_id = 0; cell_id < ncel; cell_id++) {
          ...
          energi[cell_id] =
              0.5 * (extra->cvar_rij->vals[iprev][6 * cell_id] +
                     extra->cvar_rij->vals[iprev][6 * cell_id + 1] +
                     extra->cvar_rij->vals[iprev][6 * cell_id + 2]);
          epsil_cell[cell_id] = extra->cvar_ep->vals[iprev][cell_id];

          // CH: start (add energip from Lagr stats)
          int stat_type = cs_lagr_stat_type_from_attr_id(CS_LAGR_VELOCITY_SEEN);
          cs_field_t *var_vel_part = cs_lagr_stat_get_moment(
              stat_type, CS_LAGR_MOMENT_VARIANCE, 0, -1);

          energip[cell_id] = 0.5 * (var_vel_part->val[cell_id * 6 + 0] +
                                    var_vel_part->val[cell_id * 6 + 1] +
                                    var_vel_part->val[cell_id * 6 + 2]);
          // CH: end (add energip from Lagr stats)
        }
      }
    }
      ...
      for (cs_lnum_t cell_id = 0; cell_id < ncel; cell_id++) {
        energi[cell_id] = extra->cvar_k->vals[iprev][cell_id];
        epsil_cell[cell_id] = extra->cmu * energi[cell_id] *
      ...
      bft_printf(
          _("The lagrangian module is not compatible with the selected "
            "turbulence model.\n"
            "\n"
            "Turbulent dispersion is taken into account with IDISTU = %d\n"
            " Activated turbulence model is %d, when only k-eps, Rij-eps,\n"
            " V2f or k-omega are compatible with turbulent dispersion and "
            "Lagrangian module.\n"
            "\n"),
          (int)cs_glob_lagr_time_scheme->idistu, (int)extra->iturb);
      cs_exit(1);
    }
    ...
    for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
      ...
      if (cell_id >= 0) {
        ...
        if (epsil_cell[cell_id] > 0.0 && energi[cell_id] > 0.0) {
          ...
          // CH: start (interpolate epsilon for particles near walls)

          epsil_loc[ip] = epsil_cell[cell_id];

          // CH: start (interpolate k for particles near walls)

          energi_loc[ip] = energi[cell_id];

          // CH: end (interpolate epsilon for particles)

          cs_real_t tl = cl * energi_loc[ip] / epsil_loc[ip];
          ...
          uvwdif = (3.0 * uvwdif) / (2.0 * energi_loc[ip]);
          ...
          if (cs_glob_lagr_time_scheme->modcpl > 0 &&
            ...
            if (cs_glob_lagr_time_scheme->idirla == 1) {
              ...
              bft_printf(
                  _("\n WARNING: STOP AT LAGRANGIAN MODULE EXECUTION\n"));
              bft_printf(_("Complete model direction choice value is not "
                           "correct (in LAGCAR).\n"
            ...
            else if (extra->itytur == 2 || extra->iturb == 50 ||
                     extra->iturb == 60)
              ktil = energi_loc[ip];
            ...
            for (cs_lnum_t id = 0; id < 3; id++) {
              ...
              cs_real_t bxi =
                  epsil_loc[ip] *
                  ((c0 * bbi[id] * ktil / energi_loc[ip]) +
                   ((bbi[id] * ktil / energi_loc[ip] - 1.0) * 2.0 / 3.0));
              if (bxi > 0.0)
            ...
            bx[ip][0][nor - 1] = sqrt(c0 * epsil_loc[ip]);
