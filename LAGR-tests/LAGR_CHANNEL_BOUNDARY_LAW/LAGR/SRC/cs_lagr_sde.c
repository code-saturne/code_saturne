        ...
        cs_real_3_t        *fextla)
{
  ...
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
	...
	= cs_lagr_particle_get_real(particle, p_am, CS_LAGR_RANDOM_VALUE);
      if ( CS_ABS(particle_random - 0.084) > 0.00) {
	bft_printf(" Part id: %d and rand %7.3e \n",
		   ip, particle_random);
	// Coords, vel and vel_seen
	bft_printf("   Start \n");
	bft_printf("   Part coords: X %7.3e, Y %7.3e, Z %7.3e \n",
		   part_coords[0], part_coords[1], part_coords[2] );
	bft_printf("   Part vel: Up %7.3e, Vp %7.3e, Wp %7.3e \n",
		   part_vel[0], part_vel[1], part_vel[2] );
	bft_printf("   Vel seen: Us %7.3e, Vs %7.3e, Ws %7.3e \n",
		   part_vel_seen[0], part_vel_seen[1], part_vel_seen[2] );
	// Tlag
	bft_printf("   Components \n");
	bft_printf("   Terms for Tlag: %7.3e %7.3e %7.3e  \n",
		   tlag[ip][0], tlag[ip][1], tlag[ip][2]);
	bft_printf("   Terms for taup: %7.3e \n",
		   taup[ip]);
	// Grad P
	bft_printf("   Terms for gradP: %7.3e %7.3e %7.3e  \n",
		   gradpr[cell_id][0], gradpr[cell_id][1], gradpr[cell_id][2]);
	bft_printf("   Terms for piil: %7.3e %7.3e %7.3e  \n",
		   piil[ip][0], piil[ip][1], piil[ip][2]);
	bft_printf("   Terms for grav: %7.3e %7.3e %7.3e  \n",
	...
	if (id ==0)
	  vitf = mean_vel_part->val[cell_id * 3 + id];
	else
	  vitf = 0.0;
	*/
	//CH: end (use Us instead of Uf)
	

        /* --> (2.1) Calcul preliminaires :    */
        /* ----------------------------   */
        /* calcul de II*TL+<u> et [(grad<P>/rhop+g)*tlag+<Uf>] ?  */
	...
	/*
	bft_printf("In sde: scheme added mass? %d F = %7.3e \n",
		   cs_glob_lagr_time_scheme->iadded_mass, force);
	*/
        ...
        /* --> trajectoire */
	// CH: start (modification to have no motion except on Z direction)
	if ( id == 2) {
	...
	// CH: start (print all)
	/*
	bft_printf("   Terms for coords: %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e \n",
		   ter1x, ter2x, ter3x, ter4x, ter5x, tbrix1, tbrix2);
	bft_printf("   Terms for vel: %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e \n",
		   ter1p, ter2p, ter3p, ter4p, ter5p, tbriu);
	bft_printf("   Terms for vel_seen: %7.3e %7.3e %7.3e and tci %7.3e \n",
		   ter1f, ter2f, ter3f, tci);
	*/
	// CH: end (print all)
            ...
            cs_lnum_t          *nresnew)
{
  ...
  if (cs_glob_lagr_time_scheme->idistu == 1) {
    if (cs_glob_lagr_time_step->nor > 1) {
      ...
      // CH: start (modify generation of random numbers)
      
      for (cs_lnum_t id = 0; id < 3; id++) {
	for (cs_lnum_t ivf = 0; ivf < 3; ivf++) {
	  cs_real_t *_v_gauss;
	  BFT_MALLOC(_v_gauss, p_set->n_particles, cs_real_t);
	  cs_random_normal(p_set->n_particles, &(_v_gauss[0]));
	  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++)
	    vagaus[ip][id][ivf] = _v_gauss[ip];
	  BFT_FREE(_v_gauss);
	}
      }
      ...
      */
      ...
      // CH: end (modify generation of random numbers)
    }
  }
  ...
  //CH: start (check random numbers)
  /*
  // mean
  cs_real_3_t mean_rand;
  mean_rand[0] = 0;  mean_rand[1] = 0;  mean_rand[2] = 0;
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    mean_rand[0] +=  vagaus[ip][0][0];
    mean_rand[1] +=  vagaus[ip][1][0];
    mean_rand[2] +=  vagaus[ip][2][0];
  }
  mean_rand[0] /= p_set->n_particles;
  mean_rand[1] /= p_set->n_particles;
  mean_rand[2] /= p_set->n_particles;
  
  // variance
  cs_real_3_t var_rand;
  var_rand[0] = 0;  var_rand[1] = 0;  var_rand[2] = 0;
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    var_rand[0] +=  pow( vagaus[ip][0][0] - mean_rand[0] , 2);
    var_rand[1] +=  pow( vagaus[ip][1][0] - mean_rand[1] , 2);
    var_rand[2] +=  pow( vagaus[ip][2][0] - mean_rand[2] , 2);
  }
  var_rand[0] = sqrt(var_rand[0] / p_set->n_particles);
  var_rand[1] = sqrt(var_rand[1] / p_set->n_particles);
  var_rand[2] = sqrt(var_rand[2] / p_set->n_particles);
  
  bft_printf("chtime: %d mean: %7.3e %7.3e %7.3e var:  %7.3e %7.3e %7.3e\n",
	     cs_glob_time_step->nt_cur,
	     mean_rand[0], mean_rand[1], mean_rand[2],
	     var_rand[0] , var_rand[1] , var_rand[2]  );
  //CH: end (check random numbers)

  //CH: start (force mean, var of rand numb)
  cs_real_t *nbp_cell;
  BFT_MALLOC(nbp_cell, 20, cs_real_t);
  cs_real_3_t *mean_rng;
  BFT_MALLOC(mean_rng, 20, cs_real_3_t);
  cs_real_3_t *var_rng;
  BFT_MALLOC(var_rng, 20, cs_real_3_t);

  // Init
  for (cs_lnum_t iel = 0; iel < 20; iel++) {
    nbp_cell[iel] = 0.0;
    for (cs_lnum_t ivf = 0; ivf < 3; ivf++) {
      mean_rng[iel][ivf] = 0.0;
      var_rng[iel][ivf] = 0.0;
    }
  }
  
  // Loop on particles to calculate mean and variance
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    unsigned char *particle = p_set->p_buffer + p_am->extents * ip;
    cs_lnum_t cell_id = cs_lagr_particle_get_cell_id(particle, p_am);
    nbp_cell[cell_id] +=  1;
    for (cs_lnum_t ivf = 0; ivf < 3; ivf++) {
      mean_rng[cell_id][ivf] +=  vagaus[ip][ivf][0];
      var_rng[cell_id][ivf] +=  pow(vagaus[ip][ivf][0],2);
    }
  }
  // Finish calculation
  for (cs_lnum_t iel = 0; iel < 20; iel++) {
    for (cs_lnum_t ivf = 0; ivf < 3; ivf++) {
      mean_rng[iel][ivf] /=  nbp_cell[iel];
      var_rng[iel][ivf] /= nbp_cell[iel] ;
      var_rng[iel][ivf] -= pow(mean_rng[iel][ivf],2);
    }
  }
  ...
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    ...
    cs_lnum_t cell_id = cs_lagr_particle_get_cell_id(particle, p_am);
    for (cs_lnum_t ivf = 0; ivf < 3; ivf++) {
      vagaus[ip][ivf][0] =  (vagaus[ip][ivf][0]-mean_rng[cell_id][ivf]) / sqrt(var_rng[cell_id][ivf]);
    }
  }
  ...
  //CH: end (force mean, var of rand numb)
