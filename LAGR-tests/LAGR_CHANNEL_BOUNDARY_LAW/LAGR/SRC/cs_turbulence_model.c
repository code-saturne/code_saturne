 ...
 */
double cs_turb_crij1 = 1.0 + (3./2.) * 3.5 ; // CH: Modification for Rotta model 1.80;

/*
 * Constant \f$C_2\f$ for the \f$R_{ij}-\varepsilon\f$ LRR model.
 * Useful if and only if \ref iturb=30 (\f$R_{ij}-\varepsilon\f$ LRR).
 */
double cs_turb_crij2 = 0.0; // CH: modification for Rotta model 0.60;
...
cs_turb_compute_constants(void)
{
  ...
  /* LRR constants */
  cs_turb_crij1 = 1. + (3./2.) * 2.1 ; // CH: modification for Rotta model (previously 1.80)
  cs_turb_crij2 = 0.0; // CH: modification for Rotta model (previously 0.6);
}
