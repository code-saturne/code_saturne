                    ...
                    int                       *p_move_particle)
{
  ...
  if (   b_type == CS_LAGR_OUTLET
    ...
    cs_real_t particle_random
      = cs_lagr_particle_get_real(particle, p_am, CS_LAGR_RANDOM_VALUE);
    // CH: start debug (print)
    /*
    bft_printf("Start rebound \n");
    bft_printf("   Part id: %7.3f and cur_cell: %d \n",
	       particle_random, cur_cell_id);
    bft_printf("   Part coords: X %7.3f, Y %7.3f, Z %7.3f \n",
	       particle_coord[0], particle_coord[1], particle_coord[2] );
    bft_printf("   Part vel: Up %7.3f, Vp %7.3f, Wp %7.3f \n",
	       particle_velocity[0], particle_velocity[1], particle_velocity[2] );
    bft_printf("   Vel seen: Us %7.3f, Vs %7.3f, Ws %7.3f \n",
	       particle_velocity_seen[0], particle_velocity_seen[1], particle_velocity_seen[2] );
    */
    // CH: end debug (print)
    ...
    */
    ...
    cs_real_t ustar = 0.05; //extra->uetbor[face_id];
	...
	if ( cs_glob_time_step->nt_cur == 80001) {
	  particle_velocity_seen[0] -= 2. * tmp * face_norm[2]
	  ...
	  //*(-pow(ustar,2)) / extra->cvar_rij->vals[iprev][6 * cur_cell_id + 2];
	  //
	  //* extra->cvar_rij->vals[iprev][6 * cur_cell_id + 5]
	  /// extra->cvar_rij->vals[iprev][6 * cur_cell_id + 2];
	  //
	  //* extra->cvar_rij->vals[iprev][6 * cur_cell_id + 5]
	  /// var_vel_part->val[cur_cell_id * 6 + 2];
	}
    ...
    else if ( extra->iturb == 20
	   || extra->iturb == 21) {
      
      const cs_real_t *cvar_k = CS_F_(k)->val;
      particle_velocity_seen[0] -= 2 * tmp * face_norm[2] * (-pow(ustar,2))/(2/3*cvar_k[cur_cell_id]+1e-30);
      particle_velocity_seen[1] -= 2 * tmp * face_norm[2] * (-pow(ustar,2))/(2/3*cvar_k[cur_cell_id]+1e-30);
      particle_velocity_seen[2] -= 2 * tmp * face_norm[2];
    }

    // CH: start debug
    /*
    bft_printf("After rebound \n");
    bft_printf("   Part id: %7.3f and cur_cell: %d \n",
	       particle_random, cur_cell_id);
    bft_printf("   Part coords: X %7.3f, Y %7.3f, Z %7.3f \n",
	       particle_coord[0], particle_coord[1], particle_coord[2] );
