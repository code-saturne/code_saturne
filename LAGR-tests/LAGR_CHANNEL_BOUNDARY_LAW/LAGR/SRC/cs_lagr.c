                        ...
                        const cs_real_t  *dt)
{
  ...
  for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
    cs_real_t *particle_coord = cs_lagr_particles_attr(p_set, ip, CS_LAGR_COORDS);
    cs_real_t *particle_vel = cs_lagr_particles_attr(p_set, ip, CS_LAGR_VELOCITY);
    cs_real_t *particle_vel_seen = cs_lagr_particles_attr(p_set, ip, CS_LAGR_VELOCITY_SEEN);
    bft_printf("Injection \n");
    bft_printf("   Part coord: X %7.3f, Y %7.3f, Z %7.3f \n",
	       particle_coord[0], particle_coord[1], particle_coord[2] );
    bft_printf("   Part vel: X %7.3f, Y %7.3f, Z %7.3f \n",
	       particle_vel[0], particle_vel[1], particle_vel[2] );
    bft_printf("   Vel seen: X %7.3f, Y %7.3f, Z %7.3f \n",
	       particle_vel_seen[0], particle_vel_seen[1], particle_vel_seen[2] );
  ...
  */
  ...
  /* Record particle's starting cell and rank, and update rebound time id */
    ...
    while (go_on) {
      ...
      // CH: start (gradP modified for 1D case)
      
      for (cs_lnum_t iel = 0; iel < 20; iel++) {
	cs_real_t ustar = 0.05; 
        cs_real_t romf = extra->cromf->val[iel];
	extra->vel->vals[1][iel * 3 + 1] = 0;
	extra->vel->vals[1][iel * 3 + 2] = 0;
	//extra->cvar_ep->vals[iprev][iel] = 1.0;
	//gradpr[iel][0] = - ustar * ustar * romf / 1.0 ;
	...
	int stat_type = cs_lagr_stat_type_from_attr_id(CS_LAGR_VELOCITY_SEEN);
        cs_field_t *mean_vel_part =
          cs_lagr_stat_get_moment( stat_type, CS_LAGR_MOMENT_MEAN, 0, -1);
	bft_printf("chstat: %7.3e %7.3e %7.3e \n",
		   mean_vel_part->val[iel * 3 + 0],
		   mean_vel_part->val[iel * 3 + 1],
		   mean_vel_part->val[iel * 3 + 2]   );
	bft_printf("gradP: %7.3e %7.3e %7.3e \n",
		   gradpr[iel][0],
		   gradpr[iel][1],
		   gradpr[iel][2] );
	*/
      ...
      /*
      for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
	cs_real_t  particle_coord = cs_lagr_particles_get_real(p_set, ip,
								CS_LAGR_COORDS);
	bft_printf("   Part coords: X %7.3f, Y %7.3f, Z %7.3f \n",
		   particle_coord[0], particle_coord[1], particle_coord[2] );

      }
      */
      // CH: end (gradP modified)
      ...
      // CH: start (find and print bug particles near wall)
      /*
      if ( cs_glob_time_step->nt_cur > 88000)
	cs_exit(1);
      */
      // CH: end
      ...
      //CH: start
      //MODIFICATION DE Us - version C. Henry (ancien) //
      /*
      if (1 == 1) {
        ...
        for (cs_lnum_t npt = 0; npt < p_set->n_particles; npt++) {
          ...
          cs_lnum_t iel  = cs_lagr_particles_get_lnum(p_set, npt, CS_LAGR_CELL_NUM)-1;
        
          if (iel >= 0) {

            cs_real_t *part_vel_seen = cs_lagr_particles_attr(p_set, npt, CS_LAGR_VELOCITY_SEEN);
            cs_real_t *part_vel = cs_lagr_particles_attr(p_set, npt, CS_LAGR_VELOCITY);

            for (cs_lnum_t id = 0; id < 3; id++) {
              part_vel_seen[id] -= (mean_vel_part->val[iel * 3 + id] -
                               extra->vel->vals[1][iel * 3 + id]);
              part_vel[id] = part_vel_seen[id];
            }
	...
	// CH: start (home made calculation of Lagr. stats)
	/*
	// Alloc.
	cs_real_t *sum_Np;
	cs_real_3_t *mean_Vp, *mean_Vs; 
	cs_real_33_t *var_Vp, *var_Vs;
	BFT_MALLOC(sum_Np , p_set->n_particles, cs_real_t   );
	BFT_MALLOC(mean_Vp, p_set->n_particles, cs_real_3_t );
	BFT_MALLOC(var_Vp , p_set->n_particles, cs_real_33_t);
	BFT_MALLOC(mean_Vs, p_set->n_particles, cs_real_3_t );
	BFT_MALLOC(var_Vs , p_set->n_particles, cs_real_33_t);
	// Init.
	for (cs_lnum_t iel = 0; iel < 20; iel++) {
	  sum_Np[iel] = 0.;
	  for (cs_lnum_t id = 0; id < 3; id++) {
	...
	for (cs_lnum_t npt = 0; npt < p_set->n_particles; npt++) {
	  ...
	  if (iel >= 0) {
	    ...
	    for (cs_lnum_t id = 0; id < 3; id++) {
	      mean_Vs[iel][id] += part_vel_seen[id] ;
	      mean_Vp[iel][id] += part_vel[id] ;
	    }
	  }
	}
	...
	for (cs_lnum_t iel = 0; iel < 20; iel++) {
	  for (cs_lnum_t id = 0; id < 3; id++) {
	    mean_Vs[iel][id] /= sum_Np[iel];
	    mean_Vp[iel][id] /= sum_Np[iel];
	  }
	}
	// Compute var
	for (cs_lnum_t npt = 0; npt < p_set->n_particles; npt++) {
	  ...
	  if (iel >= 0) {
	    ...
	    for (cs_lnum_t id = 0; id < 3; id++) {
	      for (cs_lnum_t id2 = 0; id2 < 3; id2++) {
		var_Vs[iel][id][id2] += (part_vel_seen[id] - mean_Vs[iel][id])
		  * (part_vel_seen[id2] - mean_Vs[iel][id2]) ;
		var_Vp[iel][id][id2] += (part_vel[id] - mean_Vp[iel][id])
		  * (part_vel[id2] - mean_Vp[iel][id2]) ;
	      }
	    }
	  }
	}
	...
	for (cs_lnum_t iel = 0; iel < 20; iel++) {
	  for (cs_lnum_t id = 0; id < 3; id++) {
	    for (cs_lnum_t id2 = 0; id2 < 3; id2++) {
	      var_Vs[iel][id][id2] /= sum_Np[iel] ;
	      var_Vp[iel][id][id2] /= sum_Np[iel] ;
	    } 
	  }
	}
	*/
	// CH: end (home made calculation of Lagr. stats)
	...
	for (cs_lnum_t iel = 0; iel < 20; iel++) {
	  ...
	  bft_printf("%d  CHMeanSat%d: %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel,
		     mean_vel_part->val[iel * 3 + 0] ,
		     mean_vel_part->val[iel * 3 + 1] ,
		     mean_vel_part->val[iel * 3 + 2] );
	  bft_printf("%d  CHMeanOwn%d: %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel,
		     mean_Vp[iel][0] ,
		     mean_Vp[iel][1] ,
		     mean_Vp[iel][2] );
	  bft_printf("%d  CHVarSat%d: %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel ,
		     var_vel_part->val[iel * 6 + 0] ,
		     var_vel_part->val[iel * 6 + 1] ,
		     var_vel_part->val[iel * 6 + 2] ,
		     var_vel_part->val[iel * 6 + 3] ,
		     var_vel_part->val[iel * 6 + 4] , 
		     var_vel_part->val[iel * 6 + 5]  );
	  bft_printf("%d  CHVarOwn%d: %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel,
		     (var_Vp[iel][0][0]) ,
		     (var_Vp[iel][1][1]) ,
		     (var_Vp[iel][2][2]) ,
	...
	if (1 == 1) {
	  ...
	  for (cs_lnum_t npt = 0; npt < p_set->n_particles; npt++) {
	    ...
	    if (iel >= 0) {
	      ...
	      for (cs_lnum_t id = 0; id < 3; id++) {
		part_vel_seen[id] -= (mean_Vp[iel][id] -
				      extra->vel->vals[1][iel * 3 + id]);
		part_vel[id] = part_vel_seen[id];
	      }
	    }
	...
	*/
	// Go in stat update to recalculate stats properly
	cs_lagr_stat_update();
	...
	*/
	...
	if (iel == 19 || iel == 9) {
	  bft_printf("%d  CHvalMean%d: %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel,
		     mean_vel_part->val[iel * 3 + 0],
		     mean_vel_part->val[iel * 3 + 1],
		     mean_vel_part->val[iel * 3 + 2]   );
	  bft_printf("%d  CHvalVar%d: %7.3e %7.3e %7.3e %7.3e %7.3e %7.3e \n",
		     cs_glob_time_step->nt_cur, iel,
		     var_vel_part->val[iel * 6 + 0],
		     var_vel_part->val[iel * 6 + 1],
		     var_vel_part->val[iel * 6 + 2],
	...
	//for (cs_lnum_t id = 0; id < 3; id++) {
	//  var_vel_part->val[iel * 6 + id] += pow(mean_vel_part->val[iel * 3 + id],2.);
	//}
