  ...
  /* Record particle's starting cell and rank, and update rebound time id */
    ...
    while (go_on) {
      ...
      if (cs_glob_lagr_time_step->nor == 1) {
        ...
        bft_printf("IN LAGR, n_particles_prev=%d, n_particles=%d\n", n_particles_prev, p_set->n_particles);
      ...
      cs_real_t tl = cs_notebook_parameter_value_by_name("tlag");
      cs_real_t _bx = cs_notebook_parameter_value_by_name("bx");
      for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {

//        taup[ip] = 0.1; // can be set in cs_user_lagr_rt
        for (cs_lnum_t ii = 0; ii < 3; ii++) {

          tlag[ip][ii] = tl; // = cl k/epsi with cl = 1/(0.5 + 0.75 *c0)

          for (cs_lnum_t nor_loc = 0; nor_loc < 3; nor_loc++)

            bx[ip][ii][nor_loc] = _bx; // \sqrt(c0 epsi) if modcpl == 0, idiffl !=0
