                        ...
                        const cs_real_t  *dt)
{
  ...
  /* Record particle's starting cell and rank, and update rebound time id */
    ...
    while (go_on) {
      ...
      //*** START MOD: paspro : modify values of coefficients to specific values
      for (cs_lnum_t ip = 0; ip < p_set->n_particles; ip++) {
        ...
        for (cs_lnum_t ii = 0; ii < 3; ii++) {
          ...
          for (cs_lnum_t nor_loc = 1; nor_loc < 3; nor_loc++)
            
            bx[ip][ii][nor_loc-1] = 1000;
  ...
  if (ts->nt_cur ==1) {
    ...
    fprintf(file1,"# Time, Omega, Gamma, gamma, upus, <xp2>, <Up2>, <Us2>, <UpUs> \n");
    fclose(file1);
  }
  ...
  if (1==1) {
    cs_real_t Bx_p = 1000.0;
    cs_real_t taup = 0.00002;
    cs_real_t tlag = 0.00001;
    cs_real_t dtot = cs_glob_lagr_time_step->ttclag;
    
    cs_real_t Omega_xp = pow(Bx_p * tlag/(tlag-taup),2) * 
      ( pow(tlag - taup, 2) * dtot +
        pow(tlag,3) * 0.5 * ( 1 - exp(-2*dtot/tlag) ) +
        pow(taup,3) * 0.5 * ( 1 - exp(-2*dtot/taup) ) -
        2* pow(tlag,2) * (tlag-taup) * ( 1 - exp(-dtot/tlag) ) +
        2* pow(taup,2) * (tlag-taup) * ( 1 - exp(-dtot/taup) ) -
        2* pow(tlag * taup,2) / (tlag+taup) * ( 1 - exp(-dtot/tlag)*exp(-dtot/taup) )
        );
    cs_real_t Gamma_up = pow(Bx_p * tlag/(tlag-taup),2) *
      ( 0.5 * tlag * ( 1 - exp(-2*dtot/tlag) ) +
        0.5 * taup * ( 1 - exp(-2*dtot/taup) ) -
        2* tlag * taup / (tlag+taup) * ( 1 - exp(-dtot/tlag)*exp(-dtot/taup) )
        );
    cs_real_t gamma_us =  pow(Bx_p ,2) *
      0.5 * tlag * ( 1 - exp(-2*dtot/tlag) );

    cs_real_t gamma_usup =  pow(Bx_p ,2) * tlag / (tlag - taup) * tlag * 
      ( 0.5 * (1 - exp(-2*dtot/tlag))
        - taup/(taup+tlag) * (1 - exp(-dtot/tlag)*exp(-dtot/taup)) );

    cs_real_t var_xpxp = 0.0;
    cs_real_t var_UpUp = 0.0;
    cs_real_t var_UsUs = 0.0;
    cs_real_t var_UpUs = 0.0;
    
    for (cs_lnum_t npt = 0; npt < p_set->n_particles; npt++) {

      cs_real_t *part_vel      = cs_lagr_particles_attr(p_set, npt,
                                                       CS_LAGR_VELOCITY);
      cs_real_t *part_vel_seen = cs_lagr_particles_attr(p_set, npt,
                                                       CS_LAGR_VELOCITY_SEEN);
      cs_real_t *part_coords   = cs_lagr_particles_attr(p_set, npt,
                                                       CS_LAGR_COORDS);
      
      var_xpxp +=
        pow(part_coords[0] - 5.0, 2) +
        pow(part_coords[1] - 5.0 , 2) +
        pow(part_coords[2] - 5.0 , 2) ;
    ...
    fprintf(file1," %e %e %e %e %e %e %e %e %e \n",
           ...
           gamma_us,
           gamma_usup,
           var_xpxp / p_set->n_particles / 3.0,
           var_UpUp / p_set->n_particles / 3.0,
           var_UsUs / p_set->n_particles / 3.0,
           var_UpUs / p_set->n_particles / 3.0 );
  }
  fclose(file1);
