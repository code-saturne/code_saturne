...
DOUBLE PRECISION UR11,UA,UC,AK0,EP0,XTETA
...
call getfbr('Injection or Inlet', nlelt, lstelt)
...
do ilelt = 1, nlelt
  ...
  radius = SQRT( CDGFBO(1,IFAC)*CDGFBO(1,IFAC) + CDGFBO(2,IFAC)*CDGFBO(2,IFAC) )
  CALL INLET(UR11,UA,UC,AK0,EP0,radius,R11,R12,R13,R22,R23,R33)

  IF ( CDGFBO(1,IFAC) .NE. 0.D0 .AND. CDGFBO(2,IFAC) .NE. 0.D0       ) THEN
    XTETA = atan(CDGFBO(2,IFAC)/CDGFBO(1,IFAC))
  ...
  rcodcl(ifac,iu,1) = UR11*COS(XTETA)-UC*SIN(XTETA)
  rcodcl(ifac,iv,1) = UR11*SIN(XTETA)+UC*COS(XTETA)
  rcodcl(ifac,iw,1) = UA

  rcodcl(ifac,ir11,1) = 2.D0/3.D0 * AK0
  rcodcl(ifac,ir12,1) = 0.D0
  rcodcl(ifac,ir13,1) = 0.D0
  rcodcl(ifac,ir22,1) = 2.D0/3.D0 * AK0
  rcodcl(ifac,ir23,1) = 0.D0
  rcodcl(ifac,ir33,1) = 2.D0/3.D0 * AK0

  rcodcl(ifac,iep,1)  = EP0
