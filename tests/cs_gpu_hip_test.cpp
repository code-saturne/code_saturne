                     ...
                     T       *g_odata)
{
  ...
  for (int j = blockSize/2; j > CS_HIP_WARP_SIZE; j /= 2) {
    if (tid < j) {
  ...
  if (tid < CS_HIP_WARP_SIZE) cs_blas_hip_warp_reduce_sum<blockSize, 1>(sdata, tid);
                     ...
                     double      *b_res)
{
  ...
  if (tid < CS_HIP_WARP_SIZE)
    cs_blas_hip_warp_reduce_sum<blockSize, 1>(stmp, tid);
...
_dot_xy_stage_1_of_2_wr(cs_lnum_t    n,
                        ...
                        double      *b_res)
{
  __shared__ double stmp[blockSize/CS_HIP_WARP_SIZE + 1];
  ...
  if ((tid % CS_HIP_WARP_SIZE) == 0) {
    stmp[tid / CS_HIP_WARP_SIZE] = t_sum;
  }
                 ...
                 double      *res)
{
  ...
  __shared__ double stmp[blockSize / CS_HIP_WARP_SIZE];
  ...
  const unsigned warp_id = threadIdx.x / CS_HIP_WARP_SIZE;
...
//#if (CUDART_VERSION >= 11000)
#if 0
    hipDeviceProp prop = {0};
    hipGetDeviceProperties(&prop, cs_glob_hip_device_id);
    int n_max_blocks
    ...
    if (n > 1) {
      hipOccupancyMaxPotentialBlockSize(&n_blocks_cg, &n_threads_cg,
...
//#if (CUDART_VERSION >= 11000)
            {
              ...
              hipLaunchCooperativeKernel((void *)_dot_xy_cg_single_pass<double>,
      ...
      if (_global == 0)
        bft_printf("\n"
                   "HIP dot product X.Y %s (%d elts., %g err)\n"
                   "----------------\n",
                   dot_name[j], (int)n, fabs(s1 -ref_s));
      else
        bft_printf("\n"
                   "HIP + MPI dot product X.Y %s (%d elts/rank)\n"
                   "----------------------\n",
                   dot_name[j], (int)n);
