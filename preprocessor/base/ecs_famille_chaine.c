                                ...
                                ecs_tab_int_t        **tab_lst_descr)
{
  ...
  /* Si les préfixes ne sont pas fournis, on en prévoit un par défaut */
  ...
  /* Création des listes d'éléments associés à chaque famille */
  /*----------------------------------------------------------*/

  ECS_MALLOC(nbr_elt_fam, num_fam_max + 1, ecs_int_t);
  ECS_MALLOC(lst_elt_fam, num_fam_max + 1, ecs_int_t *);
  ...
  for (ielt = 0; ielt < n_elts; ielt++)
    nbr_elt_fam[elt_fam[ielt]] += 1;

  for (ifam = 0; ifam <= num_fam_max; ifam++) {
    if (nbr_elt_fam[ifam] > 0)
      ECS_MALLOC(lst_elt_fam[ifam], nbr_elt_fam[ifam], ecs_int_t);
  }

  /* Détermination des attributs et familles associées */
  /*---------------------------------------------------*/
        ...
        if (tab_fam.nbr == tab_fam_cmp.nbr) {
          ...
          if (ifam == (int)(tab_fam.nbr)) {
            ...
            break; /* sortie de la boucle sur idescr_cmp, ainsi chaque
                       descripteur ayant un équivalent pointe sur
                       l'équivalent suivant (sorte de liste chaînée) */
          }

        } /* fin si le nombre de familles associé est le même */
  ...
  /* Création des listes associées aux descripteurs */
  /*------------------------------------------------*/

  ECS_MALLOC(nbr_elt_descr, tab_descr.nbr, ecs_int_t);
  ECS_MALLOC(lst_elt_descr, tab_descr.nbr, ecs_int_t *);

  for (idescr = 0; idescr < (int)(tab_descr.nbr); idescr++) {

    ecs_tab_int_t tab_fam = tab_fam_descr[tab_descr.val[idescr]];

    /* En cas d'équivalence, on utilisera la liste du descripteur
       en fin de liste d'équivalence */
    ...
    /* Sans équivalence ou pour le dernier descripteur équivalent,
    ...
    ECS_MALLOC(lst_elt_descr[idescr], nbr_elt_descr[idescr], ecs_int_t);
  ...
  ECS_MALLOC(tab_nom_descr->val, tab_nom_descr->nbr, char *);
  ECS_MALLOC(*tab_lst_descr, tab_nom_descr->nbr, ecs_tab_int_t);
  ...
  for (idescr = 0; idescr < (int)(tab_descr.nbr); idescr++) {
    ...
    case ECS_DESCR_COULEUR:
      {
        const int ide = ecs_descr__ret_ide(descr);

        int ide_div = ECS_ABS(ide);
        int l_str_ide = 0;
        ...
        ECS_MALLOC(tab_nom_descr->val[idescr],
    ...
    for (idescr_cmp = 0; idescr_cmp < idescr; idescr_cmp++) {
      if (descr_equiv[idescr_cmp] == idescr) {
        ECS_REALLOC(tab_nom_descr->val[idescr_cmp],
    ...
    /* Tri des tableaux et affectation à la liste finale */
    ...
    *(*tab_lst_descr + idescr) = ecs_tab_int__trie_et_renvoie(tab_elt_descr,
    ...
    tab_elt_descr.nbr = 0;
    ECS_FREE(tab_elt_descr.val);

  }

  ECS_FREE(nbr_elt_descr);
  ECS_FREE(lst_elt_descr);
  ...
  if (nbr_elt_fam[0] > 0) {
    ...
    tab_nom_descr->nbr += 1;
    ECS_REALLOC(tab_nom_descr->val, tab_nom_descr->nbr, char *);
    tab_nom_descr->val[tab_nom_descr->nbr - 1] = NULL;

    ECS_REALLOC(*tab_lst_descr, tab_nom_descr->nbr, ecs_tab_int_t);
    (*tab_lst_descr + tab_nom_descr->nbr - 1)->nbr = nbr_elt_fam[0];
    (*tab_lst_descr + tab_nom_descr->nbr - 1)->val = lst_elt_fam[0];
    lst_elt_fam[0] = NULL; /* propriété du tableau transférée */
  ...
  for (ifam = 0; ifam <= num_fam_max; ifam++) {
    if (lst_elt_fam[ifam] != NULL)
      ECS_FREE(lst_elt_fam[ifam]);
  }

  ECS_FREE(lst_elt_fam);
  ECS_FREE(nbr_elt_fam);

  ECS_FREE(descr_equiv);
