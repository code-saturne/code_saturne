...
cs_turbulence_kw(int phase_id)
{
  ...
  if (cs_glob_turb_rans_model->has_buoyant_term == 1) {
    ...
    if (cs_glob_velocity_pressure_model->idilat == 0) {
      ...
      cs_gradient_scalar("cromo_grad",
                         ...
                         1,             /* w_stride */
                         eqp_k->verbosity,
                         (cs_gradient_limit_t)(eqp_k->imligr),
                         eqp_k->epsrgr,
                         eqp_k->climgr,
                         NULL,
                         &bc_coeffs_loc,
...

  {
    ...
    if (cs_glob_turb_rans_model->ikecou == 0) {
      ...
      ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t c_id) {
        ...
        cs_real_t fhybr;
        if (hybrid_turb == 1 || hybrid_turb == 2)
          fhybr = w1[c_id];
        else if (hybrid_turb == 4)
  ...
  if (clip_w_id >= 0)
    cs_array_real_fill_zero(n_cells, cpro_w_clipped);
