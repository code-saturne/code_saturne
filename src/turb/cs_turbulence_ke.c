                 ...
                 cs_real_t       *prdv2f)
{
  ...
  cs_real_t *slist, *wlist;
  BFT_MALLOC(slist, n_cells_ext, cs_real_t);
  BFT_MALLOC(wlist, n_cells_ext, cs_real_t);
  ...
  if (eqp_k->verbosity >= 1) {
    if (cs_glob_turb_model->iturb == CS_TURB_K_EPSILON) {
      ...
      cs_log_printf(CS_LOG_DEFAULT,
                    "\n"
                    "  ** Solving k-epsilon, Quadratic Baglietto & al\n"
                    "     -----------------\n");
    }
    else if (cs_glob_turb_model->iturb == CS_TURB_K_EPSILON_CUBIC) {
      cs_log_printf(CS_LOG_DEFAULT,
                    "\n"
                    "  ** Solving k-epsilon, Cubic Baglietto & al\n"
                    "     -----------------\n");
    }
    else {
      cs_log_printf(CS_LOG_DEFAULT,
...
# pragma omp parallel for if(n_cells_ext > CS_THR_MIN)
  for (cs_lnum_t c_id = 0; c_id < n_cells; c_id++) {
    ...
    slist[c_id] =   cs_math_pow2(s11) + cs_math_pow2(s22) + cs_math_pow2(s33)
               + 0.5*cs_math_pow2(dudy+dvdx)
               + 0.5*cs_math_pow2(dudz+dwdx)
               + 0.5*cs_math_pow2(dvdz+dwdy);

    wlist[c_id] =  0.5*cs_math_pow2(dudy-dvdx)
               + 0.5*cs_math_pow2(dudz-dwdx)
               + 0.5*cs_math_pow2(dvdz-dwdy);
  }
...
#   pragma omp parallel for if(n_cells_ext > CS_THR_MIN)
    for (cs_lnum_t c_id = 0; c_id < n_cells; c_id++) {
      ...
      smbre[c_id] = smbrk[c_id];
        ...
        cs_real_t skjwlisklsji = 0.;
        cs_real_t sklsklsijsji = 0.;
        cs_real_t wklwklsijsji = 0.;

        for (cs_lnum_t ii = 0; ii < 3; ii++) {
          for (cs_lnum_t jj = 0; jj < 3; jj++) {
            for (cs_lnum_t kk = 0; kk < 3; kk++) {
              for (cs_lnum_t ll = 0; ll < 3; ll++){
                skiwljsklsji += xstrai[kk][ii]*xrotac[ll][jj]*xstrai[kk][ll]*xstrai[jj][ii];
                skjwlisklsji += xstrai[kk][jj]*xrotac[ll][ii]*xstrai[kk][ll]*xstrai[jj][ii];
                sklsklsijsji += xstrai[kk][ll]*xstrai[kk][ll]*xstrai[ii][jj]*xstrai[jj][ii];
                wklwklsijsji += xrotac[kk][ll]*xrotac[kk][ll]*xstrai[ii][jj]*xstrai[jj][ii];
              }
            }
          }
        }
...
# pragma omp parallel for if(n_cells_ext > CS_THR_MIN)
  for (cs_lnum_t c_id = 0; c_id < n_cells_ext; c_id++) {
    ...
    const cs_real_t xcmu = cs_turb_ca0/(cs_turb_ca1 + xss*cs_turb_ca2 + xww*cs_turb_ca3);
