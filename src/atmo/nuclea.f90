...
subroutine nuclea (nc, rom, tempc, qldia, pphy, refrad)
...
double precision nc(ncelet),qldia(ncelet)
...
do iel = 1, ncel
  ...
  if (qldia(iel).le.epzero) then
    ...
    if (vel(3,iel).gt.epzero.or.refrad(iel).lt.epzero) then
      ...
      if ((aa1*vel(3,iel)+aa4*refrad(iel)).gt.epzero) then ! alphaV/G > 0

        ! 1.1  Model of Pruppacher and Klett (1997)

        if (modnuc.eq.1) then

          nuc = (constc)**(2./(constk+2.)) * ( 0.01d0 * (aa1*vel(3,iel)                 &
            + aa4*refrad(iel))**(3./2.)                                        &
          ...
          do ii = 1, 20
            ...
            yy     = hypgeo(constmu,constk/2.d0,constk/2.d0 + 3.d0/2.d0,       &
              - constbeta*sursat**2 )
            sursat = ((0.01 * (aa1*vel(3,iel) + aa4*refrad(iel))               &
          ...
          ! mass fraction of of soluble substance in aerosol mix: msolu/mmix
          frasolu(1) = 1.d0
          frasolu(2) = 1.d0
          frasolu(3) = 0.d0
          frasolu(4) = 0.1d0
          ...
          ! supersaturation [-]
          sursat1 = (2.d0/sqrt(coefb))*((coefa/3.d0/rayonm1)**(1.5))
          sursat2 = (2.d0/sqrt(coefb))*((coefa/3.d0/rayonm2)**(1.5))
          sursat3 = (2.d0/sqrt(coefb))*((coefa/3.d0/rayonm3)**(1.5))

          ! standard deviation function
          foncf1 = 0.5d0 * dexp(2.5d0*(dlog(sigaero1))**2.)
          foncf2 = 0.5d0 * dexp(2.5d0*(dlog(sigaero2))**2.)
          foncf3 = 0.5d0 * dexp(2.5d0*(dlog(sigaero3))**2.)

          foncg1 = 1.d0 + 0.25d0 * (dlog(sigaero1))
          foncg2 = 1.d0 + 0.25d0 * (dlog(sigaero2))
          foncg3 = 1.d0 + 0.25d0 * (dlog(sigaero3))
          ...
          ! alphaV/G
          coefavg = (aa1*vel(3,iel)+aa4*refrad(iel))/aa3

          ! coefficent zeta
          coefzeta = 2.d0*coefa/3.d0*sqrt(coefavg)
