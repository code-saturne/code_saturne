...
static void
_soil_model_variable_init(void)
{
  /* Local variables */
  cs_real_t rscp;
  ...
  for (cs_lnum_t soil_id = 0; soil_id < n_elts; soil_id++) {
    ...
    /*TODO A bit too restrictive; it's possible to have water in the soil
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Soil Plant Atmo Continuum model - compute plant and soil convective
 * exchange resistances.
 *
 * \param[in] face_id and soil_id.
 */
/*----------------------------------------------------------------------------*/
                                     ...
                                     cs_lnum_t soil_id)
{
  ...
  cs_field_t *soil_air_resistance = cs_field_by_name_try("soil_air_resistance");
  ...
  cs_real_t u_star_soil = u_star_atm->val[face_id] * exp(-eta_ext_coef);;

  cs_real_t u_star_plant = pow(pow(u_star_atm->val[face_id],2)
    - pow(u_star_soil,2),0.5);
  ...
  cs_real_t g_soil_forced = u_star_soil * plant_opt->canopy_mix_l / canopy_exch_height / plant_opt->turb_prandtl;
  cs_real_t g_soil_nat = 0.005; /* TODO use expression in term of T_plant - T_air */

  /* Results */
  plant_air_resistance->val[soil_id] = 1. /(g_plant_nat + g_plant_forced) ;
  soil_air_resistance->val[soil_id] = 1. /(g_soil_nat + g_soil_forced) ;
}
...
static void
_compute_water_stress(cs_lnum_t soil_id)
{
  cs_plant_option_t *plant_opt = cs_glob_plant_option;
  ...
  cs_field_t *soil_water_potential = cs_field_by_name_try("soil_water_potential");
  ...
  cs_field_t *soil_p_water_volumetric_capacity = cs_field_by_name_try("soil_p_water_volumetric_capacity");
  cs_field_t *water_stress_factor = cs_field_by_name_try("water_stress_factor");

  /* Resistive approach */

  cs_real_t krsoil = 7.85e-10/(2*cs_math_pi
    * plant_opt->r_root);
  cs_real_t xis = 1/(plant_opt->z_root*2*cs_math_pi
    * plant_opt->ld_root
    * plant_opt->r_root*krsoil);

  cs_real_t psis = plant_opt->psi_e
    * pow(1./soil_w1->val[soil_id] , plant_opt->psi_pow);

  cs_real_t psir = psis - xis*et_plant->val[soil_id]/ct_prop->rho_l;

  cs_real_t psiv = psir - plant_opt->xiv
    * et_plant->val[soil_id]/ct_prop->rho_l;
  ...
  water_stress_factor->val[soil_id] = fpsiv;

  /* Post processing */
  soil_water_potential->val[soil_id] = psis;
  root_water_potential->val[soil_id] = psir;
  plant_water_potential->val[soil_id] = psiv;
...
static void
_compute_stomatal_conductance(cs_lnum_t soil_id,
                              ...
                              cs_real_t dtref)
{
  ...
  cs_real_t f_co2_soil = plant_opt->f_co2_0_soil
    * exp(plant_opt->ea_soil
    * (soil_temperature->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin
    - plant_opt->temp_ref_co2)
    / (cs_physical_constants_r*(soil_temperature->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin)
    * plant_opt->temp_ref_co2));
  ...
  cs_real_t ci_pho_no_a = (plant_opt->c_co2_air
    ...
    * (leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin));

  cs_real_t gamma_pho = plant_opt->gamma_0
    * (1. + plant_opt->gamma_1*(leaf_temp->val[soil_id]
    - plant_opt->tem_ref_pho
    + cs_physical_constants_celsius_to_kelvin)
    + plant_opt->gamma_2*pow(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin
  ...
  cs_real_t vlmax = plant_opt->vlmaxref
    * exp(plant_opt->ea_vlmax/cs_physical_constants_r
    / plant_opt->tem_ref_pho
    * (1. - plant_opt->tem_ref_pho/(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin )))
    / (1. + exp((plant_opt->s_vlmax*(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin )
  ...
  cs_real_t jlmax = plant_opt->jlmaxref
    ...
    * (1. - plant_opt->tem_ref_pho/(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin )))
    / (1. + exp((plant_opt->s_jlmax*(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin )
  ...
  cs_real_t gco2_previous = gco2->val[soil_id];
  ...
  for (i=0; i<30; i++){
    ...
    f_ci_pho_1 = ci_pho_1 - plant_opt->c_co2_air
      - f_co2_soil*(ra + rv) / ps*cs_physical_constants_r
      * (air_leaf_temp + cs_physical_constants_celsius_to_kelvin)
      + assimilation_rate_1*((rv+ra+1.6*plant_air_resistance->val[soil_id])
      / ps*cs_physical_constants_r
      * (air_leaf_temp + cs_physical_constants_celsius_to_kelvin)
      + 1./gco2_1);

    if (( f_ci_pho_0 == f_ci_pho_1) && (f_ci_pho_1 != 0.)){
      ci_pho->val[soil_id] = (ci_pho_0 + ci_pho_1)/2;
    }
    else if (( f_ci_pho_0 == f_ci_pho_1) && (f_ci_pho_1 == 0.)){
      ci_pho->val[soil_id] = ci_pho_1;
    }
    else if (f_ci_pho_0 == 0. ){
      ci_pho->val[soil_id] = ci_pho_0;
    }
    else if (f_ci_pho_1 == 0. ){
      ci_pho->val[soil_id] = ci_pho_1;
    }
    else {
      ci_pho->val[soil_id] = ci_pho_1 - (ci_pho_1-ci_pho_0)
        / (f_ci_pho_1-f_ci_pho_0)*f_ci_pho_1;
    }
    if(( ci_pho->val[soil_id] < 0.) || (ci_pho->val[soil_id] > 0.0006)){
      bft_printf("PROBLEM ci equal to %f\n", ci_pho->val[soil_id]);
      ci_pho->val[soil_id] = (ci_pho_0 + ci_pho_1)/2;
    }
    ci_pho_0 = ci_pho_1;
    ci_pho_1 = ci_pho->val[soil_id];
    delta_ci = 1000*(ci_pho->val[soil_id] - ci_pho_1);
  }

  if (ci_pho->val[soil_id] == 0){
    bft_printf("PROBLEM ci can't be equal to 0 \n");
  }
  else if (delta_ci/ci_pho->val[soil_id]>0.2) {
    bft_printf("PROBLEM NEW ALOGRITHM HAS NOT CONVERGED \n");
  }

  cs_real_t vc = vcmax*(ci_pho->val[soil_id]
    - gamma_pho)/(ci_pho->val[soil_id] + kc
    * (1. + plant_opt->oi/ko));

  cs_real_t vj = j/4.*(ci_pho->val[soil_id] - gamma_pho)
    / (ci_pho->val[soil_id] + 2.*gamma_pho);

  /* Assumption photorespiration is more important
   * than the other processes */
  assimilation_rate->val[soil_id] = cs::max(0., cs::min(vc , vj));
  assimilation_rate->val[soil_id] = cs::max(0.,vj);

  gco2->val[soil_id] =  plant_opt->g0_pho
    + plant_opt->a_pho*assimilation_rate->val[soil_id]
    / (ci_pho->val[soil_id] - gamma_pho)* water_stress_factor->val[soil_id];

  gco2->val[soil_id] = cs::max(plant_opt->g0_pho , gco2->val[soil_id]);
  ...
  gco2->val[soil_id] = cs::min(gco2->val[soil_id],gco2_max);
  gco2->val[soil_id] = cs::max(gco2->val[soil_id],gco2_min);
...
static void
_compute_le_h_and_leaf_temp(cs_lnum_t soil_id,
                            cs_real_t rsh2o,
                            cs_real_t air_leaf_temp,
                            ...
                            cs_real_t rn_thermal_soil_plant)
{
  cs_plant_option_t *plant_opt = cs_glob_plant_option;
  cs_air_fluid_props_t *ct_prop = cs_glob_air_props;
  ...
  cs_field_t *ray_ir_plant_to_soil
    = cs_field_by_name_try("ray_ir_plant_to_soil");
  cs_field_t *ray_ir_atm_to_plant
    = cs_field_by_name_try("ray_ir_atm_to_plant");

  cs_real_t leaf_temp_0 = leaf_temp->val[soil_id]+2.;
  cs_real_t leaf_temp_1 = leaf_temp->val[soil_id]-2.;

  cs_real_t pvsat_plant_0 = 0.;
  cs_real_t pvsat_plant_1 = 0.;

  cs_real_t hi = exp(wv_molar_mass * plant_water_potential->val[soil_id]
  ...
  for (nb=0; nb<30; nb++){
    ...
    rn_thermal_plant_0 = 2.*plant_opt->emi_plant*stephn
      * cs_math_pow4(leaf_temp_0
      + cs_physical_constants_celsius_to_kelvin)
      * cover_geometry_ratio->val[soil_id];
    rn_plant_0 = ray_net_solar_plant->val[soil_id] + rn_thermal_atm_plant
      - rn_thermal_plant_0 + rn_thermal_soil_plant;

    rn_thermal_plant_1 = 2.*plant_opt->emi_plant*stephn
      * cs_math_pow4(leaf_temp_1
      + cs_physical_constants_celsius_to_kelvin)
      * cover_geometry_ratio->val[soil_id];
    rn_plant_1 = ray_net_solar_plant->val[soil_id] + rn_thermal_atm_plant
      - rn_thermal_plant_1 + rn_thermal_soil_plant;

    et_plant_0 = cs::max(0.,wv_molar_mass/cs_physical_constants_r
      * (1./(rsh2o + plant_air_resistance->val[soil_id]))
    ...
    hs_plant_0 = rho_moist_air*cp_moist_air/plant_air_resistance->val[soil_id]
      *(leaf_temp_0 - air_leaf_temp);
    hs_plant_1 = rho_moist_air*cp_moist_air/plant_air_resistance->val[soil_id]
    ...
    if (( f_leaf_temp_0 == f_leaf_temp_1) && (f_leaf_temp_1 != 0.)){
      leaf_temp->val[soil_id] = (leaf_temp_0 + leaf_temp_1)/2.;
    }
    else if (( f_leaf_temp_0 == f_leaf_temp_1) && (f_leaf_temp_1 == 0.)){
      leaf_temp->val[soil_id] = leaf_temp_1;
    }
    else if (f_leaf_temp_0 == 0. ){
      leaf_temp->val[soil_id] = leaf_temp_0;
    }
    else if (f_leaf_temp_1 == 0. ){
      leaf_temp->val[soil_id] = leaf_temp_1;
    }
    else {
      leaf_temp->val[soil_id] = leaf_temp_1 - (leaf_temp_1-leaf_temp_0)
        / (f_leaf_temp_1-f_leaf_temp_0)*f_leaf_temp_1;
    }
    if(( leaf_temp->val[soil_id] < -20.) || (leaf_temp->val[soil_id] > 80.)){
      bft_printf("PROBLEM leaf temp equal to %f\n", leaf_temp->val[soil_id]);
      leaf_temp->val[soil_id] = (leaf_temp_0 + leaf_temp_1)/2.;
    }
    leaf_temp_0 = leaf_temp_1;
    leaf_temp_1 = leaf_temp->val[soil_id];
    delta_leaf_temp = 1000.*(leaf_temp->val[soil_id] - leaf_temp_1);
  }

  if (delta_leaf_temp/(0.1 + CS_ABS(leaf_temp->val[soil_id]))>0.2) {
    bft_printf("PROBLEM NEW ALOGRITHM HAS NOT CONVERGED \n");
  }
  cs_real_t pvsat_plant = cs_air_pwv_sat(leaf_temp->val[soil_id]);

  et_plant->val[soil_id] = cs::max(0.,wv_molar_mass/cs_physical_constants_r
    * (1./(rsh2o + plant_air_resistance->val[soil_id]))
    * (hi*pvsat_plant/(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin)
    - pvsat_dewpoint/(air_leaf_temp
    + cs_physical_constants_celsius_to_kelvin)));

  cs_real_t rn_thermal_plant = 2.*plant_opt->emi_plant*stephn
    * cs_math_pow4(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin)
    * cover_geometry_ratio->val[soil_id];
  cs_real_t rn_plant = ray_net_solar_plant->val[soil_id] + rn_thermal_atm_plant
    - rn_thermal_plant + rn_thermal_soil_plant;

  le_plant->val[soil_id] = ct_prop->hv0*et_plant->val[soil_id];

  hs_plant->val[soil_id] = rho_moist_air*cp_moist_air
    / plant_air_resistance->val[soil_id]
    *(leaf_temp->val[soil_id] - air_leaf_temp);

  ray_net_plant->val[soil_id] = rn_plant;
  ray_net_ir_plant->val[soil_id] = rn_thermal_atm_plant
    - rn_thermal_plant + rn_thermal_soil_plant;
  ray_ir_plant_to_soil->val[soil_id] = plant_opt->emi_plant*stephn
    * cs_math_pow4(leaf_temp->val[soil_id]
    + cs_physical_constants_celsius_to_kelvin)
    * cover_geometry_ratio->val[soil_id];
  ray_ir_atm_to_plant->val[soil_id] = rn_thermal_atm_plant;

}

/*----------------------------------------------------------------------------*/
/*!
 * \brief Soil Plant Atmo Continuum model - compute source term plant and soil
 * to air.
 *
 * \param[in] face_id and soil_id.
 */
...
static void
_compute_plant_to_air_source_terms(cs_lnum_t soil_id,
                                   cs_real_t dtref,
                                   cs_real_t dz_max,
                                   cs_real_t rho_moist_air)
{
  ...
  cs_real_t drho_p_to_atm  = et_plant->val[soil_id] * dtref / dz_max;

  st_rho_p->val[soil_id] = drho_p_to_atm / dtref;

  cs_real_t dq_p_to_atm = drho_p_to_atm / rho_moist_air ;

  st_q_p_exp->val[soil_id] = dq_p_to_atm / (1. + dq_p_to_atm) / dtref;
  st_q_p_imp->val[soil_id] = - dq_p_to_atm / (1. + dq_p_to_atm) / dtref;

  st_conv_energy_p->val[soil_id] = sensible_heat_plant_to_air->val[soil_id] / dz_max ;
  st_mass_energy_p->val[soil_id] = drho_p_to_atm
    * ct_prop->cp_v * ( leaf_temp->val[soil_id]
...
cs_soil_model(void)
{
  ...
  if (z_id > -1) {
    ...
    cs_field_t *ray_ir_plant_to_soil = cs_field_by_name_try("ray_ir_plant_to_soil");
    cs_field_t *ray_ir_atm_to_plant = cs_field_by_name_try("ray_ir_atm_to_plant");
    cs_field_t *et_plant = cs_field_by_name_try("et_plant");
    cs_field_t *et_soil = cs_field_by_name_try("et_soil");
    cs_field_t *assimilation_rate = cs_field_by_name_try("assimilation_rate");
    cs_field_t *gco2 = cs_field_by_name_try("gco2");
    cs_field_t *soil_w1_transpirated_by_the_plant = cs_field_by_name_try("soil_w1_transpirated_by_the_plant");
    cs_field_t *soil_w1_evaporated_by_the_soil = cs_field_by_name_try("soil_w1_evaporated_by_the_soil");
    ...
    cs_field_t *st_rho_s = cs_field_by_name_try("source_term_vapor_mass_soil");
    cs_field_t *st_conv_energy_s = cs_field_by_name_try("source_term_convective_energy_soil");
    cs_field_t *st_mass_energy_s = cs_field_by_name_try("source_term_mass_energy_soil");
    ...
    */
    ...
    for (cs_lnum_t soil_id = 0; soil_id < z->n_elts; soil_id++) {
      ...
      cs_real_t h_t_soil = h_t[face_id];
      cs_real_t h_q_soil = h_q[face_id];
      cs_real_t albedo_s = cs_glob_atmo_option->soil_cat_albedo[3];
      cs_real_t emi_s = cs_glob_atmo_option->soil_cat_emissi[3];
      ...
      if (micro_scale_option == CS_ATMO_SOIL_VEGETATION) {
        ...
        cover_geometry_ratio->val[soil_id] = (1-exp(
          - plant_opt->k_ext_coef
          * plant_opt->leaf_area_index));

        cover_reflectivity->val[soil_id] = 1 - plant_opt->emi_plant;

        /* ============================
         * Radiation on the canopy
         * ============================ */

        ray_net_solar_plant->val[soil_id] = (1.-plant_opt->albedo_plant)
          * fos*cover_geometry_ratio->val[soil_id];

        /* Note that the plant acts as a black body for PAR */
        cs_real_t photo_active_rad = ray_net_solar_plant->val[soil_id]
          / (1 - plant_opt->albedo_plant) *1.e-6/0.48;

        cs_real_t rn_thermal_atm_plant = cs::max(plant_opt->emi_plant*foir
          * cover_geometry_ratio->val[soil_id],50.);

        cs_real_t rn_thermal_soil_plant = emissivity->val[face_id] * stephn
        ...
        if (cs_glob_physical_model_flag[CS_ATMOSPHERIC] == 2){
          ...
          air_leaf_temp = atm_temp->val[cell_id] * pow(pphy / ps , rscp1
            / (1. + (rvsra - cpvcpa)* soil_total_water->val[soil_id] ))
            - cs_physical_constants_celsius_to_kelvin;
        }
        ...
        h_t_soil = rho_moist_air * cp0 / soil_air_resistance->val[soil_id];
        h_q_soil = rho_moist_air / soil_air_resistance->val[soil_id];
        ...
        cs_real_t pvsat_soil = cs_air_pwv_sat(soil_temperature->val[soil_id]);

        /* ============================
         * Water potential
         * ============================ */

        _compute_water_stress(soil_id);

        /* ============================
         * Stomatal conductance
         * ============================ */

        _compute_stomatal_conductance(soil_id,
                                      photo_active_rad,
                                      air_leaf_temp,
                                      ps,
                                      dtref);

        cs_real_t rsh2o = ps/1.6/gco2->val[soil_id]/cs_physical_constants_r
          / (air_leaf_temp + cs_physical_constants_celsius_to_kelvin);
        ...
        _compute_le_h_and_leaf_temp(soil_id,
                                    rsh2o,
                                    air_leaf_temp,
                                    pvsat_dewpoint,
                                    wv_molar_mass,
                                    ...
                                    rn_thermal_soil_plant);

        cover_temperature_radiative->val[soil_id] = leaf_temp->val[soil_id];

        /* ============================
         * Compute water exchanges
         * ============================ */

        transpire = et_plant->val[soil_id];

        et_soil->val[soil_id] = cs::max(0. , wv_molar_mass
          / cs_physical_constants_r*(1/soil_air_resistance->val[soil_id])
          *(pvsat_soil/(soil_temperature->val[soil_id]
          + cs_physical_constants_celsius_to_kelvin)
          - pvsat_dewpoint/(air_leaf_temp
          + cs_physical_constants_celsius_to_kelvin)));

        soil_w1_transpirated_by_the_plant->val[soil_id] = dtref * transpire;
        soil_w1_evaporated_by_the_soil->val[soil_id] = dtref
          * et_soil->val[soil_id];   /* To compare with the soil model*/

        q_emitted_by_the_plant->val[soil_id] = dtref * transpire
        ...
        _compute_plant_to_air_source_terms(soil_id,
                                           dtref,
                                           dz_max,
                                           rho_moist_air);
      ...
      cs_real_t w1_num = soil_w1->val[soil_id]
        ...
        / soil_water_capacity->val[soil_id]
