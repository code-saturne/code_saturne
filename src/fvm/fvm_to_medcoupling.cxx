...
/*----------------------------------------------------------------------------
 * Write block of an array of doubles to a MEDCoupling object.
 *
...
static void
_write_block_double_g(cs_gnum_t         num_start,
                      cs_gnum_t         num_end,
                      int               dim,
                      double            values[],
                      MPI_Comm          comm,
                      DataArrayDouble  *a)
{
  ...
  cs_file_serializer_t *s
    = cs_file_serializer_create(sizeof(double),
                                dim,
                                num_start,
                                num_end,
                                0, /* block_buf_size */
  ...
  do {
    ...
    _values = (double *)cs_file_serializer_advance(s, range);

    if (_values != NULL) { /* only possible on rank 0 */
      for (i = 0, j = range[0]; j < range[1]; i++, j++) {
        for (k = 0; k < dim; k++)
          a->setIJ(j, k, _values[i*dim + k]);
      }
    }
 ...
 *
 * parameters:
 *   n_elts  <-- number of elements with values to write
 *   dim     <-- dimension of array
 *   num_end <-- global number of past the last element for this block
 *   values  <-- pointer to values block array
 *   a       <-- array object to write to
 *----------------------------------------------------------------------------*/

static void
_write_block_double_l(size_t           n_elts,
                      int              dim,
                      const double     values[],
                      DataArrayDouble  *a)
{
