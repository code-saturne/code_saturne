...
#define FVM_MED_VERSION 2
#else
#define FVM_MED_VERSION 3
#endif
...
#define MED_SNAME_BLANK MED_PNOM_BLANC;
              ...
              const fvm_nodal_t    *mesh)
{
  ...
  if (rank == 0) {
    ...
    for (family_num = 0, retval = 0;
      ...
      if (retval < 0)
        bft_error(__FILE__, __LINE__, 0,
                  _("MEDfamCr() failed to create family %d"
                    "for a new med_mesh.\n"
                    "Associated med_mesh name: \"%s\"\n"
                    "Associated writer name: \"%s\"\n"),
                   ...
                   char                   *med_fieldname)
{
  ...
  if (i_field == n_fields) { /* Create a new field for this writer */
    ...
    if (rank == 0) {
      ...
      strcpy(dt_unit, "second");
      for (i = strlen(dt_unit); i < MED_LNAME_SIZE; i++)
        dt_unit[i] = ' ';
      dt_unit[MED_LNAME_SIZE] = '\0';
                        ...
                        fvm_to_med_writer_t  *writer)
{
  ...
  if (rank == 0) {
    ...
    if (global_coords_buffer != NULL) {
      ...
      for (i_dim = 0; i_dim < dim; i_dim++)
        sprintf(&coord_name[i_dim * MED_SNAME_SIZE], "Coord%s", XYZ[i_dim]);
...
#endif /* if FVM_MED_VERSION == 2 or 3 */
                   ...
                   fvm_to_med_mesh_t           *med_mesh)
{
  ...
  if (have_tesselation) {
    MPI_Scan(&block_sub_size, &block_end, 1, FVM_MPI_GNUM, MPI_SUM,
             writer->comm);
    block_end += 1;
  ...
  if (writer->rank == 0) {
    ...
    med_err retval  = MEDfamEcr(writer->fid,
                                med_mesh->name,
                                _block_values,
                                block_sub_size,
                                MED_MAILLE,
                  ...
                  char                        *export_connect)
{
  ...
  if (writer->rank == 0) {
    ...
    if (retval < 0)
      bft_error(__FILE__, __LINE__, 0,
                _("MEDconnEcr() failed to write connectivity:\n"
                  "Associated writer: \"%s\"\n"
                  "Associated med_mesh_name: \"%s\"\n"
                  "Associated MED geometrical element: \"%i\"\n"),
                writer->name, med_mesh->name, med_section_type);
