  ...
  if (rank == 0) {
    ...
    for (family_num = 0, retval = 0;
...
#if HAVE_SNPRINTF
      snprintf(family_name, MED_TAILLE_NOM, "FAMILY_%d", (int)family_num);
#else
      ...
      if (family_num < 0) {
        ...
        if (n_groups > 0) {
          BFT_MALLOC(med_groups, MED_TAILLE_LNOM  * n_groups + 1, char);
          for (i = 0; i < n_groups; i++) {
            char *group_p = med_groups + (MED_TAILLE_LNOM*i);
            memset(group_p, 0, MED_TAILLE_LNOM);
            strncpy(group_p, group_names[i], MED_TAILLE_LNOM - 1);
          }
        }
