...
/*!
  ...
  code), we have  <tt>_vertex_coords = nullptr</tt>, and \c vertex_coords
...
static int _compare_edges(const void *x, const void *y)
{
  ...
  const cs_lnum_t *e0 = static_cast<const cs_lnum_t *>(x);
  const cs_lnum_t *e1 = static_cast<const cs_lnum_t *>(y);
...
_fvm_nodal_section_copy(const fvm_nodal_section_t *this_section)
{
  ...
  new_section->_face_index = nullptr;
  new_section->_face_num   = nullptr;
  new_section->_vertex_index = nullptr;
  new_section->_vertex_num = nullptr;

  new_section->gc_id = nullptr;
  new_section->tag = nullptr;

  new_section->tesselation = nullptr;  /* TODO: copy tesselation */

  /* Numbering */
  /*-----------*/

  new_section->parent_element_id = this_section->parent_element_id;
  new_section->_parent_element_id = nullptr;
  ...
  if (this_section->global_element_num != nullptr) {
    ...
    new_section->global_element_num = nullptr;
...
_fvm_nodal_section_reduce(fvm_nodal_section_t  * this_section)
{
  ...
  /* If we have a tesselation of polyhedra (face index != nullptr),
  ...
  if (   this_section->tesselation == nullptr
      || this_section->_face_index == nullptr) {
                    ...
                    cs_lnum_t          _parent_id[])
{
  ...
  if (parent_id_size > 0 && new_parent_id != nullptr) {

    if (parent_id_p != nullptr) {
...
_remove_global_vertex_labels(fvm_nodal_t  *this_nodal)
{
  ...
  if (this_nodal->global_vertex_labels != nullptr) {
...
fvm_nodal_section_create(const fvm_element_t  type)
{
  ...
  this_section->_face_index = nullptr;
  this_section->_face_num   = nullptr;
  this_section->_vertex_index = nullptr;
  this_section->_vertex_num = nullptr;

  this_section->gc_id = nullptr;
  this_section->tag = nullptr;

  this_section->tesselation = nullptr;

  /* Numbering */
  /*-----------*/

  this_section->parent_element_id = nullptr;
  this_section->_parent_element_id = nullptr;

  this_section->global_element_num = nullptr;
                 ...
                 int          dim)
{
  ...
  this_nodal->_vertex_coords = nullptr;

  this_nodal->parent_vertex_id = nullptr;
  this_nodal->_parent_vertex_id = nullptr;

  this_nodal->global_vertex_num = nullptr;

  this_nodal->sections = nullptr;

  this_nodal->gc_set = nullptr;

  this_nodal->global_vertex_labels = nullptr;

  this_nodal->parent = nullptr;
...
fvm_nodal_copy(const fvm_nodal_t *this_nodal)
{
  ...
  new_nodal->_vertex_coords = nullptr;

  new_nodal->parent_vertex_id = this_nodal->parent_vertex_id;
  new_nodal->_parent_vertex_id = nullptr;
  ...
  new_nodal->global_vertex_labels = nullptr;
 ...
 *
 * The argument pointer to the global numbering structure is set to nullptr
 ...
 *
 * If the parent_vertex_id argument is nullptr, the list is assumed to
                              ...
                              const cs_coord_t   vertex_coords[])
{
  ...
  /* Map vertex coordinates to array passed as argument
     (this_nodal->_vertex_coords remains nullptr, so only
...
fvm_nodal_make_vertices_private(fvm_nodal_t  *this_nodal)
{
  ...
  if (this_nodal->_vertex_coords == nullptr) {
    ...
    cs_coord_t *_vertex_coords = nullptr;
 ...
 * parameters:
 *   this_nodal <-> nodal mesh structure
 *   g_labels   <-- global vertex labels, or nullptr
 *----------------------------------------------------------------------------*/
 ...
 *
 * returns:
 *   pointer to global vertex labels, or nullptr
 *----------------------------------------------------------------------------*/
                    ...
                    cs_lnum_t      *error_count)
{
  ...
  for (section_id = 0; section_id < this_nodal->n_sections; section_id++) {
    ...
    if (section->type == type && section->tesselation == nullptr) {
...
fvm_nodal_dump(const fvm_nodal_t  *this_nodal)
{
  ...
  if (this_nodal->n_vertices > 0) {
    ...
    else { /* if (this_nodal->parent_vertex_num != nullptr) */
  ...
  /* Dump group class set (nullptr allowed) */
  ...
  if (this_nodal->global_vertex_labels != nullptr) {
    ...
    for (i = 0; i < n_labels; i++) {
      if (this_nodal->global_vertex_labels[i] != nullptr)
        bft_printf("%10ld : %s\n",
