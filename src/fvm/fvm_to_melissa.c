...
#endif
{
...
#if defined(HAVE_MPI)
  {
    ...
    if (mpi_flag && comm != MPI_COMM_NULL) {
      ...
      if (rank_step > 1) {
        if (rank_step >= n_ranks)
          w->block_comm = MPI_COMM_NULL;
        else {
          int ranges[1][3] = {{0, n_ranks-1, rank_step}};
          ...
          MPI_Comm_create(cs_glob_mpi_comm, new_group, &(w->block_comm));
...
#if defined(HAVE_MELISSA_MPI)
  if (w->block_comm != w->comm && w->block_comm != MPI_COMM_NULL)
    MPI_Comm_free(&(w->block_comm));
#endif
