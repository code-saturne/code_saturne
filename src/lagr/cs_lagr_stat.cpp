...
typedef struct {
  ...
  int                       class_id;     /* Matching statistical class id */
  ...
  cs_lagr_moment_m_data_t  *m_data_func;  /* Associated mesh data value
                                             computation function, or nullptr */
...
static cs_lagr_moment_wa_t  *_lagr_moments_wa = nullptr;
static cs_lagr_moment_t     *_lagr_moments = nullptr;
static cs_lagr_mesh_stat_t  *_lagr_mesh_stats = nullptr;
...
static const char *_lagr_stat_names[] = {"particle_cumulative_weight",
                                         ...
                                         "particle_resusp_mass_flux_density",
...
_cs_lagr_stat_update_mesh_stats(const cs_time_step_t  *ts)
{
  for (int ms_id = 0; ms_id < _n_lagr_mesh_stats; ms_id++) {
    ...
    ms->m_data_func(ms->data_input, nullptr, f->location_id, ms->class_id, val);
  }
}
...
_restart_info_read(void)
{
  ...
  r = cs_restart_create("lagrangian_stats.csc", nullptr, CS_RESTART_MODE_READ);
...
_cs_lagr_moment_restart_read(void)
{
  ...
  static cs_restart_t  *cs_lag_stat_restart = nullptr;

  char const *ficsui = "lagrangian_stats.csc";
  cs_lag_stat_restart = cs_restart_create(ficsui, nullptr, CS_RESTART_MODE_READ);
  if (cs_lag_stat_restart == nullptr)
    bft_error(__FILE__, __LINE__, 0,
 ...
 * nullptr, the lifecycle of the data pointed to must be handled separately
 ...
 * \param[in]  stat_group   statistics group (particle or event)
 * \param[in]  class_id     statistical class id
                ...
                int                       prev_wa_id)
{
  ...
  cs_lagr_moment_wa_t *mwa = nullptr;
...
_find_or_add_mesh_stat(int                       location_id,
                       ...
                       double                    t_start)
{
  cs_lagr_mesh_stat_t *ms = nullptr;
 ...
 * is defined), nullptr is returned.
 *
 * \param[in, out]  mwa       moment weight accumulator
 ...
 *                            is of size 1
 *
 * \return  pointer to weight array (w0 or allocated array), or nullptr
 */
/*----------------------------------------------------------------------------*/
                          ...
                          cs_real_t             w0[1])
{
  ...
  mwa->m_data_func(mwa->data_input, nullptr, mwa->location_id, mwa->class_id, w);
 ...
 *                              (if nullptr, statistic weight assumed)
...
_event_stat_initialize(void)
{
  ...
  for (int class_id = 0;
       class_id < cs_glob_lagr_model->n_stat_classes + 1;
       class_id++) {

    /* Particle events count */

    if (_base_stat_activate[CS_LAGR_STAT_E_CUMULATIVE_WEIGHT] > 0) {
      _class_name(_lagr_stat_names[CS_LAGR_STAT_E_CUMULATIVE_WEIGHT], class_id, name);
  ...
  for (int stat_type = CS_LAGR_STAT_IMPACT_ANGLE;
    ...
    for (int class_id = 0;
      ...
      for (int i_m_type = CS_LAGR_MOMENT_MEAN;
        ...
        cs_lagr_moment_e_data_t   *data_func = nullptr;
        cs_lagr_moment_e_data_t   *w_data_func = nullptr;
                             ...
                             cs_lagr_stat_restart_t     restart_mode)
{
  return  _stat_moment_define(name,
                              ...
                              nullptr,         /* w_e_data_func */
                              nullptr,         /* w_m_data_func */
                              w_data_input,
                              nt_start,
                              t_start,
                              restart_mode);
...
cs_lagr_stat_mesh_define(const char                *name,
                         ...
                         double                     t_start)
{
  if (data_func == nullptr)
    bft_error(__FILE__, __LINE__, 0,
              _("The '%s' argument to %s must not be nullptr."),
              "data_func", __func__);
    ...
    for (int class_id = 0;
      ...
      for (int stat_type = 0; stat_type < _n_stat_types(); stat_type++) {
        ...
        if (stat_type == CS_LAGR_STAT_CUMULATIVE_WEIGHT) {
          _class_name("particle_cumulative_weight", class_id, name);
          cs_lagr_stat_accumulator_define(name,
                          ...
                          cs_lagr_stat_group_t   group)
{
  ...
  for (int ms_id = 0; ms_id < _n_lagr_mesh_stats; ms_id++) {
    ...
    ms->m_data_func(ms->data_input, events, f->location_id, ms->class_id, val);
  ...
  for (int wa_id = 0; wa_id < _n_lagr_moments_wa; wa_id++) {
    ...
    for (int m_type = CS_LAGR_MOMENT_VARIANCE;
      ...
      for (int i = 0; i < _n_lagr_moments; i++) {
        ...
        if (   (int)mt->m_type == m_type
          ...
          assert(mt->m_data_func == nullptr); /* Data should be event-based */
          ...
          for (cs_lnum_t ev_id = 0; ev_id  < events->n_events; ev_id++) {
            ...
            if (elt_id >= 0 && (p_class == mt->class_id || mt->class_id == 0)) {

              /* weight associated to current event */

              cs_real_t p_weight;

              if (mwa->e_data_func == nullptr)
                p_weight = cs_lagr_events_get_real(events,
                                                   ev_id,
                                                   CS_LAGR_STAT_WEIGHT);
              else
              ...
              if (mt->e_data_func == nullptr)
                pval = cs_lagr_events_attr_get_ptr<cs_real_t>(events,
                                                              ev_id,
                                                              attr_id);
              else
    ...
    if (l_wa_sum != nullptr) {
      ...
      for (cs_lnum_t ev_id = 0; ev_id  < events->n_events; ev_id++) {
        ...
        if (elt_id >= 0 && (p_class == mwa->class_id || mwa->class_id == 0)) {
...
cs_lagr_stat_restart_write(cs_restart_t  *restart)
{
  ...
  int *m_type, *wa_id, *l_id, *class_id, *stat_type, *stat_group;
  cs_real_t *t_start;
