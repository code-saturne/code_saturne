 ...
 *   t_intersect     <-- relative distance (in [0, 1]) of the intersection
                   ...
                   const cs_field_t               *u)
{
  ...
  for (int n_loops = displacement_step_id;
    ...
    cell_id = cs_lagr_particle_get_lnum(particle, p_am, CS_LAGR_CELL_ID);
    ...
    /* Update boundary events when particle changes */
    ...
    else if (exit_face < mesh->n_i_faces) { /* Particle moves to the
                                               neighbor cell */
    ...
    else { /* Particle moves to the boundary
              through the current face "face_id" */
...
_initialize_displacement(cs_lagr_particle_set_t  *particles)
{
  ...
  for (cs_lnum_t i = 0; i < particles->n_particles; i++) {
    ...
    int p_flag = cs_lagr_particles_get_lnum(particles, i, CS_LAGR_P_FLAG);
