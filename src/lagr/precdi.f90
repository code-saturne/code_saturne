...
!================
  ...
  !   Management of the injection of particles formed
  !   by precipitation
  ...
  ! val              ! r  ! --> ! nombre de particules à injecter (avec poids)   !
  !__________________!____!_____!________________________________________________!
  ...
  do iel = 1 , ncel
     ...
     if (mp_diss_t(iel) .gt. 0) then
        ...
        do npt  = 1 , nbpart
           do k = 1,  nbrclas
              if (ipepa(jisor,npt) .eq. iel  .and. eptp(jdp,npt) .eq. ruslag(k,1,idpt) &
                   .and. (mp(k) .lt. mp_diss(iel,k))) then
                 ! Removing of particles due to dissolution
                 ipepa(jisor,npt) = 0
                 mp(k)  = mp(k) + pepa(jrpoi,npt)* ( pi/6.d0 * (eptp(jdp,npt))**3 * rho_prec)
                 nbdiss(k) = nbdiss(k) + 1
              endif
           enddo
        enddo
