...
cs_lagr_resuspension(void)
{
  ...
  for (cs_lnum_t p_id = 0; p_id < p_set.n_particles; p_id++) {
    ...
    cs_lnum_t iel = p_set.attr_lnum(p_id, CS_LAGR_CELL_ID);
    ...
    cs_lnum_t flag = p_set.attr_lnum(p_id, CS_LAGR_P_FLAG);
    ...
    if ((    cs_glob_lagr_model->clogging == 0 && face_id > 0)
      ...
      if (flag & CS_LAGR_PART_DEPOSITED)
        ...
        if (p_set.attr_lnum(p_id, CS_LAGR_N_LARGE_ASPERITIES) == 0)
          test_colli = 1;

        cs_real_t disp_norm = p_set.attr_real(p_id, CS_LAGR_DISPLACEMENT_NORM);
        ...
        if (disp_norm > p_diam && disp_norm < 2.0 * p_diam) {
          ...
          p_set.attr_real(p_id, CS_LAGR_DISPLACEMENT_NORM) = 0.0;

          cs_lagr_adh(p_id, temp, &adhesion_energ);

          if (   test_colli == 1
              && p_set.attr_lnum(p_id, CS_LAGR_N_LARGE_ASPERITIES) > 0) {
            ...
            if (kinetic_energy > adhesion_energ) {
              ...
              p_set.attr_lnum(p_id, CS_LAGR_N_LARGE_ASPERITIES) = 0;
              p_set.attr_lnum(p_id, CS_LAGR_N_SMALL_ASPERITIES) = 0;
              p_set.attr_real(p_id, CS_LAGR_DISPLACEMENT_NORM) = 0.0;
          ...
          cs_lnum_t ii = 1;
          while (   ii <= ndiam
                 && (p_set.flag(p_id, CS_LAGR_PART_ROLLING))) {
      ...
      if (flag == CS_LAGR_PART_DEPOSITED)
        ...
        cs_real_t disp_norm = p_set.attr_real(p_id,
        ...
        while (   ii <= ndiam
          ...
          p_set.attr_lnum(p_id, CS_LAGR_N_SMALL_ASPERITIES) = ncont;

          adhes_energ *= ncont;
          adhes_force *= ncont ;
          p_set.attr_real(p_id, CS_LAGR_ADHESION_FORCE) = adhes_force;

          adhes_torque = adhes_force * p_diam * 0.5;
          p_set.attr_real(p_id, CS_LAGR_ADHESION_TORQUE) = adhes_torque;
