        ...
        cs_real_t         temp[])
{
  ...
  cs_real_t p_init_diam = p_set.attr_real(p_id, CS_LAGR_INITIAL_DIAMETER);
  cs_real_t p_shrink_diam = p_set.attr_real(p_id, CS_LAGR_SHRINKING_DIAMETER);
  cs_real_t part_cp = p_set.attr_real(p_id, CS_LAGR_CP);
  ...
  const cs_real_t *prev_part_temp
    = p_set.attr_n_real_ptr(p_id, 1, CS_LAGR_TEMPERATURE);
  ...
  cs_lnum_t co_id = p_set.attr_lnum(p_id, CS_LAGR_COAL_ID);
  ...
  if (nlayer > 1) {
    ...
    cs_real_t coefh
      =   p_set.attr_n_real(p_id, 1, CS_LAGR_MASS)
        * p_set.attr_n_real(p_id, 1, CS_LAGR_CP)
    ...
    cs_real_t  t_fluid_l
      =   p_set.attr_real(p_id, CS_LAGR_TEMPERATURE_SEEN)
        + _tkelvi;
  ...
  /* Single-layer resolution
    ...
    cs_real_t aux1      =  p_set.attr_real(p_id, CS_LAGR_TEMPERATURE_SEEN)
                         + _tkelvi
        ...
        cs_real_t         fwat[])
{
  ...
  cs_real_t prev_p_diam
    = p_set.attr_n_real(p_id, 1, CS_LAGR_DIAMETER);
  ...
  _lagtmp(p_id, dt_part, nor, layer_vol, tempct, radius, mlayer, phith, temp);
        ...
        int                   nor)
{
  ...
  tcarac = 1.0;
  pip    = p_set.attr_real(p_id, CS_LAGR_MASS);
  cs_lagr_sde_attr(CS_LAGR_MASS, p_id, nor, dt_part, tcarac, pip);
}
        ...
        int                   nor)
{
  ...
  tcarac = 1.0;
  pip    = p_set.attr_real(p_id, CS_LAGR_DIAMETER);
  cs_lagr_sde_attr(CS_LAGR_DIAMETER, p_id, nor, dt_part, tcarac, pip);
}
        ...
        const cs_real_2_t     tempct)
{
  ...
  pip = p_set.attr_n_real(p_id, 2 - nor, CS_LAGR_TEMPERATURE_SEEN);
  ...
  * ======================================================================= */
  if (extra->radiative_model > 0) {
    ...
    cs_real_t p_eps  = p_set.attr_real(p_id, CS_LAGR_EMISSIVITY);

    if (nor == 1) {

      cs_real_t prev_p_diam = p_set.attr_n_real(p_id, 1, CS_LAGR_DIAMETER);
      cs_real_t prev_p_temp = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE);

      cs_real_t srad =    cs_math_pi * pow(prev_p_diam, 2.0) * p_eps
                        * (extra->rad_energy->val[cell_id]
                      - 4.0 * _c_stephan * pow (prev_p_temp,4));
      pip =   p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE_SEEN)
      ...
      cs_real_t p_temp = p_set.attr_n_real(p_id, 0, CS_LAGR_TEMPERATURE);

      cs_real_t srad =    cs_math_pi * pow(p_diam, 2.0) * p_eps
                        * (extra->rad_energy->val[cell_id]
                      - 4.0 * _c_stephan *  pow(p_temp , 4));
      pip =  p_set.attr_real(p_id, CS_LAGR_TEMPERATURE_SEEN)
            + tcarac * srad / p_cp /p_mass;
  ...
  cs_lagr_sde_attr(CS_LAGR_TEMPERATURE, p_id, nor, dt_part, tcarac, pip);
}
                 ...
                 int                   nor)
{
  ...
  if (p_set.p_am->source_term_displ != nullptr) {
    if (p_set.p_am->source_term_displ[CS_LAGR_TEMPERATURE_SEEN] >= 0)
      ltsvar   = 1;
  }
  ...
  if (nor == 1) {
    ...
    p_set.attr_real(p_id, CS_LAGR_TEMPERATURE_SEEN) = ter1 + ter2;
        ...
        cs_real_t            *cpght)
{
  ...
  cs_real_t init_diam      = p_set.attr_real(p_id, CS_LAGR_INITIAL_DIAMETER);
  cs_real_t shrink_diam    = p_set.attr_real(p_id, CS_LAGR_SHRINKING_DIAMETER);
  ...
  cs_real_t *part_coke_mass      =
    p_set.attr_n_real_ptr(p_id, 0, CS_LAGR_COKE_MASS);
  cs_real_t *prev_part_coke_mass =
    p_set.attr_n_real_ptr(p_id, 1, CS_LAGR_COKE_MASS);

  cs_real_t *part_coal_mass      =
    p_set.attr_n_real_ptr(p_id, 0, CS_LAGR_COAL_MASS);
  cs_real_t *prev_part_coal_mass =
    p_set.attr_n_real_ptr(p_id, 1, CS_LAGR_COAL_MASS);
  ...
  cs_real_t sherw = 2 + 0.55 * sqrt(re_p) * cbrt(prt);
  ...
  aux1 = p_set.attr_real(p_id, CS_LAGR_WATER_MASS);
  ...
  _lagsec(p_id, dt_part, nor, diftl0, layer_vol, mwat_max, sherw, radius, tempct,
          mlayer, mwater, fwat);
  ...
  aux3 = p_set.attr_real(p_id, CS_LAGR_TEMPERATURE_SEEN) + _tkelvi;
  ...
  if (nor == 1) {
    ...
    cs_real_t mwat = p_set.attr_n_real(p_id, 1, CS_LAGR_WATER_MASS) - aux1;

    /* Clipping */
    if (mwat < precis)
      mwat = 0.0;

    p_set.attr_real(p_id, CS_LAGR_WATER_MASS) = mwat;
    ...
    cs_real_t mwat
      = 0.5 * (  p_set.attr_n_real(p_id, 0, CS_LAGR_WATER_MASS)
               + p_set.attr_n_real(p_id, 1, CS_LAGR_WATER_MASS)
               - aux1);
  ...
  if (part_coal_mass[l_id_het] >= 0.001 * mlayer[l_id_het]) {
    ...
    p_set.attr_real(p_id, CS_LAGR_SHRINKING_DIAMETER) = 2.0 * radius[l_id_het];
  }
  else {
    ...
    if (l_id_het == 0) {
      ...
      p_set.attr_real(p_id, CS_LAGR_SHRINKING_DIAMETER) = aux5;
  ...
  shrink_diam = p_set.attr_real(p_id, CS_LAGR_SHRINKING_DIAMETER);
  ...
  cs_real_t mwat = p_set.attr_real(p_id, CS_LAGR_WATER_MASS);

  aux1 += mwat + coal_model->xashch[co_id] * mp0;

  p_set.attr_real(p_id, CS_LAGR_MASS) = aux1;
          ...
          int                   nor)
{
  ...
  cs_real_t dia = p_set.attr_n_real(p_id, 1, CS_LAGR_DIAMETER);
  //cs_real_t surf_p = cs_math_pi*cs_math_pow2(dia) * 0.25;

  /* Drop diameter based Reynolds number */
  const cs_real_t *vel_p
    = p_set.attr_real_ptr(p_id, CS_LAGR_VELOCITY);
  const cs_real_t *vel_s
    = p_set.attr_real_ptr(p_id, CS_LAGR_VELOCITY_SEEN);
  cs_real_t rho_h = extra->cromf->val[cell_id];
  cs_real_t mu_h  = extra->viscl->val[cell_id];

  /* local Reynolds number */
  cs_real_t re_p = rho_h * cs_math_3_distance(vel_p, vel_s) * dia / mu_h;

  /* Prandtl number */
  cs_real_t cp_h = cs_air_cp_humidair(x[cell_id], x_s[cell_id]);
  cs_real_t pr   = cp_h * mu_h / lambda_h;

  cs_real_t t_l_p
    = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE);
  cs_real_t temp_h
    = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE_SEEN);
  ...
  /* Ranz-Marshall or Hughmark when re_p <= 776.06 && pr <= 250. */
  cs_real_t nusselt = 2. + 0.6*sqrt(re_p) * cbrt(pr);
  /* Hughmark when re_p > 776.06 && pr <= 250. */
  if (re_p > 776.06 && pr <= 250.) {
    nusselt = 2. + 0.27*pow(re_p, 0.62) * cbrt(pr);
  }

  /* Integration of droplet diameter and mass
     ---------------------------------------- */

  /* Droplet diameter */

  cs_real_t mass_p = p_set.attr_n_real(p_id, 1, CS_LAGR_MASS);

  // TODO use rho_p instead
  cs_real_t droplet_dia = cbrt(6.*mass_p/(cs_math_pi * rho_l));
  p_set.attr_real(p_id, CS_LAGR_DIAMETER) = droplet_dia;

  /* Evaporation
     -------------------------- */

  /* Particle's mass */
  cs_real_t mass = p_set.attr_n_real(p_id, 1, CS_LAGR_MASS);

  /* Vapour diffusion coefficient in the carrier phase */
  cs_real_t Dv = lambda_h / (rho_h * cp_h * xlew);

  /* Prandtl number */
  cs_real_t nu_h = mu_h / rho_h;
  cs_real_t prt = nu_h / Dv;
  ...
  cs_real_t temp_p = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE);
  ...
  cs_real_t d_time_evap = 2. * rho_h * Dv * sherw
                        * log((1.0 - y_v_far) / (1.0 - y_v))
                        / (rho_l * cs_math_pow2(dia));

  /* First order time scheme resolution (nor=1) */
  if (nor == 1) {

    cs_real_t aux = exp(- dt_part * d_time_evap);
    dia = dia * aux;
    p_set.attr_real(p_id, CS_LAGR_DIAMETER) = dia;
  }
    ...
    cs_real_t aux1 = exp(- dt_part * d_time_evap);
    cs_real_t ter1 = 0.5 * dia * aux1;

    cs_real_t daux2 = 0.0;//FIXME
    cs_real_t aux2 = exp(- daux2);
    cs_real_t ter2 = 0.5 * dia * aux2;
  ...
  p_set.attr_real(p_id, CS_LAGR_MASS) = new_mass;

  /* Temperature of the particle seen by the fluid */
  cs_real_t cp = p_set.attr_n_real(p_id, 1, CS_LAGR_CP);

  /* Characteristic time*/
  cs_real_t time_temp_p = mass*cp/(cs_math_pi*nusselt*lambda_h*dia);

  /* Radiative term */
  cs_real_t Srad = 0.0; //Here, no radiative term

  if (nor == 1) {
    cs_real_t temp_s = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE_SEEN);

    /* Scheme resolution (nor=1)*/
    cs_real_t xi = temp_s - time_temp_p * mass_evap * lv / (mass * cp);
    aux = exp(- dt_part / time_temp_p);

    cs_real_t ter1 = p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE) * aux;
    cs_real_t ter2 = xi * (1. - aux);

    temp_p = ter1 + ter2;
    p_set.attr_real(p_id, CS_LAGR_TEMPERATURE) = temp_p;

  }
  else if (nor == 2) {
    cs_real_t temp_s = p_set.attr_n_real(p_id, 0, CS_LAGR_TEMPERATURE_SEEN);
    ...
    cs_real_t ter1 = 0.5 * p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE) * aux1;

    cs_real_t daux2 = 0.0; //FIXME
    cs_real_t aux2 = exp(- daux2);
    cs_real_t ter2 = 0.5 * p_set.attr_n_real(p_id, 1, CS_LAGR_TEMPERATURE) * aux2;

    cs_real_t ter3 = xi * (- aux1 + (1 - aux1)/(daux1));


    cs_real_t xi_n1 = 0.0; //FIXME
    cs_real_t ter4 = xi_n1 * (1. - (1. -aux2)/(daux2));

    temp_p = ter1 + ter2 + ter3 + ter4;
