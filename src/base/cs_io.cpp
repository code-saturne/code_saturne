...
static cs_map_name_to_id_t  *_cs_io_map[2] = {nullptr, nullptr};
static cs_io_log_t  *_cs_io_log[2] = {nullptr, nullptr};
              ...
              size_t         echo)
{
  ...
  cs_io->buffer_size = 0;
  cs_io->buffer = nullptr;

  cs_io->n_vals = 0;
  cs_io->type_size = 0;
  cs_io->sec_name = nullptr;
  cs_io->type_name = nullptr;
  cs_io->data = nullptr;
...
#endif
{
  ...
  if (_cs_io_map[cs_io->mode] != nullptr) {

    int mode = cs_io->mode;
    int i = cs_map_name_to_id(_cs_io_map[mode], name);
    cs_io_log_t *l = nullptr;
  ...
  case CS_INT32:
    {
      ...
      if (buffer_type == CS_INT32) {
        int32_t *_buffer = static_cast<int32_t *>(buffer);
        for (ii = 0; ii < n_elts; ii++)
          _dest[ii] = _buffer[ii];
      }
      else if (buffer_type == CS_INT64) {
        int64_t *_buffer = static_cast<int64_t *>(buffer);
        for (ii = 0; ii < n_elts; ii++)
          _dest[ii] = _buffer[ii];
      }
      if (buffer_type == CS_UINT32) {
        ...
        uint64_t *_buffer = static_cast<uint64_t *>(buffer);
        for (ii = 0; ii < n_elts; ii++)
          _dest[ii] = _buffer[ii];
      }
    }
 ...
 * Otherwise, if this pointer is passed as nullptr, memory is allocated
 ...
 *   global_num_end   <-- global number of past-the end block item
 *                        (1 to n numbering)
 *   elts             <-> pointer to data array, or nullptr
 *   inp              --> input kernel IO structure
 *
 * returns:
                 ...
                 cs_io_t                   *inp)
{
  ...
  if (inp->data != nullptr)  /* Reset for next read */
    inp->data = nullptr;
 ...
 *
 * The magic string may be nullptr, if we choose to ignore it.
 *
 * parameters:
              ...
              cs_io_t        *outp)
{
  ...
  name_pad_size = 8-(name_size%8); /* At least 1 nullptr
 ...
 *
 * The magic string may be nullptr only in read mode, if we choose to ignore it.
 *
 * parameters:
 *   name         <-- file name
 *   magic_string <-- magic string associated with file type
...
cs_io_finalize(cs_io_t **cs_io)
{
  ...
  if (_cs_io->mode == CS_IO_MODE_WRITE)
    cs_io_write_global("EOF", 0, 0, 0, 0, CS_DATATYPE_NULL, nullptr, _cs_io);
...
cs_io_get_index_size(const cs_io_t  *inp)
{
  ...
  if (inp != nullptr && inp->index != nullptr)
 ...
 *
 * returns:
 *   pointer to section name if id in index range, nullptr otherwise
 *----------------------------------------------------------------------------*/
                           ...
                           size_t               id)
{
  ...
  inp->sec_name = (char *)(inp->buffer + 56);
  inp->type_name = nullptr; /* should not be needed now that datatype is known */
           ...
           cs_io_t                   *pp_io)
{
  ...
  if (pp_io->data == nullptr) {
    ...
    pp_io->data = nullptr; /* Reset for next read */
  }
