                  ...
                  MPI_Comm               comm)
{
  ...
  cs_gnum_t nnh_elts[2] = { (cs_gnum_t)numbering->n_no_adj_halo_elts, 0 };
 ...
 * \param[in]  numbering    pointer to cs_numbering_t structure (or nullptr)
 */
/*----------------------------------------------------------------------------*/
                      ...
                      const cs_numbering_t  *numbering)
{
  ...
  if (cs_glob_n_ranks > 1) {
    ...
    for (int i = 0; i <= CS_NUMBERING_THREADS; i++) {
      if (type_counts[i] > 0) {
...
cs_numbering_dump(const cs_numbering_t  *numbering)
{
  ...
  if (numbering->group_index != nullptr) {
