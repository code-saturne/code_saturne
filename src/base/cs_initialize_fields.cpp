...
cs_initialize_fields_stage_0(void)
{
  ...
  if (CS_F_(t) != nullptr) {
    CS_F_(t)->get_val_s().set_to_val(ctx, fluid_props->t0);
  }

  /* Initialize boundary temperature to "marker" if present */
  if (CS_F_(t_b) != nullptr) {
    CS_F_(t_b)->get_val_s().set_to_val(ctx, -cs_math_big_r);
  }
  ...
  CS_F_(dt)->get_val_s().set_to_val(ctx, cs_glob_time_step->dt_ref);

  /* Initialize physical properties
     ------------------------------ */

  /* Density */
  CS_F_(rho)->get_val_s().set_to_val(ctx, fluid_props->ro0);
  CS_F_(rho_b)->get_val_s().set_to_val(ctx, fluid_props->ro0);

  /* Boussinesq (set to impossible value at this stage) */
  if (cs_glob_velocity_pressure_model->idilat == 0) {
    cs_field_by_name("thermal_expansion")->get_val_s().set_to_val(ctx, -1.);
  }

  /* Molecular viscosity */
  CS_F_(mu)->get_val_s().set_to_val(ctx, fluid_props->viscl0);

  /* Specific heat */
  if (fluid_props->icp >= 0) {
    CS_F_(cp)->get_val_s().set_to_val(ctx, fluid_props->cp0);
  }
  ...
  if (cs_glob_physical_model_flag[CS_COMPRESSIBLE] < 0) {
    ...
    f->get_val_s().set_to_val(ctx, -cs_math_infinite_r);
  }
  ...
  if (cs_glob_physical_model_flag[CS_GAS_MIX] >= 0) {
    ...
    f->get_val_s().set_to_val(ctx, fluid_props->xmasmr);
  }
  ...
  for (int f_id = 0; f_id < n_fields; f_id++) {
    ...
    if (ifcvsl >= 0) {
      ...
      for (int i = 0; i < f_vsl->n_time_vals; i++) {
        f_vsl->get_val_s(i).set_to_val(ctx, visls_0);
      }
    }
  }
  ...
  if (cs_glob_ale != CS_ALE_NONE) {
    ...
    if (idftnp & CS_ANISOTROPIC_LEFT_DIFFUSION) {
      ...
      auto mesh_visc_view = f_mesh_visc->get_val_t();
      ...
      f_mesh_visc->get_val_s().set_to_val(ctx, 1);
    }
  ...
  if (cs_glob_porous_model > 0) {
    ...
    if (cs_glob_porosity_from_scan_opt->compute_porosity_from_scan)
      porosity->get_val_s().set_to_val(ctx, 0.);
      ...
      porosity->get_val_s().set_to_val(ctx, 1.);

    /* Tensorial porosity */
    if (cs_glob_porous_model == 2) {
      auto tens_por_view =
        cs_field_by_name("tensorial_porosity")->get_val_t();
      cs_real_t ref_val[6] = {1., 1., 1., 0., 0., 0.};
      ctx.parallel_for(m->n_cells_with_ghosts, CS_LAMBDA (cs_lnum_t c_id) {
        for (int i = 0; i < 6; i++)
  ...
  CS_F_(p)->get_val_s().set_to_val(ctx, cs_glob_fluid_properties->pred0);
  ...
  if (cs_glob_vof_parameters->vof_model > 0) {
    ...
    f_vf->get_val_s().set_to_val(ctx, clvfmn);
  }
  ...
  for (int i = 0; i < n_fields; i++) {
    ...
    if (dl_id > -1) {
      ...
      f_dl->get_val_s().set_to_val(ctx, 1.0);
    }
    ctx.wait();
  }
