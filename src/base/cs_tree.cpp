                     ...
                     const char       *path)
{
  ...
  while (start < path_len) {
    ...
    if (_nodes->children == nullptr)
      _nodes = cs_tree_add_child(_nodes, name);
    else
      _nodes = _nodes->children;

    for (_node = _nodes; _node != nullptr; _node = _node->next)
 ...
 * \brief  Search for a node located at path from node
 *
 * If the node does not exist, nullptr is returned.
 *
 * \param[in] parent  pointer to the node where we start searching
 * \param[in] path    string describing the path access
 *
 * \return a pointer to the node, or nullptr
 */
/*----------------------------------------------------------------------------*/
               ...
               const char      *sub_path)
{
  ...
  while (retval == nullptr && tn != nullptr) {
 ...
 *
 * \param[in]  name  name of the node, or nullptr
 *
...
cs_tree_node_free(cs_tree_node_t  **pnode)
{
  ...
  if (root->children != nullptr) { /* There is at least one child */
 ...
 *
 * \return string value associated to tag if found, or nullptr
 */
/*----------------------------------------------------------------------------*/
...
cs_tree_node_get_child(cs_tree_node_t  *node,
                       const char      *name)
{
  cs_tree_node_t  *child = nullptr;
  if (node != nullptr) {

    child = node->children;
    while (child != nullptr) { /* Search for the node with the given name */
 ...
 *
 * \return pointer to next sibling node with same name, or nullptr
 */
/*----------------------------------------------------------------------------*/
...
{
  cs_tree_node_t  *sibling = nullptr;
  if (node != nullptr) {

    sibling = node->next;
    while (sibling != nullptr) { /* Search for the node with the given name */
 ...
 *
 * \param[in]  node  pointer to a cs_tree_node_t to access, or nullptr
 *
 * \return  associated string, or nullptr
 */
/*----------------------------------------------------------------------------*/
 ...
 * \param[in]  node  pointer to a cs_tree_node_t to access, or nullptr
 *
 * \return  pointer to associated array, or nullptr
 */
/*----------------------------------------------------------------------------*/
...
cs_tree_node_get_values_bool(cs_tree_node_t  *node)
{
  ...
  if (node != nullptr) {
    ...
    if (node->flag & CS_TREE_NODE_BOOL)
      ...
      char *s = static_cast<char *>(node->value);
...
{
  const cs_real_t *retval = nullptr;
 ...
 *
 * \return  pointer to associated values, or nullptr
 */
/*----------------------------------------------------------------------------*/
                  ...
                  const cs_tree_node_t   *node)
{
  ...
  if (node->name == nullptr)
    cs_log_printf(log, "%sname: nullptr\n", shift);
  else
  ...
  if (node->value != nullptr) {
    ...
    case 0:
      bft_error(__FILE__, __LINE__, 0,
                " Incompatibility: node->value != nullptr and node->size = 0.\n");
      break;
                  ...
                  const char      *name)
{
  ...
  if (parent == nullptr) { /* node is a root */
    node->parent = nullptr;
  ...
  if (child == nullptr) { /* First child is this node */
                    ...
                    const char      *name)
{
  ...
  if (sibling == nullptr) { /* node is a root */
    node->parent = nullptr;
    node->prev = node->next = nullptr;
    return node;
  }
 ...
 * descendants, with the path separator indicated by a "/".
 *
 * If current is nullptr, this function behaves as \ref cs_tree_find_node.
 *
 * \param[in]  root      pointer to the root node where we start searching
                       ...
                       const char      *sub_path)
{
  ...
  if (current != nullptr) {
    ...
    if (retval == nullptr) {
      ...
      while (retval == nullptr && tn != root && tn != nullptr) {
        if (tn->next != nullptr) {
          tn = tn->next;
          if (tn != nullptr) {
             ...
             const cs_tree_node_t   *node)
{
  ...
  if (node->children != nullptr) { /* There is at least one child */
