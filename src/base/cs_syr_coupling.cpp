...
static cs_syr_coupling_t  **_syr_couplings = nullptr;
               ...
               char               *op_name_recv)
{
  ...
  if (op_name_recv != nullptr && cs_glob_rank_id > -1) {
                               ...
                               const cs_time_step_t  *ts)
{
  ...
  auto syr_coupling = static_cast<const cs_syr_coupling_t *>(coupling);
  cs_syr_coupling_ent_t *coupling_ent = nullptr;
  ...
  for (type_id = 0; type_id < 2; type_id++) {
    ...
    if (coupling_ent != nullptr) {
      ...
      if (coupling_ent->post_mesh_id != 0) {
        ...
        const cs_real_t *cell_temp = nullptr;
        const cs_real_t *face_temp = nullptr;
           ...
           cs_syr_coupling_ent_t  *coupling_ent)
{
  ...
  if (coupling_ent->n_elts > 0) {
    if (coupling_ent->solid_temp == nullptr) { /* surface coupling */
      CS_MALLOC(coupling_ent->solid_temp, coupling_ent->n_elts, cs_real_t);
    ...
    if (coupling_ent->elt_dim == syr_coupling->dim) { /* volume coupling */
      if (coupling_ent->flux == nullptr) {
        CS_MALLOC(coupling_ent->flux, coupling_ent->n_elts, float);
        for (cs_lnum_t i = 0; i < coupling_ent->n_elts; i++)
          coupling_ent->flux[i] = 0;
      }
    }
  }
  coupling_ent->tfluid_tmp = nullptr;
...
_create_coupled_ent(cs_syr_coupling_t  *syr_coupling,
                    ...
                    int                 elt_dim)
{
  char *coupled_mesh_name = nullptr;
  bool      ext_syr = false;
  cs_lnum_t n_exterior = 0;
  cs_gnum_t n_ext = 0;
  cs_coord_t *elt_centers = nullptr;
  fvm_nodal_t *location_elts = nullptr;
  float *cs_to_syr_dist = nullptr;
  float *syr_to_cs_dist = nullptr;
  ...
  assert(syr_coupling != nullptr);

  /* Initialization */

  CS_MALLOC(coupling_ent, 1, cs_syr_coupling_ent_t);

  coupling_ent->locator = nullptr;
  coupling_ent->elt_dim = elt_dim;

  coupling_ent->n_elts = 0;
  coupling_ent->elts = nullptr;

  coupling_ent->post_mesh_id = 0;
  coupling_ent->solid_temp = nullptr;
  coupling_ent->flux = nullptr;
  coupling_ent->tfluid_tmp = nullptr;

  coupling_ent->hvol = nullptr;
  ...
  if (elt_dim == syr_coupling->dim) {
    ...
    CS_MALLOC(coupling_ent->hvol, coupling_ent->n_elts, double);
    CS_MALLOC(coupling_ent->solid_temp, coupling_ent->n_elts, cs_real_t);
  ...
  if (syr_coupling->visualization != 0 && _is_cdo_thermal == 0) {
    ...
    const float *b_dist = nullptr, *v_dist = nullptr;
    ...
    cs_post_write_var(coupling_ent->post_mesh_id,
                      ...
                      nullptr,
                      b_dist,
                      nullptr);  /* time-independent variable */
  ...
  if (elt_dim == syr_coupling->dim - 1) {
    ...
    CS_MALLOC(syr_to_cs_dist, n_dist_elts, float);
...
{
  int coupl_id, ent_id;
  cs_syr_coupling_t *syr_coupling = nullptr;
  ...
  for (coupl_id = 0; coupl_id < _syr_n_couplings; coupl_id++) {
    ...
    for (ent_id = 0; ent_id < 2; ent_id++) {
      ...
      if (ce != nullptr) {
        ...
        if (syr_coupling->syr_name != nullptr)
          cs_log_printf(CS_LOG_PERFORMANCE,
                        _("\n  %s (%s):\n\n"),
...
_destroy_coupled_ent(cs_syr_coupling_ent_t  **coupling_ent)
{
  ...
  if (ce->locator != nullptr)
    ce->locator = ple_locator_destroy(ce->locator);

  if (ce->solid_temp != nullptr)
    CS_FREE(ce->solid_temp);
  if (ce->flux != nullptr)
    CS_FREE(ce->flux);
                 ...
                 const cs_real_t        *var)
{
  ...
  assert(coupling_ent != nullptr);

  if (coupling_ent->post_mesh_id == 0)
    return;

  assert(coupling_ent->solid_temp != nullptr);
  assert(step == 0 || coupling_ent->flux != nullptr);
...
{
  cs_syr_coupling_t  *retval = nullptr;
                     ...
                     int          visualization)
{
  ...
  for (int i = 0;
       i < _syr_n_couplings && syr_coupling == nullptr;
       i++) {

    if (strcmp(_syr_couplings[i]->syr_name, syr_name) == 0) {
      syr_coupling = _syr_couplings[i];

      CS_FREE(syr_coupling->syr_name);
      CS_FREE(syr_coupling->b_location_ids);
      CS_FREE(syr_coupling->v_location_ids);

      assert(syr_coupling->faces == nullptr);  /* not built yet at this stage */
      assert(syr_coupling->cells == nullptr);
    }
  }
  ...
  if (syr_coupling == nullptr) {
    CS_REALLOC(_syr_couplings,
               _syr_n_couplings + 1, cs_syr_coupling_t *);
    CS_MALLOC(syr_coupling, 1, cs_syr_coupling_t);
  ...
  syr_coupling->syr_name = nullptr;

  if (syr_name != nullptr) {
    CS_MALLOC(syr_coupling->syr_name, strlen(syr_name) + 1, char);
    strcpy(syr_coupling->syr_name, syr_name);
  }
  else {
    CS_MALLOC(syr_coupling->syr_name, 1, char);
  ...
  syr_coupling->b_location_ids = nullptr;
  syr_coupling->v_location_ids = nullptr;

  syr_coupling->faces = nullptr;
  syr_coupling->cells = nullptr;
                   ...
                   int                  location_id)
{
  ...
  if (l_type == CS_MESH_LOCATION_BOUNDARY_FACES) {
    ...
    CS_REALLOC(syr_coupling->b_location_ids, syr_coupling->n_b_locations, int);
    ...
    CS_REALLOC(syr_coupling->v_location_ids, syr_coupling->n_v_locations, int);
...
_syr_coupling_all_destroy(void)
{
  ...
  for (i_coupl = 0; i_coupl < _syr_n_couplings; i_coupl++) {
    ...
    if (syr_coupling->faces != nullptr)
      _destroy_coupled_ent(&(syr_coupling->faces));
    if (syr_coupling->cells != nullptr)
    ...
    CS_FREE(syr_coupling);

  } /* End of loop on _syr_couplings */

  _syr_n_couplings = 0;
  CS_FREE(_syr_couplings);
                         ...
                         int                 mode)
{
  ...
  CS_MALLOC(send_var, n_dist*2, double);
  ...
  if (mode == 1 && coupling_ent->n_elts > 0) {
    ...
    assert(coupling_ent->hvol != nullptr);
                         ...
                         cs_real_t                 ctbexp[])
{
  ...
  const double  *hvol = nullptr;
  const cs_real_t  *solid_temp = nullptr;
  cs_syr_coupling_ent_t  *ent = nullptr;
  ...
  assert(hvol != nullptr || ent->n_elts == 0);
  assert(solid_temp != nullptr || ent->n_elts == 0);
 ...
 * determined, so nullptr is returned.
 *----------------------------------------------------------------------------*/
 ...
 * \param[in] boundary_criteria surface selection criteria, or nullptr
 * \param[in] volume_criteria   volume selection criteria, or nullptr
                       ...
                       int          visualization)
{
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (sel_criteria[i] != nullptr) {
      for (int j = 0; j < n_locations && sel_criteria[i] != nullptr; j++) {
                              ...
                              int        bc_type[])
{
  ...
  for (int field_id = 0 ; field_id <  n_fields; field_id++) {
    ...
    for (int cpl_id = 0; cpl_id < n_cpl; cpl_id++) {
      ...
      if (coupling_ent == nullptr)  /* ignore if volume-only */
        continue;
      ...
      cs_lnum_t  *f_ids;
      CS_MALLOC(f_ids, n_cpl_faces, cs_lnum_t);
      ...
      cs_real_t *t_solid;
      CS_MALLOC(t_solid, n_cpl_faces, cs_real_t);
                              ...
                              cs_real_t        v_fluid[])
{
  ...
  CS_MALLOC(f_ids, n_b_faces, cs_lnum_t);
  CS_MALLOC(t_fluid, n_b_faces, cs_real_t);
  CS_MALLOC(h_cpl, n_b_faces, cs_real_t);
  ...
  if (cs_glob_thermal_model->thermal_variable == CS_THERMAL_MODEL_ENTHALPY) {
    CS_MALLOC(wa, n_b_faces, cs_real_t);
    ...
    CS_MALLOC(wa, n_cells, cs_real_t);
...
cs_syr_coupling_exchange_volume(void)
{
  ...
  for (int field_id = 0 ; field_id <  n_fields; field_id++) {
    ...
    for (int cpl_id = 0; cpl_id < n_cpl; cpl_id++) {
      ...
      if (coupling_ent == nullptr)  /* ignore if surface-only */
        continue;
      ...
      CS_MALLOC(c_ids, n_cpl_cells, cs_lnum_t);
      CS_MALLOC(t_fluid, n_cpl_cells, cs_real_t);
      CS_MALLOC(h_vol, n_cpl_cells, cs_real_t);
                                    ...
                                    cs_real_t  st_imp[])
{
  ...
  for (int cpl_id = 0; cpl_id < n_cpl; cpl_id++) {
    ...
    CS_MALLOC(c_ids, n_cpl_cells, cs_lnum_t);
    CS_MALLOC(t_fluid, n_cpl_cells, cs_real_t);
    CS_MALLOC(ctbimp, n_cpl_cells, cs_real_t);
    CS_MALLOC(ctbexp, n_cpl_cells, cs_real_t);
...
)
{
  ...
  if (syr_coupling == nullptr)
    ...
    CS_MALLOC(_recv_array, n_arrays * n_cpl_elts, double);
...
)
{
  ...
  if (syr_coupling == nullptr)
    ...
    CS_MALLOC(_send_array, n_dist, double);
