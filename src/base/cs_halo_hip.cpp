/*============================================================================
 * Functions dealing with ghost cells using HIP.
 *============================================================================*/
...
_gather_block_hip(const cs_lnum_t   send_blocks[],
                 const cs_lnum_t   ids[],
                 const T           src[],
                 T                 dest[])
{
...
template <typename T>
__global__ static void
_gather_block_strided_hip(const cs_lnum_t   send_blocks[],
                         cs_lnum_t         stride,
                         const cs_lnum_t   ids[],
                         const T           src[],
                         T                 dest[])
...
template <typename T>
__global__ static void
_gather_full_strided_hip(cs_lnum_t         n,
                        cs_lnum_t         stride,
                        const cs_lnum_t   ids[],
                        const T           src[],
                        T                 dest[])
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Pack cs_real_t halo data to send into dense buffer, using HIP.
 *
...
void
cs_halo_hip_pack_send_buffer(const cs_halo_t   *halo,
                              hipStream_t       stream,
  ...
  if (data_type == CS_REAL_TYPE) {
    ...
    if (stride == 1) {
      if (send_blocks == nullptr)
        _gather_full_hip<<<n_blocks, block_size, 0, stream>>>
          (n_send, send_list, var, buffer);
      else
        _gather_block_hip<<<n_blocks, block_size, 0, stream>>>
          (send_blocks, send_list, var, buffer);
    }
    else {
      if (send_blocks == nullptr)
        _gather_full_strided_hip<<<n_blocks*stride, block_size, 0, stream>>>
          (n_send, stride, send_list, var, buffer);
      else
        _gather_block_strided_hip<<<n_blocks, block_size, 0, stream>>>
    ...
    if (send_blocks == nullptr)
      _gather_full_strided_hip<<<n_blocks, block_size, 0, stream>>>
