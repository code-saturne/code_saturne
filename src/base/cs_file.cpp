                     ...
                     size_t      ni)
{
  ...
  if (f->offset + nb > f->in_mem_size) {
    if (f->allow_eof) {
      if (f->offset < (cs_file_off_t)f->in_mem_size)
 ...
 *   line:      <-> file line number if available, or nullptr.
 *   allow_eof: <-- 1 if EOF is allowed, 0 if considered an error.
 *
 * returns:
 *   s on success, nullptr on error or when end of file occurs and
                    ...
                    cs_gnum_t   global_num_end)
{
  ...
  if (f->n_ranks > 1) {
    ...
    do {
      ...
      write_buf = cs_file_serializer_advance(&s, nullptr);

      if (write_buf != nullptr) /* only on rank 0 */
        s.count[dist_rank]
 ...
 *
 * returns:
 *   pointer to gathered values buffer for gathering rank, nullptr for others
 *----------------------------------------------------------------------------*/
...
_gather_block_sizes(cs_file_t   *f,
                    ...
                    cs_gnum_t   *global_num_end)
{
  unsigned char *gather_buf = nullptr;
               ...
               cs_gnum_t   *global_num_end)
{
  ...
  unsigned char *gather_buf = static_cast<unsigned char *>(
    _gather_block_sizes(f, size, global_num_start, global_num_end));
                ...
                size_t       size)
{
  ...
  if (f->rank % f->rank_step == 0) {
    ...
    unsigned char *scatter_buf       = static_cast<unsigned char *>(io_buf);
 ...
 *
 * \return pointer to cs_file_t file descriptor (nullptr in case of failure);
                     ...
                     size_t       ni)
{
  ...
  if (f->rank == 0 && f->sh != nullptr && f->method <= CS_FILE_STDIO_PARALLEL) {
    if (f->method == CS_FILE_STDIO_PARALLEL) {
      if (_file_seek(f, f->offset, CS_FILE_SEEK_SET) != 0)
        retval = 0;
    }
...
cs_file_tell(cs_file_t  *f)
{
  ...
  if (f->method == CS_FILE_STDIO_SERIAL && f->rank == 0 && f->sh != nullptr)
    retval = _file_tell(f);
  else if (f->method == CS_FILE_IN_MEMORY_SERIAL && f->rank == 0)
 ...
 * \param [in, out]  line  file line number if available, or nullptr.
 *
 * \return s on success, nullptr on error or when end of file occurs and
 *         no characters have been read.
 */
/*----------------------------------------------------------------------------*/
 ...
 * \param[out]   method  default file access method, or nullptr
 * \param[out]   hints   MPI-IO hints, or nullptr
 */
/*----------------------------------------------------------------------------*/
 ...
 *                                distributed blocks, or nullptr
 * \param[out]   block_comm       Handle to MPI communicator used for
 *                                distributed file block access, or nullptr
 * \param[out]   comm             Handle to main MPI communicator, or nullptr
 */
/*----------------------------------------------------------------------------*/
                         ...
                         MPI_Comm  *comm)
{
  ...
  if (block_rank_step != nullptr)
    *block_rank_step = _mpi_rank_step;

  if (block_comm != nullptr) {
...
cs_file_defaults_info(void)
{
  ...
  for (int mode = CS_FILE_MODE_READ; mode < CS_FILE_MODE_APPEND; mode++) {
    MPI_Info hints;
    cs_file_access_t method;

    cs_file_get_default_access(static_cast<cs_file_mode_t>(mode),
                               &method,
                               &hints);
 ...
 *
 * Note also that for ranks > 0, this function always returns nullptr,
 ...
 *                         the current block (size: 2), or nullptr; only on rank 0
 *
 * \return a pointer to the buffer containing new data (first call counts as
 *          new), or nullptr if we are finished; always nullptr on ranks > 0.
 */
/*----------------------------------------------------------------------------*/
 ...
 *         set to nullptr. If no means to list the directory or an error
 *         occured, the return value is simply nullptr.
 */
/*----------------------------------------------------------------------------*/
                 ...
                 const char  *end)
{
  ...
  /* If either pointers is nullptr, return 0 */
  if (path == nullptr || end == nullptr)
    retval = 0;
