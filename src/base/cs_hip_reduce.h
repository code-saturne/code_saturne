#ifndef CS_HIP_REDUCE_H
#define CS_HIP_REDUCE_H
...
cs_hip_reduce_warp_reduce_sum(volatile T  *stmp,
                               size_t       tid)
{
  if (stride == 1) {
...
template <size_t blockSize, size_t stride, typename T>
__device__ static void __forceinline__
cs_hip_reduce_block_reduce_sum(T       *stmp,
                                size_t   tid,
                                T       *sum_block)
{
  __syncthreads();
...
cs_hip_reduce_sum_single_block(size_t   n,
                                T       *g_idata,
                                T       *g_odata)
{
  ...
  if (stride == 1) {
    ...
    for (size_t j = blockSize/2; j > CS_HIP_WARP_SIZE; j /= 2) {
      if (tid < j) {
...
__device__ static void __forceinline__
cs_hip_reduce_warp_reduce(volatile T  *stmp,
                           size_t       tid)
{
  R reducer;
...
template <size_t blockSize, typename R, typename T>
__device__ static void __forceinline__
cs_hip_reduce_block_reduce(T       *stmp,
                            size_t   tid,
                            T       *rd_block)
{
  R reducer;
...
template <size_t blockSize, typename R, typename T>
__global__ static void
cs_hip_reduce_single_block(size_t   n,
                            T       *g_idata,
                            T       *g_odata)
{
