...
typedef struct {
  ...
  const cs_lnum_t  *max_id;   /*!< local cell ids, or nullptr if trivial */
} cs_volume_zone_id_t;
...
#   pragma omp parallel for if (n_cells > CS_THR_MIN)
    ...
    if (overlap_error[0] < _n_zones) {
      ...
      for (int i = 1; i < _n_zones; i++) {
        ...
        for (cs_lnum_t j = 0; j < z->n_elts; j++) {
          cs_lnum_t c_id = z->elt_ids[j];
          int z_id_prev = cs::abs(_zone_id[c_id]);
          if (z_id_prev == 0)
            _zone_id[c_id] = z->id;
          else if (   _zones[z_id_prev]->allow_overlay
                   && _zone_id[c_id] > 0)
...
cs_volume_zone_print_info(void)
{
  ...
  for (int i = 0; i < _n_zones; i++) {
    ...
    if (cell_f_vol != cell_vol && cell_f_vol != nullptr)
