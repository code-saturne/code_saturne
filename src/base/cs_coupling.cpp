...
static ple_coupling_mpi_set_t *_cs_glob_coupling_mpi_app_world = nullptr;
 ...
 * \param[in]  forced_app_type  name of app type, or nullptr
 */
/*----------------------------------------------------------------------------*/
...
{
  if (_cs_glob_coupling_mpi_app_world != nullptr)
...
cs_coupling_mesh_extents(const void  *mesh,
                         ...
                         double       extents[])
{
  const fvm_nodal_t *m      = static_cast<const fvm_nodal_t *>(mesh);
  ple_lnum_t         retval = 0;
