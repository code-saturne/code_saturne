...
int  cs_glob_hip_device_id = -1;

/* Other device parameters */

int  cs_glob_hip_max_threads_per_block = -1;
int  cs_glob_hip_max_block_size = -1;
int  cs_glob_hip_max_blocks = -1;
int  cs_glob_hip_n_mp = -1;

/* Stream pool */

static int            _cs_glob_hip_n_streams = -1;
static hipStream_t  *_cs_glob_hip_streams = nullptr;
...
bool cs_glob_hip_allow_graph = false;

// Shared memory size per block (based on know GPUs, queried later).
int cs_glob_hip_shared_mem_per_block = 48*1024;
...
{
  cs_mem_hip_set_prefetch_stream(0);
  hipStreamDestroy(_cs_glob_stream_pf);

  for (int i = 0; i < _cs_glob_hip_n_streams; i++) {
    hipStreamDestroy(_cs_glob_hip_streams[i]);
    CS_FREE(_r_host[i]);
    // _r_reduce is based on _r_host's device buffer
    CS_FREE(_r_grid[i]);
  }

  CS_FREE(_cs_glob_hip_streams);
 ...
 *
 * \return  pointer to host data if shared or mapped at the HIP level,
 *          NULL otherwise.
 */
/*----------------------------------------------------------------------------*/
...
hipStream_t
cs_hip_get_stream(int  stream_id)
{
  if (stream_id >= 0 && stream_id < _cs_glob_hip_n_streams)
    return _cs_glob_hip_streams[stream_id];
  else if (stream_id < 0)
    return nullptr;

  if (_cs_glob_hip_n_streams < 0) {
    cs_base_at_finalize(finalize_streams_);
    _cs_glob_hip_n_streams = 0;
  }

  CS_REALLOC(_cs_glob_hip_streams, stream_id+1, hipStream_t);
  ...
  for (int i = _cs_glob_hip_n_streams; i < stream_id+1; i++) {
    hipStreamCreate(&_cs_glob_hip_streams[i]);
    _r_elt_size[i] = 0;
    _r_grid_size[i] = 0;
    _r_host[i] = nullptr;
    _r_reduce[i] = nullptr;
  ...
  _cs_glob_hip_n_streams = stream_id+1;
...
hipStream_t
cs_hip_get_stream_prefetch(void)
{
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Return stream id in stream pool matching a given HIP stream.
 *
...
int
cs_hip_get_stream_id(hipStream_t  stream)
{
  for (int i = 0; i < _cs_glob_hip_n_streams; i++)
    if (stream == _cs_glob_hip_streams[i])
      return i;
 ...
 *
 * These buffers are used internally by HIP 2-stage operations, and are
...
void
cs_hip_get_2_stage_reduce_buffers(int            stream_id,
                                   cs_lnum_t      n_elts,
                                   size_t         elt_size,
                                   ...
                                   void*         &r_host)
{
  assert(stream_id > -1 && stream_id < _cs_glob_hip_n_streams);
...
extern "C" void
cs_base_hip_device_info(cs_log_t  log_id)
{
  int n_devices = 0;
  ...
  for (int i = 0; i < n_devices; i++) {
    ...
    CS_HIP_CHECK(hipGetDeviceProperties(&prop, i));
    unsigned long long mem = prop.totalGlobalMem / 1000000;

    cs_glob_hip_shared_mem_per_block = prop.sharedMemPerBlock;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief  Log information on available HIP version.
 *
 * \param[in]  log_id  id of log file in which to print information
 */
/*----------------------------------------------------------------------------*/
...
extern "C" void
cs_base_hip_compiler_info(cs_log_t  log_id)
{
  cs_log_printf(log_id,
                "    %s%d\n", _("HIP ARCH Number:     "), HIP_VERSION);
}
...
extern "C" void
cs_base_hip_nccl_info(cs_log_t  log_id)
{
#if defined(HAVE_NCCL)
  int link_version;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Set HIP device based on MPI rank and number of devices.
 *
...
extern "C" int
cs_base_hip_select_default_device(void)
{
  ...
  cs_glob_hip_device_id = device_id;
  ...
  const char *s = getenv("CS_HIP_ALLOC_DEVICE_UVM");
  if (s != nullptr) {
    int i = atoi(s);
  ...
  struct hipDeviceProp_t prop;
  CS_HIP_CHECK(hipGetDeviceProperties(&prop, device_id));
  cs_glob_hip_max_threads_per_block = prop.maxThreadsPerBlock;
  cs_glob_hip_max_block_size = prop.maxThreadsPerMultiProcessor;
  cs_glob_hip_max_blocks
    =   prop.multiProcessorCount
      * (prop.maxThreadsPerMultiProcessor / prop.maxThreadsPerBlock);
  cs_glob_hip_n_mp = prop.multiProcessorCount;

  /* Create default stream for prefetching */
  if (_cs_glob_stream_pf == 0) {
    hipStreamCreate(&_cs_glob_stream_pf);
    cs_mem_hip_set_prefetch_stream(_cs_glob_stream_pf);
  }
  ...
  s = getenv("CS_HIP_ALLOW_GRAPH");
  if (s != nullptr) {
    int i = atoi(s);
    cs_glob_hip_allow_graph = (i <= 0) ? false : true;
  }
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Return currently selected HIP devices.
 *
 * \return  selected device id, or -1 if no usable device is available
 */
/*----------------------------------------------------------------------------*/
