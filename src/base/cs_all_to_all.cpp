...
static uint64_t  *_all_to_all_trace = nullptr;
static FILE      *_all_to_all_trace_bt_log = nullptr;
 ...
 *   data             <-- element values
 *   dest_id          <-- element destination id, or nullptr
 *   recv_id          <-- element receive id (for reverse mode), or nullptr
                           ...
                           const int                  dest_rank[])
{
  ...
  unsigned const char *_data = static_cast<unsigned const char *>(data);

  assert(data != nullptr || (n_elts == 0 || stride == 0));
  ...
  else if (recv_id != nullptr) { /* reverse here */
    for (size_t j = 0; j < dc->send_size; j++) {
                           ...
                           const int                  dest_rank[])
{
  ...
  if (reverse && recv_id == nullptr) {
    ...
    size_t n_sub_elts = 0;
    if (reverse == false || recv_id == nullptr)
 ...
 *   dest_rank <-- destination rank (in direct mode), used here to reorder
 *                 data in reverse mode.
 *   dest_data <-> destination data buffer, or nullptr
 *
 * returns:
                        ...
                        MPI_Comm   comm)
{
  ...
  hc->elt_rank_index = nullptr;

  hc->send_buffer = nullptr;
  hc->_send_buffer = nullptr;

  hc->send_count = nullptr;
  hc->recv_count = nullptr;
  hc->send_displ = nullptr;
  hc->recv_displ = nullptr;
  hc->recv_count_save = nullptr;
...
_is_src_rank_info_needed(cs_all_to_all_t  *d)
{
  ...
  if (d->flags & CS_ALL_TO_ALL_NO_REVERSE) {
    if (d->flags & CS_ALL_TO_ALL_NEED_SRC_RANK)
      ...
      if (d->recv_id == nullptr && d->n_elts_dest > 0)
        retval = true;
    }
  }
  else
    ...
    case CS_ALL_TO_ALL_CRYSTAL_ROUTER:
      {
        ...
        int **p_src_rank = _is_src_rank_info_needed(d) ? &(d->src_rank) : nullptr;
        cs_crystal_router_get_data(cr,
                                   p_src_rank,
                                   &(d->recv_id),
                                   &(d->src_id),
  ...
  case CS_ALL_TO_ALL_CRYSTAL_ROUTER:
    {
      ...
      if (!reverse) {
        ...
        const cs_lnum_t *elt_id
          = (d->n_elts_dest < d->n_elts_dest_e) ? d->recv_id : nullptr;
  ...
  case CS_ALL_TO_ALL_CRYSTAL_ROUTER:
    {
      ...
      if (!reverse) {
        ...
        int **p_src_rank = (d->src_rank == nullptr) ? &(d->src_rank) : nullptr;
        cs_crystal_router_get_data(cr,
                                   p_src_rank,
                                   &(d->recv_id),
                                   &(d->src_id),
 ...
 * \param[in]  d   pointer to associated all-to-all distributor
 *
 * \return  array of source ranks (or nullptr)
 */
/*----------------------------------------------------------------------------*/
 ...
 *
 * \param[out]  rne_type  type of metadata exchange algorithm, or nullptr
 */
/*----------------------------------------------------------------------------*/
