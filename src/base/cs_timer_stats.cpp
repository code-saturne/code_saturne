...
static int  *_active_id = nullptr;
static cs_time_plot_t  *_time_plot = nullptr;
...
cs_timer_stats_initialize(void)
{
  ...
  id = cs_timer_stats_create(nullptr, "operations", "total");
  cs_timer_stats_start(id);

  id = cs_timer_stats_create(nullptr, "stages", "total");
  cs_timer_stats_start(id);
...
cs_timer_stats_increment_time_step(void)
{
  ...
  if (   _time_plot == nullptr && _time_id < _start_time_id + 1
      && cs_glob_rank_id < 1)
    _build_time_plot();
 ...
 *
 * \param[in]  parent_name  name of parent statistic, or nullptr
 * \param[in]  name         associated canonical name
 * \param[in]  label        associated label, or nullptr
 *
                      ...
                      const char  *label)
{
  ...
  const char *_parent_name = nullptr;
  int parent_id = -1;
  int root_id = -1;
...
cs_timer_stats_set_plot(int  id,
                        int  plot)
{
  if (id < 0 || id > _n_stats || _time_plot != nullptr) return;
