...
static cs_mem_realloc_t   *_cs_alt_realloc_func = nullptr;
static cs_mem_free_t      *_cs_alt_free_func = nullptr;
              ...
              int          line_num)
{
  ...
  if (file_name != nullptr)
    cs_mem_update_block_info(var_name,
                             file_name,
                             line_num,
                             nullptr,
                             &mib);
               ...
               int          line_num)
{
  ...
  */
  ...
  if (_cs_mem_global_init_mode > 1)
    mib_old = cs_mem_get_block_info(ptr);
  else
    mib_old = cs_mem_get_block_info_try(ptr);
  ...
  if (mib_old.mode >= CS_ALLOC_HOST_DEVICE_PINNED) {
    return _cs_alt_realloc_func(ptr, ni, size,
...
#if defined(HAVE_ACCEL)
  if (mib_old.mode >= CS_ALLOC_HOST_DEVICE_PINNED) {
    _cs_alt_free_func(ptr, var_name, file_name, line_num);
    return nullptr;
  }
#endif
...
{
  _cs_alt_realloc_func = realloc_func;
  _cs_alt_free_func = free_func;
}
