...
/* For all current compute capabilities, the warp size is 32; If it ever
   changes, it can be obtained through hipDeviceProp_t, so we could then
   replace this macro with a global variable */
#if defined(__GFX8__) || defined(__GFX9__)
  #define CS_HIP_WARP_SIZE 64
using cs_hip_mask_t = uint64_t;
#else
...
extern int  cs_glob_hip_device_id;

/* Other device parameters */

extern int  cs_glob_hip_shared_mem_per_block;
extern int  cs_glob_hip_max_threads_per_block;
extern int  cs_glob_hip_max_block_size;
extern int  cs_glob_hip_max_blocks;
extern int  cs_glob_hip_n_mp;  /* Number of multiprocessors */
...
extern bool cs_glob_hip_allow_graph;
                    ...
                    void          **buf_d)
{
  ...
  if (val_h != NULL) {
    ...
    if (alloc_mode == CS_ALLOC_HOST) {
      CS_HIP_CHECK(hipMalloc(&_buf_d, size));
      cs_hip_copy_h2d(_buf_d, val_h, size);
