...
const char *_coeff_name[] = {"bc_coeffs::a", "bc_coeffs::b",
                             ...
                             "bc_coeffs::val_f::0", "bc_coeffs::flux_diff::0",
                             "bc_coeffs::val_f::1"};
...
_read_legacy_field_info(cs_restart_t  *r)
{
  ...
  if (   n_cur[1] == n_old[1] && n_cur[2] == n_old[2] -1
    ...
    if (CS_F_(t) != nullptr || CS_F_(h) != nullptr) {
      us_shift = -1;
      pp_shift = n_cur[2];
    }
                           ...
                           cs_map_name_to_id_t  **old_field_map)
{
  ...
  *old_field_map = nullptr;

  /* Now read field names, in id order */

  int  *type_buf = nullptr;
 ...
 *                                 or nullptr; set to sum of 2^time_id for fields
 *                                 read (size: n_fields)
 */
/*----------------------------------------------------------------------------*/
 ...
 * \param[in]       t_id_flag  -1: all time values; 0: current values;
 *                             > 0: previous values
 * \param[in, out]  write_flag  optional flag to track fields written, or nullptr;
 ...
 * \param[in, out]  read_flag      optional flag to track fields read, or nullptr;
    ...
    for (int f_id = 0; f_id < n_fields; f_id++) {
      ...
      if (lnk_f_id > -1) {
        ...
        if (f_lnk_name != nullptr) {
          ...
          if (f_lnk_name != nullptr && f_lnk_name != f_lnk->name)
        ...
        if (retval == CS_RESTART_SUCCESS) {
          ...
          if (p[c_id] == nullptr && m->n_b_faces > 0) {
            ...
            if (c_id == 8) {
              ...
              CS_MALLOC_HD(f->bc_coeffs->flux_diff, n_vals, cs_real_t,
                           cs_alloc_mode);
              p[c_id] = f->bc_coeffs->flux_diff;
            }
            else {
              assert(c_id == 10);
