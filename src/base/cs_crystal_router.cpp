...
{
  int rank_id, n_ranks;
  cs_crystal_router_t *cr = nullptr;
 ...
 *   src_id     <-- optional parent element id (elt_idx indirection), or nullptr
 *   flags      <-- ordering and metadata flags
 *   comm       <-- associated MPI communicator
 *---------------------------------------------------------------------------*/
 ...
 * parameters:
 *   <-- cr        pointer to associated Crystal Router
 *   <-> dest_id   pointer to destination id array, or nullptr
 ...
 * parameters:
 *   <-- cr         pointer to associated Crystal Router
 *   --> src_rank   pointer to source rank array, or nullptr
 *   --> src_id     pointer to source id array, or nullptr
 *   --> data  pointer to (pointer to) destination data, or nullptr
 *----------------------------------------------------------------------------*/
 ...
 *   --> src_rank  pointer to source rank array, or nullptr
 *   --> src_id    pointer to source id array, or nullptr
 *   --> data_idx  pointer to data index, or nullptr
 *   --> data      pointer to destination data, or nullptr
 *----------------------------------------------------------------------------*/
                         ...
                         void                  *data)
{
  ...
  if ((cr->flags & reverse_flags) && cr->dest_id_end < n_elts
      && dest_id != nullptr && src_id != nullptr && src_rank != nullptr) {
 ...
 *                               for elt array), or nullptr
 * \param[in]  dest_id           element destination id, or nullptr
                           ...
                           MPI_Comm          comm)
{
  ...
  unsigned const char *_elt = static_cast<unsigned const char *>(elt);
  ...
  if (add_dest_id)
    cs_assert(dest_id != nullptr || n_elts == 0);
  ...
  for (size_t i = 0; i < n_elts; i++) {
    ...
    size_t j = (src_id == nullptr) ? i : (size_t)src_id[i];
 ...
 * \param[in]  elt               element values
 * \param[in]  src_id            optional parent element id (indirection
 *                               for elt and elt_idx arrays), or nullptr
 * \param[in]  dest_id           element destination id, or nullptr
 * \param[in]  dest_rank         destination rank for each element
 * \param[in]  comm              associated MPI communicator
 *
 ...
 *                              or nullptr
 * \param[out]      src_id      pointer to (pointer to) source id array, or nullptr
                           ...
                           void                 **data)
{
  ...
  int *_src_rank = nullptr;
  cs_lnum_t *_dest_id = nullptr;
  cs_lnum_t *_src_id = nullptr;
  cs_lnum_t *_data_index = nullptr;
 ...
 *                          and send (max_sizes[1]) sizes, or nullptr
 *
