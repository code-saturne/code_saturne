/*============================================================================
 * This file define functions needed for Calculate mean profile (scalars or vectors)
 * over the mesh
...
static void _compute_min_max(cs_lnum_t           n_vals,
                            const cs_real_t     var[],
                            cs_real_t          *min,
                            cs_real_t          *max)
{
...
static user_profile_med_t * _allocate_med_mesh_struct(cs_lnum_t n_layers)
{
    ...
    for (int layer_id=0;layer_id<n_layers;layer_id++){
        ...
        MEDCoupling::MEDCouplingUMesh *mesh=MEDCoupling::MEDCouplingUMesh::New(name,3);
        med_t->layer_mesh[layer_id]=mesh;
    }

    med_t->local_mesh = cs_medcoupling_mesh_create("Case_global_mesh", "all[]", 3);
...
static void _create_1d_sample_(user_profile_t *profile_t,
                               ...
                               cs_lnum_t layer_id)
{
    ...
    /*Total selected cells weigth could be calculated once and not for each layer but keep as this because of current code structure (avoid de create a dedicated function and re compute each cell weight*/
 ...
 *
 *----------------------------------------------------------------------------*/
static void _compute_sample_moment(cs_real_t *sample,
                                   cs_real_t *weights,
...

{
    ...
    /*Reset moment
      It is assume moment[0]=mu moment[1]=sigma*/
    moment[0]=0.0;
    moment[1]=0.0;
    cs_real_t mu=0.0;
    cs_real_t sigma=0.0;
    ...
    for (int iel=0;iel<n_elts_sample;iel++)
        w_tot+=weights[iel];

    /*sum over all MPI rank*/
    cs_parall_sum(1,CS_DOUBLE,&w_tot);
    ...
    _compute_min_max(n_elts_sample,
                     sample,
                     &min_sample,
                     &max_sample);
 ...
 *      h_i         --> height of each classes
 *      l_i         --> width of each classes
 *      c_i         --> center of each classes
...
static void _fill_histogram_classes_u_bandwidth(user_histogram_t *histogram_t,
                                                cs_real_t *sample,
                                                cs_real_t *weights,
                                                cs_lnum_t n_elts_sample,
                                                cs_real_t *bandwidth)
{
    ...
    *bandwidth=(max-min)/((cs_real_t)n_bin);
    ...
    for (int b_id=1;b_id<n_bin;b_id++){
        ...
        histogram_t->c_i[b_id]=histogram_t->c_i[b_id-1]+histogram_t->l_i[b_id-1]/2.0+histogram_t->l_i[b_id]/2.0;
        }
...
/*----------------------------------------------------------------------------
 * Compute histogram quantile given a provided Quantile array
 *
 * parameters:
 *
 * histogram_t      <--> pointer to histogram structure
...
static void _compute_histogram_quantile(user_histogram_t *histogram_t,
                                       ...
                                       )
{
    ...
    cs_real_t CumulProba=0.0;
    cs_lnum_t q_id_start=0;



    for (int b_id=0;b_id<n_bin;b_id++){
        CumulProba+=histogram_t->h_i[b_id];

        for (int q_id=q_id_start;q_id<n_quantile;q_id++){
            if (CumulProba>Quantile[q_id][1]){
                q_id_start++;
                cs_real_t dp_add = Quantile[q_id][1]-(CumulProba-h_i[b_id]);
                cs_real_t percent_l_i = dp_add / h_i[b_id];
                Quantile[q_id][0]=c_i[b_id]-l_i[b_id]/2.0+l_i[b_id]*percent_l_i;
   ...
   /*update histogram proba tot : it is possible to check the correct sum of h_i*/
   histogram_t->ptot=CumulProba;
}
 ...
 * ( scott rule : see OpenTunrs Histogram factory classe).
 * Then the bandwitdh is iteratively precised usind : AMISE-optimal bandwith, known as Freedman and Diaconis rule [freedman1981]: see OpenTurns
 ...
 *      l_i         --> width of each classes
 *      c_i         --> center of each classes
 *      n_bin       --> number of bins of the histograms
 *      Q1,Q2,Q3    --> quarter quantil
 *       probatot   -->sume of h_i
...
static void _compute_histogram(user_histogram_t *histogram_t,
                                         cs_real_t *sample,
                                         cs_real_t *weights,
                                         cs_lnum_t n_elts_sample
            )
{
    ...
    /*Compute optimal bandwidth assuming gaussian distribution of sample - scott rule */
    ...
    cs_real_t Quantile[][2]={{0.0,0.25},
                             {0.0,0.5},
                             {0.0,0.75}};

    cs_lnum_t n_quantile=sizeof(Quantile)/(2*sizeof(cs_real_t*));

    _compute_histogram_quantile(histogram_t,
                                Quantile,
                                n_quantile
                               );
    ...
    /*AMISE-optimal bandwith, known as Freedman and Diaconis rule [freedman1981]: see OpenTurns*/
    bandwidth_update = IQR/(2*0.75)*pow(24.0*pow(3.14,0.5)/n_gelts_sample,1.0/3.0);
    ...
    cs_lnum_t n_loop=1;
    cs_real_t bandwidth_variation=CS_ABS(bandwidth-bandwidth_update)/bandwidth;

    /*perform a loop to try top optimize bandwidth*/
    while (n_loop<10 &&
           bandwidth_variation>0.05 &&
           IQR>DBL_EPSILON &&
           IQR_var>0.02 &&
           histogram_t->binNumber<histogram_t->n_bin_max
          ){
...
static void _dump_histogram_OT(user_histogram_t *histogram_t,
                               const char *dirname
                              )
{
#if HAVE_OT==1
...
static void _calculate_min_max_dir(user_profile_t *profile_t)
{
    ...
    cs_real_t dir_norm;
    dir_norm=pow(pow(profile_t->dir_v[0],2.0)
                +pow(profile_t->dir_v[1],2.0)
    ...
    for (int ii=0;ii<n_selected_vertices;ii++){
        cs_lnum_t vtx_id=selected_vertices[ii];
        for (int v_id=0;v_id<3;v_id++){ /* loop on the 3 vector of the new base */
            vtx_dist[v_id][ii]=0.0;
            for( int jj=0;jj<3;jj++) /* loop on x y z initial direction */
                vtx_dist[v_id][ii]+=vtx_coord[3*vtx_id+jj]*base_orth[v_id][jj]; /* project each point in the new base */
        }
    }

    for (int k=0;k<3;k++){ /* find min max in the new base*/
        _compute_min_max(n_selected_vertices,
                         vtx_dist[k],
                         &min_mesh_ijn[k],
                         &max_mesh_ijn[k]);
...
static void _compute_layer_thickness(user_profile_t *profile_t)
{
    ...
    cs_real_t dx0=0.;
        ...
        if (is_even){
            ...
            dx0 = len_p * (progression - 1.) / (progression_np1 + progression_np - 2.);
        }
        ...
        for (int l_id=0;l_id<np-1;l_id++){
            profile_t->l_thick[l_id+1]=profile_t->l_thick[l_id]*progression;
            profile_t->l_thick[n_layers-1-1-l_id]=profile_t->l_thick[n_layers-1-l_id]*progression;
        }
...
/*----------------------------------------------------------------------------
 * Calculate position of the center of each layer in dir direction (absolute and normalized
 *
 * parameters:
 *   profile_t    <--> pointer to the current profile structure
 *     folowing members of the structure are updated
...
static void _calculate_pos_dir(user_profile_t *profile_t)
{
    ...
    for (int layer_id=1;layer_id<profile_t->n_layers;layer_id++){
        profile_t->pos[layer_id]=profile_t->pos[layer_id-1]+(l_thick[layer_id-1]+l_thick[layer_id])*1.0/2.0;
...
/*----------------------------------------------------------------------------
 * Calculate the coordinates of the four edge of a square plane for stl mesh structure
 * STL mesh is triangles base, the face is split into two triangles
 *
 * parameters:
 *   i_v                <-- pointer to i_v
...
static void _square_triangle_coords(cs_real_t i_v[3],
                             cs_real_t n_v[3],
                             cs_real_t O_coord[3],
                             cs_real_t i_size,
                             cs_real_t j_size,
                            ...
                            )
{
    ...
    /* order of the point are important for future calculation of normal of the face
        giben the base, rotation is the opposite of trigonometric orientation*/
    for (int k=0;k<3;k++)
...
static void _set_stl_layers_seeds(user_profile_t *profile_t,
                                      cs_lnum_t layer_id
            )
{
   /* shorter profile variable */
    ...
    /* For each layer Calculate seed target point as the middle of the segment between layer top and bottom and the mesh sel min/max in the profile direction*/
    ...
    for (int k=0;k<3;k++)
        target_point[0][k]=O_planes_coord[0][k]
                          -n_v[k]*((l_center_nCoord-1.0/2.0*layer_thickness)-dist_min)/2.0;



    for (int k=0;k<3;k++)
        target_point[1][k]=O_planes_coord[0][k]
                          +n_v[k]*layer_thickness
                          +n_v[k]*(dist_max-(l_center_nCoord+1.0/2.0*layer_thickness))/2.0;
    ...
    /*For each target point, find the closest cell center in the cell sell over all ranks
       Doing so, the seeds are in the fluid domain */
    for (int jj=0;jj<2;jj++){
...
static void _set_layers_stl_mesh(user_profile_t *profile_t,
                                 cs_lnum_t layer_id
                                )
{
    ...
    for (int k=0;k<3;k++)
     ...
     //Get points planes (4 points per plane), 2 triangles
        ...
        for (int vtx_id=0;vtx_id<6;vtx_id ++){
            for (int k=0;k<3;k++)
                stl_mesh->coords[n_faces_square*6+vtx_id][k]=plane_vtx_coords[vtx_id][k];
        }
        n_faces_square++;
...
static void _set_med_layer_mesh(user_profile_t *profile_t,
                               cs_lnum_t layer_id)
...
//Get mesh quantities
    ...
    MEDCoupling::MEDCouplingUMesh *Umesh=profile_t->med_mesh_struct->layer_mesh[layer_id];

    MEDCoupling::MEDCouplingCMesh *Cmesh=MEDCoupling::MEDCouplingCMesh::New(Umesh->getName());
    Cmesh->setCoords(arrX,arrY,arrZ);

    /*Build unstructured mesh and associate it the the pre allocated MEDCouplingUmesh array*/
    ...
    cs_real_t j_translate=(profile_t->max_j+profile_t->min_j)/2.0;
        ...
        for (int k=0;k<3;k++){
            vec_translate[k]=l_center_nCoord*n_v[k]
                            +i_translate*i_v[k]
                            +j_translate*j_v[k];
        ...
        /*adapt rotation angle in case of negative sinus*/
        if (cos_rot_angle>0)
            if(sin_rot_angle<0)
        ...
        for (int k=0;k<3;k++){
            Ox_1[k]=cos(rot_angle)*Ox[k]
                   -sin(rot_angle)*Oz[k];

        }

        for (int k=0;k<3;k++)
            Oz_1[k]=cos(rot_angle)*Oz[k]
                   +sin(rot_angle)*Ox[k];
 ...
 *  starting with layer 0, the cells is associated to first layer met in which the cell
 *  center coordinates lies.
 *
 * parameters:
...
static void _compute_cell_volume_per_layer_basic(user_profile_t *profile_t)
{
    //Get mesh quantities
    const cs_real_t *cell_vol = cs_glob_mesh_quantities->cell_vol;
    const cs_mesh_quantities_t *mq = cs_glob_mesh_quantities;
    ...
    for (int ii=0;ii<n_selected_cells;ii++){
        ...
        cs_real_t dist=(x*dir_normalized[0]+y*dir_normalized[1]+z*dir_normalized[2]);
...
/*----------------------------------------------------------------------------
 * Calculate percent of each cells within selection criteria within each layer based
 *  on cs_stl_mesh porosity calculation
 *
 * parameters:
 *   profile_t    <--> pointer to a profile structure initalized
 *     folowing members of the structure are updated
...
static void _compute_cell_volume_per_layer_stl(user_profile_t *profile_t)
{
    ...
    for (int s_id=0;s_id<profile_t->n_layers;s_id++){
        ...
        for (int c_id=0;c_id<n_cells_with_ghosts;c_id++)
        ...
        cs_stl_compute_porosity(stl_mesh,
                                cells_l_id_vol,
                                NULL);
...
static void _compute_intersection_volume_med(user_profile_med_t *med_t,
...

{
    ...
    MEDCouplingNormalizedUnstructuredMesh<3,3>
      tMesh_wrapper(med_t->local_mesh->med_mesh);
    ...
    MEDCouplingNormalizedUnstructuredMesh<3,3>
      sMesh_wrapper(med_t->layer_mesh[layer_id]->buildPartOfMySelf(subcells->begin(),
                                                       subcells->end(),
                                                       true));
...
/*----------------------------------------------------------------------------
 * Calculate percent of each cells within selection criteria within each layer based
 *  on med coupling intersection (method derived from cs_medcoupling_intersector.cxx
 *
 * parameters:
 *   profile_t    <--> pointer to a profile structure initalized
 *     folowing members of the structure are updated
...
static void _compute_cell_vol_per_layer_med(user_profile_t *profile_t){
    #if defined(HAVE_MEDCOUPLING) && defined(HAVE_MEDCOUPLING_LOADER)
    ...
    for (int s_id=0;s_id<profile_t->n_layers;s_id++){
       ...
       _compute_intersection_volume_med(med_t,
                                        cells_l_id_vol,
                                        s_id);
       /*update intersection for profile*/
       for (int ii=0;ii<n_selected_cells;ii++){
            cs_lnum_t c_id=selected_cells[ii];
            profile_t->cells_layer_vol[s_id][c_id]=cells_l_id_vol[c_id]/cell_vol[c_id];
...
static void _free_profile_all(user_profile_t *profile_t)
{
    //free stl meshes
    for (int s_id=0;s_id<profile_t->n_layers;s_id++){
        cs_stl_mesh_t *stl_mesh=profile_t->mesh_list[s_id];
...
/*----------------------------------------------------------------------------
 * Dump the setup of the profile into a log file stored in ./profiles/<profile name>
 *
 * parameters:
 *   profile_t      <-- pointer to the current profile structure
 *   dirname        <-- pointer to directory dump path
...
static void _dump_profile_setup_log(user_profile_t *profile_t,
                                    const char *dirname
...
)
{
    ...
    ptrSetupLog = fopen(filename,"w+"); //create the file (should not exist when this function is called)
    ...
    fprintf(ptrSetupLog,
            ...
            profile_t->criteria,
            profile_t->n_layers,
            profile_t->dir_v[0],profile_t->dir_v[1],profile_t->dir_v[2],
            profile_t->min_dir,
            profile_t->max_dir,
            profile_t->weighted,
    ...
    fprintf(ptrSetupLog,
            " layer thickness : [%.3f",profile_t->l_thick[0]);

    for (int l_id=1;l_id<profile_t->n_layers;l_id++)
        fprintf(ptrSetupLog,
            ",%.3f",profile_t->l_thick[l_id]);
...
static void _dump_profile_log(user_profile_t *profile_t,
                              const char *dirname
)
{
    ...
    ptrLog = fopen(filename,"a+"); //File already created when this function is called
    ...
    fprintf(ptrLog,
            ...
            ts->nt_cur, ts->t_cur,
            profile_t->field,profile_t->min_field,profile_t->field,profile_t->max_field
           );
    ...
    for (int layer_id=0;layer_id<n_layers;layer_id++){
        ...
        fprintf(ptrLog,
                "      mean %s of layer %d (pos = %.2f) is : %.2f - sd %.2f - Q1 : %.2f - Q2 : %.2f - Q3 : %.2f - weigth : %.2f \n",
                field,layer_id,profile_t->pos[layer_id],profile_t->mean_f[layer_id],profile_t->sd_f[layer_id],
    ...
    for (int layer_id=0;layer_id<n_layers;layer_id++){
        fprintf(ptrLog,
                "      mean_n %s of layer %d (pos_n = %.2f) is : %.2f - sd_n %.2f - weigth : %.2f \n",
                field,layer_id,profile_t->pos_n[layer_id],profile_t->mean_f_n[layer_id],profile_t->sd_f_n[layer_id],profile_t->weigth[layer_id]);
    }
...
static void _dump_profile_values_csv(user_profile_t *profile_t,
                                          const char *dirname
            )
...

{
    ...
    cs_real_t *profile_var[]={profile_t->pos,
                             profile_t->weigth,
                             profile_t->mean_f,
                             profile_t->sd_f,
                             profile_t->pos_n,
    ...
    ptrResults = fopen(filename,"r+"); //create the file (should not exist when this function is called)
    ...
    if (ptrResults==NULL){
        ...
        for (int layer_id=0;layer_id<n_layers;layer_id++){
            for (int h_id=0;h_id<n_l_header;h_id++)
                fprintf(ptrResults,
                        ",l_%d_%s",
                        layer_id,layer_header[h_id]);
        }


        fprintf(ptrResults,
                ",total_l_w,cells_sel_w\n");
    ...
    ptrResults=fopen(filename,"a+"); /* Add current result in the file re using same pattern than above*/
...
void _user_dump_profile_stl_mesh(user_profile_t *profile_t,
                                const char *dirname
...
void _dump_profile_med_mesh(user_profile_t *profile_t,
                                const char *dirname
...
)
{
    ...
    if (cs_glob_rank_id<=0){
        for (int m_id=0;m_id<profile_t->n_layers;m_id++){
            ...
            MEDCoupling::WriteUMesh(outfile,profile_t->med_mesh_struct->layer_mesh[m_id],true);
...
)
{
    ...
    BFT_REALLOC(_profile_list.profile_list,_profile_list.n_profiles,user_profile_t*);
    ...
    /*In case of Medcoupling method, check CS has been compiled with MEDCouppling*/
    ...
    for (int layer_id=0;layer_id<n_layers;layer_id++){
        ...
        user_histogram_t *histogram_t=user_create_histogram(name,
                                                           profile_t->field,
                                                           300 /*n_bin_max*/
                                     );
    ...
    /* Compute position of the center of the different layer along dir_v noramlized*/
...
)
{
    ...
    _compute_histogram(histogram_t,
                                 sample,
                                 weights,
                                 n_elts_sample);
...
void user_profile_compute(user_profile_t *profile_t)
{
    ...
    for (int l_id=0;l_id<n_layers;l_id++){
        ...
        _create_1d_sample_(profile_t,
                       &n_elts_sample,
                       sample,
                       weights,
                       l_id);

        /*Compute each layer weigth - useful for checking relevant intersection calculation*/
        ...
        user_histogram_compute(histogram_t,
                               sample,
                               weights,
                               n_elts_sample);
...
)
{
    ...
    if (stat("./profiles",&file_stat) < 0 && cs_glob_rank_id<=0) { //check if folder exist with main cs rank
        mkdir("./profiles",S_IRWXU | S_IRGRP | S_IROTH); //manage acces right in the same way than other folder
    }




    if (stat(dirname,&file_stat) < 0 && cs_glob_rank_id<=0) { //check if folder exist with main cs rank
        mkdir(dirname,S_IRWXU | S_IRGRP | S_IROTH); //manage acces right in the same way than other folder
    ...
    if (stat(dirname_histograms,&file_stat) < 0 && cs_glob_rank_id<=0) { //check if folder exist with main cs rank
        mkdir(dirname_histograms,S_IRWXU | S_IRGRP | S_IROTH); //manage acces right in the same way than other folder
    }
                            ...
                            )
{
    ...
    const char *histogram_header[]={"time_step",
                                    "time",
                                    "binNumber",
                                    "n_iter",
                                    "min",
...
{
  /*wrap c++ function*/
  _dump_histogram_OT(histogram_t,
                     dirname);
}
...

{
    ...
    for (int p_id=0;p_id<n_profiles;p_id++){
        user_profile_t *profile_t=_profile_list.profile_list[p_id];
        user_profile_histogram_OT_dump(profile_t,
                                       periodicity
                                       );
    }
