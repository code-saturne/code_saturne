...
## Define a python class equivalent for its properties to the structure defined in cs_user_profile.h but storing all time step dumped
...
class Profile():
    ...
    def reset_results(self):
            ...
            # create a list of profile properties in the same order than _dump_profile_values_csv of cs_user_profile.c
    ...
    def plot_graphs(self,
        ...
        for time_step in range(len(self.__time_step)):
            ...
            ax.errorbar(self.__pos[time_step],
                        ...
                        yerr=self.__sd_f[time_step],fmt='--o',capsize=5 #errorbar layout
                        )
        ...
        i=0
        for fig in figs:
            fig.savefig(path_out+os.sep+figlabels[i]+'.png',dpi=dpi,bbox_inches='tight')
            i+=1


if __name__=='__main__':
    dir_profiles="../profiles"
    for profile in os.listdir(dir_profiles): #list all directories in profiles (each is a profile)
        if os.path.isdir(dir_profiles+os.sep+profile): #if the item is a directory : it is a profile
