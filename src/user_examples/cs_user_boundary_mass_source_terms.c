                                   ...
                                   int iappel)
{
  ...
  if (iappel == 1 || iappel == 2) {
    ...
    for (ieltcd = 0 ; ieltcd < nlelt ; ieltcd++) {
      if (iappel == 2) {
        cs_lnum_t ifac = lstelt[ieltcd];
        ...
        if (f->type & CS_FIELD_VARIABLE) {
          ...
          if (iscal > 0) {
            ...
            if (f == cs_thermal_model_field()) {
              ...
              wall_cond->spcond[ivar*nfbpcd + ieltcd] = 0.0; // non-condensable species
            }
          }
        }
      }
