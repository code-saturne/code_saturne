                            ...
                            cs_real_t                       *fluxes)
{
  ...
  for (cs_lnum_t i = f2e->idx[f_id]; i < f2e->idx[f_id + 1]; i++) {
    ...
    const double    tef
      = cs_math_surftri(cdoq->vtx_coord + 3 * v0, cdoq->vtx_coord + 3 * v1, xf);
...
static void
_compute_adv_vector_at_vertices(cs_xdef_t *def, cs_real_t *vtx_values)
{
  ...
  case CS_XDEF_BY_ARRAY: {
    ...
    if (cs_flag_test(cx->value_location, cs_flag_dual_face_byc)) {
    ...
    else if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {
                       ...
                       cs_advection_field_status_t status)
{
  ...
  adv = new cs_adv_field_t(name, new_id, status);
...
cs_advection_field_log_setup(void)
{
  ...
  cs_log_printf(CS_LOG_SETUP, "\nSummary of the advection fields\n");
...
void
cs_adv_field_t::set_postprocess(cs_flag_t post_flag_)
{
  this->post_flag |= post_flag_;
}
...
void
cs_advection_field_def_by_value(cs_adv_field_t *adv, cs_real_t *values)
{
  if (adv == nullptr)
                                ...
                                bool            is_owner)
{
  ...
  context.z_id           = 0; /* all cells */
  context.stride         = 3; /* default initialization */
  context.value_location = val_location;
  context.values         = array;
  context.is_owner       = is_owner;
  context.full_length    = true;
  /* Optional parameters */
  context.full2subset = nullptr;
  context.adjacency   = nullptr;
  context.n_list_elts = 0;
  context.elt_ids     = nullptr;
...
void
cs_advection_field_def_by_field(cs_adv_field_t *adv, cs_field_t *field)
{
  if (adv == nullptr)
    bft_error(__FILE__, __LINE__, 0, _(_err_empty_adv));
                                              ...
                                              bool            full_length)
{
  ...
  cs_xdef_array_context_t context;
  context.z_id = z_id, context.stride = 1, context.value_location = val_loc,
  context.is_owner = is_owner, context.full_length = full_length,
  context.values = array;
  /* Optional parameters (not used) */
  context.full2subset = nullptr, context.adjacency = nullptr,
  context.n_list_elts = 0, context.elt_ids = nullptr;
...
cs_advection_field_create_fields(void)
{
  ...
  for (int i = 0; i < _n_adv_fields; i++) {
    ...
    bool has_previous
      = (adv->status & CS_ADVECTION_FIELD_STEADY) ? true : false;
                                   ...
                                   cs_nvec3_t           *vect)
{
  ...
  cs_field_t *f = adv->get_field(CS_MESH_LOCATION_CELLS);
  ...
  if (adv->status & CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX) {
    ...
    case CS_XDEF_BY_ARRAY: /* P0 approximation */
    {
      ...
      if (cs_flag_test(cx->value_location, cs_flag_dual_face_byc)) {
        ...
        assert(cx->adjacency == cs_cdo_connect->c2e);
        cs_reco_dfbyc_in_cell(
          cm, cx->values + cx->adjacency->idx[cm->c_id], vector_values);
      }
      else if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {

        const cs_real_t *i_flux = cx->values;
        const cs_real_t *b_flux = cx->values + cs_cdo_quant->n_i_faces;

        cs_reco_cw_cell_vect_from_face_dofs(cm, i_flux, b_flux, vector_values);
      }
      else
        bft_error(__FILE__, __LINE__, 0,
                  " %s: Invalid location for array", __func__);
    ...
    case CS_XDEF_BY_VALUE:
      bft_error(__FILE__, __LINE__, 0,
                " %s: Type of definition not implemented yet.",
                __func__);
      break;
    ...
    case CS_XDEF_BY_ARRAY: /* P0 approximation */
      cs_xdef_cw_eval_vector_at_xyz_by_array(
        cm, 1, xyz, time_eval, def->context, vector_values);
      cs_nvec3(vector_values, eval);
      break;
                            ...
                            cs_real_t            *cell_values)
{
  ...
  cs_xdef_t *def = adv->definition;
  assert(def != nullptr); /* Sanity checks */
                               ...
                               cs_real_t               *vtx_values)
{
  ...
  if (adv->status & CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX) {
    ...
    _compute_adv_vector_at_vertices(def, vtx_values);
  ...
  else { /* Advection field define as a vector-valued potential */
...
#       pragma omp parallel for if (cdoq->n_vertices > CS_THR_MIN)
    ...
    case CS_XDEF_BY_ARRAY:
      ...
      if (cs_flag_test(cx->value_location, cs_flag_primal_vtx))
        ...
        bft_error(
          __FILE__, __LINE__, 0, " %s: Invalid location for array", __func__);
      }
      break; /* definition by array */
    ...
    case CS_XDEF_BY_ANALYTIC_FUNCTION:
      cs_evaluate_potential_at_vertices_by_analytic(
        def, time_eval, cdoq->n_vertices, nullptr, vtx_values);
      break; /* definition by analytic */
...
#       pragma omp parallel for if  (n_b_faces > CS_THR_MIN)
    ...
    case CS_XDEF_BY_ARRAY:
      ...
      if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {
        ...
        const cs_real_t *const bface_vel = cx->values + 3 * n_i_faces;
      ...
      for (int def_id = 0; def_id < adv->n_bdy_flux_defs; def_id++) {
        ...
        case CS_XDEF_BY_VALUE: {
          ...
          cs_array_real_set_scalar_on_subset(
            z->n_elts, z->elt_ids, constant_val[0], flx_values);
        } break;

        case CS_XDEF_BY_ARRAY: {
          const cs_xdef_array_context_t *cx
            = (const cs_xdef_array_context_t *)bdef->context;
          const cs_real_t *val = cx->values;

          assert(cx->stride == 1);
          assert(bdef->meta & CS_FLAG_FULL_LOC || z->elt_ids == nullptr);
                                         ...
                                         const cs_adv_field_t *adv)
{
  ...
  if (adv->status & CS_ADVECTION_FIELD_TYPE_VELOCITY_VECTOR) {
    ...
    case CS_XDEF_BY_ANALYTIC_FUNCTION: {
      ...
      assert(cs_eflag_test(
        cm->flag, CS_FLAG_COMP_PV | CS_FLAG_COMP_FEQ | CS_FLAG_COMP_EV));
      ...
      {
        short int v1, v2, v3;
        cs_cell_mesh_get_next_3_vertices(f2e_ids, cm->e2v_ids, &v1, &v2, &v3);
    ...
    if (adv->n_bdy_flux_defs == 0) {
      ...
      const cs_xdef_t *bdef = (adv->bdy_def_ids == nullptr)
                                ? adv->bdy_flux_defs[0]
                                : adv->bdy_flux_defs[adv->bdy_def_ids[bf_id]];
                                        ...
                                        cs_real_t            *fluxes)
{
  ...
  if (adv->bdy_field_id > -1 && adv->n_bdy_flux_defs == 0) {
    ...
    if (adv->n_bdy_flux_defs == 0) {
      ...
      _cw_fill_uniform_boundary_flux(cm, f, cx->values[bf_id], fluxes);
    }
    else {
      ...
      const cs_xdef_t *def = (adv->bdy_def_ids == nullptr)
                               ? adv->bdy_flux_defs[0]
                               : adv->bdy_flux_defs[adv->bdy_def_ids[bf_id]];
    ...
    assert(adv->n_bdy_flux_defs == 0);
    assert(adv->bdy_def_ids == nullptr);
    assert(adv->status & CS_ADVECTION_FIELD_TYPE_VELOCITY_VECTOR);
                                ...
                                cs_real_t            *fluxes)
{
  ...
  if (adv->status & CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX) {
    ...
    case CS_XDEF_BY_DOF_FUNCTION: {
      ...
      assert(cs_flag_test(cx->dof_location, cs_flag_primal_face));
    ...
    case CS_XDEF_BY_VALUE:
      for (short int f = 0; f < cm->n_fc; f++)
        cs_xdef_cw_eval_flux_by_vector_val(
          cm, f, time_eval, def->context, fluxes);
      break;
    ...
    case CS_XDEF_BY_ANALYTIC_FUNCTION: {
      ...
      for (short int f = 0; f < cm->n_fc; f++)
        cs_xdef_cw_eval_flux_by_vector_analytic(
          cm, f, time_eval, def->context, def->qtype, fluxes);
    } break;
    ...
    case CS_XDEF_BY_ARRAY: {
      ...
      if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {
        ...
        for (short int f = 0; f < cm->n_fc; f++) {
          ...
          cs_nvec3(cx->values + 3 * cm->f_ids[f], &nv);
        ...
        cs_nvec3(cx->values + 3 * cm->c_id, &nv);
                                 ...
                                 cs_real_t            *fluxes)
{
  ...
  case CS_XDEF_BY_ANALYTIC_FUNCTION: {
    assert(cs_eflag_test(
      cm->flag, CS_FLAG_COMP_PEQ | CS_FLAG_COMP_SEF | CS_FLAG_COMP_PFQ));
      ...
      for (short int f = 0; f < cm->n_fc; f++) {
        ...
        for (short int i = cm->f2e_idx[f]; i < cm->f2e_idx[f + 1]; i++) {
          ...
          cs_quadrature_tria_4pts(
            edge.center, face.center, cm->xc, sefc.meas, gpts, w);

          cs_xdef_cw_eval_at_xyz_by_analytic(
            cm, 4, (const cs_real_t *)gpts, time_eval, def->context, eval);
    ...
    case CS_QUADRATURE_HIGHEST: {
      ...
      for (short int f = 0; f < cm->n_fc; f++) {
        ...
        for (short int i = cm->f2e_idx[f]; i < cm->f2e_idx[f + 1]; i++) {
          ...
          cs_xdef_cw_eval_at_xyz_by_analytic(
            cm, 7, (const cs_real_t *)gpts, time_eval, def->context, eval);
    ...
    default:
      bft_error(
        __FILE__, __LINE__, 0, " %s: Invalid type of quadrature.", __func__);
      break;
  ...
  case CS_XDEF_BY_ARRAY: {
    ...
    if (cs_flag_test(cx->value_location, cs_flag_dual_face_byc)) {
      ...
      const cs_real_t *flux_array
        = cx->values + cs_cdo_connect->c2e->idx[cm->c_id];
      for (short int e = 0; e < cm->n_ec; e++)
        fluxes[e] = flux_array[e];
    }
    else if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {

      const cs_real_t *i_mflx = cx->values;
      const cs_real_t *b_mflx = cx->values + cm->bface_shift;
...
void
cs_advection_field_update(cs_real_t t_eval, bool cur2prev)
{
  for (int i = 0; i < _n_adv_fields; i++) {
...
cs_advection_get_courant(const cs_adv_field_t *adv,
                         ...
                         cs_real_t             courant[])
{
  assert(courant != nullptr); /* Sanity check */
  const cs_cdo_quantities_t *cdoq = cs_cdo_quant;
                                          ...
                                          cs_real_t             t_eval)
{
      ...
      if (cs_flag_test(cx->value_location, cs_flag_dual_face_byc)) {
        ...
        for (cs_lnum_t c_id = 0; c_id < cdoq->n_cells; c_id++) {
          for (cs_lnum_t j = c2e->idx[c_id]; j < c2e->idx[c_id + 1]; j++) {
            ...
            const cs_real_t flx    = cx->values[j];
        ...
        bft_error(__FILE__, __LINE__, 0,
                  " %s: Invalid location for the array.",
                  __func__);
  ...
  if (adv->n_bdy_flux_defs > 0) {
    ...
    for (int def_id = 0; def_id < adv->n_bdy_flux_defs; def_id++) {
      ...
      case CS_XDEF_BY_VALUE: {
        ...
        for (cs_lnum_t id = 0; id < z->n_elts; id++) {
          ...
          const cs_lnum_t bf_id = (z->elt_ids == nullptr) ? id : z->elt_ids[id];

          _fill_uniform_boundary_flux(
            cdoq, f2e, e2v, bf_id, constant_val[0], divergence);
      ...
      case CS_XDEF_BY_ARRAY: {
        ...
        if (cs_flag_test(cx->value_location, cs_flag_primal_face)) {
          ...
          for (cs_lnum_t bf_id = 0; bf_id < cdoq->n_b_faces; bf_id++)
            _fill_uniform_boundary_flux(
              cdoq, f2e, e2v, bf_id, val[bf_id], divergence);
        }
        else if (cs_flag_test(cx->value_location, cs_flag_dual_closure_byf)) {
    ...
    const cs_field_t *bflx = adv->get_field(CS_MESH_LOCATION_BOUNDARY_FACES);
...
cs_adv_field_t::cs_adv_field_t(std::string                 name_,
                               int                         id_,
                               cs_advection_field_status_t status_)
  : name(name_), id(id_), status(status_)
{
  /* Default initialization */

  this->definition = nullptr;

  this->n_bdy_flux_defs = 0;
  this->bdy_flux_defs   = nullptr;
  this->bdy_def_ids     = nullptr;

  this->cell_field_id = CS_ADVECTION_FIELD_ID_NOT_SET;
  this->int_field_id  = CS_ADVECTION_FIELD_ID_NOT_SET;

  this->vtx_field_id = CS_ADVECTION_FIELD_ID_NOT_SET;
  if (this->status & CS_ADVECTION_FIELD_DEFINE_AT_VERTICES)
    this->vtx_field_id = CS_ADVECTION_FIELD_ID_TO_BE_SET;

  this->bdy_field_id = CS_ADVECTION_FIELD_ID_NOT_SET;
  if (this->status & CS_ADVECTION_FIELD_DEFINE_AT_BOUNDARY_FACES)
    this->bdy_field_id = CS_ADVECTION_FIELD_ID_TO_BE_SET;

  /* Update the status to a scalar flux in case of an advection field coming
     from the legacy FV part */

  if ((status_ & CS_ADVECTION_FIELD_NAVSTO) &&
      (status_ & CS_ADVECTION_FIELD_LEGACY_FV))
    this->status |= CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX;

  /* If no type is set, set vector-valued by default */

  if (!(status_ & CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX) &&
      !(status_ & CS_ADVECTION_FIELD_TYPE_VELOCITY_VECTOR))
    this->status |= CS_ADVECTION_FIELD_TYPE_VELOCITY_VECTOR;
};

cs_adv_field_t::~cs_adv_field_t()
{
  this->definition = cs_xdef_free(this->definition);

  for (int id_ = 0; id_ < this->n_bdy_flux_defs; id_++)
    this->bdy_flux_defs[id_] = cs_xdef_free(this->bdy_flux_defs[id_]);

  if (this->n_bdy_flux_defs > 0)
    CS_FREE(this->bdy_flux_defs);
  if (this->bdy_def_ids != nullptr)
    CS_FREE(this->bdy_def_ids);
};
...
void
cs_adv_field_t::log_setup() const
{
  char prefix[256];
  ...
  cs_log_printf(CS_LOG_SETUP, "  * %s | Category: ", this->name.c_str());
  if (this->status & CS_ADVECTION_FIELD_NAVSTO)
    cs_log_printf(CS_LOG_SETUP, "Related to Navier-Stokes\n");
  else if (this->status & CS_ADVECTION_FIELD_GWF)
    cs_log_printf(CS_LOG_SETUP, "Related to the \"Groundwater Flow\" module\n");
  else if (this->status & CS_ADVECTION_FIELD_USER)
    cs_log_printf(CS_LOG_SETUP, "User-defined\n");

  /* Type of advection field */

  cs_log_printf(CS_LOG_SETUP, "  * %s | Type: ", this->name.c_str());
  if (this->status & CS_ADVECTION_FIELD_TYPE_VELOCITY_VECTOR)
    cs_log_printf(CS_LOG_SETUP, "Velocity vector\n");
  else if (this->status & CS_ADVECTION_FIELD_TYPE_SCALAR_FLUX)
    cs_log_printf(CS_LOG_SETUP, "Scalar flux\n");

  /* Additional information */

  if (this->status & CS_ADVECTION_FIELD_LEGACY_FV)
    cs_log_printf(CS_LOG_SETUP,
                  "  * %s | %s\n",
                  this->name.c_str(),
                  "Related to Legacy FV schemes\n");
  if (this->status & CS_ADVECTION_FIELD_STEADY)
    cs_log_printf(CS_LOG_SETUP,
  ...
  if (this->post_flag & CS_ADVECTION_FIELD_POST_NONE)
    cs_log_printf(CS_LOG_SETUP,
                  "  * %s | No postprocessing of the adv. field\n",
                  this->name.c_str());
  if (this->post_flag & CS_ADVECTION_FIELD_POST_COURANT)
    cs_log_printf(CS_LOG_SETUP,
  ...
  bool at_cells =
    (this->cell_field_id > CS_ADVECTION_FIELD_ID_NOT_SET) ? true : false;
  bool at_vertices =
    (this->vtx_field_id > CS_ADVECTION_FIELD_ID_NOT_SET) ? true : false;
  bool at_bfaces =
    (this->bdy_field_id > CS_ADVECTION_FIELD_ID_NOT_SET) ? true : false;
  bool at_ifaces =
    (this->int_field_id > CS_ADVECTION_FIELD_ID_NOT_SET) ? true : false;
  ...
  cs_xdef_log_setup(prefix, this->definition);
  ...
  cs_log_printf(CS_LOG_SETUP,
                ...
                this->name.c_str(),
                this->n_bdy_flux_defs);
  if (this->n_bdy_flux_defs > 0)
    cs_log_printf(CS_LOG_SETUP, "\n");
  for (int ib = 0; ib < this->n_bdy_flux_defs; ib++) {
    sprintf(prefix, "        Definition %2d", ib);
    cs_xdef_log_setup(prefix, this->bdy_flux_defs[ib]);
  }
}
