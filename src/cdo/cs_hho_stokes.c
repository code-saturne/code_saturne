 ...
 * \param[in, out] eqc       pointer to a cs_hho_stokes_t structure
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_HHO_STOKES_DBG > 2
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_HHO_STOKES_DBG > 2
      if (c_id % CS_HHO_STOKES_MODULO == 0) cs_cell_mesh_dump(cm);
#endif
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_HHO_STOKES_DBG > 1
        if (c_id % CS_HHO_STOKES_MODULO == 0)
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_HHO_STOKES_DBG > 0
      if (c_id % CS_HHO_STOKES_MODULO == 0)
