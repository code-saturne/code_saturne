...
static cs_cell_sys_t     **cs_macfb_cell_sys = NULL;
static cs_cell_builder_t **cs_macfb_cell_bld = NULL;
...
{
  if (cs_macfb_cell_sys == NULL || cs_macfb_cell_bld == NULL)
    return false;
  else
    return true;
}
                             ...
                             const cs_time_step_t      *time_step)
{
  ...
  for (int i = 0; i < cs_glob_n_threads; i++) {
    cs_macfb_cell_sys[i] = NULL;
    cs_macfb_cell_bld[i] = NULL;
  }
...
#pragma omp parallel
  ...
  cs_macfb_cell_bld = NULL;
  cs_macfb_cell_sys = NULL;
}
