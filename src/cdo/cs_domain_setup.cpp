                                    ...
                                    void           *func_input)
{
  ...
  domain->time_step_options->idtvar = CS_TIME_STEP_ADAPTIVE;
  ...
  domain->time_step_options->dtmin = domain->time_step->t_max;
  domain->time_step_options->dtmax = 0.;
                                 ...
                                 double       dt)
{
  ...
  domain->time_step_options->idtvar = CS_TIME_STEP_CONSTANT;
  ...
  domain->time_step_options->dtmin = dt;
  domain->time_step_options->dtmax = dt;
...
cs_domain_setup_log(const cs_domain_t   *domain)
{
  ...
  if (domain->only_steady)
    ...
    if (domain->time_step_options->idtvar == 0)
      cs_log_printf(CS_LOG_SETUP, " * Time step *constant*\n\n");
    else if (domain->time_step_options->idtvar == 1)
