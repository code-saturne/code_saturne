...
cs_cell_mesh_t       **cs_cdo_local_cell_meshes       = nullptr;
cs_face_mesh_t       **cs_cdo_local_face_meshes       = nullptr;
cs_face_mesh_light_t **cs_cdo_local_face_meshes_light = nullptr;
...
static short int **cs_cdo_local_kbuf = nullptr;
 ...
 * \param[in] block_sizes   size of each block or nullptr. Specific treatment
 * for n_blocks = 1.
 *
                   ...
                   int  *block_sizes)
{
  ...
  if (block_sizes == nullptr) { /* No block */
                   ...
                   cs_cell_mesh_t              *cm)
{
  ...
  if (build_flag & CS_FLAG_COMP_EV || build_flag & CS_FLAG_COMP_FV) {
    ...
    if (build_flag & CS_FLAG_COMP_EV) {
      for (short int e = 0; e < cm->n_ec; e++) {
        ...
        cm->e2v_ids[2 * e + 1]
          = kbuf[connect->e2v->ids[2 * e_id + 1] - v_shift];
  ...
  if (build_flag & cs_flag_need_ef) {
    ...
    for (short int i = 0; i < 2 * cm->n_ec; i++)
      cm->e2f_ids[i] = -1;
  ...
  if (build_flag & CS_FLAG_COMP_DIAM) {
    ...
    if (cm->n_fc == 4) { /* Tetrahedron */
      ...
      double cbox[6]
        = { cm->xv[0], cm->xv[1], cm->xv[2], cm->xv[0], cm->xv[1], cm->xv[2] };
