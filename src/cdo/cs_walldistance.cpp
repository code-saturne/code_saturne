...
static cs_equation_t *cs_wd_poisson_eq = nullptr;
...
#pragma omp parallel for if (cdoq->n_vertices > CS_THR_MIN) reduction(+ : count)
  for (cs_lnum_t i = 0; i < cdoq->n_vertices; i++) {
    ...
    if (vgrd2 + 2 * var[i] < 0)
      count++;
    cs_real_t d1 = std::sqrt(vgrd2 + 2 * cs::abs(var[i])),
              d2 = std::sqrt(vgrd2);
    if (d1 - d2 < 0)
      dist[i] = d1 + d2;
    else
                       ...
                       cs_real_t                  dist[])
{
  ...
  for (cs_lnum_t c_id = 0; c_id < cdoq->n_cells; c_id++) {
    ...
    for (i = connect->c2f->idx[c_id]; i < connect->c2f->idx[c_id + 1]; i++) {
      ...
      cs_nvec3_t fq               = cdoq->get_face_nvec(f_id);
    ...
    dist[c_id] = std::sqrt(tmp) - cs_math_3_norm(cell_gradient);
