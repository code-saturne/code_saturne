                    ...
                    cs_real_t                          array[])
{
  ...
  ac->func(
    t_eval, 7, nullptr, (const cs_real_t *)gpts, true, ac->input, ana_eval);
                     ...
                     cs_real_t                          array[])
{
  ...
  /* Evaluate the analytical function at the Gauss points */
  ac->func(
    t_eval, 15, nullptr, (const cs_real_t *)gpts, true, ac->input, ana_eval);
                                 ...
                                 cs_hho_builder_t          *hhob)
{
  if (hhob == nullptr)
    return;
  assert(hhob->cell_basis != nullptr && hhob->grad_basis != nullptr
         && hhob->face_basis != nullptr);
  ...
  cs_sdm_t   *rhs = nullptr, *gop = nullptr;
                         ...
                         cs_hho_builder_t          *hhob)
{
  ...
  if (cm->c_id == 0) {
    ...
    cs_sdm_block_fprintf(nullptr, nullptr, 1e-16, rhs_t);

    printf(" Diffusion matrix: grad_op_t\n");
    cs_sdm_block_fprintf(nullptr, nullptr, 1e-16, gop_t);

    printf(" Diffusion matrix: consistent part\n");
    cs_sdm_block_fprintf(nullptr, nullptr, 1e-16, cb->loc);
  }
#endif
                                       ...
                                       cs_real_t                red[])
{
  ...
  assert(hhob->cell_basis != nullptr && hhob->face_basis != nullptr);
  ...
  const cs_basis_func_t  *cbf = hhob->cell_basis;
  assert(cbf->facto != nullptr && cbf->project != nullptr);
                                 ...
                                 cs_real_t                res[])
{
  ...
  assert(hhob->face_basis != nullptr);
