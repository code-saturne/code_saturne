 ...
 *
 * \param[in]  xv1  coordinates of the first vertex
 * \param[in]  xv2  coordinates of the second vertex
 * \param[in]  xfc  vector xc --> xf
 * \param[in]  xc  coordinates of the cell center (reference point)
 *
...
static double
_voltet_optim(const cs_real_3_t xv1,
              const cs_real_3_t xv2,
              const cs_real_3_t xfc,
              const cs_real_3_t xc)
{
  constexpr cs_real_t c_1ov6 = 1./6.;

  const cs_real_3_t  x1c = { xv1[0]-xc[0], xv1[1]-xc[1], xv1[2]-xc[2] };
  const cs_real_3_t  x2c = { xv2[0]-xc[0], xv2[1]-xc[1], xv2[2]-xc[2] };

  double det
    = x1c[0] * (x2c[1]*xfc[2] - x2c[2]*xfc[1])
    - x1c[1] * (x2c[0]*xfc[2] - x2c[2]*xfc[0])
    + x1c[2] * (x2c[0]*xfc[1] - x2c[1]*xfc[0]);

  return  c_1ov6 * cs::abs(det);
}
...
# pragma omp parallel for shared(quant, topo, c2f, f2e) CS_CDO_OMP_SCHEDULE
  for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
    ...
    const cs_real_3_t xc = {quant->cell_centers[c_id][0],
                            quant->cell_centers[c_id][1],
                            quant->cell_centers[c_id][2]};
    ...
    for (cs_lnum_t jf = c2f->idx[c_id]; jf < c2f->idx[c_id + 1]; jf++) {
      ...
      cs_real_3_t xf;
      if (bf_id > -1) {
        xf[0] = quant->b_face_center[bf_id][0];
        xf[1] = quant->b_face_center[bf_id][1];
        xf[2] = quant->b_face_center[bf_id][2];
      }
      else {
        xf[0] = quant->i_face_center[f_id][0];
        xf[1] = quant->i_face_center[f_id][1];
        xf[2] = quant->i_face_center[f_id][2];
      }

      const cs_real_3_t xfc = { xf[0] - xc[0], xf[1] - xc[1], xf[2] - xc[2]};
      ...
      for (cs_lnum_t je = f2e->idx[f_id]; je < f2e->idx[f_id + 1]; je++) {
        ...
        const double pvol = 0.5 * _voltet_optim(quant->vtx_coord + 3 * v1_id,
        ...
        short int _v1 = -1, _v2 = -1;
        for (short int _v = 0; _v < n_vc; _v++) {
          cs_lnum_t v_id = c2v_ids[_v];

          if (v_id == v1_id) _v1 = _v;
          if (v_id == v2_id) _v2 = _v;

          if ((_v1 != -1) && (_v2 != -1)) break;
        }
