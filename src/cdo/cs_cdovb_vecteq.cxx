...
static cs_cell_sys_t     **_vvb_cell_system  = nullptr;
static cs_cell_builder_t **_vvb_cell_builder = nullptr;
                          ...
                          const cs_cell_sys_t    *csys)
{
  ...
  case CS_PARAM_RESNORM_WEIGHTED_RHS:
    for (short int i = 0; i < cm->n_vc; i++) {
      ...
      _rhs_norm
        += w * (_rhs[0] * _rhs[0] + _rhs[1] * _rhs[1] + _rhs[2] * _rhs[2]);
    }
...
{
  if (_vvb_cell_system == nullptr || _vvb_cell_builder == nullptr)
    return false;
  else
    return true;
}
