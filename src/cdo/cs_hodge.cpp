...
/*----------------------------------------------------------------------------*/
/*!
 * \brief  Compute the discrete EpFd Hodge operator.
 *         St algo in case of stabilization by face.
 *
 * \param[in]      n_ent    number of local entities
 * \param[in]      d        space dim.
 * \param[in]      beta     value of the stabilization coef. for each entities
 * \param[in]      vol_c    cell volume
 * \param[in]      pq       pointer to the first set of quantities
 * \param[in]      dq       pointer to the second set of quantities
 * \param[in, out] cb       temporary buffers
...
static void
_compute_stab_hodge(const int          n_ent,
                    const cs_real_t    d,
                    const cs_real_t   *beta,
                    const cs_real_t    vol_c,
                    const cs_real_3_t *pq,
                    const cs_real_3_t *dq,
                    cs_cell_builder_t *cb,
                    cs_sdm_t          *hmat)
{
  cs_real_t *kappa        = cb->values;             /* size = n_ent */
  cs_real_t *kappa_pq_dqi = cb->values + n_ent;     /* size = n_ent */
  cs_real_t *stab         = cb->values + 2 * n_ent; /* size = n_ent */
  cs_sdm_t  *qp_pq_mat    = cb->aux;
  cs_real_t *dq_pq        = qp_pq_mat->val; /* size = n_ent*n_ent */

  cs_sdm_square_init(n_ent, qp_pq_mat);

  const cs_real_t ovc  = 1.0 / vol_c;
  ...
  for (int i = 0; i < n_ent; i++) {
    const cs_real_t dqi[3] = { dq[i][0], dq[i][1], dq[i][2] };

    cs_real_t *dqi_pq = dq_pq + i * n_ent;
    for (int j = 0; j < n_ent; j++)
      dqi_pq[j] = _dp3(dqi, pq[j]);

    const cs_real_t dqi_dqi = _dp3(dqi, dqi);

    /* kappa_i = d * beta_i * (dq_i * dq_i) / (dq_i * pq_i) */
    kappa[i] = d * beta[i] * dqi_dqi / dqi_pq[i];

    cs_real_t *hi = hmat->val + i * n_ent;

    /* daigoanl term: kappa_i * (kro_ik * kro_ik - 2*kro_ik * (dq_i*pq_k / c) )
     */
    hi[i] = (1.0 - 2. * dqi_pq[i] * ovc) * kappa[i];
  }

  /* Build the upper right part of the discrete Hodge operator */

  for (int i = 0; i < n_ent; i++) {
    const cs_real_t *dqi_pq = dq_pq + n_ent * i;

    for (int k = 0; k < n_ent; k++)
      kappa_pq_dqi[k] = kappa[k] * dqi_pq[k];

    /* stab_ij = kappa_k * (dq_i*pq_k) * (dq_j*pq_k) */
    _partial_matvec(i, qp_pq_mat, kappa_pq_dqi, stab);

    cs_real_t *hi = hmat->val + i * n_ent;

    /* diagonal term : stab_ii / (c*c) */
    hi[i] += stab[i] * ovc2;

    /* Compute the extra-diagonal terms
     *  - kappa_i* (dq_j*pq_i / c) - kappa_j * (dq_i*pq_j / c) + stab_ij/(c*c)
     */

    const cs_real_t kai = kappa[i];
    for (int j = i + 1; j < n_ent; j++) {
      hi[j] += stab[j] * ovc2;
      hi[j] -= (kai * dq_pq[j * n_ent + i] + kappa[j] * dqi_pq[j]) * ovc;
    }
                        ...
                        bool                    need_eigen)
{
  ...
  switch (hp->type) {
    case CS_HODGE_TYPE_VPCD:
      hdg->matrix = cs_sdm_square_create(cm->n_max_vbyc);
      break;
    case CS_HODGE_TYPE_EPFD:
      hdg->matrix = cs_sdm_square_create(cm->n_max_ebyc);
      break;
    case CS_HODGE_TYPE_FPED:
    case CS_HODGE_TYPE_EDFP:
      hdg->matrix = cs_sdm_square_create(cm->n_max_fbyc);
      break;
    case CS_HODGE_TYPE_CPVD:
    case CS_HODGE_TYPE_VDCP:
      hdg->matrix = cs_sdm_square_create(1);
      break;
    case CS_HODGE_TYPE_FB:
      hdg->matrix = cs_sdm_square_create(cm->n_max_fbyc + 1);
      break;
    case CS_HODGE_TYPE_VC:
      hdg->matrix = cs_sdm_square_create(cm->n_max_vbyc + 1);
      break;
  ...
  CS_MALLOC(hdg->pty_data, 1, cs_property_data_t);
  cs_property_data_init(need_tensor, need_eigen, property, hdg->pty_data);
  if (hdg->pty_data->is_unity == false)
    cs_hodge_evaluate_property(0, 0, 0, hdg);
                          ...
                          const cs_real_t      *coeff_stab)
{
  ...
  bool continue_computation =
    cs_hodge_edfp_stab_get_opt(cm, hodge, cb, coeff_stab);
                              ...
                              cs_sdm_t                *hf)
{
  ...
  for (short int e = 0; e < fm->n_ef; e++) {
    ...
    const double  extra_val = c_1ov12 * fm->tef[e];
