...
typedef struct {
  ...
  /*! \var rs
   *  Pointer to a \ref cs_range_set_t structure.
   *  Useful for handling parallelism and matrix assembly
   */
  cs_range_set_t          *rs;

  /*! \var rs
   *  Pointer to a \ref cs_interface_set_t structure.
   *  Useful for handling parallelism
   */
  cs_interface_set_t      *ifs;

  /*! \var ma
   *  Pointer to a \ref cs_matrix_assembler_t structure.
   *  Helper for the assembly process of a matrix generated by the monolithic
   *  approach
   */
  cs_matrix_assembler_t   *ma;

  /*! \var ms
   *  Pointer to a \ref cs_matrix_structure_t structure.
   *  Index of the matrix related to a monolithic approach (specific to this
   *  approach)
   */
 ...
 *
 * \param[in] sc   scheme context (pointer to a cs_cdofb_monolithic_t struct.)
 */
/*----------------------------------------------------------------------------*/

static void
_build_structures(cs_cdofb_monolithic_t        *sc)
{
...
#   pragma omp parallel for if (size > CS_THR_MIN)
  ...
  sc->ifs = cs_interface_set_create(size,
                                    NULL,
                                    gnum,
                                    m->periodicity, 0, NULL, NULL, NULL);

  sc->rs = cs_range_set_create(sc->ifs,   /* interface set */
                               NULL,      /* halo */
                               size,
                               false,     /* TODO: Ask Yvan */
                               0);        /* g_id_base */
   ...
   */
  sc->ma = cs_matrix_assembler_create(sc->rs->l_range, true);
  ...
  for (cs_lnum_t frow_id = 0; frow_id < n_faces; frow_id++) {
    ...
    const cs_gnum_t  grow_ids[3]
      = {sc->rs->g_id[frow_id],              /* x-component */
         sc->rs->g_id[frow_id +   n_faces],  /* y-component */
         sc->rs->g_id[frow_id + 2*n_faces]}; /* z-component */
    ...
    for (cs_lnum_t idx = start; idx < end; idx++) {
      ...
      const cs_gnum_t  gcol_ids[3]
        = {sc->rs->g_id[fcol_id],              /* x-component */
           sc->rs->g_id[fcol_id + n_faces],    /* y-component */
           sc->rs->g_id[fcol_id + 2*n_faces]}; /* z-component */
    ...
    for (cs_lnum_t idx = f2c->idx[frow_id]; idx < f2c->idx[frow_id+1]; idx++) {
      ...
      const cs_gnum_t  gcol_id = sc->rs->g_id[3*n_faces + ccol_id];
    ...
    cs_matrix_assembler_add_g_ids(sc->ma, n_entries, grows, gcols);
    assert(shift == n_entries);

  } /* Loop on face entities */

  /* 4. Build the matrix structure */
  cs_matrix_assembler_compute(sc->ma);

  sc->ms = cs_matrix_structure_create_from_assembler(CS_MATRIX_MSR, sc->ma);
  ...
  cs_timer_counter_add_diff(&(sc->timer), &t_bld, &t_tmp);
}
          ...
          cs_real_t                       eqc_st[])
{
  ...
  const cs_lnum_t  n_faces = cs_shared_quant->n_faces;
  const cs_range_set_t  *rset = sc->rs;
  ...
  for (int bi = 0; bi < bd->n_row_blocks; bi++) {
    ...
    const cs_gnum_t  bi_gids[3]
      = {sc->rs->g_id[fi_id],              /* x-component */
         sc->rs->g_id[fi_id +   n_faces],  /* y-component */
         sc->rs->g_id[fi_id + 2*n_faces]}; /* z-component */

    for (int bj = 0; bj < bd->n_col_blocks; bj++) {

      const cs_lnum_t  fj_id = cm->f_ids[bj];
      const cs_gnum_t  bj_gids[3]
        = {sc->rs->g_id[fj_id],              /* x-component */
           sc->rs->g_id[fj_id +   n_faces],  /* y-component */
           sc->rs->g_id[fj_id + 2*n_faces]}; /* z-component */
...
cs_cdofb_monolithic_free_scheme_context(void   *scheme_context)
{
  ...
  cs_range_set_destroy(&(sc->rs));
  cs_interface_set_destroy(&(sc->ifs));
  cs_matrix_structure_destroy(&(sc->ms));
  cs_matrix_assembler_destroy(&(sc->ma));
                                   ...
                                   void                       *scheme_context)
{
  ...
  cs_matrix_t  *matrix = cs_matrix_create(sc->ms);
  cs_real_t  *rhs = NULL;
...
#       pragma omp critical
      ...
      _assemble(csys, sc, cm, _div, has_sourceterm,
  ...
  _solve_system(sles, matrix, mom_eqp, sc, vel_f, pr, rhs);
