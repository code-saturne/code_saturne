...
static cs_maxwell_t *cs_maxwell_structure = nullptr;
...
_maxwell_create(void)
{
  ...
  mxl->e_permeability = nullptr;

  mxl->m_perm_ref = cs_maxwell_vacuum_m_permeability;
  mxl->m_permittivity = nullptr;

  mxl->conductivity = nullptr;

  /* Fields and related arrays */

  mxl->scal_pot = nullptr;

  mxl->vect_pot = nullptr;

  mxl->e_field       = nullptr;
  mxl->e_field_array = nullptr; /* different location that e_field */

  mxl->d_field       = nullptr;
  mxl->d_field_array = nullptr; /* different location that d_field */

  mxl->h_field       = nullptr;
  mxl->h_field_array = nullptr; /* different location that h_field */

  mxl->b_field       = nullptr;
  mxl->b_field_array = nullptr; /* different location that b_field */

  mxl->j_field       = nullptr;
  mxl->j_field_array = nullptr; /* different location that j_field */

  /* Additional quantities (source terms for instance) */

  mxl->joule_effect = nullptr;
...
{
  if (cs_maxwell_structure == nullptr)
    return false;
  else
    return true;
}
                    ...
                    cs_flag_t     options)
{
  ...
  if (model & CS_MAXWELL_MODEL_ELECTROSTATIC) {
    ...
    cs_property_def_iso_by_value(mxl->e_permeability, nullptr, mxl->e_perm_ref);
  ...
  if (model & CS_MAXWELL_MODEL_MAGNETOSTATIC) {
    ...
    cs_property_def_iso_by_value(mxl->m_permittivity, nullptr, mxl->m_perm_ref);
...
cs_maxwell_init_setup(void)
{
  ...
  if (mxl == nullptr)
    bft_error(__FILE__, __LINE__, 0, _(_err_empty_maxwell));
