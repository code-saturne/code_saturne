...
cs_equation_bc_dirichlet_at_vertices(cs_real_t                   t_eval,
                                     ...
                                     cs_real_t                  *values)
{
  assert(face_bc != nullptr && bcflag != nullptr && values != nullptr);
  ...
  for (cs_lnum_t i = 0; i < face_bc->n_nhmg_dir_faces; i++) {
    ...
    case CS_XDEF_BY_TIME_FUNCTION:
      ...
      _assign_vb_dirichlet_values(eqp->dim,
                                  n_vf,
                                  lst,
                                  &bc_val,
                                  true, /* is constant for all vertices ? */
      ...
      case CS_XDEF_BY_TIME_FUNCTION:
        ...
        if (def->dim == 1) {
          ...
          cs_array_real_set_scalar_on_subset(
            bz->n_elts, elt_ids, bc_val, values);
        }
          else if (def->dim == 3) {
      ...
      case CS_XDEF_BY_ARRAY:
        ...
        if (bz->id == ac->z_id) {
          ...
          if (ac->z_id == 0) { /* Array is defined on the full location */
            ...
            if (ac->full_length)
              cs_array_real_copy_subset(bz->n_elts,
                                        def->dim,
                                        elt_ids,
...
#pragma omp parallel for if (quant->n_b_faces > CS_THR_MIN)
  ...
  cs_dbg_darray_to_listing(
    "DIRICHLET_VALUES", eqp->dim * quant->n_b_faces, values, 9);
#endif
}
...
cs_equation_compute_neumann_svb(cs_real_t                  t_eval,
                                ...
                                double                    *neu_values)
{
  assert(neu_values != nullptr && cm != nullptr && eqp != nullptr);
  assert(def_id > -1);
  ...
  case CS_XDEF_BY_VALUE:
    cs_xdef_cw_eval_flux_v_by_scalar_val(
      cm, f, t_eval, def->context, neu_values);
    break;
  ...
  case CS_XDEF_BY_ARRAY: {
    ...
    if (ac->full_length) {
      ...
      if (cs_flag_test(ac->value_location, cs_flag_primal_face) ||
          cs_flag_test(ac->value_location, cs_flag_boundary_face))
        cs_xdef_cw_eval_flux_v_by_scalar_val(
          cm, f, t_eval, ac->values + bf_id, neu_values);
      else
        bft_error(__FILE__, __LINE__, 0,
      ...
      if (cs_flag_test(ac->value_location, cs_flag_primal_face) ||
        ...
        cs_xdef_cw_eval_flux_v_by_scalar_val(
          cm, f, t_eval, ac->values + id, neu_values);
      }
      else
        bft_error(__FILE__, __LINE__, 0,
                  " %s: Invalid array location.", __func__);
  ...
  case CS_XDEF_BY_ARRAY: {
    ...
    if (ac->full_length) {
      ...
      if (cs_flag_test(ac->value_location, cs_flag_primal_face) ||
          cs_flag_test(ac->value_location, cs_flag_boundary_face))
        cs_xdef_cw_eval_flux_v_by_vector_val(
          cm, f, t_eval, ac->values + 3 * bf_id, neu_values);
      else
        bft_error(__FILE__, __LINE__, 0,
                  " %s: Invalid array location.", __func__);
    }
      ...
      if (cs_flag_test(ac->value_location, cs_flag_primal_face) ||
        ...
        cs_xdef_cw_eval_flux_v_by_vector_val(
          cm, f, t_eval, ac->values + 3 * id, neu_values);
      }
      else
        bft_error(__FILE__, __LINE__, 0,
                  " %s: Invalid array location.", __func__);
                                     ...
                                     double                    *neu_values)
{
  ...
  case CS_XDEF_BY_VALUE:
    cs_xdef_cw_eval_flux_by_vector_val(cm, f, t_eval, def->context, neu_values);
    break;
  ...
  case CS_XDEF_BY_TIME_FUNCTION: {
    ...
    neu_values[f]
      = cm->face[f].meas * cs_math_3_dot_product(cm->face[f].unitv, flux);
  } break;
  ...
  case CS_XDEF_BY_ARRAY: {
    ...
    cs_xdef_cw_eval_flux_by_vector_val(cm, f, t_eval, face_val, neu_values);
  } break;
