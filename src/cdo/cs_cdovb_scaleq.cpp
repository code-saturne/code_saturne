...
static cs_cell_sys_t     **_svb_cell_system  = nullptr;
static cs_cell_builder_t **_svb_cell_builder = nullptr;
 ...
 *
 * If field_tnm1 is set to nullptr, this is ignored.
 *
...
static inline void
_svb_reaction_lumped(const cs_equation_param_t    *eqp,
                     const cs_cell_mesh_t         *cm,
                     const cs_hodge_t             *mass_hodge,
                     const cs_equation_builder_t  *eqb,
                     cs_cell_builder_t            *cb,
...
/*!
 * \brief Add the reaction term for a scalar-valued CDO vertex-based scheme
 *        Case of reaction terms relying on a lumped mass matrix and a property
 *        defined on a partition of the cell (dual volume intersecting the
 *        primal cell)
 *
 * \param[in]      eqp         pointer to a cs_equation_param_t structure
 * \param[in]      cm          pointer to a cs_cell_mesh_t structure
                   ...
                   cs_cell_builder_t             *cb)
{
  if (cb->cell_flag & CS_FLAG_BOUNDARY_CELL_BY_FACE) {
    ...
    if (cs_equation_param_has_diffusion(eqp)) {
      ...
      if (csys->has_robin) {
        assert(eqc->enforce_robin_bc != nullptr);
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_CDOVB_SCALEQ_DBG > 1
    if (cs_dbg_cw_test(eqp, cm, csys))
      csys->dump("\n>> Cell system after BC treatment");
#endif
...
static void
_svb_time_do_nothing(const cs_equation_param_t   *eqp,
                     const cs_cell_mesh_t        *cm,
                     const cs_hodge_t            *mass_hodge,
                     ...
                     cs_cell_sys_t               *csys)
{
  ...
  CS_NO_WARN_IF_UNUSED(mass_hodge);
  CS_NO_WARN_IF_UNUSED(inv_dtcur);
  CS_NO_WARN_IF_UNUSED(eqb);
...
/*!
 * \brief Apply the time scheme for a scalar-valued CDO vertex-based scheme
 *        Case of an implicit Euler time scheme (no lumping of the mass matrix)
 *
 * \param[in]      eqp         pointer to a cs_equation_param_t structure
 * \param[in]      cm          pointer to a cs_cell_mesh_t structure
 * \param[in]      mass_hodge  pointer to a Hodge structure or nullptr
...
static void
_svb_imp_euler(const cs_equation_param_t   *eqp,
               const cs_cell_mesh_t        *cm,
               const cs_hodge_t            *mass_hodge,
               const double                 inv_dtcur,
               cs_equation_builder_t       *eqb,
...
static void
_svb_imp_euler_incr(const cs_equation_param_t   *eqp,
                    const cs_cell_mesh_t        *cm,
                    const cs_hodge_t            *mass_hodge,
                    const double                 inv_dtcur,
                    cs_equation_builder_t       *eqb,
...
static void
_svb_imp_euler_lumped(const cs_equation_param_t   *eqp,
                      const cs_cell_mesh_t        *cm,
                      const cs_hodge_t            *mass_hodge,
                      const double                 inv_dtcur,
                      cs_equation_builder_t       *eqb,
...
static void
_svb_imp_euler_lumped_incr(const cs_equation_param_t *eqp,
                           const cs_cell_mesh_t      *cm,
                           const cs_hodge_t          *mass_hodge,
                           const double               inv_dtcur,
                           cs_equation_builder_t     *eqb,
...
static void
_svb_imp_euler_lumped_incr_subcell(const cs_equation_param_t *eqp,
                                   const cs_cell_mesh_t      *cm,
                                   const cs_hodge_t          *mass_hodge,
                                   const double               inv_dtcur,
                                   cs_equation_builder_t     *eqb,
...
/*!
 * \brief Apply the time scheme for a scalar-valued CDO vertex-based scheme
 *        Case of a theta time scheme (no lumping of the mass matrix)
 *
 * \param[in]      eqp         pointer to a cs_equation_param_t structure
 * \param[in]      cm          pointer to a cs_cell_mesh_t structure
 * \param[in]      mass_hodge  pointer to a Hodge structure or nullptr
                  ...
                  cs_cell_sys_t               *csys)
{
  ...
  _svb_imp_euler(eqp, cm, mass_hodge, inv_dtcur, eqb, cb, csys);
}
...
static void
_svb_theta_scheme_lumped(const cs_equation_param_t   *eqp,
                         const cs_cell_mesh_t        *cm,
                         const cs_hodge_t            *mass_hodge,
                         const double                 inv_dtcur,
                         cs_equation_builder_t       *eqb,
                         ...
                         cs_cell_sys_t               *csys)
{
  ...
  _svb_imp_euler_lumped(eqp, cm, mass_hodge, inv_dtcur, eqb, cb, csys);
}
...
static void
_svb_theta_scheme_lumped_subcell(const cs_equation_param_t   *eqp,
                                 const cs_cell_mesh_t        *cm,
                                 const cs_hodge_t            *mass_hodge,
                                 const double                 inv_dtcur,
                                 cs_equation_builder_t       *eqb,
                                 ...
                                 cs_cell_sys_t               *csys)
{
  ...
  _svb_imp_euler_lumped_subcell(eqp, cm, mass_hodge, inv_dtcur, eqb, cb, csys);
}
...
{
  assert(block != nullptr && rhs != nullptr);
...
{
  if (_svb_cell_system == nullptr || _svb_cell_builder == nullptr)
    return false;
  else
    return true;
}
                             ...
                             const cs_time_step_t      *time_step)
{
  ...
  CS_MALLOC(_svb_cell_system, cs_glob_n_threads, cs_cell_sys_t *);
  CS_MALLOC(_svb_cell_builder, cs_glob_n_threads, cs_cell_builder_t *);

  for (int i = 0; i < cs_glob_n_threads; i++) {
    _svb_cell_system[i]  = nullptr;
    _svb_cell_builder[i] = nullptr;
  }
...
#pragma omp parallel
  {
    ...
    _svb_cell_system[t_id] = cs_cell_sys_create(
      connect->n_max_vbyc, connect->n_max_fbyc, 1, nullptr);
    _svb_cell_builder[t_id] = _svb_create_cell_builder(connect);
  }
#else
  assert(cs_glob_n_threads == 1);
  _svb_cell_system[0]
    = cs_cell_sys_create(connect->n_max_vbyc, connect->n_max_fbyc, 1, nullptr);
...
#pragma omp parallel
  ...
  CS_FREE(_svb_cell_system);
  CS_FREE(_svb_cell_builder);
  _svb_cell_system  = nullptr;
  _svb_cell_builder = nullptr;
}
...
void *
cs_cdovb_scaleq_init_context(cs_equation_param_t    *eqp,
                             int                     var_id,
                             int                     bflux_id,
                             cs_equation_builder_t  *eqb)
{
  ...
  cs_cdovb_scaleq_t *eqc = nullptr;

  CS_MALLOC(eqc, 1, cs_cdovb_scaleq_t);
  ...
  eqc->get_stiffness_matrix = nullptr;
  ...
  eqc->enforce_sliding  = nullptr; /* Only useful for vector-valued eq. */
  ...
  eqc->get_advection_matrix = nullptr;
    ...
    case CS_XDEF_BY_ARRAY:
      {
        ...
        if (cs_flag_test(ctx->value_location, cs_flag_primal_face))
  ...
  if (cs_equation_param_has_reaction(eqp)) {
    ...
    if (eqp->do_lumping) {
      ...
      if (cs_property_is_subcell(eqp->reaction_properties[0])) {
        ...
        eqc->add_reaction_term = _svb_reaction_lumped_subcell;
        ...
        eqc->add_reaction_term = _svb_reaction_lumped;
    ...
    if (eqc->add_reaction_term == nullptr)
      bft_error(__FILE__, __LINE__, 0,
  ...
  eqc->add_unsteady_term = _svb_time_do_nothing; // steady-state case
  ...
  if (cs_equation_param_has_time(eqp)) {
    ...
    if (eqp->do_lumping) {
      ...
      if (eqp->time_scheme == CS_TIME_SCHEME_EULER_IMPLICIT) {
        ...
        if (cs_property_is_subcell(eqp->time_property))
          eqc->add_unsteady_term = _svb_theta_scheme_lumped_subcell;
        else
          eqc->add_unsteady_term = _svb_theta_scheme_lumped;
      ...
      default:
        eqc->add_unsteady_term = _svb_time_do_nothing;
        break;
      }
  ...
  sh = cs_cdo_system_helper_create(CS_CDO_SYSTEM_DEFAULT, 1, &n_vertices, 1);
...
{
  assert(vtx_bc_flag != nullptr); /* Sanity check */
  const cs_cdo_quantities_t *quant   = cs_shared_quant;
  const cs_cdo_connect_t    *connect = cs_shared_connect;

  /* Compute the values of the Dirichlet BC */

  CS_MALLOC(eqb->dir_values, quant->n_vertices, cs_real_t);
                                ...
                                void                      *context)
{
  ...
  cs_cdovb_scaleq_t *eqc = static_cast<cs_cdovb_scaleq_t *>(context);
                                     ...
                                     cs_cell_sys_t             *csys)
{
  ...
  _svb_conv_diff_reac(eqp, eqb, eqc, cm, fm, mass_hodge, diff_hodge, csys, cb);
  ...
  double rhs_norm =
    _svb_cw_rhs_normalization(eqp->sles_param->resnorm_type, cm, csys);
  ...
  if (cs_dbg_cw_test(eqp, cm, csys))
    csys->dump(">> (END Vb scaleq system) Cell system matrix");
#endif
                                          ...
                                          cs_cell_sys_t             *csys)
{
  ...
  csys->mat->matvec(csys->val_n, mat_pk);
  for (short int i = 0; i < csys->n_dofs; i++) /* n_dofs = n_vc */
    csys->rhs[i] -= mat_pk[i];
  ...
  if (cb->has_boundary_elements() && diag_block)
  ...
  if (cb->has_boundary_elements() && csys->has_dirichlet) {
    ...
    if (diag_block)
      cs_cdo_diffusion_pena_dirichlet(eqp, cm, fm, diff_hodge, cb, csys);
...
#pragma omp for CS_CDO_OMP_SCHEDULE reduction(+ : rhs_norm)
  ...
  if (eqb->increment == nullptr)
    CS_MALLOC(eqb->increment, eqc->n_dofs, cs_real_t);
                            ...
                            void                      *context)
{
  ...
  if (eqb->init_step) {
    ...
    if (eqc->source_terms != nullptr) {
      ...
      if (eqp->default_enforcement == CS_PARAM_BC_ENFORCE_ALGEBRAIC ||
          eqp->default_enforcement == CS_PARAM_BC_ENFORCE_PENALIZED) {
        assert(eqc->vtx_bc_flag != nullptr);
        for (cs_lnum_t v = 0; v < n_vertices; v++) {
...
cs_real_t *
cs_cdovb_scaleq_get_vertex_values(void *context, bool previous)
{
  cs_cdovb_scaleq_t *eqc = (cs_cdovb_scaleq_t *)context;
                                ...
                                bool  previous)
{
  ...
  if (previous)
    ...
    vtx_values = pot->val;
  assert(vtx_values != nullptr);

  /* Reset buffer of values */

  if (eqc->cell_values == nullptr)
    CS_MALLOC(eqc->cell_values, quant->n_cells, cs_real_t);
                        ...
                        void                      *context)
{
  ...
  cs_cdo_balance_t *eb =
    cs_cdo_balance_create(cs_flag_primal_vtx, quant->n_vertices);
...
#pragma omp atomic
          ...
          mass_hodge->matrix->matvec(dp, res);
...
#pragma omp atomic
      ...
      if (cs_equation_param_has_reaction(eqp)) {
        ...
        mass_hodge->matrix->matvec(p_theta, res);
...
#pragma omp atomic
      ...
      if (cs_equation_param_has_diffusion(eqp)) {
        ...
        if (!(eqb->diff_pty_uniform))
          cs_hodge_evaluate_property_cw(cm,
                                        cb->t_pty_eval,
                                        cb->cell_flag,
                                        diff_hodge);
...
#pragma omp atomic
  ...
  for (cs_lnum_t v_id = 0; v_id < quant->n_vertices; v_id++)
    eb->balance[v_id] = eb->unsteady_term[v_id] + eb->reaction_term[v_id] +
                                ...
                                cs_real_t                 *res)
{
  if (res == nullptr)
    bft_error(__FILE__,
              __LINE__,
              0,
              "%s: Resulting array not allocated.",
              __func__);
  ...
  if (cs_flag_test(res_loc, cs_flag_primal_vtx) == false &&
    ...
    bft_error(__FILE__,
              __LINE__,
              0,
              "%s: Invalid location for the resulting array.",
              __func__);
...
#pragma omp for CS_CDO_OMP_SCHEDULE
    for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
      ...
      if (!pty_uniform)
        cs_hodge_evaluate_property_cw(cm, t_cur, cb->cell_flag, hodge);
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN)                          \
         ...
         _svb_cell_builder) firstprivate(t_eval)
  {
    ...
    const cs_lnum_t         shift   = f2c->idx[quant->n_i_faces];
    ...
    cs_eflag_t add_flag =
      CS_FLAG_COMP_EV | CS_FLAG_COMP_FE | CS_FLAG_COMP_PEQ | CS_FLAG_COMP_PFQ;
    ...
    cs_property_data_t  tmp_data =
      cs_property_data_define(need_tensor, false, diff_pty);
...
#pragma omp for CS_CDO_OMP_SCHEDULE
    for (cs_lnum_t bf_id = 0; bf_id < quant->n_b_faces; bf_id++) {
      ...
      const cs_lnum_t  c_id = f2c->ids[bf_id + shift];
      ...
      switch (face_bc->flag[bf_id]) {
        ...
        case CS_CDO_BC_ROBIN: {
          ...
          short int n_vf = 0;
          for (int i = cm->f2v_idx[f]; i < cm->f2v_idx[f + 1]; i++) {
            const cs_real_t pv = values[cm->v_ids[cm->f2v_ids[i]]];
            _flx[n_vf] = f_area * wvf[n_vf] * (alpha * (pv - u0) + beta);
            n_vf++;
          }
        } break;

        default: { /* Reconstruct a normal flux at the boundary face */
                                   ...
                                   cs_real_t                 *diff_flux)
{
  ...
  cs_cdovb_scaleq_t *eqc = (cs_cdovb_scaleq_t *)eq_context;

  assert(eqp != nullptr && eqb != nullptr && eqc != nullptr);
  assert(eqc->diffusion_hodge != nullptr);
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN)                          \
           ...
           _svb_cell_builder)
  {
    ...
    /* Each thread retrieves its related structures:
       Get the cellwise view of the mesh and the algebraic system */
    ...
    cs_property_data_t *saved_data = diff_hodge->pty_data;
    cs_property_data_t  tmp_data =
      cs_property_data_define(false, false, diff_pty);

    diff_hodge->pty_data = &tmp_data;
...
#pragma omp for CS_CDO_OMP_SCHEDULE
    for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
      ...
      if (eqp->diffusion_hodgep.algo == CS_HODGE_ALGO_WBS) {
        /* Interpolate the value of the potential at the cell center */
    ...
    diff_hodge->pty_data = saved_data;
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN)                          \
           ...
           _svb_cell_builder)
  {
    ...
    /* Settings depending on the choice on the Hodge algorithm */

    bool                        need_tensor = false;
    cs_cdo_diffusion_cw_flux_t *get_flux = cs_cdo_diffusion_svb_get_dfbyc_flux;

    if (eqp->diffusion_hodgep.algo == CS_HODGE_ALGO_WBS) {
      msh_flag |= CS_FLAG_COMP_PFQ | CS_FLAG_COMP_DEQ | CS_FLAG_COMP_SEF |
                  CS_FLAG_COMP_HFQ | CS_FLAG_COMP_PFC | CS_FLAG_COMP_FEQ;

      get_flux = cs_cdo_diffusion_wbs_get_dfbyc_flux;
...
#pragma omp for CS_CDO_OMP_SCHEDULE
    for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
      ...
      get_flux(cm, pot, diff_hodge, cb, diff_flux + connect->c2e->idx[c_id]);
