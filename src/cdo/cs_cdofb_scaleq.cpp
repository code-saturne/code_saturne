...
static cs_cell_sys_t     **cs_cdofb_cell_sys = nullptr;
static cs_cell_builder_t **cs_cdofb_cell_bld = nullptr;
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_CDOFB_SCALEQ_DBG > 1
    if (cs_dbg_cw_test(eqp, cm, csys))
      csys->dump(">> Cell system matrix after user hook");
#endif
  }
}
...
{
  assert(asb != nullptr && block != nullptr
         && rhs != nullptr); /* Sanity check */
...
# pragma omp critical
  ...
  if (eqc->source_terms != nullptr) { /* Source term */
...
{
  if (cs_cdofb_cell_sys == nullptr || cs_cdofb_cell_bld == nullptr)
    return false;
  else
    return true;
}
...
#pragma omp parallel
  {
    ...
    cs_cdofb_cell_sys[t_id] = cs_cell_sys_create(
      connect->n_max_fbyc + 1, connect->n_max_fbyc, 1, nullptr);
    cs_cdofb_cell_bld[t_id] = _cell_builder_create(connect);
  }
#else
  assert(cs_glob_n_threads == 1);
  cs_cdofb_cell_sys[0] = cs_cell_sys_create(
    connect->n_max_fbyc + 1, connect->n_max_fbyc, 1, nullptr);
                             ...
                             cs_equation_builder_t  *eqb)
{
  ...
  eqc->enforce_sliding  = nullptr; /* Not useful in the scalar-valued case */
...
# pragma omp parallel if (quant->n_cells > CS_THR_MIN)
  {
    ...
    cs_real_t *val_f_pre = nullptr, *val_c_pre = nullptr;
    if (cur2prev)
...
#if defined(DEBUG) && !defined(NDEBUG) && CS_CDOFB_SCALEQ_DBG > 1
      if (cs_dbg_cw_test(eqp, cm, csys))
        csys->dump(">> Cell system matrix after time treatment");
#endif
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN)                   \
  ...
  firstprivate(t_eval)
  {
    ...
    // 3 * n_fc is needed by Robin BCs and n_fc for Neumann BCs
    ...
    for (cs_lnum_t bf_id = 0; bf_id < quant->n_b_faces; bf_id++) {
      ...
      case CS_CDO_BC_ROBIN:
        { /* Set the local mesh structure for the current cell */
          ...
          const cs_real_t *rob_coefs = bc_values + 3*f;
