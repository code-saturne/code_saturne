...
static double **cs_cdo_connect_cw_buffer      = nullptr;
...
_build_f2f_ed_through_edges(const cs_cdo_connect_t *connect)
{
  ...
  assert(f2f_c != nullptr); /* Has to be build before */
...
# pragma omp parallel for if (n_vertices > CS_THR_MIN)
  ...
  for (cs_lnum_t c_id = 0; c_id < n_cells; c_id++) {
    ...
    for (short int v = 0; v < n_vc; v++) {
      ...
      if (v_count[v_id] > n_max_v2ec)
        n_max_v2ec = v_count[v_id];
    ...
    for (short int f = 0; f < n_fc; f++) {
      ...
      if (f_id < m->n_i_faces) { /* Interior face */
        ...
        const cs_lnum_t *f2v_idx = m->b_face_vtx_idx + f_id - m->n_i_faces;
    ...
    for (short int v = 0; v < n_vc; v++) {
      ...
      const cs_lnum_t v_id = c2v_ids[v];
      if (v_count[v_id] > n_max_v2fc)
        n_max_v2fc = v_count[v_id];
      v_count[v_id] = 0; /* reset */
    }
                     ...
                     cs_flag_t  mac_scheme_flag)
{
  ...
  cs_cdo_connect_t *connect = nullptr;
  ...
  connect->bf2v = cs_adjacency_create_from_i_arrays(
    mesh->n_b_faces, mesh->b_face_vtx_idx, mesh->b_face_vtx_lst, nullptr);

  /* Map the interior face --> vertices connectivity */

  connect->if2v = cs_adjacency_create_from_i_arrays(
    mesh->n_i_faces, mesh->i_face_vtx_idx, mesh->i_face_vtx_lst, nullptr);
  ...
  connect->vtx_rset = nullptr;
  connect->vtx_ifs  = mesh->vtx_interfaces; /* Already defined */

  connect->edge_rset = nullptr;
  connect->edge_ifs  = nullptr;

  connect->face_rset = nullptr;
  connect->face_ifs  = nullptr;
  ...
  _assign_edge_ifs_rs(
    mesh, connect, eb_scheme_flag, &(connect->edge_ifs), &(connect->edge_rset));
...
cs_cdo_connect_t *
cs_cdo_connect_free(const cs_mesh_t *mesh, cs_cdo_connect_t *connect)
{
  if (connect == nullptr)
    return connect;
                                 ...
                                 cs_range_set_t     **p_rs)
{
  ...
  case 1: /* Scalar-valued */
    ifs = *p_ifs;
    assert(ifs != nullptr || cs_glob_n_ranks < 2); /* Should be already set */
    break;
  ...
  default:
    if (interlaced)
      ...
      ifs = cs_interface_set_dup_blocks(
        mesh->vtx_interfaces, n_vertices, n_vtx_dofs);
    break;
...
{
  if (connect == nullptr || edge_values == nullptr)
    return;
  const cs_lnum_t n_faces = connect->n_faces[CS_ALL_FACES];
