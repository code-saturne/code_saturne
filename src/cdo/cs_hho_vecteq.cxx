  ...
  case CS_SPACE_SCHEME_HHO_P0:  /* TODO */
    {
      ...
      cb->loc = cs_sdm_block33_create(n_fc + 1, n_fc + 1);
  ...
  case CS_SPACE_SCHEME_HHO_P1:
    {
      ...
      */
      ...
      for (int i = 0; i < n_fc; i++) cb->ids[i] = 3*3;
      cb->ids[n_fc] = 3*4;
  ...
  case CS_SPACE_SCHEME_HHO_P2:
    {
      ...
      /* Store the face, cell and gradient basis function evaluations and
         ...
         or the factorization of the cell_cell block of size 3*10,
                               we need space for a lower triangular matrix
                               --> (3*10)*(3*10+1)/2 = 465
      ...
      */
      ...
      for (int i = 0; i < n_fc; i++) cb->ids[i] = 3*6;
      cb->ids[n_fc] = 3*10;
      cb->loc = cs_sdm_block_create(n_fc + 1, n_fc + 1, cb->ids, cb->ids);
      cb->aux = cs_sdm_block_create(n_fc + 1, 1, cb->ids, &g_size); /* R_g^T */
    }
...
static void
_vhho_init_cell_system(const cs_cell_mesh_t         *cm,
                       const cs_equation_param_t    *eqp,
                       const cs_equation_builder_t  *eqb,
                       const cs_hho_vecteq_t        *eqc,
 ...
 * \param[in, out] eqc       pointer to a cs_hho_vecteq_t structure
                    ...
                    cs_cell_sys_t           *csys)
{
  ...
  case 3*4:                       /* HHO k=1, facto. size = 78 */
    {
      assert(eqc->n_face_dofs == 3*3);
      cs_real_t  _rhs[3*4];

      cs_sdm_ldlt_compute(mCC, acc_facto, acc_facto + 78);
      cs_sdm_ldlt_solve(3*4, acc_facto, _cell_rhs, eqc->rc_tilda + c_offset);
      ...
      for (int f = 0; f < n_fc; f++) {
        ...
        for (int f_dof = 0; f_dof < 3*3; f_dof++) {
          for (int c_dof = 0; c_dof < 3*4; c_dof++)
            _rhs[c_dof] = mCF->val[3*3*c_dof + f_dof];
          cs_sdm_ldlt_solve(3*4, acc_facto, _rhs, _acf->val + 3*4*f_dof);
  ...
  case 3*10:                      /* HHO k=2, facto. size = 465 */
    {
      assert(eqc->n_face_dofs == 3*6);
      cs_real_t  _rhs[3*10];

      cs_sdm_ldlt_compute(mCC, acc_facto, acc_facto + 465);
      cs_sdm_ldlt_solve(3*10, acc_facto, _cell_rhs, eqc->rc_tilda + c_offset);

      for (int f = 0; f < n_fc; f++) {
        const cs_sdm_t  *mCF = cs_sdm_get_block(m, n_fc, f);
        cs_sdm_t  *_acf = cs_sdm_get_block(eqc->acf_tilda, c2f_shift + f, 0);
        for (int f_dof = 0; f_dof < 3*6; f_dof++) {
          for (int c_dof = 0; c_dof < 3*10; c_dof++)
            _rhs[c_dof] = mCF->val[3*6*c_dof + f_dof];
          cs_sdm_ldlt_solve(3*10, acc_facto, _rhs, _acf->val + 3*10*f_dof);
        }
      }
    }
    break;
    ...
    for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
      ...
      if (cs_equation_param_has_diffusion(eqp)) {
        ...
        for (int bi = 0; bi < n_blocks ; bi++) {
          for (int bj = 0; bj < n_blocks ; bj++) {
            ...
            /* Retrieve the scalar & vector diffusion matrix */
            /* Both have the same block structures, what is different are the
               sizes on each block*/
            const cs_sdm_t  *scalar_bij = cs_sdm_get_block(cb->loc, bi, bj);
            cs_sdm_t  *vector_bij = cs_sdm_get_block(csys->mat, bi, bj);
            ...
            for (int row_k = 0; row_k < sc_n_rows ; row_k++) {
              for (int col_l = 0; col_l < sc_n_cols ; col_l++) {
                const cs_real_t val_ = scalar_bij->val[sc_n_cols*row_k + col_l];
                /* Make it Block Diagonal */
                vector_bij->val[vec_n_cols*row_k + col_l] += val_;
                vector_bij->val[vec_n_cols*(row_k+sc_n_rows)
                                + (col_l +   sc_n_cols)]  += val_;
                vector_bij->val[vec_n_cols*(row_k+2*sc_n_rows)
                                + (col_l + 2*sc_n_cols)]  += val_;
      ...
      if (eqp->default_enforcement == CS_PARAM_BC_ENFORCE_PENALIZED) {
        ...
        /* Weakly enforced Dirichlet BCs for cells attached to the boundary
           ...
           fm = nullptr and hodge = nullptr (not useful in the current context)
        */
        if (cb->cell_flag & CS_FLAG_BOUNDARY_CELL_BY_FACE)
          cs_cdo_diffusion_pena_block_dirichlet(
            eqp, cm, nullptr, nullptr, cb, csys);
      }
...
#   pragma omp for CS_CDO_OMP_SCHEDULE
    for (cs_lnum_t c_id = 0; c_id < quant->n_cells; c_id++) {
      ...
      /* Init center values  */
      field_val[3*c_id  ] = 0.; //x-component
      field_val[3*c_id+1] = 0.; //y-component
      field_val[3*c_id+2] = 0.; //z-component
      for (short int i = 0; i < eqc->n_cell_dofs; i++)
        c_vals[i] = _rc[i] - f_contrib[i]; /* cell DOFs */
      ...
      for (short int i = 0; i < dofs_by_component ; i++) {
        ...
        field_val[3*c_id+2] += cphi_eval[i] * c_vals[i+2*dofs_by_component];
      }
  ...
  cs_dbg_darray_to_listing("FINAL CELL_VALUES", 3*quant->n_cells,
                           eqc->cell_values, eqc->n_cell_dofs);
  cs_dbg_darray_to_listing("FINAL CELL_CENTER_VALUES", 3*quant->n_cells,
                           field_val, 3);
#endif
                           ...
                           void            *scheme_context)
{
  ...
  cs_hho_vecteq_t  *eqc = (cs_hho_vecteq_t  *)scheme_context;
                            ...
                            void            *scheme_context)
{
  ...
  const cs_hho_vecteq_t  *eqc = (const cs_hho_vecteq_t  *)scheme_context;
