                  ...
                  cs_real_t                   *result)
{
  ...
  assert(thm != nullptr); /* Sanity check */
  ...
  if (elt_ids == nullptr) {
    ...
    for (cs_lnum_t i = 0; i < n_elts; i++) {
      ...
      cs_property_get_cell_tensor(
        i, t_eval, thm->lambda, false, (cs_real_t(*)[3])kappa_c);
      for (int k = 0; k < 9; k++)
        kappa_c[k] *= ov_rhocp;
    ...
    for (cs_lnum_t i = 0; i < n_elts; i++) {
      ...
      cs_property_get_cell_tensor(
        c_id, t_eval, thm->lambda, false, (cs_real_t(*)[3])kappa_c);
      for (int k = 0; k < 9; k++)
        kappa_c[k] *= ov_rhocp;
    }
  }
                     ...
                     cs_real_t            *result)
{
  ...
  cs_property_get_cell_tensor(
    cm->c_id, t_eval, thm->lambda, false, (cs_real_t(*)[3])result);
  for (int k = 0; k < 9; k++)
    result[k] *= ov_rhocp;
}
