...
cs_macfb_builder_t **cs_mac_builders = NULL;
                                ...
                                cs_macfb_builder_t        *macb)
{
  ...
  /* Sanity checks */
  assert(cm != NULL && quant != NULL && connect != NULL);
  ...
  assert(f2e != NULL); /* Has to be build before */
...
void
cs_macfb_builder_diffusion(const cs_cell_mesh_t     *cm,
