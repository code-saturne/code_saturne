...
static cs_cell_sys_t     **cs_cdoeb_cell_system  = nullptr;
static cs_cell_builder_t **cs_cdoeb_cell_builder = nullptr;
...
_ebs_create_cell_builder(const cs_cdo_connect_t   *connect)
{
  ...
  std::memset(cb->ids, 0, n_max * sizeof(int));
                   ...
                   cs_cell_builder_t             *cb)
{
  if (cb->cell_flag > 0 && csys->has_dirichlet) {
    ...
    if (eqp->default_enforcement == CS_PARAM_BC_ENFORCE_ALGEBRAIC ||
      ...
      eqc->enforce_essential_bc(eqp, cm, nullptr, hodge, cb, csys);
...
{
  assert(asb != nullptr && sh != nullptr); /* Sanity check */
...
{
  if (cs_cdoeb_cell_builder == nullptr || cs_cdoeb_cell_system == nullptr)
    return false;
  else
    return true;
}
                             ...
                             const cs_time_step_t         *time_step)
{
  ...
  for (int i = 0; i < cs_glob_n_threads; i++) {
    ...
    cs_cdoeb_cell_builder[i] = nullptr;
  }
...
#pragma omp parallel
  {
    ...
    cs_cdoeb_cell_system[t_id]
      = cs_cell_sys_create(n_max_dofs, connect->n_max_fbyc, 1, nullptr);
  }
#else
...
void  *
cs_cdoeb_vecteq_init_context(cs_equation_param_t    *eqp,
                             int                     var_id,
                             int                     bflux_id,
...
# pragma omp parallel for if (n_edges > CS_THR_MIN)
  ...
  eqc->curlcurl_hodge = nullptr;
  eqc->get_curlcurl   = nullptr;
                                   ...
                                   void                       *context)
{
  ...
  cs_cdoeb_vecteq_t  *eqc = (cs_cdoeb_vecteq_t *)context;
  assert(eqc->curlcurl_hodge != nullptr);
...
#   pragma omp for CS_CDO_OMP_SCHEDULE reduction(+:rhs_norm)
  ...
  if (cur2prev && eqc->edge_values_pre != nullptr)
