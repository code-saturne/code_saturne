...
static void
_compute_adv_vector_at_vertices(cs_xdef_t                  *def,
 ...
 *        \ref cs_xdef_array_set_adjacency or \ref cs_xdef_array_set_sublist
 *
 * \param[in, out] adv           pointer to a cs_adv_field_t structure
    ...
    case CS_XDEF_BY_ARRAY:        /* P0 approximation */
      {
        ...
        if (cs_flag_test(cx->value_location, cs_flag_dual_face_byc)) {
          ...
          cs_reco_dfbyc_in_cell(cm,
                                cx->values + cx->adjacency->idx[cm->c_id],
                                vector_values);
...
#         pragma omp parallel for if  (n_b_faces > CS_THR_MIN)
    ...
    if (adv->n_bdy_flux_defs == 0) {
      ...
      for (int def_id = 0; def_id < adv->n_bdy_flux_defs; def_id++) {
        ...
        case CS_XDEF_BY_ARRAY:
          {
            const cs_xdef_array_context_t  *cx = bdef->context;
