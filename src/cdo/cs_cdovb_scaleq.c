                                   ...
                                   void                       *context)
{
  ...
  _vbs_setup(dt_cur, mesh, eqp, eqb, eqc->vtx_bc_flag, &dir_values, &forced_ids);
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN) default(none)     \
  shared(dt_cur, quant, connect, eqp, eqb, eqc, rhs, matrix, mav,       \
         ...
         _vbs_cell_system, _vbs_cell_builder)
  {
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN) default(none)     \
  shared(dt_cur, quant, connect, eqp, eqb, eqc, rhs, matrix, mav,       \
         dir_values, forced_ids, fld, rs, _vbs_cell_system,            \
         _vbs_cell_builder)
  {
    /* Set variables and structures inside the OMP section so that each thread
       has its own value */
                        ...
                        cs_real_t                      dt_cur)
{
  ...
  /* OpenMP block */
#pragma omp parallel if (quant->n_cells > CS_THR_MIN) default(none)     \
  shared(dt_cur, quant, connect, eqp, eqb, eqc, pot, bflux, eb,         \
         _vbs_cell_builder)
  {
#if defined(HAVE_OPENMP) /* Determine default number of OpenMP threads */
    int  t_id = omp_get_thread_num();
...
#pragma omp parallel if (quant->n_cells > CS_THR_MIN) default(none)        \
  shared(t_eval, quant, connect, eqp, eqb, diff_flux, values, _vbs_cell_builder)
  {
    ...
    case CS_PARAM_HODGE_ALGO_VORONOI:
      msh_flag |= CS_CDO_LOCAL_DFQ | CS_CDO_LOCAL_EFQ | CS_CDO_LOCAL_PVQ;
      compute_flux = cs_cdo_diffusion_vbcost_get_cell_flux;
      break;
