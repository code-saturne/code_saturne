                 ...
                 const cs_real_t                   coeff)
{
  switch (scheme) {
    ...
    case CS_PARAM_ADVECTION_SCHEME_SG: /* Sharfetter-Gummel */
    {
      ...
      const cs_real_t pe_2            = 0.5 * cs::abs(coeff * beta_mean);
      cs_real_t       upwind_ratio_pe = 0.0;
      if (pe_2 > 5.) {
        ...
        assert(pe_2 > 0.0);
        upwind_ratio_pe = 1.0 - pe_2 * (1.0 / std::tanh(pe_2) - 1.0);
      }

      // hybrid centered scheme
      assert(upwind_ratio_pe >= 0.0 && upwind_ratio_pe <= 1.0);
                        ...
                        cs_sdm_t                         *adv)
{
  ...
  for (short int f = 0; f < cm->n_fc; f++) {
    ...
    if (csys->bf_ids[f] > -1) { /* This is a boundary face */
      ...
      const cs_real_t beta_min = -cs::min(0.0, beta_fc);
