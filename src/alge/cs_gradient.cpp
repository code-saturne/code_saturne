...
/*! Block size for loops on sub-blocks (use size 1 on GPU, as
  threads are already grouped by blocks in that case) */

#if defined( __CUDA_ARCH__) || defined( __SYCL_DEVICE_ONLY__)
constexpr cs_lnum_t  _loop_block_size = 1;
#else
constexpr cs_lnum_t  _loop_block_size = 128;
#endif
...
static void
_scalar_bounds(const cs_mesh_t              *m,
               ...
               cs_real_t                   (*restrict bounds)[2])
{
  ...
  /* The variable must be already synchronized */
  ...
  ctx.parallel_for(n_blocks, [=] CS_F_HOST_DEVICE (cs_lnum_t b_id) {
    ...
    for (cs_lnum_t i = 0; i < b_e_id; i++) { /* Loop on block elements */
      ...
      bounds[c_id1][0] = v_min[i];
      bounds[c_id1][1] = v_max[i];
                                  ...
                                  cs_real_t           *restrict clip_factor)
{
  ...
  ctx.wait();
  cs_halo_sync(m->halo, halo_type, ctx.use_gpu(), factor);
    ...
    for (int adj_id = 0; adj_id < n_adj; adj_id++) {
      ...
      for (cs_lnum_t c_id1 = s_id; c_id1 < e_id; c_id1++) {
        ...
        for (cs_lnum_t cidx = cell_cells_idx[c_id1];
          ...
          l_min_factor = cs::min(l_min_factor, factor[c_id2]);

        }

        clip_factor[c_id1] = cs::min(clip_factor[c_id1], l_min_factor);
                          ...
                          T                           (*restrict grad)[3])
{
  ...
  if (clip_mode == CS_GRADIENT_LIMIT_CELL) {
    ...
    ctx.parallel_for(n_blocks, [=] CS_F_HOST_DEVICE (cs_lnum_t b_id) {
      ...
      cs_real_t denum[_loop_block_size];
      cs_real_t denom[_loop_block_size];
      ...
      for (int adj_id = 0; adj_id < n_adj; adj_id++) {
        ...
        for (cs_lnum_t i = 0; i < b_e_id; i++) { /* Loop on block elements */
          ...
          for (cs_lnum_t cidx = cell_cells_idx[c_id1];
            ...
            denum[i] = cs::max(denum[i], dist1);
            denom[i] = cs::max(denom[i], dvar);
  ...
  /*------------------------------------------------*/
    ...
    _gradient_update_face_clip_factor(m, ma, ctx, halo_type, factor, clip_factor);
  ...
  /* clipping Algorithm: based on face gradient reconstructed values at I'/J' */
  /*--------------------------------------------------------------------------*/

  /* Synchronize variable */

  cs_halo_sync(m->halo, halo_type, use_gpu, clip_factor);
  ...
  if (verbosity > 1) {
    ...
    ctx.parallel_for_reduce_sum(n_cells, n_clip, [=] CS_F_HOST_DEVICE
                                (cs_lnum_t c_id,
                                 CS_DISPATCH_REDUCER_TYPE(cs_gnum_t) &_n_clip) {
 ...
 *   coefap         <-- boundary face coefficient a
 *   coefbp         <-- boundary face coefficient b
 *   pvar           <-- variable
  ...
  if (hyd_p_flag == 1) {
    ...
    ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
      ...
      */
      ...
      if (ii < n_cells)
        cs_dispatch_sum<3>(grad[ii], fctb_ii, i_sum_type);
      if (jj < n_cells)
        cs_dispatch_sum<3>(grad[jj], fctb_jj, i_sum_type);
    ...
    ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
      ...
      */
      ...
      cs_dispatch_sum<3>(grad[ii], fctb, b_sum_type);
  ...
  /*---------------------------------------------*/
    ...
    ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
      ...
      */
      ...
      cs_real_t pfaci = (1.0-ktpond) * (pvar[jj]-pvar[ii]) * i_face_surf[f_id];
      cs_real_t pfacj =     -ktpond  * (pvar[jj]-pvar[ii]) * i_face_surf[f_id];
                             ...
                             T                             (*restrict grad)[3])
{
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    cs_real_t f_surf = i_face_surf[f_id];
    const cs_real_t *f_cog = i_face_cog[f_id];
    const cs_nreal_t *f_u_normal = i_face_u_normal[f_id];

    for (cs_lnum_t i = 0; i < 3; i++) {
      cs_real_t pfaci = (f_cog[i] - cell_f_cen[ii][i]) * f_surf;
      cs_real_t pfacj = (f_cog[i] - cell_f_cen[jj][i]) * f_surf;
      ...
      if (ii < n_cells)
        cs_dispatch_sum<3>(cor_mat[ii][i], fctb_ii, i_sum_type);
      if (jj < n_cells)
        cs_dispatch_sum<3>(cor_mat[jj][i], fctb_jj, i_sum_type);
    }
  ...
  ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  face_id) {
    ...
    if (bc_type[face_id] == CS_OUTLET || bc_type[face_id] == CS_SYMMETRY) {
      ...
      const cs_nreal_t *f_u_normal = b_face_u_normal[face_id];

      /* Special treatment for outlets */
      cs_real_t xi[3], nn[3];
      for (cs_lnum_t k = 0; k < 3; k++) {
        xi[k] = cell_cen[ii][k];
        nn[k] = f_u_normal[k] * f_surf;
      }
      ...
      for (cs_lnum_t k = 0; k < 3; k++) {
        cs_real_t pfac = (xij[k] - xip[k]) * f_surf;
        cs_real_t fctb[3];
        for (cs_lnum_t j = 0; j < 3; j++)
          fctb[j] = pfac * f_u_normal[j];

        cs_dispatch_sum<3>(cor_mat[ii][k], fctb, b_sum_type);
      }
    }
  });
                      ...
                      cs_gradient_quantities_t      *gq)
{
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    cs_real_t pfac0 = -0.5 * i_face_surf[f_id];
    for (cs_lnum_t i = 0; i < 3; i++) {
      cs_real_t pfac = pfac0 * dofij[f_id][i];

      cs_real_t fctb_ii[3], fctb_jj[3];
      for (cs_lnum_t j = 0; j < 3; j++) {
        cs_real_t vecfac = pfac*i_face_u_normal[f_id][j];
        fctb_ii[j] =  vecfac * dvol1;
        fctb_jj[j] = -vecfac * dvol2;
      }

      if (cell_id1 < n_cells)
        cs_dispatch_sum<3>(cocg[cell_id1][i], fctb_ii, i_sum_type);
      if (cell_id2 < n_cells)
        cs_dispatch_sum<3>(cocg[cell_id2][i], fctb_jj, i_sum_type);
    }
  });
                           ...
                           T                             (*restrict grad)[3])
{
  ...
  for (n_sweeps = 1; n_sweeps < nswrgp; n_sweeps++) {
    ...
    if (hyd_p_flag == 1) {
      ...
      ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
        ...
        */
        ...
        if (c_id1 < n_cells)
          cs_dispatch_sum<3>(rhs[c_id1], fctb_ii, i_sum_type);
        if (c_id2 < n_cells)
          cs_dispatch_sum<3>(rhs[c_id2], fctb_jj, i_sum_type);
      ...
      ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
        ...
        */
        ...
        cs_dispatch_sum<3>(rhs[c_id], fctb, b_sum_type);
    ...
    ctx.parallel_for_reduce_sum(n_cells, l2_residual, [=] CS_F_HOST_DEVICE
      ...
      sum += cs_math_3_square_norm(rhs[c_id]);
 ...
 *   cell_hb_faces      <-- cells -> hidden boundary faces adjacency
 *   b_face_u_normal    <-- boundary faces unit normals
 *   diipb              <-- vector II' for boundary faces
 *   b_dist             <-- distance between the cell center and
                            the center of gravity of border faces
 *   cocg               <-> cocg covariance matrix for given cell
 *----------------------------------------------------------------------------*/

CS_F_HOST_DEVICE static inline void
_add_hb_faces_cocg_lsq_cell_iprime_f(cs_lnum_t         c_id,
                                     const cs_lnum_t   cell_hb_faces_idx[],
                                     const cs_lnum_t   cell_hb_faces[],
                                     const cs_nreal_t  b_face_u_normal[][3],
...

{
  ...
  for (cs_lnum_t i = s_id; i < e_id; i++) {
    ...
    const cs_real_t unddij = 1. / b_dist[f_id];
    cs_nreal_t dddij[3];
 ...
 *   cell_hb_faces      <-- cells -> hidden boundary faces adjacency
 *   b_face_u_normal    <-- boundary faces unit normals
 *   b_face_cog         <-- boundary faces center of gravity
 *   cell_cen           <-- cell center for given cell
 *   cocg               <-> cocg covariance matrix for given cell
 *----------------------------------------------------------------------------*/

CS_F_HOST_DEVICE static inline void
_add_hb_faces_cocg_lsq_cell_if(cs_lnum_t         c_id,
                               const cs_lnum_t   cell_hb_faces_idx[],
                               const cs_lnum_t   cell_hb_faces[],
...

{
  ...
  for (cs_lnum_t i = s_id; i < e_id; i++) {
    ...
    cs_nreal_t dddij[3];
    for (cs_lnum_t ll = 0; ll < 3; ll++)
      dddij[ll] = b_face_cog[f_id][ll] - cell_cen[ll];

    cs_math_3_normalize(dddij, dddij);
                            ...
                            cs_cocg_6_t                  *cocg)
{
  ...
  ctx.parallel_for(m->n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t c_id) {
    _add_hb_faces_cocg_lsq_cell_iprime_f(c_id,
                                         cell_hb_faces_idx,
                                         cell_hb_faces,
                                         b_face_u_normal,
                                         diipb,
  ...
  ctx.parallel_for(m->n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t c_id) {
    _add_hb_faces_cocg_lsq_cell_if(c_id,
                                   cell_hb_faces_idx,
                                   cell_hb_faces,
                                   b_face_cog,
                                   cell_cen[c_id],
                       ...
                       cs_gradient_quantities_t      *gq)
{
  ...
  ctx.parallel_for(m->n_b_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t idx) {
    ...
    for (cs_lnum_t i = s_id; i < e_id; i++) { /* loop on boundary faces */
      ...
      cs_math_3_normalize(dsij, dsij);
#endif

      _cocg[0] += dsij[0] * dsij[0];
      _cocg[1] += dsij[1] * dsij[1];
      _cocg[2] += dsij[2] * dsij[2];
      _cocg[3] += dsij[0] * dsij[1];
      _cocg[4] += dsij[1] * dsij[2];
      _cocg[5] += dsij[0] * dsij[2];
    }
...
template <bool is_porous, typename T>
static void
                     ...
                     T                    (*restrict grad)[3])
{
  ...
  const cs_lnum_2_t *restrict i_face_cells = m->i_face_cells;
  const cs_lnum_t *restrict b_face_cells = m->b_face_cells;
  const cs_lnum_t *restrict cell_cells_e_idx = m->cell_cells_idx;
  const cs_lnum_t *restrict cell_cells_e = m->cell_cells_lst;
...
// HIP computation if available instead of CUDA
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    if (c_weight != nullptr) {
      ...
      cs_real_t cw_ii = c_weight[ii], cw_jj = c_weight[jj];
      cs_real_t denom = 1. / (pond*cw_ii + (1. - pond)*cw_jj);

      /* (P_j - P_i) / ||d||^2 */
      cs_real_t pfac = denom * (rhsv[jj][3] - rhsv[ii][3]) * ddc;
      ...
      if (ii < n_cells)
        cs_dispatch_sum<3>(rhsv[ii], fctb_ii, i_sum_type);
      if (jj < n_cells)
        cs_dispatch_sum<3>(rhsv[jj], fctb_jj, i_sum_type);
    }
    else {
      /* (P_j - P_i) / ||d||^2 */
      cs_real_t pfac = (rhsv[jj][3] - rhsv[ii][3]) * ddc;
  ...
  if (halo_type == CS_HALO_EXTENDED && cell_cells_e_idx != nullptr) {
    ...
    ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
      for (cs_lnum_t cidx = cell_cells_e_idx[ii];
  ...
  ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    cs_dispatch_sum<3>(rhsv[c_id], fctb, b_sum_type);
  ...
  cs_halo_sync_r(m->halo, CS_HALO_STANDARD, on_device, grad);
  ...
  ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
    ...
    if (c2c_e_idx != nullptr) {
      ...
      for (cs_lnum_t cidx = s_id; cidx < e_id; cidx++) {
        ...
        pfac =   (v_jj - v_ii) / cs_math_3_square_norm(dc);
    ...
    for (cs_lnum_t fidx = s_id; fidx < e_id; fidx++) {
      ...
      const cs_real_t v_f = val_f[f_id];
      ...
      cs_real_t pfac = ddif * (v_f - v_ii);

      for (cs_lnum_t ll = 0; ll < 3; ll++)
        rhsv[ll] += dif[ll] * pfac;

      if (compute_bounds) {
        v_min = cs::min(v_min, v_f);
        v_max = cs::max(v_max, v_f);
      }
...
template <bool is_porous_f, typename T>
static void
                           ...
                           T                    (*restrict grad)[3])
{
  ...
  ctx_b.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    cs_real_t if_dot_fext = cs_math_3_distance_dot_product(cell_cen[c_id],
                                                           b_face_cog[f_id],
                                                           f_ext[c_id]);
                         ...
                         T                            (*restrict grad)[3])
{
  ...
  ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
    ...
    for (cs_lnum_t fidx = s_id; fidx < e_id; fidx++) {
      ...
      cs_real_t pfac = (val_f[f_id] - pvar[ii]) * ddif;
    ...
    ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
      ...
      for (cs_lnum_t fidx = s_id; fidx < e_id; fidx++) {
        ...
        */
        ...
        cs_real_t pfac = (val_f[f_id] - var_ii) * b_face_surf[f_id];
                              ...
                              T                             (*restrict grad)[3])
{
  ...
  cs_real_t *i_f_var, *b_f_var;
  CS_MALLOC_HD(i_f_var, m->n_i_faces, cs_real_t, cs_alloc_mode);
  CS_MALLOC_HD(b_f_var, m->n_b_faces, cs_real_t, cs_alloc_mode);
  ...
  for (int f_t = 0; f_t < 2; f_t++) {
    ...
    ctx.parallel_for(n_faces, [=] CS_F_HOST_DEVICE (cs_lnum_t f_id) {
  ...
  if (hyd_p_flag == 1) {
    ...
    ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
      ...
      */
      ...
      cs_dispatch_sum<3>(grad[c_id], fctb, b_sum_type);
      ...
      for (int adj_id = 0; adj_id < n_adj; adj_id++) {
        ...
        for (cs_lnum_t i = 0; i < b_e_id; i++) { /* Loop on block elements */
          ...
          for (cs_lnum_t cidx = cell_cells_idx[c_id1];
            ...
            denum[i] = cs::max(denum[i], dist_sq1);
            denom[i] = cs::max(denom[i], dvar_sq);
...
/*----------------------------------------------------------------------------
 * Compute the bounds of a vector or tensor based on neighboring cell values.
 * This function deals with the standard or extended neighborhood.
 *
 * template parameters:
 *   stride        3 for vectors, 6 for symmetric tensors
                ...
                cs_real_t           *restrict bounds)
{
  ...
  ctx.parallel_for(n_blocks, [=] CS_F_HOST_DEVICE (cs_lnum_t b_id) {
    ...
    /* Add bounds from boundary faces */
...
/*----------------------------------------------------------------------------
 * Initialize the gradient of a vector or tensor for gradient reconstruction.
 *
 * template parameters:
 *   stride        3 for vectors, 6 for symmetric tensors
 *
...
)
{
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    cs_real_t ktpond = (c_weight == nullptr) ?
      ...
      pond * c_weight[ii] // cell weighting active
        / (       pond * c_weight[ii]
           + (1.0-pond)* c_weight[jj]);
    ...
    */
    ...
    for (cs_lnum_t i = 0; i < stride; i++) {
      ...
      if (ii < n_cells)
        cs_dispatch_sum<3>(grad[ii][i], fctb_ii, i_sum_type);
      if (jj < n_cells)
        cs_dispatch_sum<3>(grad[jj][i], fctb_jj, i_sum_type);
    }
  ...
  ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    for (cs_lnum_t i = 0; i < stride; i++) {
      ...
      cs_dispatch_sum<3>(grad[c_id][i], fctb, b_sum_type);
    }
...
/*----------------------------------------------------------------------------
 * Compute the gradient of a vector or tensor with an iterative technique
 * in order to handle non-orthoganalities (n_r_sweeps > 1).
 *
    ...
    for (isweep = 1;
      ...
      ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
        ...
        */
        ...
        for (cs_lnum_t i = 0; i < stride; i++) {
          ...
          cs_real_t fctb_1[3], fctb_2[3];
          for (cs_lnum_t j = 0; j < 3; j++) {
            fctb_1[j] =  pfaci * f_u_normal[j];
            fctb_2[j] = -pfacj * f_u_normal[j];
          }

          if (c_id1 < n_cells)
            cs_dispatch_sum<3>(rhs[c_id1][i], fctb_1, i_sum_type);
          if (c_id2 < n_cells)
            cs_dispatch_sum<3>(rhs[c_id2][i], fctb_2, i_sum_type);
      ...
      ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
        ...
        for (cs_lnum_t i = 0; i < stride; i++) {
          ...
          */
          ...
          cs_dispatch_sum<3>(rhs[c_id][i], fctb, b_sum_type);
      ...
      ctx.parallel_for_reduce_sum(n_cells, l2_residual, [=] CS_F_HOST_DEVICE
        ...
        for (cs_lnum_t i = 0; i < stride; i++) {
          ...
          c_sum += cs_math_3_square_norm(rhs[c_id][i]);
        }
        sum += c_sum;
 ...
 *
 * On GPU, use one thread per value for coalescing.

 * parameters:
 *   ctx       <-- Reference to dispatch context
 *   on_device <-- trur if running on GPU
 *   n_cells   <-- Local number of elements
 *   cocg      <-- covariance array
 *   rhs       <-- right-hand side
 *   grad      --> gradient
 *----------------------------------------------------------------------------*/
...
#if defined(HAVE_ACCEL)
  if (on_device) {
    /* Unrolling the loop here allows dispatch to additional threads. */
    cs_lnum_t n_vals = n_cells * stride * 3;

    cs_device_context &d_ctx = static_cast<cs_device_context&>(ctx);
    d_ctx.parallel_for(n_vals, [=] CS_F_HOST_DEVICE (cs_lnum_t idx) {
      size_t c_id = idx / (stride*3);
      size_t i = (idx / 3) % stride;
      size_t j = idx % 3;
...
template <cs_lnum_t stride, bool is_porous, typename T>
static void
                      ...
                      T              (*restrict grad)[stride][3])
{
  ...
  std::chrono::high_resolution_clock::time_point t_start, t_init, t_i_faces, \
    t_ext_n, t_b_faces, t_gradient, t_halo, t_stop;
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    if (c_weight != nullptr) {
      ...
      for (cs_lnum_t kk = 0; kk < stride; kk++) {
        cs_real_t pfac = denom * (pvar[jj][kk] - pvar[ii][kk]) * ddc;
        ...
        if (jj < n_cells)
          cs_dispatch_sum<3>(rhs[jj][kk], fctb_jj, i_sum_type);
      }
    }
      ...
      for (cs_lnum_t kk = 0; kk < stride; kk++) {
        ...
        if (ii < n_cells)
          cs_dispatch_sum<3>(rhs[ii][kk], fctb, i_sum_type);
        if (jj < n_cells)
          cs_dispatch_sum<3>(rhs[jj][kk], fctb, i_sum_type);
      }
    }
  ...
  ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    for (cs_lnum_t kk = 0; kk < stride; kk++) {
      ...
      cs_dispatch_sum<3>(rhs[c_id][kk], fctb, b_sum_type);
    }
  ...
  _compute_gradient_lsq<stride>(ctx, on_device, n_cells, cocg, rhs, grad);
  ...
  if (cs_glob_timer_kernels_flag > 0) {
    ...
    elapsed = std::chrono::duration_cast
                <std::chrono::microseconds>(t_halo - t_gradient);
    printf(", halo = %ld", elapsed.count());
                             ...
                             T                 (*restrict grad)[stride][3])
{
  ...
  ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
    ...
    const cs_real_t w_ii = (c_weight != nullptr) ? c_weight[ii] : 1.;
    ...
    for (cs_lnum_t i = s_id; i < e_id; i++) {
      ...
      if (c_weight != nullptr) {  /* With cell weighting */
        cs_real_t pond = (c2f_sgn[i] > 0) ? weight[f_id] : 1. - weight[f_id];
        cs_real_t denom = 1. / (pond*w_ii + (1. - pond)*w_jj);
        ddc_w *= c_weight[jj] * denom;
      }

      for (cs_lnum_t kk = 0; kk < stride; kk++) {
        cs_real_t pfac = (pvar[jj][kk] - pvar_ii[kk]) * ddc_w;
    ...
    /* Update RHS from local copy */
    for (cs_lnum_t kk = 0; kk < stride; kk++) {
      for (cs_lnum_t ll = 0; ll < 3; ll++)
        rhs[ii][kk][ll] = _rhs[kk][ll];
    }
  ...
  if (c2c_e_idx != nullptr && halo_type == CS_HALO_EXTENDED) {
    ...
    ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
      ...
      for (cs_lnum_t cidx = c2c_e_idx[ii];
           cidx < c2c_e_idx[ii+1];
           cidx++) {
      ...
      /* Update RHS with local contribution */
      for (cs_lnum_t kk = 0; kk < stride; kk++) {
        for (cs_lnum_t ll = 0; ll < 3; ll++)
          rhs[ii][kk][ll] += _rhs[kk][ll];
      }
  ...
  ctx.parallel_for(n_b_cells, [=] CS_F_HOST_DEVICE (cs_lnum_t ii) {
                               ...
                               T                   (*restrict grad)[stride][3])
{
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    for (cs_lnum_t k = 0; k < stride; k++) {
      ...
      if (ii < n_cells)
        cs_dispatch_sum<3>(grad[ii][k], fctb_ii, i_sum_type);
      if (jj < n_cells)
        cs_dispatch_sum<3>(grad[jj][k], fctb_jj, i_sum_type);
  ...
  ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t  f_id) {
    ...
    */
    ...
    for (cs_lnum_t k = 0; k < stride; k++) {
      ...
      cs_dispatch_sum<3>(grad[ii][k], fctb, b_sum_type);
      ...
      if (w_stride == 6 && c_weight != nullptr)
        ...
        if (compute_bounds == false) {
          ...
          _lsq_scalar_gradient_gather<true>(mesh,
                                            ...
                                            r_grad,
                                            bounds);
          compute_bounds = false; // bounds have already been computed
        }
      }
                 ...
                 cs_real_t                  *bounds)
{
  ...
  case CS_GRADIENT_GREEN_ITER:
    {
      using a_t = cs_real_t[6];
      using b_t = cs_real_t[6][6];
                       ...
                       cs_real_t                    c_grad[stride][3])
{
  ...
  if (bc_coeffs != nullptr) {
    bc_coeff_a = (const a_t *)bc_coeffs->a;
    bc_coeff_b = (const b_t *)bc_coeffs->b;
  }
                   ...
                   cs_real_t           (*restrict grad)[3])
{
  ...
  cs_real_t *val_f_hmg = nullptr, *val_f_wrk = nullptr;
  ...
  if (bc_coeffs == nullptr) {
    ...
    cs_real_t *bc_coeff_loc_a = bc_coeffs_loc.a;
    cs_real_t *bc_coeff_loc_b = bc_coeffs_loc.b;
  ...
  else { // bc_coeffs != nullptr, enter here for cpl

    if (bc_coeffs->val_f == nullptr) { // work array (momemtum, ...)
      ...
      if (gradient_type != CS_GRADIENT_GREEN_ITER) {
        ...
        //TODO do a specific formula for Internal coupling
        if (hyd_p_flag) {
                   ...
                   cs_real_t                      gradv[][3][3])
{
  ...
  if (bc_coeffs == nullptr) {
    ...
    CS_MALLOC_HD(bc_coeffs_loc.a, stride*n_b_faces, cs_real_t, amode);
    CS_MALLOC_HD(bc_coeffs_loc.b, stride*stride*n_b_faces, cs_real_t, amode);

    var_t  *bc_coeff_loc_a = (var_t  *)bc_coeffs_loc.a;
    cs_real_33_t *bc_coeff_loc_b = (cs_real_33_t *)bc_coeffs_loc.b;
    ...
    if (bc_coeffs->val_f == nullptr) { // work array (momemtum, ...)
      ...
      if (gradient_type != CS_GRADIENT_GREEN_ITER) {
        ...
        var_t *val_ip = nullptr;
        CS_MALLOC_HD(val_f_wrk, n_b_faces, var_t, cs_alloc_mode);
        CS_MALLOC_HD(val_ip, n_b_faces, var_t, amode);
        ...
        /* Set work boundary value */
        bc_coeffs->val_f = (cs_real_t *)val_f_wrk;
      }
    }
  }
                   ...
                   cs_real_63_t       *restrict grad)
{
  ...
  if (cpl != nullptr) {
    ...
    if (bc_coeffs != nullptr) {
      ...
      cs_real_t _clip_coeff = (clip_mode >= 0) ? clip_coeff : -1;
      cs_internal_coupling_update_bc_coeffs_strided<6>(ctx,
                                                       bc_coeffs,
                                                       cpl,
                                                       halo_type,
                                                       _clip_coeff,
  ...
  if (bc_coeffs == nullptr) {
    ...
    var_t  *bc_coeff_loc_a = (var_t  *)bc_coeffs_loc.a;
    cs_real_66_t *bc_coeff_loc_b = (cs_real_66_t *)bc_coeffs_loc.b;

    ctx.parallel_for(n_b_faces, [=] CS_F_HOST_DEVICE (cs_lnum_t face_id) {
      for (cs_lnum_t j = 0; j < stride; j++) {
        bc_coeff_loc_a[face_id][j] = 0;
        for (cs_lnum_t k = 0; k < stride; k++)
          bc_coeff_loc_b[face_id][j][k] = 0;

        bc_coeff_loc_b[face_id][j][j] = 1;
      }
    });
    ctx.wait();
    ...
    if (bc_coeffs->val_f == nullptr) { // work array
    ...
    /* Compute face value for gradient exept for iterative_gradient
      ...
      if (gradient_type != CS_GRADIENT_GREEN_ITER) {
        ...
        var_t *val_ip = nullptr;
        CS_MALLOC_HD(val_f_wrk, n_b_faces, var_t, amode);
        CS_MALLOC_HD(val_ip, n_b_faces, var_t, amode);

        var_t *coefav = (var_t *)bc_coeffs->a;
        cs_real_66_t *coefbv = (cs_real_66_t *)bc_coeffs->b;
