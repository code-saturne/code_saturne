 ...
 * cs_blas_hip_reduce_single_block<block_size><<<1, block_size, 0>>>
                                ...
                                double            *__restrict__ sum_block)
{
  ...
  cs_blas_hip_block_reduce_sum<block_size, 1>(sdata, tid, sum_block);
}
 ...
 * \param [in]   device_id  HIP device id
...
_prefetch_h2d(const void   *dst,
              ...
              hipStream_t  stream)
{
  CS_HIP_CHECK(hipMemPrefetchAsync(dst, size, device_id, stream));
}
                    ...
                    double               res[])
{
  ...
  /* Fallback: host-side reduction via MPI
     (assumes 'res' is host-visible or HIP-aware MPI is used).
     ---------------------------------------------------------- */
                    ...
                    double               res_h[])
{
  ...
  if (c->nccl_comm != nullptr) {
    ...
    CS_HIP_CHECK(hipMemcpyAsync(res_h, res_d, sizeof(double)*tuple_size,
             ...
             const cs_real_t     *y)
{
  ...
  if (_use_hipblas == false)
    s = cs_blas_hip_dot(c->setup_data->n_rows, x, y);

#if defined(HAVE_HIPBLAS)
  if (_use_hipblas)
    s = cs_blas_hipblas_dot(c->setup_data->n_rows, x, y);
#endif
                                 ...
                                 double      *b_res)
{
  ...
  cs_blas_hip_block_reduce_sum<blockSize, 2>(stmp, tid, b_res);
}

/*----------------------------------------------------------------------------*/
/*!
 * \brief Compute dot products x.y, y.z, summing result over all
 *        threads of a block.
 *
 * blockSize must be a power of 2.
 *
...
template <size_t blockSize, typename T>
__global__ static void
_dot_products_xy_yz_stage_1_of_2(cs_lnum_t    n,
                                 const T     *x,
                                 const T     *y,
                                 const T     *z,
                                 double      *b_res)
{
  ...
  for (cs_lnum_t i = blockIdx.x*(blockDim.x) + tid;
       i < n;
       i += grid_size) {
    stmp[tid*2]     += static_cast<double>(x[i] * y[i]);
    stmp[tid*2 + 1] += static_cast<double>(y[i] * z[i]);
  }
...
template <size_t blockSize, typename T>
__global__ static void
_dot_products_xx_xy_yz_stage_1_of_2(cs_lnum_t    n,
                                    const T     *x,
                                    const T     *y,
                                    const T     *z,
                                    double      *b_res)
{
  ...
  for (cs_lnum_t i = blockIdx.x*(blockDim.x) + tid;
       i < n;
       i += grid_size) {
    stmp[tid*3]     += static_cast<double>(x[i] * x[i]);
    stmp[tid*3 + 1] += static_cast<double>(x[i] * y[i]);
    stmp[tid*3 + 2] += static_cast<double>(y[i] * z[i]);
  }

  // Output: b_res for this block

  cs_blas_hip_block_reduce_sum<blockSize, 3>(stmp, tid, b_res);
}

/*----------------------------------------------------------------------------*/
/*!
 * \brief Compute dot products x.y, y.y, x.y, x.z, y.z, summing result over all
 *        threads of a block.
 *
 * blockSize must be a power of 2.
 *
...
template <size_t blockSize, typename T>
__global__ static void
_dot_products_xx_yy_xy_xz_yz_stage_1_of_2(cs_lnum_t    n,
                                          const T     *x,
                                          const T     *y,
                                          const T     *z,
                                          double      *b_res)
{
  __shared__ double stmp[blockSize*5];
  ...
  for (cs_lnum_t i = blockIdx.x*(blockDim.x) + tid;
       i < n;
       i += grid_size) {
    stmp[tid*5]     += static_cast<double>(x[i] * x[i]);
    stmp[tid*5 + 1] += static_cast<double>(y[i] * y[i]);
    stmp[tid*5 + 2] += static_cast<double>(x[i] * y[i]);
    stmp[tid*5 + 3] += static_cast<double>(x[i] * z[i]);
    stmp[tid*5 + 4] += static_cast<double>(y[i] * z[i]);
  }

  // Output: b_res for this block

  cs_blas_hip_block_reduce_sum<blockSize, 5>(stmp, tid, b_res);
}
                                       ...
                                       double      *b_res)
{
  ...
  cs_blas_hip_block_reduce_sum<blockSize, 4>(stmp, tid, b_res);
}
                          ...
                          double              *s4)
{
  ...
  if (_use_hipblas == false) {
    ...
    cs_hip_get_2_stage_reduce_buffers
      (cs_hip_get_stream_id(stream), n, 4*sizeof(double), grid_size,
       (void *&)sum_block, (void *&)s_d, (void *&)s_h);

    _dot_products_vr_vw_vq_rr_stage_1_of_2
      <block_size><<<grid_size, block_size, 0, stream>>>
      (n, v, w, q, r, sum_block);
    cs_blas_hip_reduce_single_block<block_size, 4><<<1, block_size, 0, stream>>>
  ...
  if (_use_hipblas) {
    ...
    s[0] = cs_blas_hipblas_dot(c->setup_data->n_rows, v, r);
    s[1] = cs_blas_hipblas_dot(c->setup_data->n_rows, v, w);
    s[2] = cs_blas_hipblas_dot(c->setup_data->n_rows, v, q);
    s[3] = cs_blas_hipblas_dot(c->setup_data->n_rows, r, r);
...
)
{
  ...
  if (_use_hipblas == false) {
    double *sum_block, *s_d, *s_h;
    cs_hip_get_2_stage_reduce_buffers
      (cs_hip_get_stream_id(stream), n, sizeof(double), grid_size,
       (void *&)sum_block, (void *&)s_d, (void *&)s_h);
  ...
  if (_use_hipblas) {
    ...
    s[0] = cs_blas_hipblas_dot(c->setup_data->n_rows, x, x);
...
)
{
  ...
  if (_use_hipblas == false) {
    ...
    _dot_products_xx_xy_stage_1_of_2
      <block_size><<<grid_size, block_size, 0, stream>>>
      (n, x, y, sum_block);
    cs_blas_hip_reduce_single_block<block_size, 2><<<1, block_size, 0, stream>>>
      (grid_size, sum_block, s_d);
  ...
  if (_use_hipblas) {
    ...
    s[0] = cs_blas_hipblas_dot(c->setup_data->n_rows, x, x);
    s[1] = cs_blas_hipblas_dot(c->setup_data->n_rows, x, y);
...
)
{
  ...
  if (_use_hipblas == false) {
    double *sum_block, *s_d, *s_h;
    cs_hip_get_2_stage_reduce_buffers
      (cs_hip_get_stream_id(stream), n, 3*sizeof(double), grid_size,
       (void *&)sum_block, (void *&)s_d, (void *&)s_h);

    _dot_products_xx_xy_yz_stage_1_of_2
      <block_size><<<grid_size, block_size, 0, stream>>>
      (n, x, y, z, sum_block);
    cs_blas_hip_reduce_single_block<block_size, 3><<<1, block_size, 0, stream>>>
      (grid_size, sum_block, s_d);
  ...
  if (_use_hipblas) {
    ...
    s[0] = cs_blas_hipblas_dot(c->setup_data->n_rows, x, x);
    s[1] = cs_blas_hipblas_dot(c->setup_data->n_rows, x, y);
    s[2] = cs_blas_hipblas_dot(c->setup_data->n_rows, y, z);
...
)
{
  ...
  if (_use_hipblas == false) {
    ...
    _dot_products_xx_yy_xy_xz_yz_stage_1_of_2
      <block_size><<<grid_size, block_size, 0, stream>>>
      (n, x, y, z, sum_block);
    cs_blas_hip_reduce_single_block<block_size, 5><<<1, block_size, 0, stream>>>
      (grid_size, sum_block, s_d);

    _sync_reduction_sum(c, stream, 5, s_d, s_h);

    for (int i = 0; i < 5; i++)  // s_h is pinned memory, s is not
      s[i] = s_h[i];
  }
...
/*----------------------------------------------------------------------------
 * Solution of A.vx = Rhs using Jacobi (HIP version).
 *
 * parameters:
 *   c               <-- pointer to solver context info
                       ...
                       void                      *aux_vectors)
{
  ...
  {
    hipStream_t stream_pf = cs_hip_get_stream_prefetch();
  ...
  cs_hip_get_2_stage_reduce_buffers
    (cs_hip_get_stream_id(stream), n_rows, sizeof(double), gridsize,
  ...
  if (vx_ini != vx) {
    ...
    if (update_residual) {
      ...
      cs_blas_hip_reduce_single_block<blocksize, 1><<<1, blocksize, 0, stream>>>
  ...
  if (cs_glob_hip_allow_graph) {
    ...
    hipError_t status = hipGraphInstantiate(&graph_exec, graph, &graph_node,
                                              nullptr, 0);
...
/*----------------------------------------------------------------------------
 * Solution of A.vx = Rhs using block Jacobi (HIP version).
 *
 * parameters:
 *   c               <-- pointer to solver context info
 *   a               <-- linear equation matrix
  ...
  if (vx_ini != vx) {
    ...
    if (diag_block_size == 3) {
      _block_3_jacobi_compute_vx0<<<gridsize, blocksize, 0, stream>>>
        (n_rows, ad_inv, rhs, rk, vx_k);
      _block_3_jacobi_compute_residual
        <blocksize><<<gridsize, blocksize, 0, stream>>>
        (n_rows, ad, vx_k, rk, sum_block);
      CS_HIP_CHECK(hipStreamSynchronize(stream));
    }
    else {
      _block_jacobi_compute_vx0<<<gridsize, blocksize, 0, stream>>>
        (n_rows, diag_block_size, ad_inv, rhs, rk, vx_k);
      _block_jacobi_compute_residual
        <blocksize><<<gridsize, blocksize, 0, stream>>>
        (n_rows, diag_block_size, ad, vx_k, rk, sum_block);
    }
  ...
  while (cvg == CS_SLES_ITERATING) {
    ...
    if (diag_block_size == 3) {
      ...
      _block_jacobi_compute_vx<<<gridsize, blocksize, 0, stream>>>
        (n_rows, diag_block_size, ad_inv, rhs, vxx, vx_k);
      _block_jacobi_compute_residual
        <blocksize><<<gridsize, blocksize, 0, stream>>>
        (n_rows, diag_block_size, ad, vx_k, rk, sum_block);
    }
                    ...
                    void                      *aux_vectors)
{
  ...
  const unsigned int blocksize = CS_BLOCKSIZE;
  const unsigned int blocksize_rsb = 512; /* cs_blas_hip_reduce_single_block */

  cs_hip_grid_size(n_rows, blocksize);

  unsigned int gridsize = cs_hip_grid_size(n_rows, blocksize);
  unsigned int gridsize_blas1 = cs_hip_grid_size(n_rows, blocksize_rsb);
  ...
  while (cvg == CS_SLES_ITERATING) {
    ...
    if (n_c_iter == 1 && !(fabs(alpha[0]) > 0))
      _gcr_gkj_inv_1<<<1, CS_HIP_WARP_SIZE, 0, stream>>>
        (gkj, gkj_inv);
    else
      _gcr_gkj_inv<<<1, CS_HIP_WARP_SIZE, 0, stream>>>
