...
cs_blas_hip_warp_reduce_sum(volatile T  *stmp,
                             size_t       tid)
{
  if (stride == 1) {
...
template <size_t blockSize, size_t stride, typename T>
__device__ static void __forceinline__
cs_blas_hip_block_reduce_sum(T       *stmp,
                              size_t   tid,
                              T       *sum_block)
{
  __syncthreads();
  ...
  if (stride == 1) { /* Scalar data */
    ...
    if (tid < 32) {
      cs_blas_hip_warp_reduce_sum<blockSize, stride>(stmp, tid);
    }
#else
    if (tid < 64) {
      cs_blas_hip_warp_reduce_sum<blockSize, stride>(stmp, tid);
                                 ...
                                 T       *g_odata)
{
  ...
  if (stride == 1) {
    ...
    if (tid < CS_HIP_WARP_SIZE) cs_blas_hip_warp_reduce_sum<blockSize, stride>(sdata, tid);
