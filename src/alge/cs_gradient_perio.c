 ...
 * variable because we have to know the all components.
 *
    ...
    for (t_id = 0; t_id < n_transforms; t_id++) {
      ...
      if (   fvm_periodicity_get_type(periodicity, t_id)
        ...
        for (rank_id = 0; rank_id < halo->n_c_domains; rank_id++) {
          ...
          for (i = start_std; i < end_std; i++) {
            dpdx[n_cells + i] = _drdxyz[0 + d_var*3 + 18*i];
            dpdy[n_cells + i] = _drdxyz[1 + d_var*3 + 18*i];
            dpdz[n_cells + i] = _drdxyz[2 + d_var*3 + 18*i];
          }
