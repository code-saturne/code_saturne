...
!
! Indicators for integration on the pdf
! -------------------------------------
integer ipdf4,ipdf5
...
!===============================================================================
!
if (     ipdf1.eq.1 .or. ipdf2.eq.1 .or. ipdf3.eq.1 &
    .or. ipdf4.eq.1 .or. ipdf5.eq.1) then
  ...
  do icla = 1, nclacp
    call field_get_val_s(ixck(icla), cvar_xckcl(icla)%p)
    call field_get_val_s(ixch(icla), cvar_xchcl(icla)%p)
    call field_get_val_s(inp(icla), cvar_xnpcl(icla)%p)
    call field_get_val_s(itemp2(icla), cpro_temp2(icla)%p)
    if (ippmod(iccoal) .eq. 1) then
      call field_get_val_s(ixwt(icla), cvar_xwtcl(icla)%p)
    endif
  enddo

  do numcha = 1, ncharb
    call field_get_val_s(if1m(numcha), cvar_f1m(numcha)%p)
    call field_get_val_s(if2m(numcha), cvar_f2m(numcha)%p)
  enddo
  ...
  do iel=1, ncel
    ...
    if ((fs3no (iel).gt.fs4no(iel)) .and. (fs4no (iel).lt.1.d0)) then
      ...
      toxyd = cs_coal_ht_convert_h_to_t_gas_by_yi(hoxyd, coefe)
     ...
     do numcha=1,ncharb
       ...
       ! H(mv1, TFUEL)
        ...
        xhf1 = cs_coal_ht_convert_t_to_h_gas_by_yi_f1f2(tfuel, coefe, f1mc, f2mc)
        ...
        xhf2 = cs_coal_ht_convert_t_to_h_gas_by_yi_f1f2(tfuel, coefe, f1mc, f2mc)
      ...
      if (xxf .gt. epsicp) then
        ...
        tfs4ad = cs_coal_ht_convert_h_to_t_gas_by_yi(hfs4ad, coefe)
