...
!> \file cs_physical_properties_combustion_drift.f90
...
double precision, dimension(:), pointer :: cpro_temp, cpro_x2
...
do iflid = 0, nfld-1
  ...
  if (icla.ge.1.and.btest(iscdri, DRIFT_SCALAR_ADD_DRIFT_FLUX)) then
    ...
    do iel = 1, ncel
      cpro_taup(iel) = 0.d0
      if(visco(iel).gt.1d-17) then
        ! Simple model for Low Reynolds Numbers
        cpro_taup(iel) = cpro_rom2(iel)                               &
                       * cpro_diam2(iel)**2                           &
                       / (18.d0*visco(iel))
      endif
...
return
end subroutine cs_physical_properties_combustion_drift
