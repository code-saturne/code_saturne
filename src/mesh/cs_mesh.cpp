...
cs_mesh_t  *cs_glob_mesh = nullptr;  /* Pointer on the main mesh */
...
_print_cell_neighbor_info(const cs_mesh_t  *mesh)
{
  ...
  int  *n_cell_neighbors = nullptr;

  /* Summary of the number of cell neighbors */

  CS_MALLOC(n_cell_neighbors, mesh->n_cells_with_ghosts, int);
                 ...
                 const void *y)
{
  ...
  const cs_gnum_t *c0 = static_cast<const cs_gnum_t *>(x);
  const cs_gnum_t *c1 = static_cast<const cs_gnum_t *>(y);
...
{
  cs_lnum_t *n_perio_couples = nullptr;
  cs_gnum_t **perio_couples = nullptr;
  ...
  int perio_count = 0;
  cs_lnum_t *send_index = nullptr;
  cs_lnum_t *recv_index = nullptr;
  cs_gnum_t *send_num = nullptr, *recv_num = nullptr;
  int  *tr_id = nullptr;
  ...
  CS_MALLOC(n_perio_couples, n_perio, cs_lnum_t);
  CS_MALLOC(perio_couples, n_perio, cs_gnum_t *);

  for (i = 0; i < n_perio; i++) {
    n_perio_couples[i] = 0;
    perio_couples[i] = nullptr;
  }
  ...
  CS_MALLOC(send_index, n_ifs_faces + 1, cs_lnum_t);
  CS_MALLOC(recv_index, n_ifs_faces + 1, cs_lnum_t);
  ...
  CS_MALLOC(send_num, send_index[n_ifs_faces], cs_gnum_t);
  CS_MALLOC(recv_num, recv_index[n_ifs_faces], cs_gnum_t);
  ...
  for (i = 0; i < n_perio; i++)
    CS_MALLOC(perio_couples[i], n_perio_couples[i]*2, cs_gnum_t);
  ...
  for (i = 0; i < mesh->n_init_perio; i++) {
    ...
    if (n_perio_couples[i] > 1) {
      ...
      CS_REALLOC(perio_couples[i], n_perio_couples[i]*2, cs_gnum_t);
    }
  }
 ...
 *                      or nullptr (size: mesh->n_ghost_cells)
 *
 * returns:
                    ...
                    int               halo_rank_id[])
{
  ...
  CS_MALLOC(tr_id, halo->n_transforms, int);
  ...
  CS_REALLOC(tr_id, n_perio*2, int);
  assert(n_perio == mesh->n_init_perio);
                             ...
                             cs_lnum_t         **cell_face)
{
  ...
  CS_MALLOC(_cell_face_count, mesh->n_cells_with_ghosts, cs_lnum_t);
  CS_MALLOC(_cell_face_idx, mesh->n_cells_with_ghosts + 1, cs_lnum_t);
  ...
  CS_MALLOC(_cell_face, _cell_face_idx[mesh->n_cells_with_ghosts], cs_lnum_t);
                   ...
                   cs_gnum_t        ***perio_face_couples)
{
  ...
  cs_lnum_t  *loc_cell_num = nullptr;
  cs_lnum_t  *recv_count = nullptr, *recv_idx = nullptr;
  cs_lnum_t  *send_count = nullptr, *send_idx = nullptr;
  cs_lnum_t  *cell_face_idx, *cell_face;
  cs_gnum_t  *recv_vals = nullptr, *send_vals = nullptr;

  int  *halo_perio_num = nullptr;
  int  *halo_rank_id = nullptr;

  cs_lnum_t   *_n_perio_face_couples = nullptr;
  cs_gnum_t  **_perio_face_couples = nullptr;

  int           request_count = 0;
  MPI_Request  *halo_request = nullptr;
  MPI_Status   *halo_status = nullptr;

  const cs_halo_t  *halo = mesh->halo;

  /* Initialization */

  CS_MALLOC(_n_perio_face_couples, mesh->n_init_perio, cs_lnum_t);
  CS_MALLOC(_perio_face_couples, mesh->n_init_perio, cs_gnum_t *);

  for (i = 0; i < mesh->n_init_perio; i++) {
    _n_perio_face_couples[i] = 0;
    _perio_face_couples[i] = nullptr;
  }
  ...
  CS_MALLOC(halo_perio_num, mesh->n_ghost_cells, int);
  CS_MALLOC(halo_rank_id, mesh->n_ghost_cells, int);
  ...
  CS_MALLOC(loc_cell_num, mesh->n_cells_with_ghosts, cs_lnum_t);
  ...
  CS_MALLOC(send_count, halo->n_c_domains, cs_lnum_t);
  CS_MALLOC(recv_count, halo->n_c_domains, cs_lnum_t);
  CS_MALLOC(send_idx, halo->n_c_domains + 1, cs_lnum_t);
  CS_MALLOC(recv_idx, halo->n_c_domains + 1, cs_lnum_t);
  ...
  CS_MALLOC(send_vals, send_idx[halo->n_c_domains]*4, cs_gnum_t);
  ...
  CS_FREE(halo_rank_id);

  /* Now exchange face matching data */

  CS_MALLOC(halo_request, halo->n_c_domains*2, MPI_Request);
  CS_MALLOC(halo_status, halo->n_c_domains*2, MPI_Status);

  CS_MALLOC(recv_vals, recv_idx[halo->n_c_domains]*4, cs_gnum_t);
  ...
  for (i = 0; i < mesh->n_init_perio; i++) {
    CS_MALLOC(_perio_face_couples[i],
  ...
  CS_FREE(halo_perio_num);

  CS_FREE(cell_face_idx);
  CS_FREE(cell_face);
  CS_FREE(recv_vals);
  CS_FREE(loc_cell_num);
                   ...
                   cs_gnum_t        ***perio_face_couples)
{
  ...
  _get_halo_perio_num(mesh, halo_perio_num, nullptr);
...
_check_free_vertices(cs_mesh_t  *mesh)
{
  ...
  CS_MALLOC(ref, mesh->n_vertices, char);
...
_discard_free_vertices(cs_mesh_t  *mesh)
{
  ...
  cs_lnum_t *new_vertex_id = nullptr;

  /* Mark vertices */

  CS_MALLOC(new_vertex_id, mesh->n_vertices, cs_lnum_t);
  ...
  if (n_vertices < mesh->n_vertices) {
    ...
    CS_REALLOC(mesh->vtx_coord, n_vertices*3, cs_real_t);

    if (mesh->global_vtx_num != nullptr)
      CS_REALLOC(mesh->global_vtx_num, n_vertices, cs_gnum_t);
  }

  if (mesh->vtx_interfaces != nullptr)
    cs_interface_set_renumber(mesh->vtx_interfaces, new_vertex_id);

  CS_FREE(new_vertex_id);
  ...
  if (cs_glob_n_ranks > 1) {
    ...
    assert(mesh->global_vtx_num == nullptr);
                          ...
                          cs_gnum_t         n_perio_faces[])
{
  ...
  int *i_face_flag = nullptr;
  cs_lnum_t *f_count = nullptr;
  ...
  if (mesh->halo != nullptr) {
    ...
    CS_MALLOC(i_face_flag, mesh->n_i_faces, int);
  ...
  CS_MALLOC(f_count, mesh->n_families*4, cs_lnum_t);
...
_print_mesh_group_stats(const cs_mesh_t  *mesh)
{
  ...
  cs_gnum_t *count = nullptr, *n_elt_groups = nullptr, *n_perio_faces = nullptr;

  if (count_size == 0)
    return;

  CS_MALLOC(count, count_size, cs_gnum_t);
...
#if defined(HAVE_MPI)
  if (cs_glob_n_ranks > 1) {
    cs_gnum_t *_count = nullptr;
    CS_MALLOC(_count, count_size, cs_gnum_t);
...
_print_mesh_info(cs_mesh_t  *mesh)
{
  ...
  cs_lnum_t  *rank_buffer = nullptr;

  /* Summary of cell and ghost cell distribution */

  if (mesh->n_domains > 1) {

    CS_MALLOC(rank_buffer, mesh->n_domains, cs_lnum_t);
...
cs_mesh_create(void)
{
  cs_mesh_t  *mesh = nullptr;

  CS_MALLOC (mesh, 1, cs_mesh_t);
  ...
  mesh->vtx_coord = nullptr;
  mesh->i_face_cells = nullptr;
  mesh->b_face_cells = nullptr;
  mesh->i_face_vtx_idx = nullptr;
  mesh->b_face_vtx_idx = nullptr;
  mesh->i_face_vtx_lst = nullptr;
  mesh->b_face_vtx_lst = nullptr;

  /* Global numbering */

  mesh->global_cell_num = nullptr;
  mesh->global_i_face_num = nullptr;
  mesh->global_b_face_num = nullptr;
  mesh->global_vtx_num = nullptr;
  ...
  mesh->n_b_cells = 0;
  mesh->b_cells = nullptr;

  mesh->cell_cells_idx = nullptr;
  mesh->cell_cells_lst = nullptr;
  mesh->gcell_vtx_idx = nullptr;
  mesh->gcell_vtx_lst = nullptr;

  /* Numbering info */

  mesh->cell_numbering = nullptr;
  mesh->i_face_numbering = nullptr;
  mesh->b_face_numbering = nullptr;
  mesh->vtx_numbering = nullptr;

  /* Group and family features */

  mesh->n_groups = 0;
  mesh->group_idx = nullptr;
  mesh->group = nullptr;

  mesh->n_max_family_items = 0;
  mesh->n_families = 0;

  mesh->family_item = nullptr;
  mesh->cell_family = nullptr;
  mesh->i_face_family = nullptr;
  mesh->b_face_family = nullptr;
  ...
  mesh->class_defs = nullptr;

  mesh->select_cells = nullptr;
  mesh->select_i_faces = nullptr;
  mesh->select_b_faces = nullptr;
...
cs_mesh_reinit(cs_mesh_t  *mesh)
{
  ...
  CS_FREE(mesh->group_idx);
  CS_FREE(mesh->group);

  CS_FREE(mesh->family_item);
                         ...
                         bool        free_halos)
{
  ...
  if (mesh->gcell_vtx_idx != nullptr) {
    CS_FREE(mesh->gcell_vtx_idx);
    CS_FREE(mesh->gcell_vtx_lst);
  }
...
cs_mesh_discard_free_faces(cs_mesh_t  *mesh)
{
  ...
  if (j < i) {
    ...
    if (mesh->global_b_face_num != nullptr) {
      CS_REALLOC(mesh->global_b_face_num, j, cs_gnum_t);
    }
 ...
 *   g_i_face_vertices_size --> global interior face connectivity size, or nullptr
 *   g_b_face_vertices_size --> global boundary face connectivity size, or nullptr
 *----------------------------------------------------------------------------*/
                              ...
                              cs_gnum_t        *g_b_face_vertices_size)
{
  ...
  if (cs_glob_n_ranks > 1) {
    ...
    /* With periodicity, count faces bordering halo once for purely parallel
      ...
      int *perio_flag = nullptr;

      const cs_halo_t *halo = mesh->halo;
      const cs_lnum_t  n_transforms = halo->n_transforms;

      CS_MALLOC(perio_flag, mesh->n_ghost_cells, int);
  ...
  if (g_i_face_vertices_size != nullptr)
    *g_i_face_vertices_size = _g_face_vertices_size[0];
  if (g_b_face_vertices_size != nullptr)
...
cs_mesh_update_b_cells(cs_mesh_t  *mesh)
{
  ...
  CS_MALLOC(flag, mesh->n_cells, bool);
  ...
  CS_REALLOC(mesh->b_cells, mesh->n_b_cells, cs_lnum_t);
                  ...
                  bool                rebuild_vtx_interface)
{
  ...
  cs_lnum_t  *gcell_vtx_idx = nullptr, *gcell_vtx_lst = nullptr;
  cs_lnum_t  *n_periodic_couples = nullptr;
  cs_gnum_t  *g_i_face_num = nullptr;
  cs_gnum_t  *g_vertex_num = nullptr;
  cs_gnum_t  **periodic_couples = nullptr;

  cs_interface_set_t  *face_interfaces = nullptr;
  ...
  if (mesh->n_domains > 1 || mesh->n_init_perio > 0) {
    ...
    if (mesh->n_init_perio > 0) {
      CS_MALLOC(perio_num, mesh->n_init_perio, int);
      for (i = 0; i < mesh->n_init_perio; i++)
        perio_num[i] = i+1;
    }
    ...
    if (g_vertex_num == nullptr) {
      CS_MALLOC(g_vertex_num, n_vertices, cs_gnum_t);
    ...
    if (mesh->global_vtx_num != g_vertex_num)
      CS_FREE(g_vertex_num);

    CS_FREE(perio_num);
    CS_FREE(n_periodic_couples);

    for (i = 0; i < mesh->n_init_perio; i++)
      CS_FREE(periodic_couples[i]);
  ...
  if (halo_type == CS_HALO_EXTENDED) {
    ...
    CS_FREE(gcell_vtx_idx);
    CS_FREE(gcell_vtx_lst);
  }
...
cs_mesh_clean_families(cs_mesh_t  *mesh)
{
  ...
  CS_MALLOC(interlaced, size_tot, cs_gnum_t);
  ...
  order = cs_order_gnum_s(nullptr, interlaced, n_gc_vals, n_gc);

  /* Prepare removal of duplicates and renumbering */

  CS_MALLOC(renum, n_gc, int);
  ...
  CS_REALLOC(mesh->family_item, gc_count*n_gc_vals, int);
...
cs_mesh_create_group_classes(cs_mesh_t  *mesh)
{
  ...
  CS_MALLOC(group, mesh->n_max_family_items, char*);
                        ...
                        cs_gnum_t        ***perio_face_couples)
{
  ...
  if (n_perio_face_couples != nullptr) {
    int i;
    for (i = 0; i < mesh->n_init_perio; i++) {
                      ...
                      int               blank_perio)
{
  ...
  CS_MALLOC(cell_gnum, mesh->n_cells_with_ghosts, cs_gnum_t);
...
cs_mesh_dump(const cs_mesh_t  *mesh)
{
  ...
  if (mesh->halo != nullptr) {
    ...
    if (mesh->n_init_perio > 0 && halo->perio_lst != nullptr) {
