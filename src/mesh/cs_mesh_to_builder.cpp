                                 ...
                                 cs_part_to_block_t  *d)
{
  ...
  cs_lnum_t *face_vtx_idx = nullptr;
  cs_gnum_t *face_vtx_g = nullptr;
 ...
 *                 arrays.
 *   pp_out    <-> optional output file, or nullptr
 *----------------------------------------------------------------------------*/
                   ...
                   cs_io_t            *pp_out)
{
  ...
  int *face_gc_id = nullptr;
  cs_gnum_t *cell_gnum = nullptr, *face_gnum = nullptr;
  cs_gnum_t *face_cell_g = nullptr;
                   ...
                   cs_io_t            *pp_out)
{
  ...
  cs_lnum_t *order = nullptr, *i_order = nullptr, *b_order = nullptr;
  ...
  i_order = cs_order_gnum(nullptr, mesh->global_i_face_num, mesh->n_i_faces);
  if (mesh->n_b_faces > 0)
  ...
  else { /* if (mesh->global_vtx_num == nullptr) */
    l = 0;
    for (i = 0; i < n_i_faces; i++) {
      k = i_order[i];
      for (j = mesh->i_face_vtx_idx[k]; j < mesh->i_face_vtx_idx[k+1]; j++)
                         ...
                         cs_io_t    *pp_out)
{
  ...
  cs_gnum_t  *_perio_couples = nullptr;
  const cs_gnum_t  *couple_g_num = nullptr;
  cs_part_to_block_t *d = nullptr;
  ...
  fvm_io_num_t *c_io_num = fvm_io_num_create_from_adj_s(nullptr,
                                                        perio_couples,
                                                        n_perio_couples,
                                                        2);
 ...
 *                            temporary arrays.
 * \param[in, out]  pp_out    optional output file, or nullptr
 */
/*----------------------------------------------------------------------------*/
