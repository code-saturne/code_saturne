                          ...
                          cs_gnum_t        *new_glist[])
{
  ...
  cs_lnum_t  *_new_index = nullptr;
  cs_gnum_t   *_new_glist = nullptr;

  assert(index[0] == 0); /* case index[0] = 1 coulb be coded in the future */

  /* Build a new index */

  CS_MALLOC(_new_index, n_elts + 1, cs_lnum_t);
  ...
  CS_MALLOC(_new_glist, _new_index[n_elts], cs_gnum_t);
                   ...
                   cs_lnum_t             *send_faces[])
{
  ...
  int  *_send_rank = nullptr;
  cs_lnum_t  *_send_faces = nullptr, *reduce_ids = nullptr;
  cs_gnum_t  *reduce_index = nullptr;
  cs_join_gset_t  *new_face_rank = nullptr;
  ...
  CS_MALLOC(new_face_rank->g_list, new_face_rank->index[n_ranks], cs_gnum_t);
  ...
  CS_MALLOC(_send_rank, _n_send, int);
...
_create_face_builder(cs_lnum_t  n_faces)
{
  ...
  CS_MALLOC(builder, 1, face_builder_t);

  builder->n_faces = n_faces;

  CS_MALLOC(builder->face_index, n_faces + 1, cs_lnum_t);
  ...
  builder->subface_gnum = nullptr;
  builder->subface_gconnect = nullptr;
...
_destroy_face_builder(face_builder_t  *builder)
{
  ...
  CS_FREE(builder->face_index);

  cs_join_rset_destroy(&(builder->subface_index));
  cs_join_rset_destroy(&(builder->subface_connect));

  if (builder->subface_gnum != nullptr)
    CS_FREE(builder->subface_gnum);

  if (builder->subface_gconnect != nullptr)
    CS_FREE(builder->subface_gconnect);
...
#define _DBGTST 0
  ...
  /*
    if (f2f_connect == nullptr)  What we already do...
...
#if _DBGTST && defined(DEBUG) && !defined(NDEBUG)
  if (tst_dbg && cs_glob_join_log != nullptr && respect_plane)
           ...
           cs_lnum_t              *next_vertex)
{
  ...
  cs_lnum_t  *f2f_connect = nullptr;   /* To be implemented ... */
                  ...
                  const cs_join_mesh_t   *work)
{
  ...
  cs_gnum_t  *glob_list = nullptr, *tmp = nullptr, *vgnum = nullptr;
  fvm_io_num_t *subface_io_num = nullptr;
  ...
  CS_MALLOC(builder->subface_gnum, n_subfaces, cs_gnum_t);
  ...
  */
  ...
  CS_MALLOC(vgnum, work->n_vertices, cs_gnum_t);
  ...
  if (cs_glob_n_ranks > 1) { /* Parallel treatment */
    ...
    cs_lnum_t  *order_index = nullptr;
    cs_gnum_t  *order_glob_list = nullptr;
    cs_lnum_t  *order = cs_order_gnum_i(nullptr,
                                        glob_list,
                                        index,
                                        n_subfaces);
    ...
    subface_io_num = fvm_io_num_create_from_adj_i(nullptr,
                                                  order_index,
                                                  order_glob_list,
                                                  n_subfaces);

    assert(subface_io_num != nullptr);
                         ...
                         cs_join_gset_t        **p_o2n_hist)
{
  ...
  char  *new_mesh_name = nullptr;
  cs_lnum_t  *subfaces = nullptr;
  cs_lnum_t  *order = nullptr;
  cs_join_gset_t  *o2n_hist = nullptr;
  cs_join_mesh_t  *init_mesh = *mesh, *new_mesh = nullptr;
  ...
  CS_MALLOC(order, n_subfaces, cs_lnum_t);

  cs_order_gnum_allocated(nullptr, builder->subface_gnum, order, n_subfaces);
  ...
  CS_MALLOC(subfaces, n_new_faces, cs_lnum_t);
  ...
  CS_MALLOC(new_mesh->face_gnum, n_new_faces, cs_gnum_t);
  CS_MALLOC(new_mesh->face_vtx_idx, n_new_faces + 1, cs_lnum_t);
  ...
  CS_MALLOC(new_mesh->face_vtx_lst,
  ...
  CS_MALLOC(new_mesh->vertices, new_mesh->n_vertices, cs_join_vertex_t);
                    ...
                    cs_join_gset_t         **old2new_history)
{
  ...
  cs_lnum_t  *e2f_idx = nullptr, *e2f_lst = nullptr;
  ...
  */
  ...
  { /* Define a global number for each new sub-faces */
    ...
    CS_MALLOC(builder->subface_gconnect, sub_connect_size, cs_gnum_t);
                            ...
                            cs_join_mesh_t        **local_mesh)
{
  ...
  assert(_local_mesh != nullptr);
  assert(_o2n_hist != nullptr || (_o2n_hist == nullptr && work_mesh->n_faces == 0));
  ...
  if (n_ranks > 1) { /* Parallel mode */
    ...
    cs_gnum_t  *init_face_gnum = nullptr;
    cs_join_gset_t  *distrib_sync_hist = nullptr;
    ...
    if (param.perio_type != FVM_PERIODICITY_NULL) {
      ...
      CS_MALLOC(init_face_gnum, 2*n_init_faces, cs_gnum_t);
      ...
      CS_MALLOC(init_face_gnum, n_init_faces, cs_gnum_t);
