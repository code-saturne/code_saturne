...
static int
_cmp_gnum(const void *a, const void *b)
{
  const cs_gnum_t *A = (const cs_gnum_t *)a;
  const cs_gnum_t *B = (const cs_gnum_t *)b;
  if (*A < *B) return -1;
  if (*A > *B) return 1;
  return 0;
}
...
static bool
_distribute_face(cs_mesh_t *mesh, cs_lnum_t f_id, int rank, int cell_rank[])
{
  ...
  for (int j = 0; j < 2; j++) {
    ...
    if (c_id >= mesh->n_cells) {
      ...
      // do not distribute a proc face if remote proc id is lower than mine
...
static void
_exchange_and_order_int(cs_all_to_all_t *d,
                        int stride,
                        bool reverse,
                        const int send_data[],
                        int recv_data[],
                        const cs_lnum_t b_face_order[],
                        const cs_lnum_t b_face_map[])
{
  ...
  if (!b_face_order && !b_face_map) return;

  int *copy;
  CS_MALLOC(copy, n_elem_recv*stride, int);
  memcpy(copy, recv_data, n_elem_recv*stride*sizeof(int));

  for (cs_lnum_t new_slot = 0; new_slot < n_elem_recv; new_slot++) {
    cs_lnum_t old_slot = b_face_map ? b_face_map[new_slot] : new_slot;
    cs_lnum_t pre_slot = b_face_order ? b_face_order[old_slot] : old_slot;
    const int *orig = copy + stride*pre_slot;
    int *dest = recv_data + stride*new_slot;
...
static void
_exchange_and_order_real(cs_all_to_all_t *d,
                         int stride,
                         bool reverse,
                         const cs_real_t send_data[],
                         cs_real_t recv_data[],
                         const cs_lnum_t b_face_order[],
                         const cs_lnum_t b_face_map[])
{
  ...
  cs_real_t *copy;
  CS_MALLOC(copy, n_elem_recv*stride, cs_real_t);
  memcpy(copy, recv_data, n_elem_recv*stride*sizeof(cs_real_t));

  for (cs_lnum_t new_slot = 0; new_slot < n_elem_recv; new_slot++) {
    cs_lnum_t old_slot = b_face_map ? b_face_map[new_slot] : new_slot;
    cs_lnum_t pre_slot = b_face_order ? b_face_order[old_slot] : old_slot;
    const cs_real_t *orig = copy + stride*pre_slot;
    cs_real_t *dest = recv_data + stride*new_slot;
    for (int i = 0; i < stride; i++)
      dest[i] = orig[i];
  }
                    ...
                    const cs_lnum_t b_face_map[])
{
  ...
  _exchange_and_order_int(bfd, 1, false, copy, pm->izfppp,
                         b_face_order, b_face_map);

  if (pm->iautom) {
    memcpy(copy, pm->iautom, n_b_faces_ini*sizeof(int));
    CS_REALLOC(pm->iautom, mesh->n_b_faces, int);
    _exchange_and_order_int(bfd, 1, false, copy, pm->iautom,
                           b_face_order, b_face_map);
  }
  ...
  _exchange_and_order_int(bfd, 1, false, copy, *bc_type,
                         b_face_order, b_face_map);
...
void
_compute_cell_dest(cs_mesh_t *mesh, int cell_dest[])
{
...
void
cs_redistribute(int cell_dest[])
{
  static int iter = 0;
  iter++;

  if (cs_glob_rank_id <= 0)
    printf("Repartition iter: %d\n", iter);
  ...
  int cd_flags = 0;
  bool free_cell_dest = false;

  // build the cell part-to-part distributor

  cs_lnum_t n_cells = 0;

  // Random cell destinations if cell_dest is not prescribed.
  if (!cell_dest) {
    ...
    do {
      ...
      _compute_cell_dest(mesh, cell_dest);

      cd = cs_all_to_all_create(mesh->n_cells, cd_flags, nullptr, cell_dest, comm);
  ...
  for (cs_lnum_t f_id = 0; f_id < mesh->n_b_faces; f_id++)
    b_face_dest[f_id] = cell_dest[mesh->b_face_cells[f_id]];
  ...
  cs_halo_sync_untyped(mesh->halo, mesh->halo_type, sizeof(int), cell_dest);
  ...
  for (cs_lnum_t f_id = 0; f_id < mesh->n_i_faces; f_id++) {
    ...
    for (int j = 0; j < 2; j++) {
      dest_rank = cell_dest[mesh->i_face_cells[f_id][j]];
  ...
  // Note(Imad): to simplify implementation for now, we use the block vertices.
  ...
  cs_all_to_all_t *vd = cs_all_to_all_create_from_block(n_vertices,
  ...
  for (cs_lnum_t i = 0; i < n_b_faces; i++) {
    ...
    for (cs_lnum_t j = b_face_vtx_idx_r[b_face_order[i]];
         j < b_face_vtx_idx_r[b_face_order[i]+1]; j++) {
  ...
  // Note: attributes not updated by repartition (conf. cs_mesh.h):
  // - n_g_i_c_faces
  ...
  cs_lnum_t *cell_map, *i_face_map, *b_face_map, *vtx_map;
  cell_map = i_face_map = b_face_map = vtx_map = NULL;
  //cs_renumber_mesh(mesh, &cell_map, &i_face_map, &b_face_map, &vtx_map);
  ...
  // TODO(Imad): Does renumbering mess up selectors?
  ...
  for (int i = 0; i < n_fields; i++) {
    ...
    for (int j = 0; j < field->n_time_vals; j++) {
      ...
      for (cs_lnum_t new_slot = 0; new_slot < mesh->n_cells; new_slot++) {
        cs_lnum_t old_slot = cell_map ? cell_map[new_slot] : new_slot;
    ...
    if (limited_val_f) {
      ...
      _distribute_bc_coeff(bfd, n_b_faces_ini, field->dim, a_copy,
                           &fc->val_f_lim, b_face_order, b_face_map);
    }
    ...
    if (limited_flux) {
      ...
      _distribute_bc_coeff(bfd, n_b_faces_ini, field->dim, a_copy,
                          &fc->flux_lim, b_face_order, b_face_map);
    }

    _distribute_bc_coeff(bfd, n_b_faces_ini, field->dim, a_copy,
                         &fc->val_f_pre, b_face_order, b_face_map);
  ...
  for (int i = 0; i < n_fields; i++) {
    ...
    for (int j = 0; j < field->n_time_vals; j++) {
      ...
      for (cs_lnum_t new_slot = 0; new_slot < mesh->n_i_faces; new_slot++) {
        cs_lnum_t old_slot = i_face_map ? i_face_map[new_slot] : new_slot;
  ...
  if (free_cell_dest) CS_FREE(cell_dest);
  CS_FREE(cell_order);
  CS_FREE(cell_map);
  CS_FREE(i_face_map);
  CS_FREE(b_face_map);
