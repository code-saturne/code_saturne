...
cs_join_inter_edges_create(cs_lnum_t  n_edges)
{
  ...
  inter_edges->vtx_lst = nullptr;
  inter_edges->vtx_glst = nullptr;
  inter_edges->abs_lst = nullptr;
                             ...
                             cs_join_eset_t               *vtx_equiv)
{
  ...
  bool  *equiv_lst = nullptr;
  cs_lnum_t   *vtx_lst = nullptr, *tag_lst = nullptr;
  cs_coord_t  *abs_lst = nullptr;
  double  *tol_lst = nullptr;
  FILE  *logfile = cs_glob_join_log;

  assert(mesh != nullptr);
  assert(edges != nullptr);
  assert(inter_edges != nullptr);
  assert(vtx_equiv != nullptr);
  ...
  if (inter_edges != nullptr) {
    if (inter_edges->index[inter_edges->n_edges] > 0) {
      ...
      assert(inter_edges->vtx_lst != nullptr);
                                  ...
                                  const cs_join_inter_edges_t  *part)
{
  ...
  cs_lnum_t *orig_index = cs_all_to_all_copy_index(d,
                                                   false, /* reverse */
                                                   part->index,
                                                   nullptr);
                                ...
                                cs_join_inter_edges_t  **inter_edges)
{
  ...
  cs_lnum_t  *edge_order = nullptr, *vtx_order = nullptr;
  ...
  cs_order_gnum_allocated(nullptr, vtx_gnum, vtx_order, n_init_vertices);
                        ...
                        cs_join_inter_set_t   **inter_set)
{
  ...
  cs_join_inter_set_t  *_inter_set = nullptr;
  cs_join_eset_t  *_vtx_eset = nullptr;
  FILE  *logfile = cs_glob_join_log;
  ...
  assert(mesh != nullptr);
  assert(edges != nullptr);
  assert(edge_edge_vis != nullptr);

  assert(vtx_eset != nullptr);
  assert(inter_set != nullptr);
  ...
  if (cs_glob_n_ranks > 1) {
    ...
    join_type = static_cast<cs_join_type_t>(sync_tag);
                        ...
                        cs_join_stats_t        *stats)
{
  ...
  int  box_dim = 0;
  cs_coord_t  *f_extents = nullptr;
  fvm_neighborhood_t  *face_neighborhood = nullptr;
  cs_join_gset_t  *face_visibility = nullptr;
                               ...
                               const cs_join_gset_t   *face_visib)
{
  ...
  assert(mesh != nullptr);
  assert(edges != nullptr);
  assert(face_visib != nullptr);

  cs_lnum_t  size = 0, size_max = 0;
  cs_lnum_t  *count = nullptr, *face2edge_idx = nullptr, *face2edge_lst = nullptr;
  cs_gnum_t  *tmp = nullptr;
  cs_join_gset_t  *edge_visib = nullptr;
