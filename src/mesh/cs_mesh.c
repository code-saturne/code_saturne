                                   ...
                                   cs_lnum_t        *e_id)
{
  ...
  *s_id = 0;
  ...
  /* With OpenMP and compatible mesh numbering, compute block range */

  if (m->b_face_numbering->type != CS_NUMBERING_THREADS)
    return;
  ...
  /* Adjust to avoid different threads accessing the same value
     if needed. */

  if (*s_id > 1) {
    while (adj[*s_id - 1 ] == adj[*s_id]) {
      *s_id += 1;
      if (*s_id == n)
        break;
    }
