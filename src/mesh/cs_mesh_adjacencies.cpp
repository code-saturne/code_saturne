...
const cs_mesh_adjacencies_t  *cs_glob_mesh_adjacencies = nullptr;
...
_update_cell_faces(cs_mesh_adjacencies_t  *ma,
                   const cs_mesh_t        *m)
{
  if (ma->_c2f == nullptr && ma->c2f != nullptr)   /* not owner */
    return;
...
_update_cell_vertices(cs_mesh_adjacencies_t  *ma,
                      const cs_mesh_t        *m)
{
  if (ma->_c2v == nullptr && ma->c2v != nullptr)   /* not owner */
    return;
  ...
  for (int f_t = 0; f_t < 2; f_t++) {
    ...
    const cs_lnum_t *f2v_idx = nullptr, *f2c = nullptr;
    if (f_t == 0) {
  ...
  for (int f_t = 0; f_t < 2; f_t++) {
    ...
    const cs_lnum_t n_f = (f_t == 0) ? m->n_i_faces : m->n_b_faces;
    const cs_lnum_t stride = (f_t == 0) ? 2 : 1;
    const cs_lnum_t *f2v_idx= nullptr, *f2v = nullptr, *f2c = nullptr;
    if (f_t == 0) {
      f2v_idx = m->i_face_vtx_idx;
...
cs_mesh_adjacencies_initialize(void)
{
  ...
  ma->cell_cells_idx = nullptr;
  ma->cell_cells = nullptr;

  ma->cell_cells_e_idx = nullptr;
  ma->cell_cells_e = nullptr;

  ma->cell_i_faces = nullptr;
  ma->cell_i_faces_sgn = nullptr;

  ma->cell_b_faces_idx = nullptr;
  ma->cell_b_faces = nullptr;

  ma->cell_hb_faces_idx = nullptr;
  ma->cell_hb_faces = nullptr;
...
cs_mesh_adjacencies_finalize(void)
{
  ...
  cs_glob_mesh_adjacencies = nullptr;
}
...
cs_mesh_adjacencies_update_device(cs_alloc_mode_t  alloc_mode)
{
  ...
  if (ma->cell_i_faces_sgn != nullptr) {
 ...
 * \param[in]  sgn       array storing the orientation (may be nullptr)
 *
 * \return  a pointer to a new allocated cs_adjacency_t structure
 */
/*----------------------------------------------------------------------------*/
...
{
  assert(a2b_1 != nullptr && a2b_2 != nullptr);
 ...
 *          standard output
 *
 * \param[in]  name    name of the dump file. Can be set to nullptr
 * \param[in]  _f      pointer to a FILE structure. Can be set to nullptr.
 * \param[in]  adj     pointer to a cs_adjacency_t structure
 */
/*----------------------------------------------------------------------------*/
...
cs_mesh_adjacency_c2f(const cs_mesh_t  *m,
                      int               boundary_order)
{
  cs_lnum_t  *cell_shift = nullptr;
  cs_adjacency_t  *c2f = nullptr;
