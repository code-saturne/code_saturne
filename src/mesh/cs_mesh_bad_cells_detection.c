...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Private function to evaluate cell's non-orthogonality.
 *
 ...
 * \param [out]  bad_ortho_norm_cells  local array storring bad cells for postprocessing.
 * \param [out]  bad_cells             global array storring bad cells for various usage.
 */
/*----------------------------------------------------------------------------*/
                    ...
                    cs_gnum_t                    bad_cells[])
{
    ...
    for (face_id = 0; face_id < mesh->n_i_faces; face_id++)
    {
        ...
        /* Evaluates the non-orthogonality. */
        for (i = 0; i < dim; i++)
        ...
        if (i_face_ortho[face_id] < 0.1)
        {
            ...
            bad_cells[cell1] = _type_flag_mask[0];
        }
    }
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Private function to evaluate cell's center offsetting.
 *
 ...
 * \param [in]   mesh                    pointer to associated mesh structure.
 * \param [in]   mesh_quantities         pointer to associated mesh quantities structure.
 * \param [out]  bad_ortho_aframe_cells  local array storring bad cells for postprocessing.
 * \param [out]  bad_cells               global array storring bad cells for various usage.
 */
/*----------------------------------------------------------------------------*/
                              ...
                              cs_gnum_t                    bad_cells[])
{
    ...
    for (face_id = 0; face_id < mesh->n_i_faces; face_id++)
    {
        ...
        offsetting[face_id] = 1 - sqrt(_DOT_PRODUCT_3D(v1, v1) / _MODULE_3D(face_normal));

        if (offsetting[face_id] < 0.1)
        {
            bad_ortho_aframe_cells[cell1] = 1;
            bad_cells[cell1] = _type_flag_mask[1];
        }
    }
}
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Private function to evaluate cell's distorsion.
 *
 * Compute Least Squares Gradient coefficient for cells.
 * Evaluates a distorsion level (based on LSQ Gradient Method) and tags identified bad cells.
 *
 * \param [in]   mesh                pointer to associated mesh structure.
 * \param [in]   mesh_quantities     pointer to associated mesh quantities structure.
 * \param [out]  bad_lsq_grad_cells  local array storring bad cells for postprocessing.
 * \param [out]  bad_cells           global array storring bad cells for various usage.
 */
/*----------------------------------------------------------------------------*/
                       ...
                       cs_gnum_t                    bad_cells[])
{
    ...
    double *surfbo  = mesh_quantities->b_face_normal;

    cs_lnum_t  i, face_id, cell1, cell2, iel_;
    cs_real_t  cell_center1[3], cell_center2[3];
    cs_real_t  vect[3], dij[3];

    double unsdij, surfn, unssbn, min_diag, max_diag;

    double *w1 = NULL;
    BFT_MALLOC(w1, 6 * n_cells_wghosts, double);
    ...
    for (face_id = 0; face_id < mesh->n_b_faces; face_id++)
    {
        ...
        for (i = 0; i < dim; i++)
            dij[i] = surfbo[face_id * 3 + i] * unssbn;
    ...
    /* Approximative method using the Frobenius norm to estimate the min/max eigenvalues ratio */
    for (iel_ = 0; iel_ < n_cells; iel_++)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            min_diag = fmin(min_diag, fabs(w1[iel_ + i * n_cells_wghosts]));
            max_diag = fmax(max_diag, fabs(w1[iel_ + i * n_cells_wghosts]));
        }

        lsq[iel_] = min_diag / max_diag;

        if (lsq[iel_] < 0.1)
        {
            bad_lsq_grad_cells[iel_] = 1;
            bad_cells[iel_] = _type_flag_mask[2];
        }
    }
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Private function to evaluate cell's volume ratio.
 *
 * Compute Volume Ratio coefficient for cells.
 * Evaluates a the cell's geometric continuity (based on volume ratio) and tags identified bad cells.
 *
 * \param [in]   mesh                 pointer to associated mesh structure.
 * \param [in]   mesh_quantities      pointer to associated mesh quantities structure.
 * \param [out]  bad_vol_ratio_cells  local array storring bad cells for postprocessing.
 * \param [out]  bad_cells            global array storring bad cells for various usage.
 */
/*----------------------------------------------------------------------------*/
                      ...
                      cs_gnum_t                    bad_cells[])
{
    ...
    for (face_id = 0; face_id < mesh->n_i_faces; face_id++)
    {
        ...
        vol_ratio[face_id] = fmin(volume[cell1] / volume[cell2], volume[cell2] / volume[cell1 ]);

        if (vol_ratio[face_id] < 0.1*0.1)
        {
            bad_vol_ratio_cells[cell1] = 1;
            bad_vol_ratio_cells[cell2] = 1;
            bad_cells[cell1] = _type_flag_mask[3];
            bad_cells[cell2] = _type_flag_mask[3];
        }
    }
}
                            ...
                            const cs_mesh_quantities_t     *mesh_quantities)
{
    ...
    /* Eventual warning printed in the listing --> flag initialization */
    iwarning = 0;
    n_cells_tot = n_cells;
    if (cs_glob_rank_id >= 0)
        cs_parall_counter(&n_cells_tot, 1);

    bft_printf(_("\n  Number of Cells: %llu\n"), (unsigned long long)n_cells_tot);
    ...
    ibad = 0;
    for (i = 0; i < n_cells; i++)
        if (bad_cells[i] & _type_flag_mask[0])
        {
            ibad++;
            iwarning++;
        }
  ...
  /*----------------------------------------------*/
    ...
    ibad = 0;
    for (i = 0; i < n_cells; i++)
        if (bad_cells[i] & _type_flag_mask[1])
        {
            ibad++;
            iwarning++;
        }
    ...
    /*--------------------------------------*/
    /* Condition 3: Least Square Gradient   */
    /*--------------------------------------*/
    ...
    BFT_MALLOC(working_array, n_cells_wghosts, double);
    ...
    ibad = 0;
    for (i = 0; i < n_cells; i++)
        if (bad_cells[i] & _type_flag_mask[2])
        {
            ibad++;
            iwarning++;
        }
    ...
    /* Display listing output */
    bft_printf(_("\n  Criteria 3: LSQ Gradient Quality:\n"));
    bft_printf(_("    Number of bad cells detected: %llu --> %3.0f %%\n"),
                 (unsigned long long)ibad, (double)ibad / (double)n_cells_tot * 100.0);

    /* Post processing */
    cs_post_write_var(-1, "LSQ_Gradient_Tag", 1, false, true, CS_POST_TYPE_cs_int_t,
                -1, 0.0, bad_lsq_grad_cells, NULL, NULL);
    ...
    double  *vol_ratio = NULL;

    BFT_MALLOC(bad_vol_ratio_cells, n_cells_wghosts, cs_lnum_t);
    BFT_MALLOC(working_array, n_i_faces, double);
    ...
    ibad = 0;
    for (i = 0; i < n_cells; i++)
        if (bad_cells[i] & _type_flag_mask[3])
        {
            ibad++;
            iwarning++;
        }
    ...
    /* Post processing */
    cs_post_write_var(-1, "Volume_Ratio_Tag", 1, false, true, CS_POST_TYPE_cs_int_t,
                    -1, 0.0, bad_vol_ratio_cells, NULL, NULL);
    ...
    for (face_id = 0; face_id < n_i_faces; face_id++)
    {
        ...
        if (bad_cells[cell2] != 0)
            bad_guilt_cells[cell1]++;
    }

    ibad = 0;
    for (i = 0; i < n_cells; i++)
        if (bad_guilt_cells[i] >= 5 && bad_cells[i] == 0)
        {
            ibad++;
            iwarning++;
            bad_guilt_cells[i] = 1;
            bad_cells[i] = _type_flag_mask[4];
        }
        else
            bad_guilt_cells[i] = 0;
    ...
    /* Display listing output */
    bft_printf(_("\n  Criteria 5: Guilt by Association Cells:\n"));
    bft_printf(_("    Number of bad cells detected: %llu --> %3.0f %%\n"),
                 (unsigned long long)ibad, (double)ibad / (double)n_cells_tot * 100.0);

    /* Post processing */
    cs_post_write_var(-1, "Guilt_Cells_Tag", 1, false, true, CS_POST_TYPE_cs_int_t,
                    -1, 0.0, bad_guilt_cells, NULL, NULL);

    /*-----------------------------------------------------*/
    /* Eventual warning printed in the listing             */
    /*-----------------------------------------------------*/

    if (iwarning > 0)
    {
        bft_printf(_("\n*****   WARNING: MESH QUALITY ISSUE HAS BEEN DETECTED   *****\n"));
        bft_printf(_("Mesh should be re-considered by taking into account above criteria.\n"));
        bft_printf(_("The calculation will run but results may seriously be altered...\n"));
    }
