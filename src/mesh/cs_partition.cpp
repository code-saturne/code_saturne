...
static int                       *_part_extra_partitions_list = nullptr;
                          ...
                          MPI_Comm                  comm)
{
  ...
  cs_real_t *_vtx_coord = cs_all_to_all_copy_array(dv,
                                                   3,
                                                   true, /* reverse */
                                                   mb->vertex_coords);
               ...
               MPI_Comm    comm)
{
...

  {
    ...
    int retval = ParMETIS_V3_PartKway(vtxdist,
                                      cell_idx,
                                      cell_neighbors,
                                      nullptr, /* vwgt:   cell weights */
                                      nullptr, /* adjwgt: face weights */
    ...
    if (retval != METIS_OK)
      bft_error(__FILE__, __LINE__, 0, _("Error in ParMETIS partitioning.\n"));
  }
               ...
               MPI_Comm     comm)
{
  ...
  if (retval == 0) {
    ...
    if (dgraph_save_name != nullptr) {

      FILE *graph_save = nullptr;
               ...
               cs_gnum_t                **g_face_cells)
{
  ...
  cs_gnum_t *_g_face_cells = nullptr;
  ...
  *g_face_cells = mb->face_cells;
  ...
  if (ignore_perio == false && mb->n_perio > 0) {
    ...
    cs_gnum_t *_per_face_couples = nullptr;
    const cs_gnum_t *per_face_couples = nullptr;
  ...
  if (cs_glob_n_ranks > 1) {
    cs_gnum_t *g_face_cells_tmp = nullptr;
                   ...
                   int                      **cell_rank)
{
  ...
  if (cs_glob_n_ranks > 1 && (mb->cell_bi.rank_step != rank_step)) {
    ...
    cs_gnum_t *global_cell_num = nullptr;

    int *_cell_rank = nullptr;
