 ...
 *
 * \return  pointer to interface set for selected faces, or nullptr
 */
/*----------------------------------------------------------------------------*/
                                      ...
                                      cs_interface_set_t  **ifs)
{
  ...
  cs_lnum_t  *_ifs_face_id = nullptr;

  cs_interface_set_t  *_face_ifs = nullptr;
                                 ...
                                 const cs_lnum_t  v2c[])
{
  ...
  cs_lnum_t  *ifs_face_id = nullptr;
  cs_interface_set_t  *face_ifs = nullptr;
  cs_lnum_t *send_idx = nullptr, *recv_idx = nullptr;
  cs_gnum_t *v2c_send = nullptr, *v2c_recv = nullptr;
                ...
                cs_lnum_t   b_face_end)
{
  ...
  if (cs_glob_n_ranks > 1)
    ifs = cs_interface_set_create(n_vertices, nullptr, mesh->global_vtx_num,
  ...
  cs_lnum_t  *v2c_idx = nullptr, *v2c = nullptr;
  ...
  cs_lnum_t  *v_min_c_add = nullptr;
  cs_gnum_t  *v_g_min_c_add = nullptr;
  ...
  if (cs_glob_n_ranks > 1 || mesh->global_vtx_num != nullptr) {

    fvm_io_num_t *v_add_io_num = fvm_io_num_create_from_adj_s(nullptr,
                                                              v_g_min_c_add,
                                                              n_add_vertices,
                                                              2);
...

  {
    ...
    for (cs_lnum_t i = 0; i < n_vertices; i++) {
      if (v2c_idx[i+1] > v2c_idx[i]) {
        cs_gnum_t g_vtx_num = (mesh->global_vtx_num != nullptr) ?
 ...
 * \param[in]  mesh      pointer to mesh structure to modify
 *
 * \return  pointer to interface set for faces, or nullptr
 */
/*----------------------------------------------------------------------------*/
 ...
 * \param[in]       perio_num       periodicity number for each face, or nullptr
                 ...
                 cs_lnum_t            face_id[])
{
  ...
  cs_gnum_t *b_face_gnum = nullptr;

  cs_mesh_builder_t *mb = cs_glob_mesh_builder;

  cs_lnum_t *face_id_c = nullptr;
  ...
  if (cs_glob_n_ranks > 1) {
    cs_gnum_t counter[2] = { (cs_gnum_t)mesh->n_b_faces, 0 };
    if (perio_num != nullptr)
      counter[1] = 1;
                        ...
                        cs_lnum_t   face_id[])
{
  ...
  _boundary_insert(mesh, nullptr, perio_num, true, n_faces, face_id);
                                             ...
                                             cs_lnum_t   face_id[])
{
  ...
  _boundary_insert(mesh, nullptr, perio_num, false, n_faces, face_id);
...
cs_mesh_boundary_remove_periodicity(cs_mesh_t  *mesh)
{
  ...
  if (mesh->halo != nullptr || mesh->halo_type == CS_HALO_EXTENDED) {
    cs_lnum_t n_cells = mesh->n_cells;
    ...
    cs_interface_set_destroy(&(mesh->vtx_interfaces));
    cs_mesh_init_halo(mesh, nullptr, mesh->halo_type, mesh->verbosity, true);
  }
