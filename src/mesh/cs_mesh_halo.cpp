                   ...
                   const cs_interface_set_t  *ifs)
{
  ...
  vtx_lookup_table_t  *vtx_lookup = nullptr;

  const cs_interface_t  *interface = nullptr;
  const cs_lnum_t  *vtx_ids = nullptr;
  const fvm_periodicity_t  *periodicity = cs_interface_set_periodicity(ifs);
  const cs_lnum_t n_transforms = fvm_periodicity_get_n_transforms(periodicity);
  const cs_lnum_t n_interfaces = cs_interface_set_size(ifs);
  ...
  if (loc_rank_id > 0) {
    ...
    if (n_interfaces > 2) {
      ...
      cs_lnum_t  *buffer = nullptr;
      cs_lnum_t *_rank_ids = nullptr;
                ...
                cs_lnum_t                 *gcell_faces_lst)
{
  ...
  cs_halo_t  *halo = mesh->halo;
  vtx_lookup_table_t  *vtx_lookup = nullptr;
  cs_halo_type_t  *cell_tag = nullptr;
  cs_halo_type_t  *face_checker = nullptr;
  cs_lnum_t *vtx_checker = nullptr;
  cs_lnum_t *counter = nullptr;
  ...
  if (mesh->i_face_vtx_lst == nullptr)
    return;
...
_fill_halo(cs_mesh_t  *mesh)
{
  ...
  if (mesh->n_init_perio > 0) {
    ...
    cs_lnum_t n_elts;
    cs_lnum_t *exchange_buffer = nullptr;
...
_get_list_buffer_size(const cs_interface_set_t  *ifs)
{
  ...
  const cs_interface_t  *interface = nullptr;
  const cs_lnum_t  *tr_index = nullptr;
                          ...
                          cs_lnum_t                 *vtx_interface_idx)
{
  ...
  for (id = 0; id < cs_interface_set_size(ifs); id++) {
    ...
    if (rank_id == cs_interface_rank(interface)) {
      ...
      if (tr_index == nullptr) {  /*  purely parallel elements */
                               ...
                               cs_lnum_t           *p_gcell_dist_vtx_lst[])
{
  ...
  cs_lnum_t *gcell_dist_vtx_idx = nullptr, *gcell_dist_vtx_lst = nullptr;
  cs_lnum_t *vtx_interface_idx = nullptr;
  cs_lnum_t *dist_id_lst = nullptr;
                            ...
                            cs_lnum_t  *p_gcell_dist_vtx_lst[])
{
  ...
  cs_lnum_t *send_idx_buffer = nullptr;
  cs_lnum_t *gcell_dist_vtx_idx = nullptr, *gcell_dist_vtx_lst = nullptr;
  cs_lnum_t *send_buffer = nullptr, *recv_buffer = nullptr;
                     ...
                     cs_lnum_t           *cell_faces_lst)
{
  ...
  cs_halo_t  *halo = mesh->halo;
  cs_lnum_t *gcell_face_count = nullptr, *l2d_fids = nullptr;
  cs_lnum_t *send_buffer = nullptr, *recv_buffer = nullptr, *buffer = nullptr;
  cs_lnum_t *send_shift = nullptr, *recv_shift = nullptr, *halo2ifs_rank = nullptr;
  ...
  const cs_interface_t  *interface = nullptr;
  const cs_lnum_t  *loc_ids = nullptr;
  const cs_lnum_t  *dist_ids = nullptr;
  const cs_lnum_t  *tr_index = nullptr;
...
_clean_halo(cs_mesh_t  *mesh)
{
  ...
  cs_lnum_t counter = 0;
  cs_lnum_t *new_c_domain_rank = nullptr;
  cs_lnum_t *new_perio_lst = nullptr;
                            ...
                            cs_lnum_t                 *p_cell_faces_lst[])
{
  ...
  cs_lnum_t *vtx_tag = nullptr;
  cs_lnum_t *cell_buffer = nullptr, *cell_tag = nullptr, *counter = nullptr;
...
{
  cs_lnum_t *send_gcell_vtx_idx = nullptr, *send_gcell_vtx_lst = nullptr;
  cs_lnum_t *gcell_vtx_idx = nullptr, *gcell_vtx_lst = nullptr;
  cs_lnum_t *gcell_faces_idx = nullptr, *gcell_faces_lst = nullptr;
  cs_halo_t  *halo = mesh->halo;
