...
cs_ctwr_add_variable_fields(void)
{
...

  {
    ...
    int f_id = cs_variable_field_create("y_l_r",
                                        "Yl rain",
                                        CS_MESH_LOCATION_CELLS,
                                        1);
    f = cs_field_by_id(f_id);

    /* Clipping of rain mass 0 <= y_l_r */
    ...
    f_id = cs_variable_field_create("yh_l_r",
                                    "Yl.hl rain",
                                    CS_MESH_LOCATION_CELLS,
                                    1);
    f = cs_field_by_id(f_id);
...
cs_ctwr_add_property_fields(void)
{
  ...
  }
  {
    ...
    cs_field_set_key_int(f, keyvis, post_flag);
    cs_field_set_key_int(f, keylog, 1);
    cs_field_set_key_str(f, klbl, "ym liq packing");
  }
  ...
  }
  {
    ...
    cs_field_set_key_int(f, keyvis, post_flag);
    cs_field_set_key_int(f, keylog, 1);
    cs_field_set_key_str(f, klbl, "ym liq rain");
  }
  ...
  if (ct_opt->solve_rain_velocity) {
    ...
    sprintf(f_label, "Terminal velocity_p %02d", class_id);
    ...
    sprintf(f_label, "Drift velocity p %02d", class_id);
    f = cs_field_create(f_name,
                        field_type,
                        CS_MESH_LOCATION_CELLS,
                        3,
