...
try:
    ...
    if config.has_option('install', 'syrthes'):
        syr4path = os.path.join(config.get('install', 'syrthes'), 'bin')
        sys.path.insert(0, syr4path)
    import syrthes
except Exception:
    pass
...
class syrthes4_domain(base_domain):
    ...
    def __init__(self,
        ...
        # Additional parameters for Code_Saturne/Syrthes4 coupling
        # Names, directories, and files in case structure
        ...
        self.app_id = self.syr4case.app_id
    ...
    def set_case_dir(self, case_dir):
        ...
        self.syrthes_case.set_data_dir(self.data_dir)
        self.syrthes_case.set_src_dir(self.src_dir)
    ...
    def set_exec_dir(self, exec_dir):
        ...
        if self.tag != None:
            ...
            self.exec_dir = os.path.join(self.exec_dir, 'SYR')

        self.syr4_case.set_exec_dir(self.exec_dir)
    ...
    def set_result_dir(self, name, by_suffix=True):
        ...
        if by_suffix == True:
            self.result_dir = os.path.join(self.case_dir, 'RESU_SYR4')
            self.result_suffix = name
    ...
    def init_syrthes_case(self):
        ...
        args = '-d ' + self.param
        args += ' -l ' + self.log_file

        if self.n_procs != None and self.n_procs != 1:
            args += ' -n ' + str(self.syr4case.n_procs)

        if self.n_procs_radiation > 0:
            args += ' -r ' + str(self.n_procs_radiation)

        self.syrthes_case = syrthes.process_cmd_line(args)
    ...
    def compile_and_link(self):
        """
        Compile and link user subroutines for Syrthes4 if necessary
        """
        ...
        dir_files = os.listdir(self.src_dir)
        src_files = (fnmatch.filter(dir_files, '*.c')
                     + fnmatch.filter(dir_files, 'Makefile'))
        copy_dir = None
        exec_src = None
        ...
        retval = syrthes.build_syrthes(parall = True,
                                       cfd_coupling = True,
                                       fname = log_name,
                                       srcdir = exec_src,
        ...
        if retval != 0:
            raise RunCaseError('Syrthes4: Compile or link error.')
    ...
    def solver_args(self, **kw):
        ...
        args += ' -d ' + self.syr4case.data_file
        args += ' -n ' + str(self.syr4case.n_procs)

        if self.syr4case.n_procs_ray > 0:
        ...
        if self.coupling_mode == 'MPI':
            ...
            # Coupled app_names
            # (if not given, a default initialization has been done)
            args += ' --coupling'
            for c_app_id in self.coupled_app_ids:
                args += ' ' + str(c_app_id)

        # Output to a logfile
        args += ' > ' + self.logfile + ' 2>&1\n'
    ...
    def copy_data(self):
        ...
        self.syr4_case.read_data_file()

        # Sumary of the parameters (DEBUG)
        self.syr4_case.dump()

        # Preparation of the execution directory
        retval = self.syr4_case.prepare_data()
        if retval != 0:
    ...
    def preprocess(self):
        ...
        Partion mesh for parallel run if required by user
        """

        # Initialize output file if needed
        self.syr4_case.listing_init()

        # Handle interpreted functions
        self.syr4_case.interpret_func()

        # Pre-processing (only if Syrthes computation is done in parallel)
        retval = self.syr4_case.preprocessing()
        if retval != 0:
