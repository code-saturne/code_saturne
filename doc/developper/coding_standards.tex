...
\item one routine per file (except if all routines except the first
      ...
      \texttt{usini1.f90} user file which contains several subroutines
      (it initially followed the rule, but subroutines were split,
...
\item at least 2 characters per variable names 
      (banish variables \texttt{i}, \texttt{j}, or \texttt{k},
       preferring \texttt{ii}, \texttt{jj}, or \texttt{kk}).
...
\item use \texttt{do}~/~\texttt{enddo} constructs instead of
      for where \texttt{do}~/~\texttt{continue}.

\item avoid \texttt{goto} constructs for where \texttt{select}~/~\texttt{case}
      would be more appropriate.

\item avoid using ``pointer'' of ``pointer'' of ``pointer'' indices
      in loops. Compute the pointer integer first (for example,
      \texttt{ivar = ipr(iphas)}. This makes things more readable and
      may help the compiler optimize the loop, leading to faster
      code.
...
\item avoid \texttt{a = 3.d0*b}, preferring \texttt{three = 3.d0} followed
      by \texttt{a = three * b}.  
