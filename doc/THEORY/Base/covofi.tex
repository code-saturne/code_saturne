...
\\
$\bullet$ Si les temres sources ne sont pas extrapolés, toutes les contributions
...
\begin{itemize}
\item mise à zéro des vecteurs représentants le second membre (\var{SMBRS}) et
...
(stockés dans \var{PROPCE}). La contribution des termes sources utisateurs (au
...
le cas par exemple où $T_s^{imp}$ change de signe\footnote{cf. \fort{preduv}}.
\\\\
...
de la m\^eme manière que précédemment avec \var{THETAP} = 1.
\\
\item une fois la contribution de tous les termes sources calculés, le second
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Dans cette section, on va \'etudier plus particulièrement l'inversibilit\'e de
la matrice $\tens{EM}_{\,n}$, matrice du syst\`eme lin\'eaire
\`a r\'esoudre associ\'ee \`a $\mathcal{EM}_{n}$ pour le cas d'un schéma en temps
de type Euler implicite d'ordre un ($\theta=1$). Pour toutes les notations, on
se reportera à la documentation sur le sous-programme \fort{covofi}.
...
Cette propri\'et\'e\footnote{Lascaux, P. et Théodor,
...
dissipation $\rho\frac{1}{R_f}\,\frac{\varepsilon}{k}$, sp\'ecifique \`a l'étude de la
...
Apr\`es intégration spatiale, la somme de ces deux termes discrets s'\'ecrit~:\\
...
considérations expos\'ees dans le pr\'esent document.\\
\end{itemize}
...
s'\'ecrit, d'après les équations (\ref{Base_Covofi_Eq_scal_final}) et la discrétisation
