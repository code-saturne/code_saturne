...
so compiler wrappers are not necessary, but they can lead to
...
its configuration and that of third-party sofware must be taken.

in addition to local connectivity arrays, \CS uses gobal element ids
for some operations, such as reading and writing meshes and restart files,
parallel interface element matching, and postprocessing output.
...
as \CS does not yet exloit MPI-IO for these libraries.
