...
\cite{boucker00}, and to the theoretical documentation \cite{gest02},
...
It is usually the root of a specific account \texttt{/home/saturne} (as it
is the case on the servers of the MFEE department or on the Chatou cluster) or a
subdirectory \texttt{/home/saturne/Saturne} (as it is the case on the CCRT
machines). For installs outside EDF R\&D, please refer to the administrator who
...
``\texttt{.profile}''\footnote{or \texttt{.monprofile} if the
 ...
 administators, as is the case in the MFEE departement}:

\hspace*{2cm}\texttt{. }\tildeunix\texttt{saturne/Noyau/ncs-\verscs/bin/cs\_profile} for the MFEE
server and Chatou cluster.

\hspace*{2cm}\texttt{. }\tildeunix\texttt{saturne/Saturne/Noyau/ncs-\verscs/bin/cs\_profile}
for the CCRT machines.

\hspace*{2cm}\texttt{. }\texttt{xxxxxxx/Noyau/ncs-\verscs/bin/cs\_profile} for
the other cases, where \texttt{xxxxxxx} represents the PATHCS directory where
...
\begin{list}{$\bullet$}{}
\item A directory \texttt{MAILLAGE} containing the mesh(es)
...
\begin{list}{$\bullet$}{}
\item A directory \texttt{FORT} for the potential user subroutines
...
a directory \texttt{SUITE.mmddhhmm} for the calculation
      restart files,
a directory  \texttt{HIST.mmddhhmm} for the files of chronological
      record of the results at specific locations (probes),
\texttt{listenv.mmddhhmm} and \texttt{listing.mmddhhmm} files reporting the
...
a directory \texttt{FORT.mmddhhmm} in the directory \texttt{RESU}. The {\em Xml}
...
\texttt{lance.mmddhhmm}). \texttt{compil.log.mmddhhmm} and
\texttt{resume.mmddhhmm} are respectively reports of the compilation phase and
...
names have to be given in the variable \texttt{FICHIERS\_RESULTATS\_UTILISATEUR}
of the launch script, see \S\ref{prg_lance}}.
...
\begin{list}{$\bullet$}{}
\item a directory \texttt{FORT\_SYR} for the potential \syrthes\ user subroutines  
...
for the \CS\ review) under the name \texttt{listsyr.mmddhhmm}. The compilation
report file is the same for \syrthes\ and \CS. It is placed in the \texttt{RESU}
directory under the name  \texttt{compil.log.mmddhhmm}.
For an easier follow-up of the modifications
in former calculations, the potential \syrthes\ user-subroutines used in a
calculation are stored in a directory \texttt{FORT\_SYR.mmddhhmm} in the
...
\multicolumn{3}{r}{(\CS\ calculation coupled with \syrthes):}\\
...
&                                   & physics modules for gas combustion,
                                      pulverised coal \\
                                \multicolumn{3}{r}{or electric arcs)}\\
...
\multicolumn{2}{l}{\texttt{STUDY/CASE1/FORT:}}&{\bf \CS\ user subroutines }\\
&	\texttt{USERS}   		&	examples of a user subroutines\\
...
&                                        &the contents of the directory are user
                                          modifiable)\\
&	\texttt{FORT.08211921}	      &\CS\ user subroutines  
                                        used for the\\
&                                      &calculation 08211921\\
&	\texttt{FORT\_SYR.08211921}   &\syrthes\ user subroutines
...
&	\texttt{lance}                &launch script (compliant with all
...
information. Indeed, the case preparer \texttt{cree\_sat} automatically extracts the
necessary files and prepares the launch script without the user having to go
directly into the \CS\ base directories (see \S\ref{prg_creesat}).
The \texttt{info\_cs} gives direct
access to the most needed information (especially the user and progammer's
guides) without the user having to look for them in the \CS\ directories.


The subdirectory \texttt{lib} contains the libraries. 
...
\item\texttt{Linux\_CCRT} for the Tantale cluster at CCRT
\item\texttt{Linux\_Ch} for the Chatou cluster
\item\texttt{OSF1} for the Nickel/Chrome clusters at CCRT
\item\texttt{SunOS} for Sun machines
\end{list}


For each architecture, a subdirectory (named after \texttt{NOM\_ARCH}) 
stores the compilated libraries
\texttt{libsaturne*.a}. The launch script automatically loads them for
...
\noindent 
When a specific module is not available in the distribution of \CS,
the corresponding file \texttt{libsaturne*.a} is replaced by an empty library
which is marked out by \texttt{STUB}
(\texttt{libsaturneCPLV\_STUB.a} for example) to allow the complilation. 
...
\item PUR to use the ``Purify'' utility.
\end{list}
...
under subdirectories corresponding to each module: \texttt{base\_1} (general
routines of moderate confidentiality), \texttt{base\_2} (general
routines of high confidentiality), \texttt{cfbl} (compressible flows),
...
under similar subdirectories corresponding to each module: \texttt{base} (general
routines), \texttt{cfbl}, \texttt{cogz}, \texttt{cplv}, \texttt{elec},
\texttt{pprt} and \texttt{rayt}.
The case preparer \texttt{cree\_sat} copies all these files in the
user directory \texttt{FORT/USERS} during the case preparation.
...
\texttt{mati}, \texttt{pprt} and \texttt{rayt}.
...
(\texttt{info\_cs version}). If it does not correspond to the desired version,
...
\item Prepare the different directories using \texttt{cree\_sat}(see
\S\ref{prg_creesat}) and, when needed, add the directories \texttt{DATA\_SYR}
and \texttt{FORT\_SYR} which are required to accomodate the \syrthes\ files.

\item Place the mesh(es) in the directory \texttt{MAILLAGE}. Make sure they are
...
(Lagrangian mode not compliant with the Graphical User Interface in version \verscs)
    \begin{list}{}{}
        \item {\em compulsory without Graphical User Interface:}
        \begin{list}{-}{}
            \item \texttt{usini1} to specify the calculation parameters
...
\item Prepare the launch script \texttt{lance}, directly or through the
      Graphical Interface (see \S\ref{prg_lance})

\item Run the calculation and analyse the results

\item Purge the temporary files (in the directory \texttt{RUN} defined
      in the launch script, see \S\ref{prg_lance})
\end{list}
...
\texttt{RUN=\$HOME/tmp\_Saturne/\$ETUDE/\$CAS.mmddhhmm} for stand-alone
workstations or for the Chatou cluster\\
\texttt{RUN=\$TMPDIR} on Nickel/Chrome at the CCRT (\texttt{\$TMPDIR} is a
directory created by the batch manager for the job and deleted after its
completion)\\
\texttt{RUN=\$SCRATCHDIR/tmp\_Saturne/\$ETUDE/\$CAS.mmddhhmm} for Tantale at the
CCRT\\
where \texttt{\$ETUDE} and \texttt{\$CAS} are the names of the study and the
...
disks on a machine when the \texttt{\$HOME} account is on an NFS disk. Or, on
Nickel/Chrome, it might be useful to use an alternate \texttt{RUN} directory
that will not be deleted at the end (for debugging purposes for instance).

For this matter, the variable \texttt{CS\_TMP\_PREFIX} of the launch script (see
\S\ref{prg_lance}) allows  the user to change this directory.
...
\texttt{\$CS\_TMP\_PREFIX/tmp\_Saturne/\$ETUDE/\$CAS.mmddhhmm}.
...
Kernel. The Preprocessor is in charge of the pre-processing. It reads the meshes and
...
Kernel will be running on. For a monoprocessor calculation, the file is named
\texttt{enveloppe\_vers\_solveur}. For a parallel calculation on $N$ processors,
the files are named \texttt{enveloppe\_vers\_solveur\_n0001} to
\texttt{enveloppe\_vers\_solveur\_n$N$}. In a standard calculation, these files
are created in the temporary execution directory and left there after
calculation. They have no interest for data analysis and are too dependent on
...
be specified in the variable \texttt{MODE\_EXEC} (see \S\ref{prg_lance}):\\
\hspace*{0.5cm}\texttt{complet}: complete mode. The Preprocessor module is executed
for pre-processing, followed by the Kernel for the calculation. The 
\texttt{enveloppe\_vers\_solveur*} files are created and left in the
temporary execution directory.\\
\hspace*{0.5cm}\texttt{pre-traitement}: only the Preprocessor module is
executed. The \texttt{enveloppe\_vers\_solveur*} files are stored in a directory
\texttt{PRE\_TRAITEMENT.mmddhhmm} automatically created in the \texttt{RESU}
directory, for later use.\\
\hspace*{0.5cm}\texttt{calcul}: only the Kernel is executed. The
\texttt{enveloppe\_vers\_solveur*} files are read from the directory specified
in the variable \texttt{PRE\_TRAITEMENT\_AMONT} of the launch script
...
%==================================
\label{prg_creesat}%
The case preparer \texttt{cree\_sat} automatically creates a
study directory according to the typical architecture and copies and
pre-fills an example of calculation launch script.

The syntax of \texttt{cree\_sat} is as follows:

\noindent
\texttt{cree\_sat -etude STUDY CASE\_NAME1 CASE\_NAME2...}\\
creates a study directory \texttt{STUDY} with case subdirectories
\texttt{CASE\_NAME1} and \texttt{CASE\_NAME2}... 
If no case name is given, a default case directory called \texttt{CAS1} is
created.

\noindent
\texttt{cree\_sat -cas DEBIT3 DEBIT4}\\
executed in the directory \texttt{STUDY} adds the case directories
\texttt{DEBIT3} and \texttt{DEBIT4}.  

An option \texttt{-noihm} is available for the use of \CS\ 
without Graphic Interface (see \S\ref{prg_ihm}). This option must
be either the first or the last argument and appear only once.

In the directory \texttt{DATA}, \texttt{cree\_sat} places a
...
in the launch script in the variable DONNEES\_THERMOCHIMIE. All other files in
the \texttt{DATA} or in the \texttt{THCH} will be ignored.\\
\texttt{cree\_sat} also places in the directory \texttt{DATA} the launch script
for the Graphical User Interface:  \texttt{SaturneGUI}.


In the directory \texttt{FORT}, \texttt{cree\_sat} creates a
subdirectory \texttt{USERS} containing all the user subroutines,
classified by module type:  \texttt{base} (general
routines), \texttt{cfbl}, \texttt{cogz}, \texttt{cplv}, \texttt{elec},
\texttt{pprt} and \texttt{rayt}. Only the user subroutines placed directly under
the directory \texttt{FORT} will be considered. The others will be ignored.

In the directory \texttt{SCRIPTS}, \texttt{cree\_sat} copies and pre-fills an
example of launch scripts: \texttt{lance}.
The study, case and user name are filled
automatically in the launch script, as are the paths leading to the
different directories.  The name of the mesh file(s) must be specified
(by default its name is \texttt{\$ETUDE.unv}). 
If necessary, optional instructions may be added (see \S\ref{prg_lance}).


%==================================
\subsection{Pre-processing}
%==================================
The Preprocessor module of \CS\ is in charge of the pre-processing. It reads the
...
\item Universal I-DEAS format (Master Series, versions 6 to 9) generated
      by I-DEAS, ICEM, SIMAIL, ...\\
{\footnotesize\it It should be noted, though, that NOPO files
(\texttt{*.des}) created by SIMAIL are directly usable by \CS. It is therefore
not necessary to convert them to the I-DEAS format. Moreover the conversion
procedure is quite tedious if one wishes to keep the colour of the boundary
faces. Indeed, once the mesh M is created and its boundary faces couloured, the
boundary faces have to be extracted, and the resuting surface mesh has to be
repasted with the original volume mesh M (using the option to paste all
nodes). The resulting composite mesh must then be exported in the SIMAIL format
(M.des) and transformed into the I-DEAS format with the
\texttt{sim2sup}\footnote{this utility comes with the SIMAIL distribution. The
current version on the MFEE servers is \texttt{/logiciels/simulog/SIMAIL-6.5/bin/sim2sup}/}
utility, using mode 1 (\mbox{\texttt{SIMAIL --> MASTERSERIES}}). This procedure
can be very long for meshes of more than 100\,000 cells.}

\item SIMAIL NOPO format: the \texttt{*.des} files may be read directly
      by \CS.

\item Hexa NUMECA format (\texttt{*.hex}files): this format is seldom used. It
is the product of {\em IggHexa} (which has since become {\em Hespress}. It is
not maintained, since the corresponding mesh generator in not available at
MFEE. The filter is based on specifications and some example meshes provided by
the NUMECA company in 2001.

\item MED 2.2. format.

\item CGNS 2.0 (or later) format: this format is seldom used or tested at EDF,
but more largely used elsewhere.

\item EnSight 6 or later and EnSight Gold format.

\item Gmsh format (free 3D mesh generation tool with integrated pre- and
      post-processing functions).

\item Comet-Design format (polyhedric mesh generation tool).
...
The variable IFOENV must
 be set to 0 in the launch scripts. Many potentialities of \CS\ are not
...
each internal face possesses two and only two neighbouring cells, and each
boundary face only one neighbouring cell. For non-converted,
or non-convertale meshes of the 'Common Solver' type,
the calculation must be done without using the Preprocessor (keyword IFOENV=0).
For all the other formats, the Preprocessor must be used (IFOENV=1).

The Preprocessor can also accept zipped mesh file (for the formats I-DEAS, SIMAIL and Hexa NUMECA).
...
{\em``\texttt{.hex}''} fot the Hexa NUMECA format,
...
on these constraints, see the keyword \texttt{ITURB} p.\pageref{prg_iturb}. 
...
\cite{ecsmu}, accessible by the command \texttt{info\_cs ecsmu}. The executable
...
For the main options, the launch script \texttt{lance} contains corresponding
...
\texttt{-m \$MAILLAGE}, where \texttt{MAILLAGE} is the variable where the user has
...
processors used is specified in the \texttt{NOMBRE\_DE\_PROCESSEURS} variable,
or through the batch headers. If necessary, the launch script then automatically
passes the \texttt{-p} option to the Preprocessor command line (see \ref{prg_lance}).

\item \texttt{-rc}: triggers the mesh pasting functions. If nothing more is
...
the faces of colours 6 and 7, the full option will be \mbox{\texttt{-rc -couleur
6 7}}. These options are to be specified in the \texttt{COMMANDE\_RC} variable in
...
invariant point\\
\texttt{-perio -rota -angle a -dir dx dy dz -ptinv px py pz}\\
(with obvious notations), or by giving the rotation matrix and an invariant point\\
\texttt{-perio -rota -matrice m11 m12 m13 m21 m22 m23 m31 m32 m33 -ptinv px py
pz}\\
...
arguments like \texttt{-couleur}. It is also possible to specify up to 3
...
\texttt{-perio -trans -10.2 0 0 -couleur 2} \verb+\+\\
\texttt{-perio -rota -angle 90 -dir 0 0 1 -ptinv 0 0 0 -couleur 3 4} \verb+\+\\
\texttt{-perio -trans 0 1 0 -rota -matrice 1 0  0 0 0 -1 0 1 0 -ptinv 0 0 -0.2}\\
This option is to be specified in the \texttt{COMMANDE\_PERIO} variable in
the launch script, to be automatically passed to the command line.

\item \texttt{-df}: triggers the division of boundary and internal faces which
...
but it is sometimes required (for the Lagrangian modelling, for instance, where
...
be \mbox{\texttt{-df 0.02}}.\\
This option is to be specified in the \texttt{COMMANDE\_DF} variable in
the launch script, to be automatically passed to the command line.

\item \texttt{-ve}: to trigger the determination of the ``extended
neighbourhood''. Indeed, for certain gradient calculation methods (IMRGRA=2 or
3) as well as for the dynamic LES model (ITURB=41), the code needs the list of
the different cells sharing at least one vertex with a given cell. This
``extended neighbourhood'' list is created by the Preprocessor when necessary and
passed to the Kernel. The launch script automatically detects whether this list
is needed and applies the \texttt{-ve} option, without any user intervention.
\end{list}
...
Preprocessor module and the Kernel.\\
...
\item \texttt{-ifoenv n}: specifies the use (\texttt{n}=1) or not (\texttt{n}=0)
of the Preprocessor module. If the \texttt{-ifoenv 0} option is used, only the mehes
of the ``Common Solver'' type can be used (see \S\ref{prg_maillages} for
restrictions).\\
The value of \texttt{n} is to be placed in the \texttt{IFOENV} variable in the launch
script. The option \mbox{\texttt{-ifoenv \$IFOENV}} is automatically added to the
Kernel command line.\\
The value of \texttt{n} is accessible in the variable IFOENV\index{IFOENV} in
the FORTRAN code.

\item \texttt{-longia n}: specifies the size \texttt{n} of the macro-array of
integers IA (number of integers in the array). If the size is not sufficient,
\CS\ stops and indicates the additional size needed.\\
The value of \texttt{n} is to be placed in the \texttt{LONGIA} variable in the launch
script. The option \mbox{\texttt{-longia \$LONGIA}} is automatically added to the
Kernel command line.\\
The value of \texttt{n} is accessible in the variable LONGIA\index{LONGIA} in
the FORTRAN code.

\item \texttt{-longra n}: specifies the size \texttt{n} of the macro-array of
reals RA (number of reals in the array). If the size is not sufficient,
\CS\ stops and indicates the additional size needed.\\
The value of \texttt{n} is to be placed in the \texttt{LONGRA} variable in the launch
script. The option \mbox{\texttt{-longra \$LONGRA}} is automatically added to the
Kernel command line.\\
The value of \texttt{n} is accessible in the variable LONGRA\index{LONGRA} in
the FORTRAN code.

\item \texttt{-mc xx} or \texttt{--mode-comm xx}: indicates the communication
mode between the Preprocessor and the Kernel.\\
\hspace*{0.5cm}\texttt{xx=bin}: binary files\\
\hspace*{0.5cm}\texttt{xx=txt}: ASCII files\\
The usage of this option is very limited, and generally restricted to
developpers. Hence it does not appear in the launch script, which induces the
default behaviour (binary files).

\item \texttt{-p n} or \texttt{-parallel n}: specifies the number of processors
(potentially virtual) on
which the Kernel will run.  In the launch script, the number of
processors used is specified in the \texttt{NOMBRE\_DE\_PROCESSEURS} variable,
or through the batch headers. If necessary, the launch script then automatically
passes the \texttt{-p} option to the Kernel command line (see \ref{prg_lance}).
...
with keywords \texttt{couleur} (for colour selection), \texttt{groupe} (for
...
except the faces of colour 2 and 3, the command line option would be\\
\texttt{-syrthes -couleur 2 3 -invsel}\\
...
\texttt{-syrthes -groupe PAROI -2d -X}\\
All these options are to be placed in the \texttt{COMMANDE\_SYRTHES} variable of
...
\item \texttt{-v n} or \texttt{--verif n}: triggers the verification mode.
...
\hspace*{0.5cm}\texttt{n=-1}: no test (default value if no \texttt{-v} option is
...
The value of \texttt{n} is to be placed in the \texttt{ARG\_CS\_VERIF} variable
in the launch script. The option \mbox{\texttt{-v \$ARG\_CS\_VERIF}} is
automatically added to the Kernel command line.\\
...
The case preparer \texttt{cree\_sat} places an example of launch script,
\texttt{lance}, in the \texttt{SCRIPTS} directory. This script is quite general
...
found on the machines of the CCRT (Nickel/Chrome/Tantale). The data expected are
...
\item \texttt{IFOENV}: value of the parameter IFOENV to be passed to the Kernel
(see \ref{prg_optappelnoy})

\item \texttt{LONGIA}: value of the parameter LONGIA to be passed to the Kernel
(see \ref{prg_optappelnoy}).

\item \texttt{LONGRA}: value of the parameter LONGIA to be passed to the Kernel
(see \ref{prg_optappelnoy})
\end{list}

{\it For parallel computations, LONGIA and LONGRA can theoretically
be divided by the number of processors, since each execution only deals with a
...
\begin{list}{$\bullet$}{}
\item \texttt{ETUDE}: name of the study directory (automatically set by
\texttt{cree\_sat}, see \S\ref{prg_architecture})

\item \texttt{CAS}: name of the case directory (automatically set by
\texttt{cree\_sat}, see \S\ref{prg_architecture}) 

\item \texttt{PARAM}: name of the Interface parameter file, if necessary (see
\ref{prg_optappelnoy})

\item \texttt{MAILLAGE}: name(s) of the mesh(es) used for the calculation (see
\ref{prg_optappelecs} and \ref{prg_maillages}). The files will be looked for in
the directory \texttt{REPMAIL} (see below).

\item \texttt{COMMANDE\_RC}: Preprocessor command line option for mesh pasting (see
\ref{prg_optappelecs})

\item \texttt{COMMANDE\_DF}: Preprocessor command line option for the division of
faces with too large a warp angle (see \ref{prg_optappelecs})

\item \texttt{COMMANDE\_PERIO}: Preprocessor command line option for the definition
of periodic boundary conditions (see \ref{prg_optappelecs})

\item \texttt{COMMANDE\_SYRTHES}: Kernel command line option for coupling with
\syrthes\ (see \ref{prg_optappelnoy})

\item \texttt{DONNEES\_THERMOCHIMIE}: name of the thermochemical data file, if
necessary (the file is looked for in the directory \texttt{DATA}, see
\S\ref{prg_usppmo})

\item \texttt{NOMBRE\_DE\_PROCESSEURS}: number of processors (potentially virtual)
to be used for the calculation.\\
If the variable is left empty, the launch script
will fill it automatically: on a batch system, \texttt{NOMBRE\_DE\_PROCESSEURS}
will be equal to the number of processors reserved, and in case of an
interactive calculation, it will be set to 1.\\
When using a batch system,
\texttt{NOMBRE\_DE\_PROCESSEURS} should ideally be equal to the number of
processors reserved, and can never be larger (one executable per
processor). With an interactive calculation (like a Linux PC),
\texttt{NOMBRE\_DE\_PROCESSEURS} can be larger than the total number of processors
...
\texttt{NOMBRE\_DE\_PROCESSEURS-1} processors.

\item \texttt{LISTE\_PROCESSEURS}: list of nodes on which the calculation is to
...
\item \texttt{FICHIERS\_DONNEES\_UTILISATEUR}: list of the user data files to be
...
\texttt{FICHIERS\_DONNEES\_UTILISATEUR} (see \S\ref{prg_usvort}) 

\item \texttt{FICHIERS\_RESULTATS\_UTILISATEUR}: list of user result files to be
...
all the files will be stored in. The files automatically created by the code
...
to be specified in \texttt{FICHIERS\_RESULTATS\_UTILISATEUR}},
restart files) do not need to be specified in
\texttt{FICHIERS\_RESULTATS\_UTILISATEUR}.

\item \texttt{CS\_TMP\_PREFIX}: alternate temporary directory for the
calculation (see \S\ref{prg_temporarydirectory})

\item \texttt{OPTIMISATION}: optimisation level for compilation
 (LO, DBG, EF, PROF or PUR; see \S\ref{prg_library}). This optimisation
level will be applied to all the modules of \CS\ (BASE, COGZ, CFBL, CPLV, ELEC,
LAGR, MATI, RAYT).
Leaving the variable empty stands for ``standard''
optimisation.

\item \texttt{LISTE\_LIB\_SAT}: list of \CS\ module libraries to use (see
\S\ref{prg_library}). Specifying
a list allows to use the ``STUB'' libraries (for modules that are not available)
or to use different optimisation options for different modules. If a list is
given, all modules must be referenced once (and only once). Leaving
the variable empty implies taking the standard optimisation library for all the
modules (BASE, COGZ, CFBL, CPLV, ELEC, LAGR, MATI, RAYT). An example of list
could be:\\ 
\texttt{LISTE\_LIB\_SAT=``libsaturneBASEDBG.a libsaturneCOGZ\_STUB.a
libsaturneCFBLDBG.a}\verb+ \+\\
\texttt{\phantom{LISTE\_LIB\_SAT=``}libsaturneCPLV\_STUB.a libsaturneELECDBG.a
libsaturneLAGR\_STUB.a}\verb+ \+\\ 
\texttt{\phantom{LISTE\_LIB\_SAT=``}libsaturneMATI\_STUB.a libsaturneRAYT\_STUB.a''}

\item \texttt{OPTION\_LIB\_EXT}: additional commands for the link stage of the
...
\texttt{OPTION\_LIB\_EXT=``-L/opt/foo/lib -lfoo''}
...
command \texttt{\$VALGRIND cs13.exe ...}). It is especially designed to use the
...
\item \texttt{PILOTAGE\_ADAPTATION}: commands to trigger the automatic mesh
adaptation with the software Homard. This option is still under development and
restricted to developpers use.

\item \texttt{REPBASE}: root directory of the calculation. This variable is
automatically set by \texttt{cree\_sat} and should not be changed.

\item \texttt{DATA}: DATA directory of the case (see \ref{prg_architecture}).
This variable is automatically set by \texttt{cree\_sat} and should not be
changed.
...
\item \texttt{FORT}: FORT directory of the case (see \ref{prg_architecture}).
This variable is automatically set by \texttt{cree\_sat} and should not be
changed.

\item \texttt{SCRIPTS}: SCRIPTS directory of the case (see
\ref{prg_architecture}). This variable is automatically set by
\texttt{cree\_sat} and should not be changed.

\item \texttt{SUITE\_AMONT}: directory containing the files for calculation
restart.

\item \texttt{PRE\_TRAITEMENT\_AMONT}: directory containing the
\texttt{enveloppe\_vers\_solveur*} files for a calculation in ``calculation''
mode (see \ref{prg_executionmodes})

\item \texttt{REPMAIL}: directory containing the mesh files (see
\ref{prg_architecture}). This variable is automatically set by
\texttt{cree\_sat} and should generally not be changed.

\item \texttt{DATA\_SYR}: directory for the \syrthes\ data. This directory has
to be created by the user. It is advised to keep the location proposed in the
launch script, which complies with the standard architecture of \CS\ (see
...
\item \texttt{FORT\_SYR}: directory for the \syrthes\ user subroutines.
...
\item \texttt{MODE\_COUPLAGE}: coupling mode between \CS\ and \syrthes, when
such coupling is activated (see \texttt{COMMANDE\_SYRTHES}). Three options are
available:\\
\hspace*{0.5cm}\texttt{MPI}: for a coupling based on MPI messages (requires an
MPI library)\\
\hspace*{0.5cm}\texttt{pipes}: for a coupling based on pipe files (on clusters,
such a coupling is valid only if all the processors are located on the same
node)\\
\hspace*{0.5cm}\texttt{sockets}: for a coupling based on sockets

\item \texttt{MODE\_EXEC}: execution mode for \CS\ (\ref{prg_executionmodes})
...
a specific \CS\ syntax which is then interpretated by the code.

In version \verscs, the Graphical Interface manages calculation parameters,
standard initialisation values and boundary
conditions for standard physics and pulverised coal combustion. The other specific
...
- define in \texttt{usclim} the inlet area with its velocity
profile. The wall and output areas do not need to appear in
\texttt{usclim}. Moreover, if the inlet area has already been set in
the interface (for example with a uniform velocity), it is the boundary
conditions defined in the subroutine \texttt{usclim} which will be taken
into account.

The Graphical User Interface is launched with the \texttt{./SaturneGUI} commande
...
\item Analysis environment: definition of the calculation directories
      ...
      Preprocessor module (used by the Interface to get the colours of the boundary
      faces).

\item Thermophysical models: physical model, turbulence model, thermal model,
      initialisation of the variables.

\item Physical properties: fluid characteristics, gravity.
...
\item Boundary conditions: definition of the boundary conditions for
      each variable. The colours of the boundary faces may be read
...
\item Analysis control: parameters concerning the time step, location of
...
\S\ref{prg_lance}). When the launch
...
\minititre{Note: option \texttt{-noihm} of cree\_sat}
...
must be placed in the directory \texttt{FORT}. But, while doing this, all the
...
he has already specified through the Interface, \texttt{cree\_sat} automatically
comments out the examples in \texttt{usini1} (\texttt{Cex} at the beginning of
each line) while copying it in the directory \texttt{USERS}. Therefore, the user
...
\texttt{usini1} have to be specified. In that case, using the \texttt{-noihm}
option of \texttt{cree\_sat} will prevent it from commenting \texttt{usini1}
...
Developpers can also refer to \cite{boucker00} and \cite{gest02}.
...
store temporarily the information coming from this ``distant''
...
creation of ghost cells allows to store temporarily the information
...
face in the array NODFAC (see note 3 in paragraph \ref{prg_nnod}).}

\variab{IPNFBR}{IPNFBR(NFABOR+1)}{IA}{Position of the first node of the each boundary
face in the array NODFBR (see note 3 in paragraph \ref{prg_nnod}).}


\variab{NODFAC}{NODFAC(LNDFAC)}{IA} {Index-numbers of the nodes of each
internal face (see note 3 in paragraph \ref{prg_nnod}).}

\variab{NODFBR}{NODFBR(LNDFBR)}{IA}{Index-numbers of the nodes of each
boundary face (see note 3 in paragraph \ref{prg_nnod}).}
...
In addition, additionnal geometric variables are accessible in sections of
the unidimensional macro-arrays\footnote{in FORTRAN, a multidimensional
array A(3,2) is in fact a unidimensional array containing the elements
A(1,1), A(2,1), A(3,1), A(1,2), A(2,2) and A(3,2) in this order.} IA
(for integers) and RA (for real numbers) which are passed as arguments
in every subroutine (apart from a few ones of very low level). The
index-number of the first element of these sections in IA and RA is indicated by
an integer stored in a ``common'' (in the file
\texttt{pointe.h})\footnote{With this system, it is possible for example to
reach the surface of an
internal face IFAC with RA(ISRFAN+IFAC-1) in every subroutine where RA
is known and \texttt{pointe.h} is present. \\
In the same manner, DOFIJ(II,IFAC) can be obtained with
RA(IDOFIJ+(IFAC-1)*NDIM+II-1).} By analogy, these integers will be referred here
as ``pointers''. The main ones are listed below.
...
respectively the orthogonal projections on a straight line orthogonal to
the face of the centres of the first and the second neighbouring cells}
...
orthogonal projection of I, centre of the neighbouring cell, on the
straight line perpendicular to the face and crossing its centre}

\variab{IDIST}{IDIST}{I}{In RA, pointer to DIST(NFAC), real array
giving, for every internal face, the scalar product between the
vectors $\vect{IJ}$ and $\vect{n}$. I and J are respectively the centre
...
the vectors $\vect{IF}$ and $\vect{n}$. I is the centre of the
neighbouring cell. F is the face centre. The vector $\vect{n}$ is the
...
the straight line joining the centres of the two neighbouring cells. F
is the face centre} 

\variab{IICELB}{IICELB}{I}{In IA, pointer to ICELBR(NFABOR), integer array
giving, for every boundary face, the index-number of the neighbouring cell}
...
giving the absolute value of the surface of the internal faces}

\variab{ISRFBN}{ISRFBN}{I}{In RA, pointer to SURFBN(NFABOR), real array
giving the absolute value of the surface of the boundary faces}

%==================================
%\subsubsection{Variables physiques}
...
\variab{RTP}{RTP(NCELET,NVAR)}{RA}{Array storing the variables,
solved at the current time step} 

\variab{RTPA}{RTPA(NCELET,NVAR)}{RA}{Array storing the variables,
solved at the previous time step} 
...
\texttt{numvar.h}. Some solved variables (pressure, speed, turbulence)
depend on the considered phase, and the index which refers to it is then a
array whose size is the maximum number of phases NPHSMX.
...
variable index-numbers do not only allow to access to the value of the
variable at the cell centre (for instance RTP(IEL,IPR(IPHAS))) but are also
used to access to arrays whose size is the number of variables, like
some numerical options (for instance, BLENCV(IK(IPHAS)) is the
percentage of second-order convective scheme for the turbulent energy of
the phase IPHAS when the $k-\varepsilon$ model is used).

\begin{list}{$\bullet$}{}
\item IPR(IPHAS)\index{IPR}: pressure.
...
\item IK(IPHAS)\index{IK}: turbulent energy, in $k-\varepsilon$ modelling.
...
$R_{ij}-\varepsilon$ or v2f ($\varphi$-model) modelling.
...
\item The J$^{\text{th}}$ scalar related to a specific physics is, in
      ...
      whose value in the cell IEL at the current time step is given by
      RTP(IEL,ISCA(ISCAPP(J))). 

\item The J$^{\text{th}}$ ``user'' scalar
      (0$\leqslant$J$\leqslant$NSCAUS$\leqslant$NSCAL) is,si $J>0$, the
 scalar number J in the thorough list of NSCAL scalars' In the list of 
solved NVAR variables, it corresponds to the variable number cell IEL 
and its value in the cell is RTP(IEL,ISCA(J)).
...
\item The temperature (or the enthalpy) is the scalar number
      ...
      value in the cell IEL is RTP(IEL,ISCA(ISCALT(IPHAS))=1 if there is no thermal scalar.
\item A ``user'' scalar number J may represent the mean value of the
      square of the fluctuations ({\em i.e.}
      $\overline{\varphi^\prime\varphi^\prime}$) of a ``user'' scalar K
      (called $\varphi$). This can be made either {\em via} the
...
The indexes referring  to the different properties stored in the generic
...
All the properties (used or not) have a unique and distinct index-number, given
automatically by the code and stored in an integer or an integer array
(whose size may be the maximum number of phases, the maximum
number of scalars or the maximum number of variables).

For instance, the number corresponding to the density of the phase
IPHAS is IROM(IPHAS). \\
In the list of the properties defined at the cell centre, the density of
the phase IPHAS is therefore the IPPROC(IROM(IPHAS))$^{\text{th}}$
property: its value at the centre of the cell IEL is given by \\
...
The number of each foreseen property is given below:
...
corresponding to the specific heat, in the case where it is variable
...
number for the NESTMX error estimators of Navier-Stokes. These are
...
convective flow associated with each variable ({\em i.e.} for each face
with the surface $S$, $\rho \vect{u} \,.\,\vect{S}$ in $kg.s^{-1}$). It
must be noticed that the convective flows are associated with the
...
convective flow associated with each variable ({\em i.e.} for each face
with the surface $S$, $\rho \vect{u} \,.\,\vect{S}$ in $kg.s^{-1}$) at
...
corresponding to the ``constant'' $C$ of the dynamic model, {\em i.e}
...
In that case, in order to diminish the necessary memory, these
properties are stored in a simple real number rather than in a real
array whose size would be the number of cells.
\begin{list}{$\bullet$}{}
\item It is the case for the specific heat $C_p$. 
\begin{list}{-}{}
\item If $C_p$ is constant in the phase IPHAS, it can be specified in 
...
every average defined by the user. In fact it is not really a
time-average, but a cumulation over time (the division by the cumulated
...
The cumulated duration associated with the calculation of a time-average
...
real number DTCMOM(-IDTMOM(IMOM)). \\
When this cumulated duration can vary in space (for instance in the case of a
spatially variable time step), it must be stored in a array whose size is
the number of cells, which may be accessed to in PROPCE. It must be
noticed that the cumulated duration associated with the calculation of 
...
ICDTMO(IDTMOM(IMOM)). For instance, for the average IMOM, the
...
how to calculate a time-average in a particular case (printing of extrem
values, writing of the results).

\bigskip

The 2 variables, HBORD and TBORD should be noted as they are relatively local
(in the treatment of boundary conditions) and are used only by developers.

\variab{HBORD}{HBORD(NFABOR)}{RA}{Array of the coefficients of exchange of 
temperature (or enthalpy) at the boundary faces (must set ISVHB=1\index{ISVHB}
 in \texttt{tridim}, or else the array could crash. Used in coupling with \syrthes.}

\variab{TBORD}{TBORD(NFABOR)}{RA}{Temperature (or enthalpy) array of the boundary
 faces (must set ISVTB=ISCAL\index{ISVTB} (number of the scalar representing temperature
 or enthalpy) in \texttt{tridim}, or else the array could crash).
 Used in coupling with \syrthes.}   



%Les deux variables HBORD et TBORD peuvent \^etre not\'ees, 
%bien qu'elles soient relativement locales (traitement des conditions aux
%limites) et utiles aux d\'eveloppeurs uniquement.   

%\variab{HBORD}{HBORD(NFABOR)}{RA}{Tableau du coefficient d'\'echange de la
%temp\'erature  (ou de l'enthalpie) sur les faces de bord (n\'ecessite d'avoir
%pos\'e ISVHB=1\index{ISVHB} dans \texttt{tridim}, sinon on risque des \'ecrasements de
%tableaux). Utilis\'e pour 
%le couplage avec \syrthes. Le tableau est de taille NFABOR bien que l'utilit\'e
%de la variable soit limit\'ee aux faces de paroi}

%\variab{TBORD}{TBORD(NFABOR)}{RA}{Tableau de la temp\'erature (ou de
%l'enthalpie) sur les faces de bord (n\'ecessite d'avoir
%pos\'e ISVTB=ISCAL\index{ISVTB} (num\'ero du scalaire repr\'esentant la temp\'erature ou
%l'enthalpie) dans \texttt{tridim}, sinon on risque des \'ecrasements de 
%tableaux). Utilis\'e pour le couplage avec \syrthes. 
%Le tableau est de taille NFABOR bien que l'utilit\'e
%de la variable soit limit\'ee aux faces de paroi}  


%==================================
%\subsubsection{Variables des m\'ethodes num\'eriques}
...
\variab{IIFAPA}{IIFAPA(NPHSMX)}{IA}{For each phase, the pointer in IA which
 ...
 to the centre of a given volume when necessary ($R_{ij}-\varepsilon$ with
 wall echo, LES with van Driest-wall damping,
or SST $k-\omega$ turbulence model) and when ICDPAR=2. The number of the wall
face (for the phase IPHAS) which is the closest to the centre of the
...
allowing to mark out the distance between the centre of a given volume and the
...
and when ICDPAR=1. The distance between the centre of the cell IEL and the
...
between the centre of the cell IEL and the closest wall is therefore
RA(IYPPAR+IEL-1)} 
...
\minititre{Note 1: families and properties}
...
\begin{list}{$\bullet$}{}
\item colour.
When the mesh is created using I-DEAS, boundary faces and volume
      elements may be given colours. When it is created using SIMAIL, the
      colours of boundary faces are called ``references'', and the colours
      of volume elements are called ``subdomains''.
\item groups.
For instance, in I-DEAS, mesh entities may be gathered in ``groups''
      and manipulated together. In \CS, every mesh entity belonging to
      a group is then given a group number.
\end{list}
From \CS 's point of view, all the references to mesh entities (boundary faces
and volume elements) correspond to a number (colour number or group
number) associated with the entity. An entity may have several
references (for instance, one entity may have one colour and belong to
...
the properties (in particular the colour) of an entity (a boundary face
...
given one colour (for example using SIMAIL). The family of the boundary
face IFAC is IFML=IFMFBR(IFAC). The first (and only) property of this
family is the colour ICOUL, obtained for the face IFAC with
ICOUL=IPRFML(IFML,1). In order to know the property number corresponding
to a group, the user may refer to the listing of the Preprocessor, or use
...
\minititre{Note 2: boundary conditions}
...
projection of the centre of the adjacent cell on the straight line
perpendicular to the boundary face and crossing its centre:
...
%==================================
%\subsubsection{Tableaux utilisateurs}
...
%==================================
%\subsubsection{Tableaux d\'eveloppeurs}
...
%==================================
%\subsubsection{Parall\'elisme et p\'eriodicit\'e}
...
\item On the MFTT cluster, the launching of a parallel running calculation
      requires to complete the batch cards \texttt{PBS} located in the
      beginning of \texttt{cntl}, and particularly to set the number of
      physical nodes (\texttt{nodes}) and the number of physical
      processors per node (\texttt{ppn}) wanted. Generally, the number of
      used virtual processors is the same as the number of reserved physical
      processors, that is to say:
\texttt{NOMBRE\_DE\_PROCESSEURS}=\texttt{nodes}$\times$\texttt{ppn}.

\item On the C.C.R.T., the launching of a parallel running calculation
      requires to complete the batch cards \texttt{LSF} located in the
      beginning of \texttt{lance\_ccrt}, and particularly to set the
      number of processors (\texttt{\#BSUB -n}) wanted and the limit CPU
      time (\texttt{\#BSUB -c}). The variable
      \texttt{NOMBRE\_DE\_PROCESSEURS} which stands for the number of
      virtual processors used for the calculation must be completed,
      too. In the case of a communication by files between the Preprocessor
      and the Kernel ((\texttt{IFOENV=1}), the number of used virtual
      processors is the same as the number of reserved processors. In
      the case of a communication by MPI (\texttt{IFOENV=2}), a
      supplementary processor must be reserved to run the Preprocessor.

\item Concerning the periodicity, it is enough to complete the option
      \texttt{COMMANDE\_PERIO} of the launch scripts. The transformation
...
\begin{list}{-}{}
\item that the periodicity is activated by testing the variable IPERIO
      (=1 if the periodicity is activated);
\item that rotation periodicities are present by testing the variable
      IPEROT (number of rotation periodicities);
\item that the calculation is run in parallel mode by testing the
...
\item calculation of an internal face value from the values of $A$  in
      the neighbouring cells (use of IFACEL).
\end{list}
...
\begin{list}{-}{}
\item periodicity is incompatible with 
  \begin{list}{-}{}
   \item  multigrid algorithm,
   \item  Lagrangian module, 
   \end{list}
\item rotation periodicity is incompatible with 
  ...
  \end{list}
\item although it has not been the case so far, eventual problemes might be met
      in the case of rotation periodicity with the $R_{ij}-\varepsilon$
...
\end{list} 
%==================================
%\subsubsection[Tableaux g\'eom\'etriques et particulaires li\'ees \`a la
%      mod\'elisation lagrangienne] 
%{Tableaux g\'eom\'etriques et particulaires li\'ees \`a la
%      mod\'elisation lagrangienne} 
...
The directory \texttt{SUITE*} contains:
\begin{list}{-}{}
...
the head of the \texttt{listenv} file, which repeats the command line passed to
...
subroutine. In the case that a parameter is specified in the interdace
 ...
 user subroutines are placed in the \texttt{USERS} directory by the
 case preparer \texttt{cree\_sat}. 
 


%==================================
%\subsection[Initialisation des principaux mots cl\'es : \texttt{usini1}]
%{Initialisation des principaux mots cl\'es : \textmd{\texttt{usini1}}}
...
can be provided by the user in  \texttt{FORT/USERS/base}; in this case, understanding
...
utility program \texttt{cree\_sat} comments automatically all the
...
\texttt{cree\_sat} can be inactivated with
the option \texttt{-noihm} (useful if the user knows that he will not
use the interface).

%==================================
%\subsection[Gestion des conditions aux limites : \texttt{usclim}]
%{Gestion des conditions aux limites : \textmd{\texttt{usclim}}}
...
faces. A sequence of \texttt{IF ... ELSE IF ... ENDIF} allows to
differentiate the boundary faces according to their colour (or, more
generally, the properties IPRFML of the family IFMFBR to which the face
belongs). If needed, disposal geometric and
...
%==================================
%\subsubsection{Codage des conditions aux limites standards}
...
%==================================
%\subsubsection{Codage des condition limites non standards}
...
\item For the other scalars, the condition ICODCL=5 is similar to
      ...
      RCODCL(IFAC,IVAR,2) must therefore be specified: see \cite{gest02}.
\end{list}
...
%==================================
%\subsubsection{V\'erification des conditions aux limites}
...
%==================================
%\subsubsection{Tri des faces de bord}
...
\item The 2nd part (IAPPEL=1) defines the boundary faces at which the vortex method
 is applicable. The boundary faces can be represented as colours (ICOUL) or by the
 geometric co-ordinates. The IREPVO\index{IREPVO} array is informed by IENT whic
h defines the number of inlets concerned with the vortex (essentially the vortex
...
defined as a product of DIR1 and DIR2. DIR1 and DIR2 must be chosen impreitavely to
 ...
 orientated in the entry domain.If ICAS=2, the position CEN has to be the centre of
...
%
%Si ICAS=1, les conditions aux limites aux fronti\`eres (arêtes) du
%rectangle d'entrée doivent \^etre d\'efinies. Elles sont définies par le tableau
%ICLVOR\index{ICLVOR}. ICLVOR(II,IENT) représente le type de condition à la
%limite pour l'arête II (1$\leqslant$II$\leqslant$4) de l'entrée IENT.
...
%\end{list}
%Les quatre arêtes sont numérotées à partir des directions de DIR1 et DIR2, comme
%indiqué dans la figure \ref{Fig_vortex}.
%\begin{figure}[hp]
%\centerline{
%\includegraphics*[width=8cm]{\repgraphics/vort{\repgraphics/vortex.\extgraphics}}ex.\extgraphics}}
%\caption{Numérotation des arêtes d'une entrée rectangulaire (ICAS=1) traitée par
%la méthode des vortex}\label{Fig_vortex}
...
%Si ICAS=1, l'utilisateur doit définir LLX et LLY qui donnent les longueurs de la
...
%seront reg\'en\'er\'es après un temps fixe de TMPLIM\index{TMPLIM} secondes (\`a
...
is a langrangian method in which the eddy centres are replaced by a set velocity). If IDELPA=1, 
...
(a sample number r, is taken for each vortex, at each time step and for each direction and
 the centre of the vortex is replaced by the 2 principle directions, $r \mbox{UD} \Delta t$ where 
...
%==================================
 %\subsection[Gestion des propri\'et\'es physiques variables : \texttt{usphyv}]
%{Gestion des propri\'et\'es physiques variables : \textmd{\texttt{usphyv}}}
...
%==================================
%\subsection[Initialisation non standard des variables : \texttt{usiniv}]
%{Initialisation non standard des variables : \textmd{\texttt{usiniv}}}
...
is NCELET, defined at the cell centres whatever the chosen time step type).
...
%==================================
%\subsection[Gestion non standard des fichiers historiques : \texttt{ushist}]
%{Gestion non standard des fichiers historiques :
%\textmd{\texttt{ushist}}}
...
\item Positionning of the probes (only at the first passage): the index
      ...
      gives then the number ICAPT(II)\index{ICAPT} of the cell centre
...
calculation. The interface allows the specification of the name of the copied
 ...
 be inputted in FICHIERS\_RESULTATS\_UTILISATEUR in the launch script.

%==================================
%\subsection[Termes sources utilisateur dans Navier-Stokes : \texttt{ustsns}]
%{Termes sources utilisateur dans Navier-Stokes : \textmd{\texttt{ustsns}}}
...
the theoretical and computer documentation \cite{gest02} of the
...
%==================================
%\subsection[Termes sources utilisateur pour $k$ et $\varepsilon$ :
%\texttt{ustske}] 
%{Termes sources utilisateur pour $k$ et $\varepsilon$ :
%\textmd{\texttt{ustske}}}
...
%==================================
%\subsection[Termes sources utilisateur pour $R_{ij}$ et $\varepsilon$ :
%  \texttt{ustsri}] 
%{Termes sources utilisateur pour $R_{ij}$ et $\varepsilon$ :
%\textmd{\texttt{ustsri}}}
...
variable IVAR (refering successively to IR11(IPHAS), IR22(IPHAS),
...
%==================================
%\subsection[Termes sources utilisateur pour $\varphi$ et $\overline{f}$ : \texttt{ustsv2}]
%{Termes sources utilisateur pour $\varphi$ et $\overline{f}$ :\textmd{\texttt{ustsv2}}}
...
CRVEXP for IVAR refering successively to IPHI(IPHAS) and
...
%==================================
%\subsection[Termes sources utilisateur pour $k$ et $\omega$ : \texttt{ustskw}]
%{Termes sources utilisateur pour $k$ et $\omega$ : \textmd{\texttt{ustskw}}}
...
%==================================
%\subsection[Termes sources utilisateur pour les scalaires utilisateur : \texttt{ustssc}]
%{Termes sources utilisateur pour les scalaires utilisateur : \textmd{\texttt{ustssc}}}
...
%==================================
%\subsection[Gestion des pertes de charge : \texttt{uskpdc}]
%{Gestion des pertes de charge : \textmd{\texttt{uskpdc}}}
...
%==================================
%\subsection[Gestion des sources de masse : \texttt{ustsma}]
%{Gestion des sources de masse : \textmd{\texttt{ustsma}}}
...
\item RCPT1D(NFPT1D)\index{RCPT1D}: Volumetric heat capapcity $\rho C_p$ of the
...
%==================================
%\subsection[Modification de la viscosit\'e turbulente: \texttt{usvist}]
%{Modification de la viscosit\'e turbulente : \textmd{\texttt{usvist}}}
...
%==================================
%\subsection[Modification de la vitesse de frottement : \texttt{usruet}]
%{Modification de la vitesse de frottement : \textmd{\texttt{usruet}}}
...
to the theoretical and computer documentation \cite{gest02} of the
subroutine \texttt{clptur}. To access to this document from a
workstation, use the command \texttt{info\_cs noyau}.
...
%==================================
%\subsection[Modification de la ``constante'' du modèle LES dynamique: \texttt{ussmag}]
%{Modification de la ``constante'' du modèle LES dynamique: \textmd{\texttt{ussmag}}}
\subsection{Modification of the ``constant'' of the dynamic LES model:
  \textmd{\texttt{ussmag}}}  
%==================================
...
This subroutine is used to modify the calculation of the ``constant'' of
...
The notations used for the definition of the ``constant'' used in the
...
time and in space. It is determined by $\displaystyle C=\frac{M_{ij}L{ij}}{M_{kl}M_{kl}}$.
...
%==================================
%\subsection[Conversion temp\'erature-enthalpie et enthalpie-temp\'erature : \texttt{usthht}]
%{Conversion temp\'erature-enthalpie et enthalpie-temp\'erature : \textmd{\texttt{usthht}}}
...
%==================================
%\subsection[Modification de la g\'eom\'etrie du maillage : \texttt{usmodg}]
%{Modification de la g\'eom\'etrie du maillage : \textmd{\texttt{usmodg}}}
...
This subroutine may be used to modify ``manually'' the mesh
characteristics\footnote{The family properties must not be modified:
they have already been used to define the periodic faces.In fact, a face 
defined as periodic in the script cannot be changed at the inlet, even 
if the colour of the inlet face is altered}:
\begin{list}{$\bullet$}{}
\item \mbox{XYZCEN(3,NCELET)} (cell centre coordinates),
\item \mbox{CDGFAC(3,NFAC)} (internal face centre coordinates),
\item \mbox{CDGFBO(3,NFABOR)} (boundary face centre coordinates),
\item \mbox{SURFAC(3,NFAC)} (internal face surface vector components) and
\item \mbox{SURFBO(3,NFABOR)} (boundary face surface vector components).
\end{list}

This subroutine may also be used to test the mesh quality (which is done by
default). But it must be reminded that the Preprocessor, when running in
the ``verification'' mode, allows to get quality criterions which are
more complete and easier to exploit (using EnSight or in the execution
report). For instance, for a mesh ``\texttt{mesh}'' (in \texttt{des},
\texttt{unv} or any other acceptable format), the command to execute is: 
\mbox{\texttt{verifmail mesh.unv}} (this command calls the Preprocessor
with the necessary options).\\

%==================================
%\subsection[Gestion des sorties interm\'ediaires de post-traitement : \texttt{usnpst}]
%{Gestion des sorties interm\'ediaires de post-traitement : \textmd{\texttt{usnpst}}}
\subsection{Management of the post-processing intermediary outputs: \textmd{\texttt{usnpst}}}
%==================================

\noindent
\textit{Subroutine called every time step(even if the user hasn't moved it to the FORT directroy).}
...
\newpage
%==================================
%\subsection[Gestion des sorties au format {\em Rubens} : \texttt{usrubm}]
%{Gestion des sorties au format {\em Rubens} : \textmd{\texttt{usrubm}}}
\subsection{Management of the outputs in {\em Rubens} format: \textmd{\texttt{usrubm}}}
%==================================

\noindent
\textit{Subroutine called at every chronological output.}

This subroutine gives the possibility to generate chronological outputs
using the {\em Rubens} mesh tool format. The given example manages the
writing of the pressure, the velocity and the first four scalars. All
the variables are however passed as arguments, to allow the user to add
everry variable he wants.

%==================================
%\subsection[D\'efinition de coupes pour les sorties par listes de faces : \texttt{uslfac}]
%{D\'efinition de coupes pour les sorties par listes de faces : \textmd{\texttt{uslfac}}}
\subsection{Definition of sections for outputs in the form of lists of faces: \textmd{\texttt{uslfac}}}
%==================================

\noindent
\textit{Subroutine called at the calculation beginning.}

This subroutine gives the possibility to define sections in the form of
lists of \texttt{NLFAC} internal faces (\texttt{LSTFAC}) and
\texttt{NLFAB} boundary faces (\texttt{LSTFAB}), in order to generate
chronological outputs in {\em EnSight}, {\em MED} or {\em CGNS} format.

The given example proposes to define two sections (\texttt{NBCOUP =
2}). The first one is consituted of internal and boundary faces marked
out by means of the coordinates of their centres. The second one is constituted
of internal faces marked out by means of the colour of their neighbouring
cells and of boundary faces marked out by means of their colour.

It must be noticed that the lists of faces can not be modified during
the calculation (they are set at the calculation beginning).

In order to ease the post-processing, a distinct name should be given to
each section (\texttt{COUPE = }): this character string will be the name
of the corresponding {\em EnSight} ``part'' and will be used in the eventual
{\em MED} file.

The content of the character string \texttt{CAS = } will be used as a
prefix in the name of the EnSight directory containing the
post-processing files and in the eventual MED file. If several sections
are requested and the user does not wish to post-process the data on each
one at the same time step (see \texttt{usvfac}), it
may be useful, in order to ease the post-processing, to specify a
different value of \texttt{CAS} for each section (for instance
\texttt{CAS=''CAS\_COUPE1''}, \texttt{CAS=''CAS\_COUPE2''}...), so that
a distinct {\em EnSight} directory will be generated for each one. If
\texttt{CAS} is not specified, the post-processing files are stored with
the usual chronological post-processing files (it is recommended to
always specify the value of the variable \texttt{CAS}).

The post-processing format is set by means of the variable \texttt{FORMA
= }. By default, the {\em EnSight} format is generated. Outputs in the
{\em MED} or {\em CGNS} formats may be also requested.

The values to post-process on the sections are precisely defined in the subroutine \texttt{usvfac}

%==================================
%\subsection[D\'efinition des informations \`a post-traiter sur les coupes : \texttt{usvfac}]
%{D\'efinition des informations \`a post-traiter sur les coupes : \textmd{\texttt{usvfac}}}
\subsection{Definition of the data to post-process on the sections:
  \textmd{\texttt{usvfac}}}  
%==================================

\noindent
\textit{Subroutine called every time step and for every section.}

For the sections defined in \texttt{uslfac}, the subroutine
\texttt{usvfac} is used to define the instants at which outputs will
be generated and to specify which variables have to be
post-processed. Basically, the communication for the current section is
initialised by calling \texttt{MSGDSC}, which provides the section number,
the time step number and the physical time. For this section, the arrays
of values at internal (\texttt{TRAFAI}) and boundary faces
(\texttt{TRAFAB}) are sent to the Preprocessor by the subroutine
\texttt{MSGVAC}, as are the variable name (\texttt{NAMEVR}), the length
of the character string which contains it (\texttt{LNAME}), the
dimension of the variable (\texttt{IDIMT}=1 for a scalar and =3 for a
vector) and the numbers of the section internal (\texttt{NFAICP}) and
boundary (\texttt{NFABCP}) faces. 

More precisely, the user must first define the pairs (section, time
step) corresponding to expected post-processing outputs. This is the goal
of the first part of the subroutine: an instruction \texttt{RETURN} is
placed in the beginning of the subroutine according to the section
number \texttt{NUMCOU} and the time step number \texttt{NTCABS}. It is
important to note that the section outputs are independent from the
other post-processing outputs. In particular, there is no automatically
generated output after the last calculation time step (if an output
after the last time step is wanted, this must then be specified).
 
For each section, the given example proposes then to define the
variables to post-process. For instance, concerning the section number 1,
the velocity, the pressure and the temperature at the boundary are
generated. Concerning the section number 2, all the usually post-processed
unknowns (according to \texttt{NTCHR} and \texttt{ICHRVR}) are generated.

%==================================
%\subsection[D\'efinition de post traitments et zones de maillage~: \texttt{usdpst}]
%{D\'efinition de post traitements et zones de maillage~: \textmd{\texttt{usdpst}}}
...
%==================================
%\subsection[Modification de zones de maillage post traiter~: \texttt{usmpst}]
%{Modification de zones de maillag post traiter~: \textmd{\texttt{usmpst}}}
...
argument \texttt{IMODIF} must be given the value 1.If this argument
 ...
 modifiable assebmly (if in doubt, let IMODIF=1). 
...
velocity is lower than this value (and boundary faces neighbouring a
...
%==================================
%\subsection[D\'efinition des champs \`a post-traiter : \texttt{usvpst}]
%{D\'efinition des champs \`a post-traiter : \textmd{\texttt{usvpst}}}
...
%==================================
%\subsection[Modifications des variables en fin de pas de temps : \texttt{usproj}]
%{Modifications des variables en fin de pas de temps : \textmd{\texttt{usproj}}}
...
\item It shows the precautions to take before doing some operations in
      ...
      access to values at the cells neighbouring a face). 
...
%==================================
%\subsection[Transferts thermiques radiatifs en milieu gris semi-transparent]
%{Transferts thermiques radiatifs en milieu gris semi-transparent}
\subsection{Radiative thermal transfers in semi-transparent gray media}
%==================================

%==================================
%\subsubsection[Initialisation des principaux mots cl\'es du rayonnement : \texttt{usray1}]
%{Initialisation des principaux mots cl\'es du rayonnement : \textmd{\texttt{usray1}}}
...
%==================================
%\subsubsection[Gestion des conditions aux limites du rayonnement : \texttt{usray2}]
%{Gestion des conditions aux limites du rayonnement : \textmd{\texttt{usray2}}}
...
according to the data given in \texttt{usray2}.\\ 
...
%==================================
%\subsubsection[Coefficient d'absortion du milieu, conditions aux limites
%pour la luminance et calcul du flux net radiatif : \texttt{usray3}]
%{Coefficient d'absortion du milieu, conditions aux limites
%pour la luminance et calcul du flux net radiatif : \textmd{\texttt{usray3}}}
...
%==================================
%\subsubsection[Encapsulation de la conversion temp\`erature-enthalpie : \texttt{usray4}]
%{Encapsulation de la conversion temp\'erature-enthalpie : \textmd{\texttt{usray4}}}
...
%==================================
%\subsection[Utilisation d'une physique particuli\`ere : \texttt{usppmo}]
%{Utilisation d'une physique particuli\`ere : \textmd{\texttt{usppmo}}}
...
DONNEES\_THERMOCHIMIE\index{DONNEES\_THERMOCHIMIE} in the launch script
(for instance: DONNEES\_THERMOCHIMIE=''dp\_C3P''). Some example files
...
\clearpage
%==================================
%\subsection[Gestion des conditions aux limites charbon pulv\'eris\'e et combustion gaz~: \texttt{usebuc, usd3pc,uslwcc, uscpcl et uscplc}]
%{Gestion des conditions aux limites charbon pulv\'eris\'e et combustion gaz~:
%\textmd{\texttt{usebuc, usd3pc, uslwcc, uscpcl et uscplc}}}
...
modelling. The way of using them is the same as the way of using
...
generally by a property of their family), where are defined: the type of
face, the type of boundary condition for each variable and eventually
...
In the case of a specific physics modelling, a zone number IZONE
...
instance the colour ICOUL) is associated with every boundary face, in
...
%==================================
%\subsection[Initialisation des variables charbon pulv\'eris\'e et
%combustion gaz~: \texttt{usebui, usd3pi, uslwci et uscpiv}]
%{Initialisation des variables charbon pulv\'eris\'e et combustion
%gaz~: \textmd{\texttt{usebui, usd3pi, uslwci et uscpiv}}}
...
%==================================
%\subsection[Initialisation des options des variables charbon pulv\'eris\'e et combustion gaz~: \texttt{usebu1, usd3p1, uslwc1, uscpi1 et uscpl1}]
%{Initialisation des options des variables charbon pulv\'eris\'e et
%combustion gaz~: \textmd{\texttt{usebu1, usd3p1, uslwc1,  uscpi1 et uscpl1}}}
...
The variables which can be activated by the user for each specific
	...
	\item Properties PROPCE(IEL,IPPROC(IPROP))
       ...
       \item rapid complete chemistry diffusion flame modelling: 
	     ...
	     \end{list}
       \item pulverised coal modelling with 3 comustables: 
	     \begin{list}{}{}
...
%==================================
%\subsection[Gestion des conditions aux limites module \'electrique~: \texttt{uselcl}]
%{Gestion des conditions aux limites module \'electrique~: \textmd{\texttt{uselcl}}}
...
is required to run an electric calculation. The main use is the same as
 occurs in \texttt{usclim} for the standard \CS calculations, in that 
a loop on the boundary face, referenced by their colour (or more genrally
 the properties of their familly), where it is defined ;the face type,
 the boundary conditions for each variable, and finally the value of 
each variable or imposed flow. 
...
{ppvar.h} and cannot be modified.}( the colour ICOUL for example)in order
...
%==================================
%\subsection[Initialisation des variables  module \'electrique~: \texttt{useliv}]
%{Initialisation des variables  module \'electrique~: \textmd{\texttt{useliv}}}
...
\end{list}
%====================================
%\subsection[Initialisation des options des variables  module \'electrique~: \texttt{useli1}]
%{Initialisation des options des variables  module \'electrique~: \textmd{\texttt{useli1}}}
...
%==================================
%\subsection[Gestion des propri\'et\'es physiques variables module \'electrique~: \texttt{uselph}]
%{Gestion des propri\'et\'es physiques variables module \'electrique~: \textmd{\texttt{uselph}}}
...
%==================================
%\subsection[Gestion des sorties au format {\em EnSight} module \'electrique~: \texttt{uselen}]
%{Gestion des sorties au format {\em EnSight} module \'electrique~: \textmd{\texttt{uselen}}}
...
If it is convenient for the user, there is no need to add this subroutine into the
 FORT directory: the post-processing will be done automatically (at the same frequency
 (NTCHR) as the other calculation variables) 


%==================================
%\subsection[Module compressible]
%{Module compressible }
...
%==================================
%\subsubsection[Initialisation des options des variables module compressible : \texttt{uscfx1}
%et \texttt{uscfx2}]
%{Initialisation des options des variables module compressible : \textmd{\texttt{uscfx1}}
%et \textmd{\texttt{uscfx2}}}
...
%==================================
%\subsubsection[Gestion des conditions aux limites module compressible : \texttt{uscfcl}]
%{Gestion des conditions aux limites module compressible : \textmd{\texttt{uscfcl}}} 
...
\texttt{usclim} in the framework of standard calculations, that is to
say a loop on the boundary faces marked out by their colour (or more
generally by a property of their family), where the type of
face, the type of boundary condition for each variable and eventually
the value of each variable are defined.
...
%==================================
%\subsubsection[Initialisation des variables module compressible : \texttt{uscfxi}]
%{Initialisation des variables module compressible : \textmd{\texttt{uscfxi}}} 
...
%==================================
%\subsubsection[Thermodynamique module compressible : \texttt{uscfth}]
%{Thermodynamique module compressible : \textmd{\texttt{uscfth}}} 
...
%==================================
%\subsubsection[Gestion des propri\'et\'es physiques variables module compressible~: \texttt{uscfpv}]
%{Gestion des propri\'et\'es physiques variables module compressible~: \textmd{\texttt{uscfpv}}}
...
%==================================
%\subsection[Approche lagrangienne des \'ecoulements multiphasiques \`a inclusions dispers\'ees]
%{Approche lagrangienne des \'ecoulements multiphasiques \`a inclusions dispers\'ees}
\subsection{Lagrangian modelling of multiphasic flows with dipersed inclusions}
%==================================


%==================================
%\subsubsection[Initialisation des principaux mots cl\'es de l'approche lagrangienne : \texttt{uslag1}]
%{Initialisation des principaux mots cl\'es de l'approche lagrangienne : \textmd{\texttt{uslag1}}}
\subsubsection{Initialisation of the main key words in the lagrangian
   modelling: \textmd{\texttt{uslag1}}}
%==================================
...
the case of a calculation modelling a lagrangian multiphasic flow. This
...
modelling, the user must make sure that the volumetric statistics are
...
%==================================
%\subsubsection[Gestion des conditions aux limites particulaires :
%\texttt{uslag2} et \texttt{uslain}]
%{Gestion des conditions aux limites particulaires : \textmd{\texttt{uslag2}} et
%\textmd{\texttt{uslain}}} 
\subsubsection{Management of the boundary conditions related to the
...
\noindent
In the framework of the multiphasic lagrangrian modelling, the management
...
module, and a type of particle behaviour is associated with each one. \\
...
defines NFRLAG\index{NFRLAG} boundary zones from the colour of the
...
\item if IUSLAG(ICLAS,IZONE,IJUVW) = -1, the particle injection velocity
      is equal to the fluid velocity at the centre of the cell
...
%==================================
%\subsubsection[Traitement de l'interaction particules/fronti\`ere : \texttt{uslabo}]
%{Traitement de l'interaction particules/fronti\`ere : \textmd{\texttt{uslabo}}}
...
%==================================
%\subsubsection[Option de clonage/fusion des particules : \texttt{uslaru}]
%{Option de clonage/fusion des particules : \textmd{\texttt{uslaru}}}
...
%==================================
%\subsubsection[Manipulation des variables particulaires en fin d'itération et statistiques volumiques utilisateur: \texttt{uslast} et \texttt{uslaen}]
%{Manipulation des variables particulaires en fin d'itération et statistiques volumiques utilisateur: \textmd{\texttt{uslast}} et \textmd{\texttt{uslaen}}}
...
%==================================
%\subsubsection[\'Equations diff\'erentielles stochastiques utilisateur : \texttt{uslaed}]
%{\'Equations diff\'erentielles stochastiques utilisateur : \textmd{\texttt{uslaed}}}
...
%==================================
%\subsubsection[Temps de relaxation des particules : \texttt{uslatp}]
%{Temps de relaxation des particules : \textmd{\texttt{uslatp}}}
...
%==================================
%\subsubsection[Temps caractéristique thermique des particules: \texttt{uslatc}]
%{Temps caractéristique thermique des particules: \textmd{\texttt{uslatc}}}
