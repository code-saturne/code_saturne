...
\hline
Documentation:     & online documentation. Download link at \url{http://files.opencascade.com/Salome/Salome5.1.4/med-fichier_2.3.6.tar.gz}\\
\hline
\end{tabular}
...
\item Analysis environment: definition of the calculation directories
      (STUDY, CASE), mesh file(s), coupling with \syrthes, stand-alone
      execution of the Preprocessor module (used by the Interface to get
      the colors of the boundary faces).
...
We consider 3 types of output: the volume mesh, informational meshes
(such as reoriented elements), and meshes outlining zones with errors.
By default, all types of meshes are output. If one or several of
the two filtering sub-options is used , only the meshes of the
...
are: \texttt{--volume} and \texttt{--info}.
...
is usually not needed. In this case, the \texttt{--no-write} option may be
avoid creating a {\tt preprocessor\_output} file.
...
\item Periodicity is defined through the Graphical Interface or using
      the \texttt{usperio} user subroutine.
...
to them. By default, the input read is a file named \texttt{preprocessor\_output},
but if this function is used, any file may be selected, and the same
file may be read multiple times (applying a different coordinate transformation
each time).
...
%==================================
\subsection{Define mesh faces joining: \textmd{\texttt{usjoin}}}
%==================================

\noindent
\textit{Subroutine called only during the calculation initialisation.}

This subroutine may be used to define mesh joinings. It allows access
to advanced joining parameters (such as search tree memory/speed
tuning parameters) not accessible through the GUI.

It is possible (and usually recommended) to restrict the operation to
faces to those which are ``probably'' joined,
using appropriate selection criteria.

The most useful parameter to \texttt{defjoi} is the \texttt{fract}
parameter, which assigns value $r$ (where $0 < r < 0,49$) to the maximum
...
The \texttt{--plane} parameter also defined the maximum angle between
face normals up to which two faces are considered coplanar (25° by default);
...
%==================================
\subsection{Define mesh faces periodicity: \textmd{\texttt{usperi}}}
%==================================

\noindent
\textit{Subroutine called only during the calculation initialisation.}

This subroutine may be used to define mesh periodicities. It allows access
to advanced joining parameters (such as search tree memory/speed
tuning parameters) not accessible through the GUI.
...
connections between faces to those which are ``probably'' periodic,
using appropriate selection criteria.
...
may be selected when the \texttt{indgrp} of \texttt{pstcma} is set to 1,
so as to maintain group information, but those faces will
only be written for formats supporting this (such as MED),
and will only bear groups, not variable fields}. If the user
