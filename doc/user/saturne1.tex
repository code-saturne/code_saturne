...
The first thing to do before running \CS is to add in the user \textasciitilde\texttt{/.profile} file the path leading to the chosen \CS version. At the time of writing it was :\
\begin{center}
\texttt{export PATH=/home/saturne/Code\_Saturne/2.0-rc1/arch/Linux\_x86\_64/bin:\$PATH}.\
\end{center}
...
\begin{center}
\texttt{code\_saturne create -s STUDY}\
\end{center}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{V-39}
...
%==================================
If the calculation does not run properly, the user is advised to check the following points:
\begin{list}{$\bullet$}{}
\item  if the calculation stops in the pre-processor, the user should check for error messages in the file \texttt{listpre.mmddhhmm} found in \texttt{CASE1/RESU},
...
\item if the calculation stops in the \CS core, the user should look for messages at the end of the files \texttt{listing.mmddhhmm} and \texttt{err.mmddhhmm}. In addition, the user can track the following keywords in the listing. They are specific error signals:
  \begin{list}{-}{}
  \item  \texttt{SIGFPE}: a floating point exception occurred. It happens when there is a division by 0, when the calculation did not converge, or when a real number reached a value over $10^{300}$.
...
\item Calculation environment: definition of the mesh file(s), periodicity,
...
%==================================
\subsubsubsection{Developer arrays}
%==================================
...
\pcs's main functions. It can also be done by the solver. When done in the \pcs, this operation is performed on a single processor while it can be done in parallel in the solver. With the \pcs, it can be done in the Graphical User Interface (GUI) or with a command line. In the GUI, the user needs to tick the box ``Join
 meshes'' in the tab ``Meshes'' in the item ``Calculation environment
 $\rightarrow$ Meshes selection''. Then, new elements appear, the user must
 specify the pairs of references to be joined. He can also modify basic joining
...
\end{figure}
%
To activate it without the GUI, use the \texttt{--join} or
...
for the periodic faces to be conform (though it usually leads to better
...
This operation can also be performed either in the \pcs or in the solver by specific Fortran files. When done in the \pcs, the task is done on a single processor while it can be done in parallel in the solver.
...
The setting of these parameters is done in the user subroutine \texttt{usjoin} (called once). The user can specify the parameters used for the joining of different meshes. Below is given the list of the standard parameters which can me modified:
\begin{list}{-}{}
...
\end{list}
In the call of the subroutine '\texttt{defjoi}' are specified the references of the mesh faces to be joined.
...
Periodicities can be set directly in the Graphical User Interface (GUI) or using the user subroutine \texttt{usperi} (called when once during the calculation initialisation). In the GUI, the user can choose between 5 types of periodicities: one periodicity of translation and four periodicities of rotation, see fig. \ref{fig:periodicities}. Then specific parameters must be set.

\begin{figure}[ht]
\begin{center}
\includegraphics[width=14cm]{periodicity}
\caption{Periodicity}
\label{fig:periodicities}
\end{center}
\end{figure}

\texttt{usperi} can be used instead of the GUI, it allows also the user to specify the parameters used to set periodicities and gives access to more advanced parameters. Below is given the list of the main parameters which can me modified:
\begin{list}{-}{}
\item \texttt{fract}: the initial tolerance radius associated to each vertex is equal to the lenght of the shortest incident edge, multiplied by this fraction,
...
The user subroutine \texttt{usmodg} may be used to modify ``manually'' the mesh vertices
coordinates, \textit{i.e.} the array \mbox{\texttt{xyznod(3,nnod)}} (vertex coordinates).
...
\item Calculation control: parameters related to the time averages, the time step, the
      locations of
      the probes where some variables will be monitored over time (if the GUI is not used, this information is specified in Section \ref{sec:usiniv}), the       definition of the frequency of the outputs in the calculation
      listing and in the chronological records and of the {\itshape EnSight} outputs, see figs. \ref{fig:24_GUI} and \ref{fig:25_GUI}. The item ``Profiles'' allows to save, with a frequency defined by the user, 1D profiles on an axis defined by two points, see fig. \ref{fig:26_GUI}.
\item Calculation management: user arrays definition, see fig. \ref{fig:28_GUI}.
\end{list}

\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_6}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_7}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_8}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_11}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_12}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_13}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{V-60}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_15}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_21}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_22}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_23}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_24}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_25}
\caption{Parameters of chronological outputs}
\label{fig:25_GUI}
\end{center}
\end{figure}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_26}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_28}
\caption{Definition of user array size}
\label{fig:28_GUI}
\end{center}
\end{figure}
...
\item For calculations with non-constant time step
      (\texttt{idtvar}=1 or 2) which are a restart (\texttt{isuite}=1) from a
...
or 2) which is a restart (\texttt{isuite}=1) from a calculation in which
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_19}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{GUI_20}
...
\S\ref{prg_clstandard}), it is sufficient to complete \texttt{itypfb(ifac,iphas)} and
...
\minititre{Notes}
$\bullet\ $ Whatever is the value of the indicator \texttt{itypfb(ifac,iphas)}, if
...
\texttt{itypfb(ifac,iphas)}=$N$ where $N$ range is 1 to
...
\texttt{rcodcl}. \texttt{itypfb(ifac,iphas)} is then equal to \texttt{iindef}
...
1.\\
\texttt{ifac=itrifb(i+n,iphas)} is the number of the i$^{\text{th}}$ face of type
2, if there are $n$ faces of type 1.\\
... etc.

Two auxiliary arrays of size \texttt{ntypmx} are also defined.\\
\texttt{idebty(ityp,iphas)\index{idebty}} is the index
corresponding to the first
face of type \texttt{ityp} in the array \texttt{itrifb}.\\
\texttt{ifinty(ityp,iphas)\index{ifinty}} is the index
corresponding to the last
face of type \texttt{ityp} in the array \texttt{itrifb}.

Therefore, a value \texttt{ifac0} found between \texttt{idebty(ityp,iphas)} and
\texttt{ifinty(ityp,iphas)} is associated to each face \texttt{ifac} of type
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=9cm]{density_law}
...
\item By default, the $C_p$ coefficient of phase \texttt{iphas} and the
...
To assign a variable value to $C_p$, the user \textbf{must}t specify it in the
      interface or assign the value 1 to \texttt{icp(iphas)} in \texttt{usini1},
...
equations (at each time step). For each phase \texttt{iphas}, it is called three times every time
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=9cm]{head_loss5}
...
The subroutine \texttt{usdpst}, called at the calculation start, allows for the definition of  surface or volume
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{homogeneous-coal1}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{homogeneous-coal2}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{homogeneous-coal3}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{homogeneous-coal4}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{homogeneous-coal5}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{cl-coal1}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{time_average-coal}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{solution_control-coal}
...
Physical properties are stored in \texttt{propce} (cell center), \texttt{propfa} (inner face) and \texttt{propfb}. For instance, \texttt{propce(iel, ipproc(irom  (iphas)))} is \texttt{rom(iel,iphas)}, the mean density (in $kg.m^{-3}$), and \texttt{propfa(ifac,ipprof(ifluma(ivar)))} is \texttt{flumas(IFAC,IVAR)}, the convective flux of the variable \texttt{ivar}.\\
...
\begin{list}{.}{}
\item for ``standard'' boundary conditions (inlet, free outlet, wall, symmetry): a code is defined in the array \texttt{itypfb} (of dimensions equal to the number of boundary faces, and to the number of phases). This code will then be used by a non-user subroutine to assign the conditions (scalars in particular will receive the conditions of the phase they are associated with).
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=13cm]{rad_transf0}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=10cm]{rad_transf1}
\caption{Radiative transfers - parameters}
\label{fig:1_ray}
\end{center}
\end{figure}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=7cm]{rad_transf2}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=10cm]{rad_transf3}
\caption{Radiative transfers - parameters}
\label{fig:3_ray}
\end{center}
\end{figure}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=7cm]{rad_transf4}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=6cm]{rad_transf5}
\caption{Calculation control - Radiative transfers history}
\label{fig:5_ray}
\end{center}
\end{figure}
\clearpage
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{rad_transf6}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{rad_transf7}
...
done in the Graphical User Interface (GUI) or in the user subroutine \texttt{ussyrc} 
(called when ???). In the GUI, to set such a coupling, a thermal scalar must be 
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=8cm]{syrthes_coupling1}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{Lag-global_settings3}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=10cm]{Lag-global_settings2}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=8cm]{global-Lag3}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{Lag-Statistics}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=11cm]{Lag-output}
...
the case of a calculation using a Lagrangian multiphasic flow model. This
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=16cm]{Lag-cl}
...
\item For the electric arc module, the enthalpy value is taken from the temperature
 of reference \texttt{t0(iphas)} (given in \texttt{usini1}) from the temperature-enthalpy
 tables
...
. An example is given of how to obtain the enthalpy from the temperature of reference
 \texttt{t0(iphas)}(given in \texttt{usini1}), the temperature-enthalpy law must be
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{ale1}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{ale2}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{ale3}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=12cm]{atmo1}
...
\begin{list}{-}{}
 \item the air velocity with the array \texttt{rtp(iel,iu(iphas))} (with
...
\begin{list}{-}{}
 ...
 \item the air velocity by filling the array \texttt{rtp(iel,iu(iphas))} (with
 \texttt{iv} and \texttt{iw} for the other components),
\end{list}
