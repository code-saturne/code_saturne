...
is referred to \fort{cs\_user\_physical\_properties} (see the \doxygenfile{cs__user__physical__properties_8f90.html}{programmers reference of the dedicated subroutine})
 or \fort{cs\_physical\_properties\_update} for more information).
},
...
The code (see the \doxygenfile{cs__boundary__conditions__set__coeffs_8c.html}{programmers reference of the dedicated subroutine})
...
\end{itemize}
In this reference-frame the wall distance of the cell center to the wall $\norm{\centf - \centip}$ is denoted by $y$.
...
proportional to the cell center to wall dimensionless distance, $y^+$, over which a linear profile
...
make explicit the wall velocity in the final expression:
%
The internal pair of boundary condition coefficients for the diffusive term for the velocity are
\begin{equation}
\left\lbrace
...
substituted by a Robin-type one where the diffusive flux is function of the cell center scalar value,
...
\begin{equation*}
  P_\fib = P_{\centip} +  \grad_\celli P \cdot (\centf -\centip).
\end{equation*}
...
a single plane parallel to the outlet, then the value of $ \grad_\celli P \cdot (\centf- \centip) $
