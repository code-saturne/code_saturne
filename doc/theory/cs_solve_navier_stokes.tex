...
-\vect{\widetilde{u}}^{n+1}\,\dive{(\rho\vect{u})}
  =
  \divv \left(\tens{\sigma}^{n+\theta}\right) + \vect{TS}^{n+\theta_{S}}-\tens{K}^{n}\,\vect{\widetilde{u}}^{n+\theta} ,
...
\subsubsection*{Pressure correction step (or projection of velocities)}
...
Following the classical approach, the time-discretized equations are then integrated over the control volumes ${\Omega_\celli}$ (or cells).
\subsubsection*{Velocity prediction step}
...
${\Omega_\celli}$ and $\gamma_b(i)$ the set of the centers of the boundary faces of
${\Omega_\celli}$, we obtain:

\begin{equation*}
\int_{\Omega_\celli}{\dive\left(\vect{u}\otimes (\rho \vect{u})\right)\, d\Omega} =
...
\begin{equation*}
\int_{\vol{\celli}}{\divv(\mu \gradt{\vect{u}}) \dd \Omega} =
...
\begin{array}{ll}
\mathcal{E}(\vect{\widetilde{u}}, \celli) &= \theta\, \mathcal{J}(\vect{\widetilde{u}}, \celli)\,+\,(1-\theta)\,\mathcal{J}(\vect{u}^{n}, \celli)\,+ |{\Omega_\celli}|\displaystyle\frac{\rho_\celli}{\Delta t}\,(\vect{\widetilde{u}}_{\celli}-\vect{u}^n_{\celli}) \\
...
{i.e.} comprising the $\vect{F}_{\ib}((\rho \vect{u}),\vect{u})$ and
...
\begin{equation}
\int_{\vol{\celli}}{\dive(\Delta t \grad\ \delta P^{n+\theta})\, \dd\Omega} =
...
and:
\begin{equation}
\int_{\Omega_\celli}{\dive(\rho \vect{\widetilde{u}})^{n+1}  \dd \Omega} =
...
\mathcal{F}(\delta P, \celli) &=
 \sum\limits_{\cellj\in Neigh(\celli)}{\left[D_{\ij}(\Delta t,\delta
  P)-\left[(\rho \vect{\widetilde{u}})^{n+1}_{\fij} \cdot \vect{S}_\ij \right]^{Corr}\right]}\\
&+\sum\limits_{k\in {\gamma_b(\celli)}} {\left[D_{\ib}(\Delta
  t^n,\delta P)-\left[(\rho \vect{\widetilde{u}} \cdot \vect{S}_\ij )^{n+1}_{\fib}\right]^{Corr}\right]\,+\,\Gamma \vol{\celli}}
\end{array}
\end{equation}
%
and:
...
and thus:
\begin{equation}\notag
\int_{\vol{\celli}}{\dive(\rho \vect{u})^{n+1}  \dd \Omega} = \Gamma
\end{equation}
\end{remark}
