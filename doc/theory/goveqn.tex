...
+\divt \left( \vect{u}\otimes \rho \vect{u} \right)
=\divt \left( \tens{\sigma} \right) +\vect{ST}_{\vect{u}}-\tens{K}\,\vect{u},
\end{equation}
...
\nomenclature[gtaut2]{$\tens{\tau}$}{viscous stress tensor, which is the deviatoric part of the stress tensor}
 defined from $\mu =\mu_l $ (dynamic molecular viscosity) 
...
\tens{\tau}=2\,\mu \ \tens{S}-\frac{2}{3}\mu \ \trace{\tens{S}}\ \tens{Id},
\end{equation}
with
...
\begin{itemize}
\item with an eddy-viscosity model (EVM) such as the $%
...
= -\left[-2 \mu_t \tens{S} + \frac{2}{3}\mu_t\trace{\tens{S}} \tens{1}%
...
\tens{ \mathcal{G}} & = & \frac{3}{2}\frac{C_{\mu }}{\sigma _{t}}
\frac{k}{\varepsilon }
...
\begin{equation}
\displaystyle \Phi_{ij} =\phi_{ij,1}+ \phi_{ij,2}+\phi_{ij,3}+\phi_{ij,w}
\end{equation}%
%
\begin{equation}
\begin{array}{r c l}
\phi _{ij,1} &=& -\rho \,C_{1}\frac{\varepsilon }{k}%
\left( R_{ij}-\frac{2}{3}k\delta _{ij} \right), \\
\phi _{ij,2} &=& -\rho \,C_{2} \left(
\mathcal{P}_{ij}-\frac{2}{3}\mathcal{P} \right)\delta _{ij}, \\
\phi_{ij,3} &=& -C_{3} \left( G_{ij}-\frac{1}{3}\delta _{ij}G_{kk} \right).
\end{array}
\end{equation}
...
\sqrt{2\overline{S_{ij}}\,\overline{S_{ij}}}
\end{equation}%
...
\begin{equation}
L_{ij} -\frac{1}{3} \delta_{ij} L_{kk} =  C \left(
\alpha_{ij}-\widetilde{\beta}_{ij} \right),
\end{equation}
with:
\begin{equation}
\begin{array}{rcl}
...
\begin{equation}
E_{ij} = L_{ij}-C \left(\alpha_{ij} - \widetilde{\beta}_{ij} \right),
\end{equation}
...
%-------------------------------------------------------------------------------
\subsection{Equations for the scalars}
