...
\texttt{RUN=\$SCRATCHDIR/tmp\_Saturne/\$ETUDE/\$CAS.mmddhhmm} for Tantale and
Platine at the CCRT\\
...
example of the launch script: \texttt{lance}.
...
(see \S\ref{prg_lance}),
especially the mesh file(s) to use (by default a name \texttt{\$ETUDE.unv} is
put), but everything can be specified throuhg the Graphical Interface.
...
procedure is quite tedious if one wishes to keep the color of the boundary
...
the faces of colors 6 and 7, the full option will be \mbox{\texttt{-rc -color
...
\item \texttt{-iasize n}: specifies the size \texttt{n} of the macro-array of
...
script. The option \mbox{\texttt{-iasize \$LONGIA}} is automatically added to the
Kernel command line.\\
The value of \texttt{n} is accessible in the variable LONGIA\index{LONGIA} in
the FORTRAN code.

\item \texttt{-rasize n}: specifies the size \texttt{n} of the macro-array of
...
script. The option \mbox{\texttt{-rasize \$LONGRA}} is automatically added to the
Kernel command line.\\
...
through the \texttt{-iasize} option (see \ref{prg_optappelnoy}).

\item \texttt{LONGRA}: value of the parameter LONGIA to be passed to the Kernel
through the \texttt{-rasize} option (see \ref{prg_optappelnoy})
\end{list}
...
\item \texttt{COMMANDE\_DF}: Kernel command line option for the division of
...
a list allows to use different optimisation options for different modules. If a
list is 
given, all modules must be referenced once (and only once). Leaving
the variable empty implies taking the same optimisation for all the
modules (BASE, COGZ, CFBL, CPLV, ELEC, FUEL, LAGR, MATI, RAYT), corresponding to
the value of the \texttt{OPTIMISATION} variable (see above). An example of list
could be:\\ 
\texttt{LISTE\_LIB\_SAT=``libsaturneBASEDBG.a libsaturneCFBL.a
libsaturneCOGZDBG.a}\verb+ \+\\
\texttt{\phantom{LISTE\_LIB\_SAT=``}libsaturneCPLV.a libsaturneELECDBG.a
libsaturneFUELDBG.a}\verb+ \+\\ 
\texttt{\phantom{LISTE\_LIB\_SAT=``}libsaturneLAGR.a libsaturneMATI.a
libsaturneRAYT.a''}
...
teh associated conditions are intended to be modified in \texttt{usclim}, and
...
When the mesh is created using I-DEAS, boundary faces and volume
      elements may be given colors. When it is created using SIMAIL, the
      colors of boundary faces are called ``references'', and the colors
...
and volume elements) correspond to a number (color number or group
...
differentiate the boundary faces according to their color (or, more
...
\item The 2nd part (IAPPEL=1) defines the boundary faces at which the vortex method
 is applicable. The boundary faces can be represented as colors (ICOUL) or by the
...
if the color of the inlet face is altered}:
\begin{list}{$\bullet$}{}
\item \mbox{XYZCEN(3,NCELET)} (cell center coordinates),
\item \mbox{CDGFAC(3,NFAC)} (internal face center coordinates),
\item \mbox{CDGFBO(3,NFABOR)} (boundary face center coordinates),
...
out by means of the coordinates of their centers. The second one is constituted
of internal faces marked out by means of the color of their neighboring
cells and of boundary faces marked out by means of their color.
...
say a loop on the boundary faces marked out by their color (or more
...
a loop on the boundary face, referenced by their color (or more genrally
